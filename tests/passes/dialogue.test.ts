...
`;
  ...
  describe('Wave 1148 — dialoguePass: dialogue turn-highlight aftermath void, dialogue revelation-curiosity aftermath void, dialogue suspense-emotional aftermath void', async () => {
    ...
    const buildScenes1148 = (count: number): string => {
      let f = '';
      for (let i = 0; i < count; i++) {
        f += `INT. SCENE ${i} - DAY\n\nA figure moves through the room.\n\n`;
      }
    ...
    const runD1148 = async (fountain: string, records: any[] = []) => {
      const { dialoguePass } = await import('../../server/nvm/revision/passes/dialogue.ts');
      return dialoguePass({ fountain, original: fountain, records, structure: {} as any, annotations: [], approvedSpans: [] });
    };
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('DIALOGUE_TURN_HIGHLIGHT_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no dialogue highlight', async () => {
      const records1148a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1148(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeRec1148(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1148(i);
      });
      const res = await runD1148(buildScenes1148(10), records1148a);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_TURN_HIGHLIGHT_AFTERMATH_VOID'), 'DIALOGUE_TURN_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_TURN_HIGHLIGHT_AFTERMATH_VOID does not fire when a dramatic turn is followed by a dialogue highlight within its window', async () => {
      const records1148an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1148(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeRec1148(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1148(i);
      });
      const res = await runD1148(buildScenes1148(10), records1148an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_TURN_HIGHLIGHT_AFTERMATH_VOID'), 'DIALOGUE_TURN_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_REVELATION_CURIOSITY_AFTERMATH_VOID fires when every revelation is followed by two scenes with no rise in curiosity', async () => {
      const records1148b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1148(i, { revelation: 'the truth about the letter' });
        if (i === 8 || i === 9) return makeRec1148(i, { curiosityDelta: 1 });
        return makeRec1148(i);
      });
      const res = await runD1148(buildScenes1148(10), records1148b);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_REVELATION_CURIOSITY_AFTERMATH_VOID'), 'DIALOGUE_REVELATION_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_REVELATION_CURIOSITY_AFTERMATH_VOID does not fire when a revelation is followed by a rise in curiosity within its window', async () => {
      const records1148bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1148(i, { revelation: 'the truth about the letter' });
        if (i === 1 || i === 9) return makeRec1148(i, { curiosityDelta: 1 });
        return makeRec1148(i);
      });
      const res = await runD1148(buildScenes1148(10), records1148bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_REVELATION_CURIOSITY_AFTERMATH_VOID'), 'DIALOGUE_REVELATION_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_SUSPENSE_EMOTIONAL_AFTERMATH_VOID fires when every suspense rise is followed by two scenes with no emotional shift', async () => {
      const records1148c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1148(i, { suspenseDelta: 1 });
        if (i === 8 || i === 9) return makeRec1148(i, { emotionalShift: 'positive' });
        return makeRec1148(i);
      });
      const res = await runD1148(buildScenes1148(10), records1148c);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_SUSPENSE_EMOTIONAL_AFTERMATH_VOID'), 'DIALOGUE_SUSPENSE_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_SUSPENSE_EMOTIONAL_AFTERMATH_VOID does not fire when a suspense rise is followed by an emotional shift within its window', async () => {
      const records1148cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1148(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeRec1148(i, { emotionalShift: 'positive' });
        return makeRec1148(i);
      });
      const res = await runD1148(buildScenes1148(10), records1148cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_SUSPENSE_EMOTIONAL_AFTERMATH_VOID'), 'DIALOGUE_SUSPENSE_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1134 — dialoguePass: dialogue clock-staging aftermath void, dialogue turn-relational aftermath void, dialogue turn-staging aftermath void', async () => {
    const makeRec1134 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: `INT. SC${idx} - DAY`,
      emotionalShift: 'neutral', suspenseDelta: 0, curiosityDelta: 0,
      clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [], relationshipShifts: [], visualBeats: [],
    ...
    const buildScenes1134 = (count: number): string => {
      let f = '';
      for (let i = 0; i < count; i++) {
        f += `INT. SCENE ${i} - DAY\n\nA figure moves through the room.\n\n`;
      }
    ...
    const runD1134 = async (fountain: string, records: any[] = []) => {
      const { dialoguePass } = await import('../../server/nvm/revision/passes/dialogue.ts');
      return dialoguePass({ fountain, original: fountain, records, structure: {} as any, annotations: [], approvedSpans: [] });
    };
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('DIALOGUE_CLOCK_STAGING_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no heavily-staged scene', async () => {
      const records1134a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1134(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeRec1134(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1134(i);
      });
      const res = await runD1134(buildScenes1134(10), records1134a);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_CLOCK_STAGING_AFTERMATH_VOID'), 'DIALOGUE_CLOCK_STAGING_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_CLOCK_STAGING_AFTERMATH_VOID does not fire when a clock-raise is followed by a heavily-staged scene within its window', async () => {
      const records1134an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1134(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeRec1134(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1134(i);
      });
      const res = await runD1134(buildScenes1134(10), records1134an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_CLOCK_STAGING_AFTERMATH_VOID'), 'DIALOGUE_CLOCK_STAGING_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_TURN_RELATIONAL_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no relationship shift', async () => {
      const records1134b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1134(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeRec1134(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeRec1134(i);
      });
      const res = await runD1134(buildScenes1134(10), records1134b);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_TURN_RELATIONAL_AFTERMATH_VOID'), 'DIALOGUE_TURN_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_TURN_RELATIONAL_AFTERMATH_VOID does not fire when a dramatic turn is followed by a relationship shift within its window', async () => {
      const records1134bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1134(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeRec1134(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeRec1134(i);
      });
      const res = await runD1134(buildScenes1134(10), records1134bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_TURN_RELATIONAL_AFTERMATH_VOID'), 'DIALOGUE_TURN_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_TURN_STAGING_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no heavily-staged scene', async () => {
      const records1134c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1134(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeRec1134(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1134(i);
      });
      const res = await runD1134(buildScenes1134(10), records1134c);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_TURN_STAGING_AFTERMATH_VOID'), 'DIALOGUE_TURN_STAGING_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_TURN_STAGING_AFTERMATH_VOID does not fire when a dramatic turn is followed by a heavily-staged scene within its window', async () => {
      const records1134cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1134(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeRec1134(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1134(i);
      });
      const res = await runD1134(buildScenes1134(10), records1134cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_TURN_STAGING_AFTERMATH_VOID'), 'DIALOGUE_TURN_STAGING_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1120 — dialoguePass: dialogue clock-suspense aftermath void, dialogue clock-highlight aftermath void, dialogue turn-emotional aftermath void', async () => {
    const makeRec1120 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: `INT. SC${idx} - DAY`,
      emotionalShift: 'neutral', suspenseDelta: 0, curiosityDelta: 0,
      clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [], relationshipShifts: [], visualBeats: [],
    ...
    const buildScenes1120 = (count: number): string => {
      let f = '';
      for (let i = 0; i < count; i++) {
        f += `INT. SCENE ${i} - DAY\n\nA figure moves through the room.\n\n`;
      }
    ...
    const runD1120 = async (fountain: string, records: any[] = []) => {
      const { dialoguePass } = await import('../../server/nvm/revision/passes/dialogue.ts');
      return dialoguePass({ fountain, original: fountain, records, structure: {} as any, annotations: [], approvedSpans: [] });
    };
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('DIALOGUE_CLOCK_SUSPENSE_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no suspense rise', async () => {
      const records1120a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1120(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeRec1120(i, { suspenseDelta: 1 });
        return makeRec1120(i);
      });
      const res = await runD1120(buildScenes1120(10), records1120a);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_CLOCK_SUSPENSE_AFTERMATH_VOID'), 'DIALOGUE_CLOCK_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_CLOCK_SUSPENSE_AFTERMATH_VOID does not fire when a clock-raise is followed by a suspense rise within its window', async () => {
      const records1120an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1120(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeRec1120(i, { suspenseDelta: 1 });
        return makeRec1120(i);
      });
      const res = await runD1120(buildScenes1120(10), records1120an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_CLOCK_SUSPENSE_AFTERMATH_VOID'), 'DIALOGUE_CLOCK_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_CLOCK_HIGHLIGHT_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no highlighted dialogue', async () => {
      const records1120b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1120(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeRec1120(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1120(i);
      });
      const res = await runD1120(buildScenes1120(10), records1120b);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_CLOCK_HIGHLIGHT_AFTERMATH_VOID'), 'DIALOGUE_CLOCK_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_CLOCK_HIGHLIGHT_AFTERMATH_VOID does not fire when a clock-raise is followed by highlighted dialogue within its window', async () => {
      const records1120bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1120(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeRec1120(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1120(i);
      });
      const res = await runD1120(buildScenes1120(10), records1120bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_CLOCK_HIGHLIGHT_AFTERMATH_VOID'), 'DIALOGUE_CLOCK_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_TURN_EMOTIONAL_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no emotional shift', async () => {
      const records1120c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1120(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeRec1120(i, { emotionalShift: 'positive' });
        return makeRec1120(i);
      });
      const res = await runD1120(buildScenes1120(10), records1120c);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_TURN_EMOTIONAL_AFTERMATH_VOID'), 'DIALOGUE_TURN_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_TURN_EMOTIONAL_AFTERMATH_VOID does not fire when a dramatic turn is followed by an emotional shift within its window', async () => {
      const records1120cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1120(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeRec1120(i, { emotionalShift: 'positive' });
        return makeRec1120(i);
      });
      const res = await runD1120(buildScenes1120(10), records1120cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_TURN_EMOTIONAL_AFTERMATH_VOID'), 'DIALOGUE_TURN_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1092 — dialoguePass: dialogue shift-relational aftermath void, dialogue clock-curiosity aftermath void, dialogue turn-suspense aftermath void', async () => {
    ...
    const buildScenes1092 = (count: number): string => {
      let f = '';
      for (let i = 0; i < count; i++) {
        f += `INT. SCENE ${i} - DAY\n\nA figure moves through the room.\n\n`;
      }
    ...
    const runD1092 = async (fountain: string, records: any[] = []) => {
      const { dialoguePass } = await import('../../server/nvm/revision/passes/dialogue.ts');
      return dialoguePass({ fountain, original: fountain, records, structure: {} as any, annotations: [], approvedSpans: [] });
    };
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('DIALOGUE_SHIFT_RELATIONAL_AFTERMATH_VOID fires when every significant shift is followed by two scenes with no further relationship movement', async () => {
      const records1092a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1092(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.5 }] });
        if (i === 8 || i === 9) return makeRec1092(i, { relationshipShifts: [{ pairKey: 'c|d', dimension: 'respect', amount: 0.1 }] });
        return makeRec1092(i);
      });
      const res = await runD1092(buildScenes1092(10), records1092a);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_SHIFT_RELATIONAL_AFTERMATH_VOID'), 'DIALOGUE_SHIFT_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_SHIFT_RELATIONAL_AFTERMATH_VOID does not fire when a significant shift is followed by relationship movement within its window', async () => {
      const records1092an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1092(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.5 }] });
        if (i === 1 || i === 9) return makeRec1092(i, { relationshipShifts: [{ pairKey: 'c|d', dimension: 'respect', amount: 0.1 }] });
        return makeRec1092(i);
      });
      const res = await runD1092(buildScenes1092(10), records1092an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_SHIFT_RELATIONAL_AFTERMATH_VOID'), 'DIALOGUE_SHIFT_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_CLOCK_CURIOSITY_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no curiosity rise', async () => {
      const records1092b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1092(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeRec1092(i, { curiosityDelta: 1 });
        return makeRec1092(i);
      });
      const res = await runD1092(buildScenes1092(10), records1092b);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_CLOCK_CURIOSITY_AFTERMATH_VOID'), 'DIALOGUE_CLOCK_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_CLOCK_CURIOSITY_AFTERMATH_VOID does not fire when a clock-raise is followed by a curiosity rise within its window', async () => {
      const records1092bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1092(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeRec1092(i, { curiosityDelta: 1 });
        return makeRec1092(i);
      });
      const res = await runD1092(buildScenes1092(10), records1092bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_CLOCK_CURIOSITY_AFTERMATH_VOID'), 'DIALOGUE_CLOCK_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_TURN_SUSPENSE_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no suspense rise', async () => {
      const records1092c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1092(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeRec1092(i, { suspenseDelta: 1 });
        return makeRec1092(i);
      });
      const res = await runD1092(buildScenes1092(10), records1092c);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_TURN_SUSPENSE_AFTERMATH_VOID'), 'DIALOGUE_TURN_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_TURN_SUSPENSE_AFTERMATH_VOID does not fire when a dramatic turn is followed by a suspense rise within its window', async () => {
      const records1092cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1092(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeRec1092(i, { suspenseDelta: 1 });
        return makeRec1092(i);
      });
      const res = await runD1092(buildScenes1092(10), records1092cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_TURN_SUSPENSE_AFTERMATH_VOID'), 'DIALOGUE_TURN_SUSPENSE_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1050 — dialoguePass: dialogue shift-suspense aftermath void, dialogue shift-emotional aftermath void, dialogue seed-staging aftermath void', async () => {
    ...
    const buildScenes1050 = (count: number): string => {
      let f = '';
      for (let i = 0; i < count; i++) {
        f += `INT. SCENE ${i} - DAY\n\nA figure moves through the room.\n\n`;
      }
    ...
    const runD1050 = async (fountain: string, records: any[] = []) => {
      const { dialoguePass } = await import('../../server/nvm/revision/passes/dialogue.ts');
      return dialoguePass({ fountain, original: fountain, records, structure: {} as any, annotations: [], approvedSpans: [] });
    };
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('DIALOGUE_SHIFT_SUSPENSE_AFTERMATH_VOID fires when every relationship-shift scene is followed by two scenes with no suspense rise', async () => {
      const records1050a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1050(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.5 }] });
        if (i === 8 || i === 9) return makeRec1050(i, { suspenseDelta: 1 });
        return makeRec1050(i);
      });
      const res = await runD1050(buildScenes1050(10), records1050a);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_SHIFT_SUSPENSE_AFTERMATH_VOID'), 'DIALOGUE_SHIFT_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_SHIFT_SUSPENSE_AFTERMATH_VOID does not fire when a relationship-shift scene is followed by a suspense rise within its window', async () => {
      const records1050an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1050(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.5 }] });
        if (i === 1 || i === 9) return makeRec1050(i, { suspenseDelta: 1 });
        return makeRec1050(i);
      });
      const res = await runD1050(buildScenes1050(10), records1050an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_SHIFT_SUSPENSE_AFTERMATH_VOID'), 'DIALOGUE_SHIFT_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_SHIFT_EMOTIONAL_AFTERMATH_VOID fires when every relationship-shift scene is followed by two scenes with no emotional shift', async () => {
      const records1050b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1050(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.5 }] });
        if (i === 8 || i === 9) return makeRec1050(i, { emotionalShift: 'positive' });
        return makeRec1050(i);
      });
      const res = await runD1050(buildScenes1050(10), records1050b);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_SHIFT_EMOTIONAL_AFTERMATH_VOID'), 'DIALOGUE_SHIFT_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_SHIFT_EMOTIONAL_AFTERMATH_VOID does not fire when a relationship-shift scene is followed by an emotional shift within its window', async () => {
      const records1050bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1050(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.5 }] });
        if (i === 1 || i === 9) return makeRec1050(i, { emotionalShift: 'positive' });
        return makeRec1050(i);
      });
      const res = await runD1050(buildScenes1050(10), records1050bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_SHIFT_EMOTIONAL_AFTERMATH_VOID'), 'DIALOGUE_SHIFT_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_SEED_STAGING_AFTERMATH_VOID fires when every seed is followed by two scenes with no substantial staging', async () => {
      const records1050c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1050(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeRec1050(i, { visualBeats: ['beat1', 'beat2'] });
        return makeRec1050(i);
      });
      const res = await runD1050(buildScenes1050(10), records1050c);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_SEED_STAGING_AFTERMATH_VOID'), 'DIALOGUE_SEED_STAGING_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_SEED_STAGING_AFTERMATH_VOID does not fire when a seed is followed by substantial staging within its window', async () => {
      const records1050cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1050(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeRec1050(i, { visualBeats: ['beat1', 'beat2'] });
        return makeRec1050(i);
      });
      const res = await runD1050(buildScenes1050(10), records1050cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_SEED_STAGING_AFTERMATH_VOID'), 'DIALOGUE_SEED_STAGING_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1036 — dialoguePass: dialogue open-thread-suspense aftermath void, dialogue seed-emotional aftermath void, dialogue shift-curiosity aftermath void', async () => {
    const makeRec1036 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: `INT. SC${idx} - DAY`,
      emotionalShift: 'neutral', suspenseDelta: 0, curiosityDelta: 0,
      clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [], relationshipShifts: [], visualBeats: [],
    ...
    const buildScenes1036 = (count: number): string => {
      let f = '';
      for (let i = 0; i < count; i++) {
        f += `INT. SCENE ${i} - DAY\n\nA figure moves through the room.\n\n`;
      }
    ...
    const runD1036 = async (fountain: string, records: any[] = []) => {
      const { dialoguePass } = await import('../../server/nvm/revision/passes/dialogue.ts');
      return dialoguePass({ fountain, original: fountain, records, structure: {} as any, annotations: [], approvedSpans: [] });
    };
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('DIALOGUE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no suspense rise', async () => {
      const records1036a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1036(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeRec1036(i, { suspenseDelta: 1 });
        return makeRec1036(i);
      });
      const res = await runD1036(buildScenes1036(10), records1036a);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID'), 'DIALOGUE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a suspense rise within its window', async () => {
      const records1036an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1036(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeRec1036(i, { suspenseDelta: 1 });
        return makeRec1036(i);
      });
      const res = await runD1036(buildScenes1036(10), records1036an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID'), 'DIALOGUE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_SEED_EMOTIONAL_AFTERMATH_VOID fires when every seed is followed by two scenes with no emotional shift', async () => {
      const records1036b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1036(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeRec1036(i, { emotionalShift: 'positive' });
        return makeRec1036(i);
      });
      const res = await runD1036(buildScenes1036(10), records1036b);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_SEED_EMOTIONAL_AFTERMATH_VOID'), 'DIALOGUE_SEED_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_SEED_EMOTIONAL_AFTERMATH_VOID does not fire when a seed is followed by an emotional shift within its window', async () => {
      const records1036bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1036(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeRec1036(i, { emotionalShift: 'positive' });
        return makeRec1036(i);
      });
      const res = await runD1036(buildScenes1036(10), records1036bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_SEED_EMOTIONAL_AFTERMATH_VOID'), 'DIALOGUE_SEED_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_SHIFT_CURIOSITY_AFTERMATH_VOID fires when every relationship-shift scene is followed by two scenes with no curiosity rise', async () => {
      const records1036c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1036(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.5 }] });
        if (i === 8 || i === 9) return makeRec1036(i, { curiosityDelta: 1 });
        return makeRec1036(i);
      });
      const res = await runD1036(buildScenes1036(10), records1036c);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_SHIFT_CURIOSITY_AFTERMATH_VOID'), 'DIALOGUE_SHIFT_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_SHIFT_CURIOSITY_AFTERMATH_VOID does not fire when a relationship-shift scene is followed by a curiosity rise within its window', async () => {
      const records1036cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1036(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.5 }] });
        if (i === 1 || i === 9) return makeRec1036(i, { curiosityDelta: 1 });
        return makeRec1036(i);
      });
      const res = await runD1036(buildScenes1036(10), records1036cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_SHIFT_CURIOSITY_AFTERMATH_VOID'), 'DIALOGUE_SHIFT_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1022 — dialoguePass: dialogue stakes-suspense aftermath void, dialogue seed-curiosity aftermath void, dialogue open-thread-emotional aftermath void', async () => {
    const makeRec1022 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: `INT. SC${idx} - DAY`,
      emotionalShift: 'neutral', suspenseDelta: 0, curiosityDelta: 0,
      clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [], relationshipShifts: [], visualBeats: [],
    ...
    const buildScenes1022 = (count: number): string => {
      let f = '';
      for (let i = 0; i < count; i++) {
        f += `INT. SCENE ${i} - DAY\n\nA figure moves through the room.\n\n`;
      }
    ...
    const runD1022 = async (fountain: string, records: any[] = []) => {
      const { dialoguePass } = await import('../../server/nvm/revision/passes/dialogue.ts');
      return dialoguePass({ fountain, original: fountain, records, structure: {} as any, annotations: [], approvedSpans: [] });
    };
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('DIALOGUE_STAKES_SUSPENSE_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no suspense rise', async () => {
      const records1022a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1022(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeRec1022(i, { suspenseDelta: 1 });
        return makeRec1022(i);
      });
      const res = await runD1022(buildScenes1022(10), records1022a);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_STAKES_SUSPENSE_AFTERMATH_VOID'), 'DIALOGUE_STAKES_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_STAKES_SUSPENSE_AFTERMATH_VOID does not fire when a stakes-raise is followed by a suspense rise within its window', async () => {
      const records1022an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1022(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeRec1022(i, { suspenseDelta: 1 });
        return makeRec1022(i);
      });
      const res = await runD1022(buildScenes1022(10), records1022an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_STAKES_SUSPENSE_AFTERMATH_VOID'), 'DIALOGUE_STAKES_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_SEED_CURIOSITY_AFTERMATH_VOID fires when every seed is followed by two scenes with no curiosity rise', async () => {
      const records1022b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1022(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeRec1022(i, { curiosityDelta: 1 });
        return makeRec1022(i);
      });
      const res = await runD1022(buildScenes1022(10), records1022b);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_SEED_CURIOSITY_AFTERMATH_VOID'), 'DIALOGUE_SEED_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_SEED_CURIOSITY_AFTERMATH_VOID does not fire when a seed is followed by a curiosity rise within its window', async () => {
      const records1022bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1022(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeRec1022(i, { curiosityDelta: 1 });
        return makeRec1022(i);
      });
      const res = await runD1022(buildScenes1022(10), records1022bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_SEED_CURIOSITY_AFTERMATH_VOID'), 'DIALOGUE_SEED_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('DIALOGUE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no emotional shift', async () => {
      const records1022c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1022(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeRec1022(i, { emotionalShift: 'positive' });
        return makeRec1022(i);
      });
      const res = await runD1022(buildScenes1022(10), records1022c);
      assert.ok(res.issues.some((i: any) => i.rule === 'DIALOGUE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID'), 'DIALOGUE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('DIALOGUE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by an emotional shift within its window', async () => {
      const records1022cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1022(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeRec1022(i, { emotionalShift: 'positive' });
        return makeRec1022(i);
      });
      const res = await runD1022(buildScenes1022(10), records1022cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'DIALOGUE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID'), 'DIALOGUE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });
