...
import { relationshipArcPass } from '../../server/nvm/revision/passes/relationship-arc.ts';
  ...
  describe('Wave 1155 — relationshipArcPass: relationship revelation-staging aftermath void, relationship turn-curiosity aftermath void, relationship payoff-suspense aftermath void', async () => {
    ...
    it('RELATIONSHIP_REVELATION_STAGING_AFTERMATH_VOID fires when every revelation is followed by two scenes with no heavily-staged scene', async () => {
      const recs1155a = Array.from({ length: 10 }, (_, i) => {
      ...
      const res = await runRA1155(recs1155a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_REVELATION_STAGING_AFTERMATH_VOID'), 'RELATIONSHIP_REVELATION_STAGING_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_REVELATION_STAGING_AFTERMATH_VOID does not fire when a revelation is followed by a heavily-staged scene within its window', async () => {
      const recs1155an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1155(recs1155an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_REVELATION_STAGING_AFTERMATH_VOID'), 'RELATIONSHIP_REVELATION_STAGING_AFTERMATH_VOID should not fire');
    });

    it('RELATIONSHIP_TURN_CURIOSITY_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no curiosity rise', async () => {
      const recs1155b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1155(recs1155b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_TURN_CURIOSITY_AFTERMATH_VOID'), 'RELATIONSHIP_TURN_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_TURN_CURIOSITY_AFTERMATH_VOID does not fire when a dramatic turn is followed by a curiosity rise within its window', async () => {
      const recs1155bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1155(recs1155bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_TURN_CURIOSITY_AFTERMATH_VOID'), 'RELATIONSHIP_TURN_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('RELATIONSHIP_PAYOFF_SUSPENSE_AFTERMATH_VOID fires when every payoff is followed by two scenes with no suspense rise', async () => {
      const recs1155c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1155(recs1155c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_PAYOFF_SUSPENSE_AFTERMATH_VOID'), 'RELATIONSHIP_PAYOFF_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_PAYOFF_SUSPENSE_AFTERMATH_VOID does not fire when a payoff is followed by a suspense rise within its window', async () => {
      const recs1155cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1155(recs1155cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_PAYOFF_SUSPENSE_AFTERMATH_VOID'), 'RELATIONSHIP_PAYOFF_SUSPENSE_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1141 — relationshipArcPass: relationship clock-dialogue-highlight aftermath void, relationship clock-staging aftermath void, relationship revelation-dialogue-highlight aftermath void', async () => {
    const runRA1141 = async (records: ScreenplaySceneRecord[]) => {
      const { relationshipArcPass } = await import('../../server/nvm/revision/passes/relationship-arc.ts');
      return relationshipArcPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('RELATIONSHIP_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no highlighted dialogue', async () => {
      const recs1141a = Array.from({ length: 10 }, (_, i) => {
      ...
      const res = await runRA1141(recs1141a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'RELATIONSHIP_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a clock-raise is followed by highlighted dialogue within its window', async () => {
      const recs1141an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1141(recs1141an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'RELATIONSHIP_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('RELATIONSHIP_CLOCK_STAGING_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no heavily-staged scene', async () => {
      const recs1141b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1141(recs1141b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_CLOCK_STAGING_AFTERMATH_VOID'), 'RELATIONSHIP_CLOCK_STAGING_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_CLOCK_STAGING_AFTERMATH_VOID does not fire when a clock-raise is followed by a heavily-staged scene within its window', async () => {
      const recs1141bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1141(recs1141bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_CLOCK_STAGING_AFTERMATH_VOID'), 'RELATIONSHIP_CLOCK_STAGING_AFTERMATH_VOID should not fire');
    });

    it('RELATIONSHIP_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every revelation is followed by two scenes with no highlighted dialogue', async () => {
      const recs1141c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'the truth about the letter' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1141(recs1141c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'RELATIONSHIP_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a revelation is followed by highlighted dialogue within its window', async () => {
      const recs1141cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'the truth about the letter' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1141(recs1141cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'RELATIONSHIP_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1127 — relationshipArcPass: relationship clock aftermath void, relationship revelation-emotional aftermath void, relationship revelation aftermath void', async () => {
    const runRA1127 = async (records: ScreenplaySceneRecord[]) => {
      const { relationshipArcPass } = await import('../../server/nvm/revision/passes/relationship-arc.ts');
      return relationshipArcPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('RELATIONSHIP_CLOCK_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no relationship shift', async () => {
      const recs1127a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runRA1127(recs1127a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_CLOCK_AFTERMATH_VOID'), 'RELATIONSHIP_CLOCK_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_CLOCK_AFTERMATH_VOID does not fire when a clock-raise is followed by a relationship shift within its window', async () => {
      const recs1127an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runRA1127(recs1127an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_CLOCK_AFTERMATH_VOID'), 'RELATIONSHIP_CLOCK_AFTERMATH_VOID should not fire');
    });

    it('RELATIONSHIP_REVELATION_EMOTIONAL_AFTERMATH_VOID fires when every revelation is followed by two scenes with no emotional shift', async () => {
      const recs1127b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'the truth about the letter' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runRA1127(recs1127b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_REVELATION_EMOTIONAL_AFTERMATH_VOID'), 'RELATIONSHIP_REVELATION_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_REVELATION_EMOTIONAL_AFTERMATH_VOID does not fire when a revelation is followed by an emotional shift within its window', async () => {
      const recs1127bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'the truth about the letter' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runRA1127(recs1127bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_REVELATION_EMOTIONAL_AFTERMATH_VOID'), 'RELATIONSHIP_REVELATION_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('RELATIONSHIP_REVELATION_AFTERMATH_VOID fires when every revelation is followed by two scenes with no relationship shift', async () => {
      const recs1127c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'the truth about the letter' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runRA1127(recs1127c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_REVELATION_AFTERMATH_VOID'), 'RELATIONSHIP_REVELATION_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_REVELATION_AFTERMATH_VOID does not fire when a revelation is followed by a relationship shift within its window', async () => {
      const recs1127cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'the truth about the letter' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runRA1127(recs1127cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_REVELATION_AFTERMATH_VOID'), 'RELATIONSHIP_REVELATION_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1113 — relationshipArcPass: relationship clock-suspense aftermath void, relationship clock-emotional aftermath void, relationship revelation-curiosity aftermath void', async () => {
    const runRA1113 = async (records: ScreenplaySceneRecord[]) => {
      const { relationshipArcPass } = await import('../../server/nvm/revision/passes/relationship-arc.ts');
      return relationshipArcPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('RELATIONSHIP_CLOCK_SUSPENSE_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no suspense rise', async () => {
      const recs1113a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1113(recs1113a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_CLOCK_SUSPENSE_AFTERMATH_VOID'), 'RELATIONSHIP_CLOCK_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_CLOCK_SUSPENSE_AFTERMATH_VOID does not fire when a clock-raise is followed by a suspense rise within its window', async () => {
      const recs1113an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1113(recs1113an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_CLOCK_SUSPENSE_AFTERMATH_VOID'), 'RELATIONSHIP_CLOCK_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('RELATIONSHIP_CLOCK_EMOTIONAL_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no emotional shift', async () => {
      const recs1113b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runRA1113(recs1113b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_CLOCK_EMOTIONAL_AFTERMATH_VOID'), 'RELATIONSHIP_CLOCK_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_CLOCK_EMOTIONAL_AFTERMATH_VOID does not fire when a clock-raise is followed by an emotional shift within its window', async () => {
      const recs1113bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runRA1113(recs1113bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_CLOCK_EMOTIONAL_AFTERMATH_VOID'), 'RELATIONSHIP_CLOCK_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('RELATIONSHIP_REVELATION_CURIOSITY_AFTERMATH_VOID fires when every revelation is followed by two scenes with no curiosity rise', async () => {
      const recs1113c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'the truth about the letter' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1113(recs1113c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_REVELATION_CURIOSITY_AFTERMATH_VOID'), 'RELATIONSHIP_REVELATION_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_REVELATION_CURIOSITY_AFTERMATH_VOID does not fire when a revelation is followed by a curiosity rise within its window', async () => {
      const recs1113cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'the truth about the letter' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1113(recs1113cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_REVELATION_CURIOSITY_AFTERMATH_VOID'), 'RELATIONSHIP_REVELATION_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1085 — relationshipArcPass: relationship-shift magnitude-curiosity aftermath void, relationship-shift magnitude-suspense aftermath void, relationship-shift highlight aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('RELATIONSHIP_SHIFT_MAGNITUDE_CURIOSITY_AFTERMATH_VOID fires when every significant shift is followed by two scenes with no curiosity rise', async () => {
      const recs1085a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.5 }] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1085(recs1085a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_SHIFT_MAGNITUDE_CURIOSITY_AFTERMATH_VOID'), 'RELATIONSHIP_SHIFT_MAGNITUDE_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_SHIFT_MAGNITUDE_CURIOSITY_AFTERMATH_VOID does not fire when a significant shift is followed by a curiosity rise within its window', async () => {
      const recs1085an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.5 }] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1085(recs1085an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_SHIFT_MAGNITUDE_CURIOSITY_AFTERMATH_VOID'), 'RELATIONSHIP_SHIFT_MAGNITUDE_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('RELATIONSHIP_SHIFT_MAGNITUDE_SUSPENSE_AFTERMATH_VOID fires when every significant shift is followed by two scenes with no suspense rise', async () => {
      const recs1085b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.5 }] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1085(recs1085b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_SHIFT_MAGNITUDE_SUSPENSE_AFTERMATH_VOID'), 'RELATIONSHIP_SHIFT_MAGNITUDE_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_SHIFT_MAGNITUDE_SUSPENSE_AFTERMATH_VOID does not fire when a significant shift is followed by a suspense rise within its window', async () => {
      const recs1085bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.5 }] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1085(recs1085bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_SHIFT_MAGNITUDE_SUSPENSE_AFTERMATH_VOID'), 'RELATIONSHIP_SHIFT_MAGNITUDE_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('RELATIONSHIP_SHIFT_HIGHLIGHT_AFTERMATH_VOID fires when every relationship shift is followed by two scenes with no highlighted dialogue', async () => {
      const recs1085c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.1 }] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1085(recs1085c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONSHIP_SHIFT_HIGHLIGHT_AFTERMATH_VOID'), 'RELATIONSHIP_SHIFT_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('RELATIONSHIP_SHIFT_HIGHLIGHT_AFTERMATH_VOID does not fire when a relationship shift is followed by highlighted dialogue within its window', async () => {
      const recs1085cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 0.1 }] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1085(recs1085cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONSHIP_SHIFT_HIGHLIGHT_AFTERMATH_VOID'), 'RELATIONSHIP_SHIFT_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1057 — relationshipArcPass: relational seed-dialogue-highlight aftermath void, relational open-thread-dialogue-highlight aftermath void, relational stakes-staging aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('RELATIONAL_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every seed is followed by two scenes with no highlighted dialogue', async () => {
      const recs1057a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1057(recs1057a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'RELATIONAL_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a seed is followed by highlighted dialogue within its window', async () => {
      const recs1057an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1057(recs1057an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'RELATIONAL_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('RELATIONAL_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no highlighted dialogue', async () => {
      const recs1057b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1057(recs1057b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'RELATIONAL_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by highlighted dialogue within its window', async () => {
      const recs1057bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1057(recs1057bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'RELATIONAL_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('RELATIONAL_STAKES_STAGING_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no visually dense scene', async () => {
      const recs1057c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1057(recs1057c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_STAKES_STAGING_AFTERMATH_VOID'), 'RELATIONAL_STAKES_STAGING_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_STAKES_STAGING_AFTERMATH_VOID does not fire when a stakes-raise is followed by a visually dense scene within its window', async () => {
      const recs1057cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1057(recs1057cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_STAKES_STAGING_AFTERMATH_VOID'), 'RELATIONAL_STAKES_STAGING_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1043 — relationshipArcPass: relational stakes-dialogue-highlight aftermath void, relational seed-suspense aftermath void, relational open-thread-suspense aftermath void', async () => {
    const runRA1043 = async (records: ScreenplaySceneRecord[]) => {
      const { relationshipArcPass } = await import('../../server/nvm/revision/passes/relationship-arc.ts');
      return relationshipArcPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('RELATIONAL_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no highlighted dialogue', async () => {
      const recs1043a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1043(recs1043a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'RELATIONAL_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a stakes-raise is followed by highlighted dialogue within its window', async () => {
      const recs1043an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runRA1043(recs1043an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'RELATIONAL_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('RELATIONAL_SEED_SUSPENSE_AFTERMATH_VOID fires when every seed is followed by two scenes with no suspense rise', async () => {
      const recs1043b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1043(recs1043b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_SEED_SUSPENSE_AFTERMATH_VOID'), 'RELATIONAL_SEED_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_SEED_SUSPENSE_AFTERMATH_VOID does not fire when a seed is followed by a suspense rise within its window', async () => {
      const recs1043bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1043(recs1043bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_SEED_SUSPENSE_AFTERMATH_VOID'), 'RELATIONAL_SEED_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('RELATIONAL_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no suspense rise', async () => {
      const recs1043c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1043(recs1043c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID'), 'RELATIONAL_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a suspense rise within its window', async () => {
      const recs1043cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1043(recs1043cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID'), 'RELATIONAL_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1029 — relationshipArcPass: relational stakes-emotional aftermath void, relational seed aftermath void, relational open-thread aftermath void', async () => {
    const runRA1029 = async (records: ScreenplaySceneRecord[]) => {
      const { relationshipArcPass } = await import('../../server/nvm/revision/passes/relationship-arc.ts');
      return relationshipArcPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('RELATIONAL_STAKES_EMOTIONAL_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no emotional shift', async () => {
      const recs1029a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runRA1029(recs1029a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'RELATIONAL_STAKES_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_STAKES_EMOTIONAL_AFTERMATH_VOID does not fire when a stakes-raise is followed by an emotional shift within its window', async () => {
      const recs1029an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runRA1029(recs1029an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'RELATIONAL_STAKES_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('RELATIONAL_SEED_AFTERMATH_VOID fires when every seed is followed by two scenes with no relationship shift', async () => {
      const recs1029b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runRA1029(recs1029b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_SEED_AFTERMATH_VOID'), 'RELATIONAL_SEED_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_SEED_AFTERMATH_VOID does not fire when a seed is followed by a relationship shift within its window', async () => {
      const recs1029bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runRA1029(recs1029bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_SEED_AFTERMATH_VOID'), 'RELATIONAL_SEED_AFTERMATH_VOID should not fire');
    });

    it('RELATIONAL_OPEN_THREAD_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no relationship shift', async () => {
      const recs1029c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runRA1029(recs1029c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_OPEN_THREAD_AFTERMATH_VOID'), 'RELATIONAL_OPEN_THREAD_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_OPEN_THREAD_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a relationship shift within its window', async () => {
      const recs1029cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runRA1029(recs1029cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_OPEN_THREAD_AFTERMATH_VOID'), 'RELATIONAL_OPEN_THREAD_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1015 — relationshipArcPass: relational seed-emotional aftermath void, relational open-thread-curiosity aftermath void, relational stakes-suspense aftermath void', async () => {
    const runRA1015 = async (records: ScreenplaySceneRecord[]) => {
      const { relationshipArcPass } = await import('../../server/nvm/revision/passes/relationship-arc.ts');
      return relationshipArcPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('RELATIONAL_SEED_EMOTIONAL_AFTERMATH_VOID fires when every seed is followed by two scenes with no emotional shift', async () => {
      const recs1015a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runRA1015(recs1015a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_SEED_EMOTIONAL_AFTERMATH_VOID'), 'RELATIONAL_SEED_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_SEED_EMOTIONAL_AFTERMATH_VOID does not fire when a seed is followed by an emotional shift within its window', async () => {
      const recs1015an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runRA1015(recs1015an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_SEED_EMOTIONAL_AFTERMATH_VOID'), 'RELATIONAL_SEED_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('RELATIONAL_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no new curiosity', async () => {
      const recs1015b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1015(recs1015b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID'), 'RELATIONAL_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by new curiosity within its window', async () => {
      const recs1015bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1015(recs1015bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID'), 'RELATIONAL_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('RELATIONAL_STAKES_SUSPENSE_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no rise in suspense', async () => {
      const recs1015c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1015(recs1015c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_STAKES_SUSPENSE_AFTERMATH_VOID'), 'RELATIONAL_STAKES_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_STAKES_SUSPENSE_AFTERMATH_VOID does not fire when a stakes-raise is followed by rising suspense within its window', async () => {
      const recs1015cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1015(recs1015cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_STAKES_SUSPENSE_AFTERMATH_VOID'), 'RELATIONAL_STAKES_SUSPENSE_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1001 — relationshipArcPass: relational stakes-curiosity aftermath void, relational seed-curiosity aftermath void, relational open-thread-emotional aftermath void', async () => {
    const runRA1001 = async (records: ScreenplaySceneRecord[]) => {
      const { relationshipArcPass } = await import('../../server/nvm/revision/passes/relationship-arc.ts');
      return relationshipArcPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('RELATIONAL_STAKES_CURIOSITY_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no new curiosity', async () => {
      const recs1001a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1001(recs1001a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_STAKES_CURIOSITY_AFTERMATH_VOID'), 'RELATIONAL_STAKES_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_STAKES_CURIOSITY_AFTERMATH_VOID does not fire when a stakes-raise is followed by new curiosity within its window', async () => {
      const recs1001an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1001(recs1001an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_STAKES_CURIOSITY_AFTERMATH_VOID'), 'RELATIONAL_STAKES_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('RELATIONAL_SEED_CURIOSITY_AFTERMATH_VOID fires when every seed is followed by two scenes with no new curiosity', async () => {
      const recs1001b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1001(recs1001b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_SEED_CURIOSITY_AFTERMATH_VOID'), 'RELATIONAL_SEED_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_SEED_CURIOSITY_AFTERMATH_VOID does not fire when a seed is followed by new curiosity within its window', async () => {
      const recs1001bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runRA1001(recs1001bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_SEED_CURIOSITY_AFTERMATH_VOID'), 'RELATIONAL_SEED_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('RELATIONAL_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no emotional shift', async () => {
      const recs1001c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runRA1001(recs1001c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID'), 'RELATIONAL_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('RELATIONAL_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by an emotional shift within its window', async () => {
      const recs1001cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runRA1001(recs1001cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID'), 'RELATIONAL_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 959 — relationshipArcPass: relational suspense zone imbalance, relational revelation zone imbalance, relational seed zone imbalance', async () => {
    ...
    // Zone geometry n=10: Z0={0,1,2}, Z1={3,4}, Z2={5,6,7}, Z3={8,9}. Target at 0,1,2,8,9 →
    // Z0 3/5=60% (bloat), Z1 and Z2 empty → fires. Target at 0,3,5,8 → every zone touched → no-fire.
    it('RELATIONAL_SUSPENSE_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of suspense-raising scenes', async () => {
      const recs959a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { suspenseDelta: [0, 1, 2, 8, 9].includes(i) ? 1 : 0 }));
      const res = await runRA959(recs959a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_SUSPENSE_ZONE_IMBALANCE'), 'RELATIONAL_SUSPENSE_ZONE_IMBALANCE should fire');
    });

    it('RELATIONAL_SUSPENSE_ZONE_IMBALANCE does not fire when suspense-raising scenes touch every zone', async () => {
      const recs959an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { suspenseDelta: [0, 3, 5, 8].includes(i) ? 1 : 0 }));
      const res = await runRA959(recs959an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_SUSPENSE_ZONE_IMBALANCE'), 'RELATIONAL_SUSPENSE_ZONE_IMBALANCE should not fire');
    });

    it('RELATIONAL_REVELATION_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of revelation scenes', async () => {
      const recs959b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { revelation: [0, 1, 2, 8, 9].includes(i) ? 'a hidden truth surfaces' : null }));
      const res = await runRA959(recs959b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_REVELATION_ZONE_IMBALANCE'), 'RELATIONAL_REVELATION_ZONE_IMBALANCE should fire');
    });

    it('RELATIONAL_REVELATION_ZONE_IMBALANCE does not fire when revelation scenes touch every zone', async () => {
      const recs959bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { revelation: [0, 3, 5, 8].includes(i) ? 'a hidden truth surfaces' : null }));
      const res = await runRA959(recs959bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_REVELATION_ZONE_IMBALANCE'), 'RELATIONAL_REVELATION_ZONE_IMBALANCE should not fire');
    });

    it('RELATIONAL_SEED_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of seeding scenes', async () => {
      const recs959c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { seededClueIds: [0, 1, 2, 8, 9].includes(i) ? ['c1'] : [] }));
      const res = await runRA959(recs959c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_SEED_ZONE_IMBALANCE'), 'RELATIONAL_SEED_ZONE_IMBALANCE should fire');
    });

    it('RELATIONAL_SEED_ZONE_IMBALANCE does not fire when seeding scenes touch every zone', async () => {
      const recs959cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { seededClueIds: [0, 3, 5, 8].includes(i) ? ['c1'] : [] }));
      const res = await runRA959(recs959cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_SEED_ZONE_IMBALANCE'), 'RELATIONAL_SEED_ZONE_IMBALANCE should not fire');
    });
  });

  describe('Wave 945 — relationshipArcPass: relational positive emotion zone imbalance, relational curiosity zone imbalance, relational payoff zone imbalance', async () => {
    const runRA945 = async (records: ScreenplaySceneRecord[]) => {
      const { relationshipArcPass } = await import('../../server/nvm/revision/passes/relationship-arc.ts');
      return relationshipArcPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // Zone geometry n=10: Z0={0,1,2}, Z1={3,4}, Z2={5,6,7}, Z3={8,9}. Target at 0,1,2,8,9 →
    // Z0 3/5=60% (bloat), Z1 and Z2 empty → fires. Target at 0,3,5,8 → every zone touched → no-fire.
    it('RELATIONAL_POSITIVE_EMOTION_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of positive-shift scenes', async () => {
      const recs945a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2, 8, 9].includes(i) ? 'positive' : 'neutral' }));
      const res = await runRA945(recs945a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_POSITIVE_EMOTION_ZONE_IMBALANCE'), 'RELATIONAL_POSITIVE_EMOTION_ZONE_IMBALANCE should fire');
    });

    it('RELATIONAL_POSITIVE_EMOTION_ZONE_IMBALANCE does not fire when positive-shift scenes touch every zone', async () => {
      const recs945an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 3, 5, 8].includes(i) ? 'positive' : 'neutral' }));
      const res = await runRA945(recs945an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_POSITIVE_EMOTION_ZONE_IMBALANCE'), 'RELATIONAL_POSITIVE_EMOTION_ZONE_IMBALANCE should not fire');
    });

    it('RELATIONAL_CURIOSITY_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of curiosity-raising scenes', async () => {
      const recs945b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { curiosityDelta: [0, 1, 2, 8, 9].includes(i) ? 1 : 0 }));
      const res = await runRA945(recs945b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_CURIOSITY_ZONE_IMBALANCE'), 'RELATIONAL_CURIOSITY_ZONE_IMBALANCE should fire');
    });

    it('RELATIONAL_CURIOSITY_ZONE_IMBALANCE does not fire when curiosity-raising scenes touch every zone', async () => {
      const recs945bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { curiosityDelta: [0, 3, 5, 8].includes(i) ? 1 : 0 }));
      const res = await runRA945(recs945bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_CURIOSITY_ZONE_IMBALANCE'), 'RELATIONAL_CURIOSITY_ZONE_IMBALANCE should not fire');
    });

    it('RELATIONAL_PAYOFF_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of payoff scenes', async () => {
      const recs945c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { payoffSetupIds: [0, 1, 2, 8, 9].includes(i) ? ['s1'] : [] }));
      const res = await runRA945(recs945c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_PAYOFF_ZONE_IMBALANCE'), 'RELATIONAL_PAYOFF_ZONE_IMBALANCE should fire');
    });

    it('RELATIONAL_PAYOFF_ZONE_IMBALANCE does not fire when payoff scenes touch every zone', async () => {
      const recs945cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { payoffSetupIds: [0, 3, 5, 8].includes(i) ? ['s1'] : [] }));
      const res = await runRA945(recs945cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_PAYOFF_ZONE_IMBALANCE'), 'RELATIONAL_PAYOFF_ZONE_IMBALANCE should not fire');
    });
  });
  ...
  describe('Wave 917 — relationshipArcPass: relational revelation purpose zone cluster, relational revelation purpose drought run, relational character moment zone imbalance', async () => {
    ...
    // RELATIONAL_REVELATION_PURPOSE_ZONE_CLUSTER fire: n=9, 3 thirds; revelation-purposed scenes
    // at 0,1,2 (opening third) → 3/3 = 100% > 75%. Filler 'establish_world'.
    it('RELATIONAL_REVELATION_PURPOSE_ZONE_CLUSTER fires when >75% of revelation-purposed scenes cluster in one third', async () => {
      const recs917a = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runRA917(recs917a);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_REVELATION_PURPOSE_ZONE_CLUSTER'), 'RELATIONAL_REVELATION_PURPOSE_ZONE_CLUSTER should fire');
    });

    it('RELATIONAL_REVELATION_PURPOSE_ZONE_CLUSTER does not fire when revelation-purposed scenes spread across thirds', async () => {
      const recs917an = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runRA917(recs917an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_REVELATION_PURPOSE_ZONE_CLUSTER'), 'RELATIONAL_REVELATION_PURPOSE_ZONE_CLUSTER should not fire');
    });

    // RELATIONAL_REVELATION_PURPOSE_DROUGHT_RUN fire: n=10, revelation-purposed scenes at 0, 8, 9
    // (minPresentCount 3), leaving a 7-scene gap (indices 1-7) — run of 7 >= threshold 6.
    it('RELATIONAL_REVELATION_PURPOSE_DROUGHT_RUN fires when a long run has no revelation-purposed scene', async () => {
      const recs917b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 8, 9].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runRA917(recs917b);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_REVELATION_PURPOSE_DROUGHT_RUN'), 'RELATIONAL_REVELATION_PURPOSE_DROUGHT_RUN should fire');
    });

    it('RELATIONAL_REVELATION_PURPOSE_DROUGHT_RUN does not fire when revelation-purposed scenes are evenly spread', async () => {
      const recs917bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runRA917(recs917bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_REVELATION_PURPOSE_DROUGHT_RUN'), 'RELATIONAL_REVELATION_PURPOSE_DROUGHT_RUN should not fire');
    });

    // RELATIONAL_CHARACTER_MOMENT_ZONE_IMBALANCE fire: n=10, Z0={0,1,2}, Z1={3,4}, Z2={5,6,7},
    // Z3={8,9}; character_moment at 0,1,2,8,9 → Z0 3/5=60% (bloat), Z1 and Z2 empty.
    it('RELATIONAL_CHARACTER_MOMENT_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of character-moment scenes', async () => {
      const recs917c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2, 8, 9].includes(i) ? 'character_moment' : 'establish_world' }));
      const res = await runRA917(recs917c);
      assert.ok(res.issues.some((i: any) => i.rule === 'RELATIONAL_CHARACTER_MOMENT_ZONE_IMBALANCE'), 'RELATIONAL_CHARACTER_MOMENT_ZONE_IMBALANCE should fire');
    });

    it('RELATIONAL_CHARACTER_MOMENT_ZONE_IMBALANCE does not fire when character-moment scenes touch every zone', async () => {
      const recs917cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 5, 8].includes(i) ? 'character_moment' : 'establish_world' }));
      const res = await runRA917(recs917cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'RELATIONAL_CHARACTER_MOMENT_ZONE_IMBALANCE'), 'RELATIONAL_CHARACTER_MOMENT_ZONE_IMBALANCE should not fire');
    });
  });
