...
`;
  ...
  describe('Wave 1166 — originalityPass: originality revelation-dialogue-highlight aftermath void, originality turn-staging aftermath void, originality turn-dialogue-highlight aftermath void', async () => {
    ...
    const PURPOSE_POOL_1166 = ['complicate', 'introduce_conflict', 'establish_world', 'character_moment'];
    const SENTENCE_POOL_1166 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor1166 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec1166 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor1166(idx),
      emotionalShift: 'neutral',
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_1166[idx % PURPOSE_POOL_1166.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain1166 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor1166(i)}\n\n${SENTENCE_POOL_1166[i % SENTENCE_POOL_1166.length]}`).join('\n\n');
    const runO1166 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain1166(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    it('ORIGINALITY_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every revelation is followed by two scenes with no highlighted dialogue', async () => {
      const recs1166a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1166(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeRec1166(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1166(i);
      });
      const res = await runO1166(recs1166a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'ORIGINALITY_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a revelation is followed by highlighted dialogue within its window', async () => {
      const recs1166an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1166(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeRec1166(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1166(i);
      });
      const res = await runO1166(recs1166an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'ORIGINALITY_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_TURN_STAGING_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no heavily-staged scene', async () => {
      const recs1166b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1166(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeRec1166(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1166(i);
      });
      const res = await runO1166(recs1166b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_TURN_STAGING_AFTERMATH_VOID'), 'ORIGINALITY_TURN_STAGING_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_TURN_STAGING_AFTERMATH_VOID does not fire when a dramatic turn is followed by a heavily-staged scene within its window', async () => {
      const recs1166bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1166(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeRec1166(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1166(i);
      });
      const res = await runO1166(recs1166bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_TURN_STAGING_AFTERMATH_VOID'), 'ORIGINALITY_TURN_STAGING_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no highlighted dialogue', async () => {
      const recs1166c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1166(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeRec1166(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1166(i);
      });
      const res = await runO1166(recs1166c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'ORIGINALITY_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a dramatic turn is followed by highlighted dialogue within its window', async () => {
      const recs1166cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1166(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeRec1166(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1166(i);
      });
      const res = await runO1166(recs1166cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'ORIGINALITY_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1138 — originalityPass: originality revelation-emotional aftermath void, originality turn-suspense aftermath void, originality turn-curiosity aftermath void', async () => {
    ...
    // (revelation string / dramaticTurn string) never overlap the filler purpose pool, so
    // cycling never contaminates a fixture.
    const PURPOSE_POOL_1138 = ['complicate', 'introduce_conflict', 'establish_world', 'character_moment'];
    const SENTENCE_POOL_1138 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor1138 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec1138 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor1138(idx),
      emotionalShift: 'neutral',
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_1138[idx % PURPOSE_POOL_1138.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain1138 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor1138(i)}\n\n${SENTENCE_POOL_1138[i % SENTENCE_POOL_1138.length]}`).join('\n\n');
    const runO1138 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain1138(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('ORIGINALITY_REVELATION_EMOTIONAL_AFTERMATH_VOID fires when every revelation is followed by two scenes with no emotional shift', async () => {
      const recs1138a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1138(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeRec1138(i, { emotionalShift: 'positive' });
        return makeRec1138(i);
      });
      const res = await runO1138(recs1138a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_REVELATION_EMOTIONAL_AFTERMATH_VOID'), 'ORIGINALITY_REVELATION_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_REVELATION_EMOTIONAL_AFTERMATH_VOID does not fire when a revelation is followed by an emotional shift within its window', async () => {
      const recs1138an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1138(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeRec1138(i, { emotionalShift: 'positive' });
        return makeRec1138(i);
      });
      const res = await runO1138(recs1138an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_REVELATION_EMOTIONAL_AFTERMATH_VOID'), 'ORIGINALITY_REVELATION_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_TURN_SUSPENSE_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no suspense rise', async () => {
      const recs1138b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1138(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeRec1138(i, { suspenseDelta: 1 });
        return makeRec1138(i);
      });
      const res = await runO1138(recs1138b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_TURN_SUSPENSE_AFTERMATH_VOID'), 'ORIGINALITY_TURN_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_TURN_SUSPENSE_AFTERMATH_VOID does not fire when a dramatic turn is followed by a suspense rise within its window', async () => {
      const recs1138bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1138(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeRec1138(i, { suspenseDelta: 1 });
        return makeRec1138(i);
      });
      const res = await runO1138(recs1138bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_TURN_SUSPENSE_AFTERMATH_VOID'), 'ORIGINALITY_TURN_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_TURN_CURIOSITY_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no curiosity rise', async () => {
      const recs1138c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1138(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeRec1138(i, { curiosityDelta: 1 });
        return makeRec1138(i);
      });
      const res = await runO1138(recs1138c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_TURN_CURIOSITY_AFTERMATH_VOID'), 'ORIGINALITY_TURN_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_TURN_CURIOSITY_AFTERMATH_VOID does not fire when a dramatic turn is followed by a curiosity rise within its window', async () => {
      const recs1138cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1138(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeRec1138(i, { curiosityDelta: 1 });
        return makeRec1138(i);
      });
      const res = await runO1138(recs1138cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_TURN_CURIOSITY_AFTERMATH_VOID'), 'ORIGINALITY_TURN_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1124 — originalityPass: originality revelation-curiosity aftermath void, originality revelation-suspense aftermath void, originality turn-emotional aftermath void', async () => {
    // Same truncation pitfall as Waves 592/606/…/1096/1110 above — every fixture cycles purpose/
    // sentence/slug per scene to avoid tripping unrelated 'major' rules that would crowd these
    // 'minor' checks out of originality's top-8-by-severity slice. The tested signals here
    // (revelation string / dramaticTurn string) never overlap the filler purpose pool, so
    // cycling never contaminates a fixture.
    const PURPOSE_POOL_1124 = ['complicate', 'introduce_conflict', 'establish_world', 'character_moment'];
    const SENTENCE_POOL_1124 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor1124 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec1124 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor1124(idx),
      emotionalShift: 'neutral',
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_1124[idx % PURPOSE_POOL_1124.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain1124 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor1124(i)}\n\n${SENTENCE_POOL_1124[i % SENTENCE_POOL_1124.length]}`).join('\n\n');
    const runO1124 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain1124(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('ORIGINALITY_REVELATION_CURIOSITY_AFTERMATH_VOID fires when every revelation is followed by two scenes with no curiosity rise', async () => {
      const recs1124a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1124(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeRec1124(i, { curiosityDelta: 1 });
        return makeRec1124(i);
      });
      const res = await runO1124(recs1124a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_REVELATION_CURIOSITY_AFTERMATH_VOID'), 'ORIGINALITY_REVELATION_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_REVELATION_CURIOSITY_AFTERMATH_VOID does not fire when a revelation is followed by a curiosity rise within its window', async () => {
      const recs1124an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1124(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeRec1124(i, { curiosityDelta: 1 });
        return makeRec1124(i);
      });
      const res = await runO1124(recs1124an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_REVELATION_CURIOSITY_AFTERMATH_VOID'), 'ORIGINALITY_REVELATION_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_REVELATION_SUSPENSE_AFTERMATH_VOID fires when every revelation is followed by two scenes with no suspense rise', async () => {
      const recs1124b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1124(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeRec1124(i, { suspenseDelta: 1 });
        return makeRec1124(i);
      });
      const res = await runO1124(recs1124b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_REVELATION_SUSPENSE_AFTERMATH_VOID'), 'ORIGINALITY_REVELATION_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_REVELATION_SUSPENSE_AFTERMATH_VOID does not fire when a revelation is followed by a suspense rise within its window', async () => {
      const recs1124bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1124(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeRec1124(i, { suspenseDelta: 1 });
        return makeRec1124(i);
      });
      const res = await runO1124(recs1124bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_REVELATION_SUSPENSE_AFTERMATH_VOID'), 'ORIGINALITY_REVELATION_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_TURN_EMOTIONAL_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no emotional shift', async () => {
      const recs1124c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1124(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeRec1124(i, { emotionalShift: 'positive' });
        return makeRec1124(i);
      });
      const res = await runO1124(recs1124c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_TURN_EMOTIONAL_AFTERMATH_VOID'), 'ORIGINALITY_TURN_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_TURN_EMOTIONAL_AFTERMATH_VOID does not fire when a dramatic turn is followed by an emotional shift within its window', async () => {
      const recs1124cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1124(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeRec1124(i, { emotionalShift: 'positive' });
        return makeRec1124(i);
      });
      const res = await runO1124(recs1124cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_TURN_EMOTIONAL_AFTERMATH_VOID'), 'ORIGINALITY_TURN_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1110 — originalityPass: originality payoff-staging aftermath void, originality open-thread-relational aftermath void, originality open-thread-staging aftermath void', async () => {
    // Same truncation pitfall as Waves 592/606/…/1082/1096 above — every fixture cycles purpose/
    // sentence/slug per scene to avoid tripping unrelated 'major' rules that would crowd these
    // 'minor' checks out of originality's top-8-by-severity slice. The tested signals here
    // (payoffSetupIds array / unresolvedClues array length>=3) never overlap the filler purpose
    // pool, so cycling never contaminates a fixture.
    const PURPOSE_POOL_1110 = ['turning_point', 'complicate', 'introduce_conflict', 'establish_world'];
    const SENTENCE_POOL_1110 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor1110 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec1110 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor1110(idx),
      emotionalShift: 'neutral',
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_1110[idx % PURPOSE_POOL_1110.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain1110 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor1110(i)}\n\n${SENTENCE_POOL_1110[i % SENTENCE_POOL_1110.length]}`).join('\n\n');
    const runO1110 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain1110(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('ORIGINALITY_PAYOFF_STAGING_AFTERMATH_VOID fires when every payoff is followed by two scenes with no visually dense scene', async () => {
      const recs1110a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1110(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeRec1110(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1110(i);
      });
      const res = await runO1110(recs1110a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_PAYOFF_STAGING_AFTERMATH_VOID'), 'ORIGINALITY_PAYOFF_STAGING_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_PAYOFF_STAGING_AFTERMATH_VOID does not fire when a payoff is followed by a visually dense scene within its window', async () => {
      const recs1110an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1110(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeRec1110(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1110(i);
      });
      const res = await runO1110(recs1110an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_PAYOFF_STAGING_AFTERMATH_VOID'), 'ORIGINALITY_PAYOFF_STAGING_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no relationship shift', async () => {
      const recs1110b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1110(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeRec1110(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeRec1110(i);
      });
      const res = await runO1110(recs1110b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID'), 'ORIGINALITY_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a relationship shift within its window', async () => {
      const recs1110bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1110(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeRec1110(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeRec1110(i);
      });
      const res = await runO1110(recs1110bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID'), 'ORIGINALITY_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_OPEN_THREAD_STAGING_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no visually dense scene', async () => {
      const recs1110c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1110(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeRec1110(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1110(i);
      });
      const res = await runO1110(recs1110c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_OPEN_THREAD_STAGING_AFTERMATH_VOID'), 'ORIGINALITY_OPEN_THREAD_STAGING_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_OPEN_THREAD_STAGING_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a visually dense scene within its window', async () => {
      const recs1110cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1110(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeRec1110(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1110(i);
      });
      const res = await runO1110(recs1110cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_OPEN_THREAD_STAGING_AFTERMATH_VOID'), 'ORIGINALITY_OPEN_THREAD_STAGING_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1082 — originalityPass: originality stakes-staging aftermath void, originality clock-dialogue-highlight aftermath void, originality seed-dialogue-highlight aftermath void', async () => {
    ...
    // sentence/slug per scene to avoid tripping unrelated 'major' rules that would crowd these
    // 'minor' checks out of originality's top-8-by-severity slice. The tested signals here
    // (purpose==='raise_stakes' / clockRaised bool / seededClueIds array) never overlap the
    // filler purpose pool, so cycling never contaminates a fixture.
    const PURPOSE_POOL_1082 = ['turning_point', 'complicate', 'introduce_conflict', 'establish_world'];
    const SENTENCE_POOL_1082 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor1082 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec1082 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor1082(idx),
      emotionalShift: 'neutral',
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_1082[idx % PURPOSE_POOL_1082.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain1082 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor1082(i)}\n\n${SENTENCE_POOL_1082[i % SENTENCE_POOL_1082.length]}`).join('\n\n');
    const runO1082 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain1082(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('ORIGINALITY_STAKES_STAGING_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no visually dense scene', async () => {
      const recs1082a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1082(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeRec1082(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1082(i);
      });
      const res = await runO1082(recs1082a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_STAKES_STAGING_AFTERMATH_VOID'), 'ORIGINALITY_STAKES_STAGING_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_STAKES_STAGING_AFTERMATH_VOID does not fire when a stakes-raise is followed by a visually dense scene within its window', async () => {
      const recs1082an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1082(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeRec1082(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1082(i);
      });
      const res = await runO1082(recs1082an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_STAKES_STAGING_AFTERMATH_VOID'), 'ORIGINALITY_STAKES_STAGING_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no highlighted dialogue', async () => {
      const recs1082b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1082(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeRec1082(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1082(i);
      });
      const res = await runO1082(recs1082b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'ORIGINALITY_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a clock-raise is followed by highlighted dialogue within its window', async () => {
      const recs1082bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1082(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeRec1082(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1082(i);
      });
      const res = await runO1082(recs1082bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'ORIGINALITY_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every seed is followed by two scenes with no highlighted dialogue', async () => {
      const recs1082c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1082(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeRec1082(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1082(i);
      });
      const res = await runO1082(recs1082c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'ORIGINALITY_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a seed is followed by highlighted dialogue within its window', async () => {
      const recs1082cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1082(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeRec1082(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1082(i);
      });
      const res = await runO1082(recs1082cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'ORIGINALITY_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1054 — originalityPass: originality open-thread-suspense aftermath void, originality clock-suspense aftermath void, originality seed-relational aftermath void', async () => {
    ...
    // sentence/slug per scene to avoid tripping unrelated 'major' rules that would crowd these
    // 'minor' checks out of originality's top-8-by-severity slice. The tested signals here
    // (unresolvedClues array / clockRaised bool / seededClueIds array) never overlap the filler
    // purpose pool, so cycling never contaminates a fixture.
    const PURPOSE_POOL_1054 = ['turning_point', 'complicate', 'introduce_conflict', 'establish_world'];
    const SENTENCE_POOL_1054 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor1054 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec1054 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor1054(idx),
      emotionalShift: 'neutral',
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_1054[idx % PURPOSE_POOL_1054.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain1054 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor1054(i)}\n\n${SENTENCE_POOL_1054[i % SENTENCE_POOL_1054.length]}`).join('\n\n');
    const runO1054 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain1054(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('ORIGINALITY_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no suspense rise', async () => {
      const recs1054a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1054(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeRec1054(i, { suspenseDelta: 1 });
        return makeRec1054(i);
      });
      const res = await runO1054(recs1054a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID'), 'ORIGINALITY_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a suspense rise within its window', async () => {
      const recs1054an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1054(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeRec1054(i, { suspenseDelta: 1 });
        return makeRec1054(i);
      });
      const res = await runO1054(recs1054an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID'), 'ORIGINALITY_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_CLOCK_SUSPENSE_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no suspense rise', async () => {
      const recs1054b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1054(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeRec1054(i, { suspenseDelta: 1 });
        return makeRec1054(i);
      });
      const res = await runO1054(recs1054b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_CLOCK_SUSPENSE_AFTERMATH_VOID'), 'ORIGINALITY_CLOCK_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_CLOCK_SUSPENSE_AFTERMATH_VOID does not fire when a clock-raise is followed by a suspense rise within its window', async () => {
      const recs1054bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1054(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeRec1054(i, { suspenseDelta: 1 });
        return makeRec1054(i);
      });
      const res = await runO1054(recs1054bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_CLOCK_SUSPENSE_AFTERMATH_VOID'), 'ORIGINALITY_CLOCK_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_SEED_RELATIONAL_AFTERMATH_VOID fires when every seed is followed by two scenes with no relationship shift', async () => {
      const recs1054c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1054(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeRec1054(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeRec1054(i);
      });
      const res = await runO1054(recs1054c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_SEED_RELATIONAL_AFTERMATH_VOID'), 'ORIGINALITY_SEED_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_SEED_RELATIONAL_AFTERMATH_VOID does not fire when a seed is followed by a relationship shift within its window', async () => {
      const recs1054cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1054(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeRec1054(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeRec1054(i);
      });
      const res = await runO1054(recs1054cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_SEED_RELATIONAL_AFTERMATH_VOID'), 'ORIGINALITY_SEED_RELATIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1040 — originalityPass: originality clock-curiosity aftermath void, originality seed-emotional aftermath void, originality payoff-suspense aftermath void', async () => {
    // Same truncation pitfall as Waves 592/606/…/1012/1026 above — every fixture cycles purpose/
    // sentence/slug per scene to avoid tripping unrelated 'major' rules that would crowd these
    // 'minor' checks out of originality's top-8-by-severity slice. The tested signals here
    // (clockRaised bool / seededClueIds array / payoffSetupIds array) never overlap the filler
    // purpose pool, so cycling never contaminates a fixture.
    const PURPOSE_POOL_1040 = ['turning_point', 'complicate', 'introduce_conflict', 'establish_world'];
    const SENTENCE_POOL_1040 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor1040 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec1040 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor1040(idx),
      emotionalShift: 'neutral',
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_1040[idx % PURPOSE_POOL_1040.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain1040 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor1040(i)}\n\n${SENTENCE_POOL_1040[i % SENTENCE_POOL_1040.length]}`).join('\n\n');
    const runO1040 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain1040(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('ORIGINALITY_CLOCK_CURIOSITY_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no curiosity rise', async () => {
      const recs1040a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1040(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeRec1040(i, { curiosityDelta: 1 });
        return makeRec1040(i);
      });
      const res = await runO1040(recs1040a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_CLOCK_CURIOSITY_AFTERMATH_VOID'), 'ORIGINALITY_CLOCK_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_CLOCK_CURIOSITY_AFTERMATH_VOID does not fire when a clock-raise is followed by a curiosity rise within its window', async () => {
      const recs1040an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1040(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeRec1040(i, { curiosityDelta: 1 });
        return makeRec1040(i);
      });
      const res = await runO1040(recs1040an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_CLOCK_CURIOSITY_AFTERMATH_VOID'), 'ORIGINALITY_CLOCK_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_SEED_EMOTIONAL_AFTERMATH_VOID fires when every seed is followed by two scenes with no emotional shift', async () => {
      const recs1040b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1040(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeRec1040(i, { emotionalShift: 'positive' });
        return makeRec1040(i);
      });
      const res = await runO1040(recs1040b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_SEED_EMOTIONAL_AFTERMATH_VOID'), 'ORIGINALITY_SEED_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_SEED_EMOTIONAL_AFTERMATH_VOID does not fire when a seed is followed by an emotional shift within its window', async () => {
      const recs1040bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1040(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeRec1040(i, { emotionalShift: 'positive' });
        return makeRec1040(i);
      });
      const res = await runO1040(recs1040bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_SEED_EMOTIONAL_AFTERMATH_VOID'), 'ORIGINALITY_SEED_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_PAYOFF_SUSPENSE_AFTERMATH_VOID fires when every payoff is followed by two scenes with no suspense rise', async () => {
      const recs1040c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1040(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeRec1040(i, { suspenseDelta: 1 });
        return makeRec1040(i);
      });
      const res = await runO1040(recs1040c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_PAYOFF_SUSPENSE_AFTERMATH_VOID'), 'ORIGINALITY_PAYOFF_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_PAYOFF_SUSPENSE_AFTERMATH_VOID does not fire when a payoff is followed by a suspense rise within its window', async () => {
      const recs1040cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1040(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeRec1040(i, { suspenseDelta: 1 });
        return makeRec1040(i);
      });
      const res = await runO1040(recs1040cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_PAYOFF_SUSPENSE_AFTERMATH_VOID'), 'ORIGINALITY_PAYOFF_SUSPENSE_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1026 — originalityPass: originality stakes-emotional aftermath void, originality seed-suspense aftermath void, originality payoff-curiosity aftermath void', async () => {
    // Same truncation pitfall as Waves 592/606/…/998/1012 above — every fixture cycles purpose/
    // sentence/slug per scene to avoid tripping unrelated 'major' rules that would crowd these
    // 'minor' checks out of originality's top-8-by-severity slice. The tested signals here
    // (purpose==='raise_stakes' / seededClueIds array / payoffSetupIds array) never overlap the
    // filler purpose pool, so cycling never contaminates a fixture.
    const PURPOSE_POOL_1026 = ['turning_point', 'complicate', 'introduce_conflict', 'establish_world'];
    const SENTENCE_POOL_1026 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor1026 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec1026 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor1026(idx),
      emotionalShift: 'neutral',
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_1026[idx % PURPOSE_POOL_1026.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain1026 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor1026(i)}\n\n${SENTENCE_POOL_1026[i % SENTENCE_POOL_1026.length]}`).join('\n\n');
    const runO1026 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain1026(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('ORIGINALITY_STAKES_EMOTIONAL_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no emotional shift', async () => {
      const recs1026a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1026(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeRec1026(i, { emotionalShift: 'positive' });
        return makeRec1026(i);
      });
      const res = await runO1026(recs1026a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'ORIGINALITY_STAKES_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_STAKES_EMOTIONAL_AFTERMATH_VOID does not fire when a stakes-raise is followed by an emotional shift within its window', async () => {
      const recs1026an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1026(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeRec1026(i, { emotionalShift: 'positive' });
        return makeRec1026(i);
      });
      const res = await runO1026(recs1026an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'ORIGINALITY_STAKES_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_SEED_SUSPENSE_AFTERMATH_VOID fires when every seed is followed by two scenes with no suspense rise', async () => {
      const recs1026b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1026(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeRec1026(i, { suspenseDelta: 1 });
        return makeRec1026(i);
      });
      const res = await runO1026(recs1026b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_SEED_SUSPENSE_AFTERMATH_VOID'), 'ORIGINALITY_SEED_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_SEED_SUSPENSE_AFTERMATH_VOID does not fire when a seed is followed by a suspense rise within its window', async () => {
      const recs1026bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1026(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeRec1026(i, { suspenseDelta: 1 });
        return makeRec1026(i);
      });
      const res = await runO1026(recs1026bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_SEED_SUSPENSE_AFTERMATH_VOID'), 'ORIGINALITY_SEED_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('ORIGINALITY_PAYOFF_CURIOSITY_AFTERMATH_VOID fires when every payoff is followed by two scenes with no curiosity rise', async () => {
      const recs1026c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1026(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeRec1026(i, { curiosityDelta: 1 });
        return makeRec1026(i);
      });
      const res = await runO1026(recs1026c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_PAYOFF_CURIOSITY_AFTERMATH_VOID'), 'ORIGINALITY_PAYOFF_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('ORIGINALITY_PAYOFF_CURIOSITY_AFTERMATH_VOID does not fire when a payoff is followed by a curiosity rise within its window', async () => {
      const recs1026cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1026(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeRec1026(i, { curiosityDelta: 1 });
        return makeRec1026(i);
      });
      const res = await runO1026(recs1026cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_PAYOFF_CURIOSITY_AFTERMATH_VOID'), 'ORIGINALITY_PAYOFF_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 956 — originalityPass: originality curiosity zone imbalance, originality open thread zone imbalance, originality revelation zone imbalance', async () => {
    ...
    // emotion/slug/sentence per scene to avoid tripping unrelated 'major' rules that would crowd
    // these 'minor' checks out of originality's top-8-by-severity slice. The tested signals here
    // are non-purpose (curiosity delta / open-thread array / revelation field), so filler defaults
    // (curiosityDelta 0, empty unresolvedClues, null revelation) never contaminate them.
    const EMOTION_POOL_956 = ['neutral', 'neutral', 'neutral'];
    const PURPOSE_POOL_956 = ['turning_point', 'complicate', 'introduce_conflict', 'establish_world'];
    const SENTENCE_POOL_956 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor956 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec956 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor956(idx),
      emotionalShift: EMOTION_POOL_956[idx % EMOTION_POOL_956.length],
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_956[idx % PURPOSE_POOL_956.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain956 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor956(i)}\n\n${SENTENCE_POOL_956[i % SENTENCE_POOL_956.length]}`).join('\n\n');
    const runO956 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain956(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // Zone geometry n=10: Z0={0,1,2}, Z1={3,4}, Z2={5,6,7}, Z3={8,9}. Target at 0,1,2,8,9 →
    // Z0 3/5=60% (bloat), Z1 and Z2 empty → fires. Target at 0,3,5,8 → every zone touched → no-fire.
    it('ORIGINALITY_CURIOSITY_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of curiosity-raising scenes', async () => {
      const recs956a = Array.from({ length: 10 }, (_, i) => makeRec956(i, [0, 1, 2, 8, 9].includes(i) ? { curiosityDelta: 1 } : {}));
      const res = await runO956(recs956a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_CURIOSITY_ZONE_IMBALANCE'), 'ORIGINALITY_CURIOSITY_ZONE_IMBALANCE should fire');
    });

    it('ORIGINALITY_CURIOSITY_ZONE_IMBALANCE does not fire when curiosity-raising scenes touch every zone', async () => {
      const recs956an = Array.from({ length: 10 }, (_, i) => makeRec956(i, [0, 3, 5, 8].includes(i) ? { curiosityDelta: 1 } : {}));
      const res = await runO956(recs956an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_CURIOSITY_ZONE_IMBALANCE'), 'ORIGINALITY_CURIOSITY_ZONE_IMBALANCE should not fire');
    });

    it('ORIGINALITY_OPEN_THREAD_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of open-thread scenes', async () => {
      const recs956b = Array.from({ length: 10 }, (_, i) => makeRec956(i, [0, 1, 2, 8, 9].includes(i) ? { unresolvedClues: ['q1'] } : {}));
      const res = await runO956(recs956b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_OPEN_THREAD_ZONE_IMBALANCE'), 'ORIGINALITY_OPEN_THREAD_ZONE_IMBALANCE should fire');
    });

    it('ORIGINALITY_OPEN_THREAD_ZONE_IMBALANCE does not fire when open-thread scenes touch every zone', async () => {
      const recs956bn = Array.from({ length: 10 }, (_, i) => makeRec956(i, [0, 3, 5, 8].includes(i) ? { unresolvedClues: ['q1'] } : {}));
      const res = await runO956(recs956bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_OPEN_THREAD_ZONE_IMBALANCE'), 'ORIGINALITY_OPEN_THREAD_ZONE_IMBALANCE should not fire');
    });

    it('ORIGINALITY_REVELATION_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of revelation scenes', async () => {
      const recs956c = Array.from({ length: 10 }, (_, i) => makeRec956(i, [0, 1, 2, 8, 9].includes(i) ? { revelation: 'a hidden truth surfaces' } : {}));
      const res = await runO956(recs956c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_REVELATION_ZONE_IMBALANCE'), 'ORIGINALITY_REVELATION_ZONE_IMBALANCE should fire');
    });

    it('ORIGINALITY_REVELATION_ZONE_IMBALANCE does not fire when revelation scenes touch every zone', async () => {
      const recs956cn = Array.from({ length: 10 }, (_, i) => makeRec956(i, [0, 3, 5, 8].includes(i) ? { revelation: 'a hidden truth surfaces' } : {}));
      const res = await runO956(recs956cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_REVELATION_ZONE_IMBALANCE'), 'ORIGINALITY_REVELATION_ZONE_IMBALANCE should not fire');
    });
  });

  describe('Wave 942 — originalityPass: originality positive emotion zone imbalance, originality suspense zone imbalance, originality payoff zone imbalance', async () => {
    // Same truncation pitfall as Waves 592/606/…/914/928 above — every fixture cycles purpose/
    // emotion/slug/sentence per scene to avoid tripping unrelated 'major' rules that would crowd
    // these 'minor' checks out of originality's top-8-by-severity slice. The tested signals here
    // are non-purpose (positive shift / suspense delta / payoff array), so filler defaults
    // (neutral emotion, suspenseDelta 0, empty payoffSetupIds) never contaminate them.
    const EMOTION_POOL_942 = ['neutral', 'neutral', 'neutral'];
    const PURPOSE_POOL_942 = ['turning_point', 'complicate', 'introduce_conflict', 'establish_world'];
    const SENTENCE_POOL_942 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor942 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec942 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor942(idx),
      emotionalShift: EMOTION_POOL_942[idx % EMOTION_POOL_942.length],
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_942[idx % PURPOSE_POOL_942.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain942 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor942(i)}\n\n${SENTENCE_POOL_942[i % SENTENCE_POOL_942.length]}`).join('\n\n');
    const runO942 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain942(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // Zone geometry n=10: Z0={0,1,2}, Z1={3,4}, Z2={5,6,7}, Z3={8,9}. Target at 0,1,2,8,9 →
    // Z0 3/5=60% (bloat), Z1 and Z2 empty → fires. Target at 0,3,5,8 → every zone touched → no-fire.
    it('ORIGINALITY_POSITIVE_EMOTION_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of positive-shift scenes', async () => {
      const recs942a = Array.from({ length: 10 }, (_, i) => makeRec942(i, [0, 1, 2, 8, 9].includes(i) ? { emotionalShift: 'positive' } : {}));
      const res = await runO942(recs942a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_POSITIVE_EMOTION_ZONE_IMBALANCE'), 'ORIGINALITY_POSITIVE_EMOTION_ZONE_IMBALANCE should fire');
    });

    it('ORIGINALITY_POSITIVE_EMOTION_ZONE_IMBALANCE does not fire when positive-shift scenes touch every zone', async () => {
      const recs942an = Array.from({ length: 10 }, (_, i) => makeRec942(i, [0, 3, 5, 8].includes(i) ? { emotionalShift: 'positive' } : {}));
      const res = await runO942(recs942an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_POSITIVE_EMOTION_ZONE_IMBALANCE'), 'ORIGINALITY_POSITIVE_EMOTION_ZONE_IMBALANCE should not fire');
    });

    it('ORIGINALITY_SUSPENSE_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of suspense-raising scenes', async () => {
      const recs942b = Array.from({ length: 10 }, (_, i) => makeRec942(i, [0, 1, 2, 8, 9].includes(i) ? { suspenseDelta: 1 } : {}));
      const res = await runO942(recs942b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_SUSPENSE_ZONE_IMBALANCE'), 'ORIGINALITY_SUSPENSE_ZONE_IMBALANCE should fire');
    });

    it('ORIGINALITY_SUSPENSE_ZONE_IMBALANCE does not fire when suspense-raising scenes touch every zone', async () => {
      const recs942bn = Array.from({ length: 10 }, (_, i) => makeRec942(i, [0, 3, 5, 8].includes(i) ? { suspenseDelta: 1 } : {}));
      const res = await runO942(recs942bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_SUSPENSE_ZONE_IMBALANCE'), 'ORIGINALITY_SUSPENSE_ZONE_IMBALANCE should not fire');
    });

    it('ORIGINALITY_PAYOFF_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of payoff scenes', async () => {
      const recs942c = Array.from({ length: 10 }, (_, i) => makeRec942(i, [0, 1, 2, 8, 9].includes(i) ? { payoffSetupIds: ['s1'] } : {}));
      const res = await runO942(recs942c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_PAYOFF_ZONE_IMBALANCE'), 'ORIGINALITY_PAYOFF_ZONE_IMBALANCE should fire');
    });

    it('ORIGINALITY_PAYOFF_ZONE_IMBALANCE does not fire when payoff scenes touch every zone', async () => {
      const recs942cn = Array.from({ length: 10 }, (_, i) => makeRec942(i, [0, 3, 5, 8].includes(i) ? { payoffSetupIds: ['s1'] } : {}));
      const res = await runO942(recs942cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_PAYOFF_ZONE_IMBALANCE'), 'ORIGINALITY_PAYOFF_ZONE_IMBALANCE should not fire');
    });
  });
  ...
  describe('Wave 914 — originalityPass: originality resolution zone imbalance, originality complicate zone imbalance, originality introduce conflict zone imbalance', async () => {
    ...
    // that would crowd these 'minor' checks out. PURPOSE_POOL_914 excludes the tested purpose
    // values (resolution/complicate/introduce_conflict) so filler scenes never contaminate them.
    const EMOTION_POOL_914 = ['neutral', 'neutral', 'neutral'];
    const PURPOSE_POOL_914 = ['raise_stakes', 'character_moment', 'turning_point', 'establish_world'];
    const SENTENCE_POOL_914 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor914 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec914 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor914(idx),
      emotionalShift: EMOTION_POOL_914[idx % EMOTION_POOL_914.length],
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_914[idx % PURPOSE_POOL_914.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain914 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor914(i)}\n\n${SENTENCE_POOL_914[i % SENTENCE_POOL_914.length]}`).join('\n\n');
    const runO914 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain914(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // Zone geometry n=10: Z0={0,1,2}, Z1={3,4}, Z2={5,6,7}, Z3={8,9}. Target at 0,1,2,8,9 →
    // Z0 3/5=60% (bloat), Z1 and Z2 empty → fires. Target at 0,3,5,8 → every zone touched → no-fire.
    it('ORIGINALITY_RESOLUTION_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of resolution-purposed scenes', async () => {
      const recs914a = Array.from({ length: 10 }, (_, i) => makeRec914(i, [0, 1, 2, 8, 9].includes(i) ? { purpose: 'resolution' } : {}));
      const res = await runO914(recs914a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_RESOLUTION_ZONE_IMBALANCE'), 'ORIGINALITY_RESOLUTION_ZONE_IMBALANCE should fire');
    });

    it('ORIGINALITY_RESOLUTION_ZONE_IMBALANCE does not fire when resolution-purposed scenes touch every zone', async () => {
      const recs914an = Array.from({ length: 10 }, (_, i) => makeRec914(i, [0, 3, 5, 8].includes(i) ? { purpose: 'resolution' } : {}));
      const res = await runO914(recs914an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_RESOLUTION_ZONE_IMBALANCE'), 'ORIGINALITY_RESOLUTION_ZONE_IMBALANCE should not fire');
    });

    it('ORIGINALITY_COMPLICATE_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of complicating scenes', async () => {
      const recs914b = Array.from({ length: 10 }, (_, i) => makeRec914(i, [0, 1, 2, 8, 9].includes(i) ? { purpose: 'complicate' } : {}));
      const res = await runO914(recs914b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_COMPLICATE_ZONE_IMBALANCE'), 'ORIGINALITY_COMPLICATE_ZONE_IMBALANCE should fire');
    });

    it('ORIGINALITY_COMPLICATE_ZONE_IMBALANCE does not fire when complicating scenes touch every zone', async () => {
      const recs914bn = Array.from({ length: 10 }, (_, i) => makeRec914(i, [0, 3, 5, 8].includes(i) ? { purpose: 'complicate' } : {}));
      const res = await runO914(recs914bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_COMPLICATE_ZONE_IMBALANCE'), 'ORIGINALITY_COMPLICATE_ZONE_IMBALANCE should not fire');
    });

    it('ORIGINALITY_INTRODUCE_CONFLICT_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of conflict-introducing scenes', async () => {
      const recs914c = Array.from({ length: 10 }, (_, i) => makeRec914(i, [0, 1, 2, 8, 9].includes(i) ? { purpose: 'introduce_conflict' } : {}));
      const res = await runO914(recs914c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_INTRODUCE_CONFLICT_ZONE_IMBALANCE'), 'ORIGINALITY_INTRODUCE_CONFLICT_ZONE_IMBALANCE should fire');
    });

    it('ORIGINALITY_INTRODUCE_CONFLICT_ZONE_IMBALANCE does not fire when conflict-introducing scenes touch every zone', async () => {
      const recs914cn = Array.from({ length: 10 }, (_, i) => makeRec914(i, [0, 3, 5, 8].includes(i) ? { purpose: 'introduce_conflict' } : {}));
      const res = await runO914(recs914cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_INTRODUCE_CONFLICT_ZONE_IMBALANCE'), 'ORIGINALITY_INTRODUCE_CONFLICT_ZONE_IMBALANCE should not fire');
    });
  });
  ...
  describe('Wave 872 — originalityPass: originality climax drought run, originality establish world drought run, originality resolution zone cluster', async () => {
    ...
    // above — every fixture cycles purpose/emotion/dialogue/slug/sentence per scene to avoid
    // tripping unrelated 'major' rules that would crowd these 'minor' checks out.
    const EMOTION_POOL_872 = ['neutral', 'neutral', 'neutral'];
    const PURPOSE_POOL_872 = ['complicate', 'raise_stakes', 'turning_point', 'character_moment'];
    const SENTENCE_POOL_872 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor872 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec872 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor872(idx),
      emotionalShift: EMOTION_POOL_872[idx % EMOTION_POOL_872.length],
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_872[idx % PURPOSE_POOL_872.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain872 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor872(i)}\n\n${SENTENCE_POOL_872[i % SENTENCE_POOL_872.length]}`).join('\n\n');
    const runO872 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain872(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // ORIGINALITY_CLIMAX_DROUGHT_RUN fire:
    // n=10; climax at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('ORIGINALITY_CLIMAX_DROUGHT_RUN fires when a long run has no climax-purposed scene', async () => {
      const recs872a = Array.from({ length: 10 }, (_, i) => makeRec872(i, {
        purpose: (i === 0 || i === 1 || i === 2) ? 'climax' : PURPOSE_POOL_872[i % PURPOSE_POOL_872.length],
      }));
      const res = await runO872(recs872a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_CLIMAX_DROUGHT_RUN'), 'ORIGINALITY_CLIMAX_DROUGHT_RUN should fire');
    });

    it('ORIGINALITY_CLIMAX_DROUGHT_RUN does not fire when climax-purposed scenes are evenly spread', async () => {
      const recs872an = Array.from({ length: 10 }, (_, i) => makeRec872(i, {
        purpose: (i === 0 || i === 3 || i === 6 || i === 9) ? 'climax' : PURPOSE_POOL_872[i % PURPOSE_POOL_872.length],
      }));
      const res = await runO872(recs872an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_CLIMAX_DROUGHT_RUN'), 'ORIGINALITY_CLIMAX_DROUGHT_RUN should not fire');
    });

    // ORIGINALITY_ESTABLISH_WORLD_DROUGHT_RUN fire:
    // n=10; establish_world at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('ORIGINALITY_ESTABLISH_WORLD_DROUGHT_RUN fires when a long run has no world-establishing scene', async () => {
      const recs872b = Array.from({ length: 10 }, (_, i) => makeRec872(i, {
        purpose: (i === 0 || i === 1 || i === 2) ? 'establish_world' : PURPOSE_POOL_872[i % PURPOSE_POOL_872.length],
      }));
      const res = await runO872(recs872b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_ESTABLISH_WORLD_DROUGHT_RUN'), 'ORIGINALITY_ESTABLISH_WORLD_DROUGHT_RUN should fire');
    });

    it('ORIGINALITY_ESTABLISH_WORLD_DROUGHT_RUN does not fire when world-establishing scenes are evenly spread', async () => {
      const recs872bn = Array.from({ length: 10 }, (_, i) => makeRec872(i, {
        purpose: (i === 0 || i === 3 || i === 6 || i === 9) ? 'establish_world' : PURPOSE_POOL_872[i % PURPOSE_POOL_872.length],
      }));
      const res = await runO872(recs872bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_ESTABLISH_WORLD_DROUGHT_RUN'), 'ORIGINALITY_ESTABLISH_WORLD_DROUGHT_RUN should not fire');
    });

    // ORIGINALITY_RESOLUTION_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; resolution scenes at 0,1,2 → 100% opening third
    it('ORIGINALITY_RESOLUTION_ZONE_CLUSTER fires when >75% of resolution-purposed scenes cluster in one third', async () => {
      const recs872c = Array.from({ length: 9 }, (_, i) => makeRec872(i, {
        purpose: (i === 0 || i === 1 || i === 2) ? 'resolution' : PURPOSE_POOL_872[i % PURPOSE_POOL_872.length],
      }));
      const res = await runO872(recs872c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_RESOLUTION_ZONE_CLUSTER'), 'ORIGINALITY_RESOLUTION_ZONE_CLUSTER should fire');
    });

    it('ORIGINALITY_RESOLUTION_ZONE_CLUSTER does not fire when resolution-purposed scenes spread across thirds', async () => {
      const recs872cn = Array.from({ length: 9 }, (_, i) => makeRec872(i, {
        purpose: (i === 0 || i === 4 || i === 8) ? 'resolution' : PURPOSE_POOL_872[i % PURPOSE_POOL_872.length],
      }));
      const res = await runO872(recs872cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_RESOLUTION_ZONE_CLUSTER'), 'ORIGINALITY_RESOLUTION_ZONE_CLUSTER should not fire');
    });
  });
  ...
  describe('Wave 844 — originalityPass: originality introduce conflict drought run, originality negative emotion drought run, originality positive emotion zone cluster', async () => {
    ...
    // above — every fixture cycles purpose/emotion/dialogue/slug/sentence per scene to avoid
    // tripping unrelated 'major' rules that would crowd these 'minor' checks out.
    const EMOTION_POOL_844 = ['neutral', 'neutral', 'neutral'];
    const PURPOSE_POOL_844 = ['establish_world', 'complicate', 'raise_stakes', 'resolution'];
    const SENTENCE_POOL_844 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor844 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec844 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor844(idx),
      emotionalShift: EMOTION_POOL_844[idx % EMOTION_POOL_844.length],
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_844[idx % PURPOSE_POOL_844.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain844 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor844(i)}\n\n${SENTENCE_POOL_844[i % SENTENCE_POOL_844.length]}`).join('\n\n');
    const runO844 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain844(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // ORIGINALITY_INTRODUCE_CONFLICT_DROUGHT_RUN fire:
    // n=10; introduce_conflict at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('ORIGINALITY_INTRODUCE_CONFLICT_DROUGHT_RUN fires when a long run has no new conflict', async () => {
      const recs844a = Array.from({ length: 10 }, (_, i) => makeRec844(i, {
        purpose: (i === 0 || i === 1 || i === 2) ? 'introduce_conflict' : PURPOSE_POOL_844[i % PURPOSE_POOL_844.length],
      }));
      const res = await runO844(recs844a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_INTRODUCE_CONFLICT_DROUGHT_RUN'), 'ORIGINALITY_INTRODUCE_CONFLICT_DROUGHT_RUN should fire');
    });

    it('ORIGINALITY_INTRODUCE_CONFLICT_DROUGHT_RUN does not fire when conflict-introducing scenes are evenly spread', async () => {
      const recs844an = Array.from({ length: 10 }, (_, i) => makeRec844(i, {
        purpose: (i === 0 || i === 3 || i === 6 || i === 9) ? 'introduce_conflict' : PURPOSE_POOL_844[i % PURPOSE_POOL_844.length],
      }));
      const res = await runO844(recs844an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_INTRODUCE_CONFLICT_DROUGHT_RUN'), 'ORIGINALITY_INTRODUCE_CONFLICT_DROUGHT_RUN should not fire');
    });

    // ORIGINALITY_NEGATIVE_EMOTION_DROUGHT_RUN fire:
    // n=10; negative-emotion at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('ORIGINALITY_NEGATIVE_EMOTION_DROUGHT_RUN fires when a long run has no negative-emotion charge', async () => {
      const recs844b = Array.from({ length: 10 }, (_, i) => makeRec844(i, {
        emotionalShift: (i === 0 || i === 1 || i === 2) ? 'negative' : EMOTION_POOL_844[i % EMOTION_POOL_844.length],
      }));
      const res = await runO844(recs844b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_NEGATIVE_EMOTION_DROUGHT_RUN'), 'ORIGINALITY_NEGATIVE_EMOTION_DROUGHT_RUN should fire');
    });

    it('ORIGINALITY_NEGATIVE_EMOTION_DROUGHT_RUN does not fire when negative-emotion scenes are evenly spread', async () => {
      const recs844bn = Array.from({ length: 10 }, (_, i) => makeRec844(i, {
        emotionalShift: (i === 0 || i === 3 || i === 6 || i === 9) ? 'negative' : EMOTION_POOL_844[i % EMOTION_POOL_844.length],
      }));
      const res = await runO844(recs844bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_NEGATIVE_EMOTION_DROUGHT_RUN'), 'ORIGINALITY_NEGATIVE_EMOTION_DROUGHT_RUN should not fire');
    });

    // ORIGINALITY_POSITIVE_EMOTION_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; positive-emotion scenes at 0,1,2 → 100% opening third
    it('ORIGINALITY_POSITIVE_EMOTION_ZONE_CLUSTER fires when >75% of positive-emotion scenes cluster in one third', async () => {
      const recs844c = Array.from({ length: 9 }, (_, i) => makeRec844(i, {
        emotionalShift: (i === 0 || i === 1 || i === 2) ? 'positive' : EMOTION_POOL_844[i % EMOTION_POOL_844.length],
      }));
      const res = await runO844(recs844c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_POSITIVE_EMOTION_ZONE_CLUSTER'), 'ORIGINALITY_POSITIVE_EMOTION_ZONE_CLUSTER should fire');
    });

    it('ORIGINALITY_POSITIVE_EMOTION_ZONE_CLUSTER does not fire when positive-emotion scenes spread across thirds', async () => {
      const recs844cn = Array.from({ length: 9 }, (_, i) => makeRec844(i, {
        emotionalShift: (i === 0 || i === 4 || i === 8) ? 'positive' : EMOTION_POOL_844[i % EMOTION_POOL_844.length],
      }));
      const res = await runO844(recs844cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_POSITIVE_EMOTION_ZONE_CLUSTER'), 'ORIGINALITY_POSITIVE_EMOTION_ZONE_CLUSTER should not fire');
    });
  });

  describe('Wave 830 — originalityPass: originality turning point drought run, originality introduce conflict zone cluster, originality negative emotion zone cluster', async () => {
    // Same truncation pitfall as Waves 592/606/620/634/648/662/676/690/704/718/732/746/760/774/788/802/816
    // above — every fixture cycles purpose/emotion/dialogue/slug/sentence per scene to avoid
    // tripping unrelated 'major' rules that would crowd these 'minor' checks out.
    const EMOTION_POOL_830 = ['positive', 'neutral', 'neutral'];
    const PURPOSE_POOL_830 = ['establish_world', 'complicate', 'raise_stakes', 'resolution'];
    const SENTENCE_POOL_830 = [
      'Alice studies the map by lamplight.', 'Bob paces the length of the corridor.',
      'Rain streaks the tall window.', 'A phone buzzes on the counter.',
      'Footsteps echo down the stairwell.', 'The kettle whistles on the stove.',
      'A drawer sticks halfway open.', 'Wind rattles the loose shutter.',
    ...
    const slugFor830 = (idx: number) => `${idx % 2 === 0 ? 'INT.' : 'EXT.'} LOCATION ${idx} - ${idx % 3 === 0 ? 'DAY' : idx % 3 === 1 ? 'NIGHT' : 'DUSK'}`;
    const makeRec830 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: slugFor830(idx),
      emotionalShift: EMOTION_POOL_830[idx % EMOTION_POOL_830.length],
      suspenseDelta: 0, curiosityDelta: 0, clockRaised: false, clockDelta: 0, revelation: null,
      dialogueHighlights: [],
      relationshipShifts: [], seededClueIds: [], payoffSetupIds: [],
      unresolvedClues: [], visualBeats: [], purpose: PURPOSE_POOL_830[idx % PURPOSE_POOL_830.length],
      dramaticTurn: 'nothing',
      ...overrides,
    });
    const buildFountain830 = (count: number): string =>
      Array.from({ length: count }, (_, i) => `${slugFor830(i)}\n\n${SENTENCE_POOL_830[i % SENTENCE_POOL_830.length]}`).join('\n\n');
    const runO830 = async (records: any[], fountain?: string) => {
      const { originalityPass } = await import('../../server/nvm/revision/passes/originality.ts');
      const f = fountain ?? buildFountain830(records.length);
      return originalityPass({
        fountain: f, original: f, records,
        structure: { escalating: false, avgSuspensePerScene: 0, completionPercent: 50,
          approachingClimax: false, revelationCount: 0, actBreaks: [] } as any,
    ...
    // ORIGINALITY_TURNING_POINT_DROUGHT_RUN fire:
    // n=10; turning_point at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('ORIGINALITY_TURNING_POINT_DROUGHT_RUN fires when a long run has no turning point', async () => {
      const recs830a = Array.from({ length: 10 }, (_, i) => makeRec830(i, {
        purpose: (i === 0 || i === 1 || i === 2) ? 'turning_point' : PURPOSE_POOL_830[i % PURPOSE_POOL_830.length],
      }));
      const res = await runO830(recs830a);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_TURNING_POINT_DROUGHT_RUN'), 'ORIGINALITY_TURNING_POINT_DROUGHT_RUN should fire');
    });

    it('ORIGINALITY_TURNING_POINT_DROUGHT_RUN does not fire when turning points are evenly spread', async () => {
      const recs830an = Array.from({ length: 10 }, (_, i) => makeRec830(i, {
        purpose: (i === 0 || i === 3 || i === 6 || i === 9) ? 'turning_point' : PURPOSE_POOL_830[i % PURPOSE_POOL_830.length],
      }));
      const res = await runO830(recs830an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_TURNING_POINT_DROUGHT_RUN'), 'ORIGINALITY_TURNING_POINT_DROUGHT_RUN should not fire');
    });

    // ORIGINALITY_INTRODUCE_CONFLICT_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; introduce_conflict scenes at 0,1,2 → 100% opening third
    it('ORIGINALITY_INTRODUCE_CONFLICT_ZONE_CLUSTER fires when >75% of conflict-introducing scenes cluster in one third', async () => {
      const recs830b = Array.from({ length: 9 }, (_, i) => makeRec830(i, {
        purpose: (i === 0 || i === 1 || i === 2) ? 'introduce_conflict' : PURPOSE_POOL_830[i % PURPOSE_POOL_830.length],
      }));
      const res = await runO830(recs830b);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_INTRODUCE_CONFLICT_ZONE_CLUSTER'), 'ORIGINALITY_INTRODUCE_CONFLICT_ZONE_CLUSTER should fire');
    });

    it('ORIGINALITY_INTRODUCE_CONFLICT_ZONE_CLUSTER does not fire when conflict-introducing scenes spread across thirds', async () => {
      const recs830bn = Array.from({ length: 9 }, (_, i) => makeRec830(i, {
        purpose: (i === 0 || i === 4 || i === 8) ? 'introduce_conflict' : PURPOSE_POOL_830[i % PURPOSE_POOL_830.length],
      }));
      const res = await runO830(recs830bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_INTRODUCE_CONFLICT_ZONE_CLUSTER'), 'ORIGINALITY_INTRODUCE_CONFLICT_ZONE_CLUSTER should not fire');
    });

    // ORIGINALITY_NEGATIVE_EMOTION_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; negative-emotion scenes at 0,1,2 → 100% opening third
    it('ORIGINALITY_NEGATIVE_EMOTION_ZONE_CLUSTER fires when >75% of negative-emotion scenes cluster in one third', async () => {
      const recs830c = Array.from({ length: 9 }, (_, i) => makeRec830(i, {
        emotionalShift: (i === 0 || i === 1 || i === 2) ? 'negative' : EMOTION_POOL_830[i % EMOTION_POOL_830.length],
      }));
      const res = await runO830(recs830c);
      assert.ok(res.issues.some((i: any) => i.rule === 'ORIGINALITY_NEGATIVE_EMOTION_ZONE_CLUSTER'), 'ORIGINALITY_NEGATIVE_EMOTION_ZONE_CLUSTER should fire');
    });

    it('ORIGINALITY_NEGATIVE_EMOTION_ZONE_CLUSTER does not fire when negative-emotion scenes spread across thirds', async () => {
      const recs830cn = Array.from({ length: 9 }, (_, i) => makeRec830(i, {
        emotionalShift: (i === 0 || i === 4 || i === 8) ? 'negative' : EMOTION_POOL_830[i % EMOTION_POOL_830.length],
      }));
      const res = await runO830(recs830cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'ORIGINALITY_NEGATIVE_EMOTION_ZONE_CLUSTER'), 'ORIGINALITY_NEGATIVE_EMOTION_ZONE_CLUSTER should not fire');
    });
  });
