...
import { relationshipArcPass } from '../../server/nvm/revision/passes/relationship-arc.ts';
  ...
  describe('Wave 1174 — beliefPass: belief suspense-curiosity aftermath void, belief suspense-emotional aftermath void, belief emotion-curiosity aftermath void', async () => {
    ...
    it('BELIEF_SUSPENSE_CURIOSITY_AFTERMATH_VOID fires when every suspense-spike has no curiosity rise within 2 scenes', async () => {
      const recs1174a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { suspenseDelta: 1 } : ([8, 9].includes(i) ? { curiosityDelta: 1 } : {})));
      const res = await runBF1174(recs1174a);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_SUSPENSE_CURIOSITY_AFTERMATH_VOID'), 'BELIEF_SUSPENSE_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('BELIEF_SUSPENSE_CURIOSITY_AFTERMATH_VOID does not fire when a suspense-spike is followed by a curiosity rise within 2 scenes', async () => {
      const recs1174an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { suspenseDelta: 1 } : ([1, 9].includes(i) ? { curiosityDelta: 1 } : {})));
      const res = await runBF1174(recs1174an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_SUSPENSE_CURIOSITY_AFTERMATH_VOID'), 'BELIEF_SUSPENSE_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_SUSPENSE_EMOTIONAL_AFTERMATH_VOID fires when every suspense-spike has no emotional shift within 2 scenes', async () => {
      const recs1174b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { suspenseDelta: 1 } : ([8, 9].includes(i) ? { emotionalShift: 'positive' } : {})));
      const res = await runBF1174(recs1174b);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_SUSPENSE_EMOTIONAL_AFTERMATH_VOID'), 'BELIEF_SUSPENSE_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('BELIEF_SUSPENSE_EMOTIONAL_AFTERMATH_VOID does not fire when a suspense-spike is followed by an emotional shift within 2 scenes', async () => {
      const recs1174bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { suspenseDelta: 1 } : ([1, 9].includes(i) ? { emotionalShift: 'positive' } : {})));
      const res = await runBF1174(recs1174bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_SUSPENSE_EMOTIONAL_AFTERMATH_VOID'), 'BELIEF_SUSPENSE_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_EMOTION_CURIOSITY_AFTERMATH_VOID fires when every emotionally-charged scene has no curiosity rise within 2 scenes', async () => {
      const recs1174c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { emotionalShift: 'positive' } : ([8, 9].includes(i) ? { curiosityDelta: 1 } : {})));
      const res = await runBF1174(recs1174c);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_EMOTION_CURIOSITY_AFTERMATH_VOID'), 'BELIEF_EMOTION_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('BELIEF_EMOTION_CURIOSITY_AFTERMATH_VOID does not fire when an emotionally-charged scene is followed by a curiosity rise within 2 scenes', async () => {
      const recs1174cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { emotionalShift: 'positive' } : ([1, 9].includes(i) ? { curiosityDelta: 1 } : {})));
      const res = await runBF1174(recs1174cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_EMOTION_CURIOSITY_AFTERMATH_VOID'), 'BELIEF_EMOTION_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1146 — beliefPass: belief clock-dialogue-highlight aftermath void, belief turn-curiosity aftermath void, belief turn-emotional aftermath void', async () => {
    ...
    // FIRE: aftermath signal only at 8,9 — outside both trigger windows {1,2} and {4,5} → every trigger
    // void → fires. NO-FIRE: aftermath at 1 (inside trigger 0's window) and 9 → trigger 0 not void → no fire.
    it('BELIEF_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every clock-raise has no highlighted dialogue within 2 scenes', async () => {
      const recs1146a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { clockRaised: true } : ([8, 9].includes(i) ? { dialogueHighlights: ['a memorable line'] } : {})));
      const res = await runBF1146(recs1146a);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'BELIEF_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('BELIEF_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a clock-raise is followed by highlighted dialogue within 2 scenes', async () => {
      const recs1146an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { clockRaised: true } : ([1, 9].includes(i) ? { dialogueHighlights: ['a memorable line'] } : {})));
      const res = await runBF1146(recs1146an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'BELIEF_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_TURN_CURIOSITY_AFTERMATH_VOID fires when every dramatic turn has no curiosity rise within 2 scenes', async () => {
      const recs1146b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { dramaticTurn: 'reveal' } : ([8, 9].includes(i) ? { curiosityDelta: 1 } : {})));
      const res = await runBF1146(recs1146b);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_TURN_CURIOSITY_AFTERMATH_VOID'), 'BELIEF_TURN_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('BELIEF_TURN_CURIOSITY_AFTERMATH_VOID does not fire when a dramatic turn is followed by a curiosity rise within 2 scenes', async () => {
      const recs1146bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { dramaticTurn: 'reveal' } : ([1, 9].includes(i) ? { curiosityDelta: 1 } : {})));
      const res = await runBF1146(recs1146bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_TURN_CURIOSITY_AFTERMATH_VOID'), 'BELIEF_TURN_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_TURN_EMOTIONAL_AFTERMATH_VOID fires when every dramatic turn has no emotional shift within 2 scenes', async () => {
      const recs1146c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { dramaticTurn: 'reveal' } : ([8, 9].includes(i) ? { emotionalShift: 'positive' } : {})));
      const res = await runBF1146(recs1146c);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_TURN_EMOTIONAL_AFTERMATH_VOID'), 'BELIEF_TURN_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('BELIEF_TURN_EMOTIONAL_AFTERMATH_VOID does not fire when a dramatic turn is followed by an emotional shift within 2 scenes', async () => {
      const recs1146cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { dramaticTurn: 'reveal' } : ([1, 9].includes(i) ? { emotionalShift: 'positive' } : {})));
      const res = await runBF1146(recs1146cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_TURN_EMOTIONAL_AFTERMATH_VOID'), 'BELIEF_TURN_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1132 — beliefPass: belief clock-emotional aftermath void, belief clock-relational aftermath void, belief turn-suspense aftermath void', async () => {
    const runBF1132 = async (records: ScreenplaySceneRecord[]) => {
      const { beliefPass } = await import('../../server/nvm/revision/passes/belief.ts');
      return beliefPass({ fountain: '', original: '', records, structure: {} as any, annotations: [], approvedSpans: [] });
    };
    ...
    // FIRE: aftermath signal only at 8,9 — outside both trigger windows {1,2} and {4,5} → every trigger
    // void → fires. NO-FIRE: aftermath at 1 (inside trigger 0's window) and 9 → trigger 0 not void → no fire.
    it('BELIEF_CLOCK_EMOTIONAL_AFTERMATH_VOID fires when every clock-raise has no emotional shift within 2 scenes', async () => {
      const recs1132a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { clockRaised: true } : ([8, 9].includes(i) ? { emotionalShift: 'positive' } : {})));
      const res = await runBF1132(recs1132a);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_CLOCK_EMOTIONAL_AFTERMATH_VOID'), 'BELIEF_CLOCK_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('BELIEF_CLOCK_EMOTIONAL_AFTERMATH_VOID does not fire when a clock-raise is followed by an emotional shift within 2 scenes', async () => {
      const recs1132an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { clockRaised: true } : ([1, 9].includes(i) ? { emotionalShift: 'positive' } : {})));
      const res = await runBF1132(recs1132an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_CLOCK_EMOTIONAL_AFTERMATH_VOID'), 'BELIEF_CLOCK_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_CLOCK_RELATIONAL_AFTERMATH_VOID fires when every clock-raise has no relationship shift within 2 scenes', async () => {
      const recs1132b = Array.from({ length: 10 }, (_, i) =>
      ...
      const res = await runBF1132(recs1132b);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_CLOCK_RELATIONAL_AFTERMATH_VOID'), 'BELIEF_CLOCK_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('BELIEF_CLOCK_RELATIONAL_AFTERMATH_VOID does not fire when a clock-raise is followed by a relationship shift within 2 scenes', async () => {
      const recs1132bn = Array.from({ length: 10 }, (_, i) =>
      ...
      const res = await runBF1132(recs1132bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_CLOCK_RELATIONAL_AFTERMATH_VOID'), 'BELIEF_CLOCK_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_TURN_SUSPENSE_AFTERMATH_VOID fires when every dramatic turn has no suspense rise within 2 scenes', async () => {
      const recs1132c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { dramaticTurn: 'reveal' } : ([8, 9].includes(i) ? { suspenseDelta: 1 } : {})));
      const res = await runBF1132(recs1132c);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_TURN_SUSPENSE_AFTERMATH_VOID'), 'BELIEF_TURN_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('BELIEF_TURN_SUSPENSE_AFTERMATH_VOID does not fire when a dramatic turn is followed by a suspense rise within 2 scenes', async () => {
      const recs1132cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { dramaticTurn: 'reveal' } : ([1, 9].includes(i) ? { suspenseDelta: 1 } : {})));
      const res = await runBF1132(recs1132cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_TURN_SUSPENSE_AFTERMATH_VOID'), 'BELIEF_TURN_SUSPENSE_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1076 — beliefPass: belief seed-staging aftermath void, belief open-thread-dialogue-highlight aftermath void, belief open-thread-staging aftermath void', async () => {
    ...
    // FIRE: aftermath signal only at 8,9 — outside both trigger windows {1,2} and {4,5} → every trigger
    // void → fires. NO-FIRE: aftermath at 1 (inside trigger 0's window) and 9 → trigger 0 not void → no fire.
    it('BELIEF_SEED_STAGING_AFTERMATH_VOID fires when every seed has no visually dense scene within 2 scenes', async () => {
      const recs1076a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { seededClueIds: ['c1'] } : ([8, 9].includes(i) ? { visualBeats: ['beat one', 'beat two'] } : {})));
      const res = await runBF1076(recs1076a);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_SEED_STAGING_AFTERMATH_VOID'), 'BELIEF_SEED_STAGING_AFTERMATH_VOID should fire');
    });

    it('BELIEF_SEED_STAGING_AFTERMATH_VOID does not fire when a seed is followed by a visually dense scene within 2 scenes', async () => {
      const recs1076an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { seededClueIds: ['c1'] } : ([1, 9].includes(i) ? { visualBeats: ['beat one', 'beat two'] } : {})));
      const res = await runBF1076(recs1076an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_SEED_STAGING_AFTERMATH_VOID'), 'BELIEF_SEED_STAGING_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every heavy clue-debt scene has no highlighted dialogue within 2 scenes', async () => {
      const recs1076b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { unresolvedClues: ['c1', 'c2', 'c3'] } : ([8, 9].includes(i) ? { dialogueHighlights: ['a memorable line'] } : {})));
      const res = await runBF1076(recs1076b);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'BELIEF_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('BELIEF_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by highlighted dialogue within 2 scenes', async () => {
      const recs1076bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { unresolvedClues: ['c1', 'c2', 'c3'] } : ([1, 9].includes(i) ? { dialogueHighlights: ['a memorable line'] } : {})));
      const res = await runBF1076(recs1076bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'BELIEF_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_OPEN_THREAD_STAGING_AFTERMATH_VOID fires when every heavy clue-debt scene has no visually dense scene within 2 scenes', async () => {
      const recs1076c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { unresolvedClues: ['c1', 'c2', 'c3'] } : ([8, 9].includes(i) ? { visualBeats: ['beat one', 'beat two'] } : {})));
      const res = await runBF1076(recs1076c);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_OPEN_THREAD_STAGING_AFTERMATH_VOID'), 'BELIEF_OPEN_THREAD_STAGING_AFTERMATH_VOID should fire');
    });

    it('BELIEF_OPEN_THREAD_STAGING_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a visually dense scene within 2 scenes', async () => {
      const recs1076cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { unresolvedClues: ['c1', 'c2', 'c3'] } : ([1, 9].includes(i) ? { visualBeats: ['beat one', 'beat two'] } : {})));
      const res = await runBF1076(recs1076cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_OPEN_THREAD_STAGING_AFTERMATH_VOID'), 'BELIEF_OPEN_THREAD_STAGING_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1034 — beliefPass: belief seed-curiosity aftermath void, belief open-thread-emotional aftermath void, belief open-thread-relational aftermath void', async () => {
    ...
    // FIRE: aftermath signal only at 8,9 — outside both trigger windows {1,2} and {4,5} → every trigger
    // void → fires. NO-FIRE: aftermath at 1 (inside trigger 0's window) and 9 → trigger 0 not void → no fire.
    it('BELIEF_SEED_CURIOSITY_AFTERMATH_VOID fires when every seed has no curiosity rise within 2 scenes', async () => {
      const recs1034a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { seededClueIds: ['c1'] } : ([8, 9].includes(i) ? { curiosityDelta: 1 } : {})));
      const res = await runBF1034(recs1034a);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_SEED_CURIOSITY_AFTERMATH_VOID'), 'BELIEF_SEED_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('BELIEF_SEED_CURIOSITY_AFTERMATH_VOID does not fire when a seed is followed by a curiosity rise within 2 scenes', async () => {
      const recs1034an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { seededClueIds: ['c1'] } : ([1, 9].includes(i) ? { curiosityDelta: 1 } : {})));
      const res = await runBF1034(recs1034an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_SEED_CURIOSITY_AFTERMATH_VOID'), 'BELIEF_SEED_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID fires when every heavy clue-debt scene has no emotional shift within 2 scenes', async () => {
      const recs1034b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { unresolvedClues: ['c1', 'c2', 'c3'] } : ([8, 9].includes(i) ? { emotionalShift: 'positive' } : {})));
      const res = await runBF1034(recs1034b);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID'), 'BELIEF_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('BELIEF_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by an emotional shift within 2 scenes', async () => {
      const recs1034bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { unresolvedClues: ['c1', 'c2', 'c3'] } : ([1, 9].includes(i) ? { emotionalShift: 'positive' } : {})));
      const res = await runBF1034(recs1034bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID'), 'BELIEF_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID fires when every heavy clue-debt scene has no relationship shift within 2 scenes', async () => {
      const recs1034c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { unresolvedClues: ['c1', 'c2', 'c3'] } : ([8, 9].includes(i) ? { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] } : {})));
      const res = await runBF1034(recs1034c);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID'), 'BELIEF_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('BELIEF_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a relationship shift within 2 scenes', async () => {
      const recs1034cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { unresolvedClues: ['c1', 'c2', 'c3'] } : ([1, 9].includes(i) ? { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] } : {})));
      const res = await runBF1034(recs1034cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID'), 'BELIEF_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1006 — beliefPass: belief stakes-emotional aftermath void, belief seed-suspense aftermath void, belief payoff-emotional aftermath void', async () => {
    ...
    // FIRE: aftermath signal only at 8,9 — outside both trigger windows {1,2} and {4,5} → every trigger
    // void → fires. NO-FIRE: aftermath at 1 (inside trigger 0's window) and 9 → trigger 0 not void → no fire.
    it('BELIEF_STAKES_EMOTIONAL_AFTERMATH_VOID fires when every stakes-raise has no emotional shift within 2 scenes', async () => {
      const recs1006a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { purpose: 'raise_stakes' } : ([8, 9].includes(i) ? { emotionalShift: 'positive' } : {})));
      const res = await runBF1006(recs1006a);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'BELIEF_STAKES_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('BELIEF_STAKES_EMOTIONAL_AFTERMATH_VOID does not fire when a stakes-raise is followed by an emotional shift within 2 scenes', async () => {
      const recs1006an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { purpose: 'raise_stakes' } : ([1, 9].includes(i) ? { emotionalShift: 'positive' } : {})));
      const res = await runBF1006(recs1006an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'BELIEF_STAKES_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_SEED_SUSPENSE_AFTERMATH_VOID fires when every seed has no rise in suspense within 2 scenes', async () => {
      const recs1006b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { seededClueIds: ['c1'] } : ([8, 9].includes(i) ? { suspenseDelta: 1 } : {})));
      const res = await runBF1006(recs1006b);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_SEED_SUSPENSE_AFTERMATH_VOID'), 'BELIEF_SEED_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('BELIEF_SEED_SUSPENSE_AFTERMATH_VOID does not fire when a seed is followed by rising suspense within 2 scenes', async () => {
      const recs1006bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { seededClueIds: ['c1'] } : ([1, 9].includes(i) ? { suspenseDelta: 1 } : {})));
      const res = await runBF1006(recs1006bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_SEED_SUSPENSE_AFTERMATH_VOID'), 'BELIEF_SEED_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_PAYOFF_EMOTIONAL_AFTERMATH_VOID fires when every payoff has no emotional shift within 2 scenes', async () => {
      const recs1006c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { payoffSetupIds: ['s1'] } : ([8, 9].includes(i) ? { emotionalShift: 'positive' } : {})));
      const res = await runBF1006(recs1006c);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_PAYOFF_EMOTIONAL_AFTERMATH_VOID'), 'BELIEF_PAYOFF_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('BELIEF_PAYOFF_EMOTIONAL_AFTERMATH_VOID does not fire when a payoff is followed by an emotional shift within 2 scenes', async () => {
      const recs1006cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { payoffSetupIds: ['s1'] } : ([1, 9].includes(i) ? { emotionalShift: 'positive' } : {})));
      const res = await runBF1006(recs1006cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_PAYOFF_EMOTIONAL_AFTERMATH_VOID'), 'BELIEF_PAYOFF_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 992 — beliefPass: belief stakes suspense aftermath void, belief payoff relationship aftermath void, belief open-thread curiosity aftermath void', async () => {
    const runBF992 = async (records: ScreenplaySceneRecord[]) => {
      const { beliefPass } = await import('../../server/nvm/revision/passes/belief.ts');
      return beliefPass({ fountain: '', original: '', records, structure: {} as any, annotations: [], approvedSpans: [] });
    };
    ...
    // FIRE: aftermath signal only at 8,9 — outside both trigger windows {1,2} and {4,5} → every trigger
    // void → fires. NO-FIRE: aftermath at 1 (inside trigger 0's window) and 9 → trigger 0 not void → no fire.
    it('BELIEF_STAKES_SUSPENSE_AFTERMATH_VOID fires when every stakes-raise has no rise in suspense within 2 scenes', async () => {
      const recs992a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { purpose: 'raise_stakes' } : ([8, 9].includes(i) ? { suspenseDelta: 1 } : {})));
      const res = await runBF992(recs992a);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_STAKES_SUSPENSE_AFTERMATH_VOID'), 'BELIEF_STAKES_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('BELIEF_STAKES_SUSPENSE_AFTERMATH_VOID does not fire when a stakes-raise is followed by rising suspense within 2 scenes', async () => {
      const recs992an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { purpose: 'raise_stakes' } : ([1, 9].includes(i) ? { suspenseDelta: 1 } : {})));
      const res = await runBF992(recs992an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_STAKES_SUSPENSE_AFTERMATH_VOID'), 'BELIEF_STAKES_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_PAYOFF_RELATIONSHIP_AFTERMATH_VOID fires when every payoff has no relationship shift within 2 scenes', async () => {
      const recs992b = Array.from({ length: 10 }, (_, i) =>
      ...
      const res = await runBF992(recs992b);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_PAYOFF_RELATIONSHIP_AFTERMATH_VOID'), 'BELIEF_PAYOFF_RELATIONSHIP_AFTERMATH_VOID should fire');
    });

    it('BELIEF_PAYOFF_RELATIONSHIP_AFTERMATH_VOID does not fire when a payoff is followed by a relationship shift within 2 scenes', async () => {
      const recs992bn = Array.from({ length: 10 }, (_, i) =>
      ...
      const res = await runBF992(recs992bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_PAYOFF_RELATIONSHIP_AFTERMATH_VOID'), 'BELIEF_PAYOFF_RELATIONSHIP_AFTERMATH_VOID should not fire');
    });

    it('BELIEF_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID fires when every heavy clue-debt scene has no curiosity raised within 2 scenes', async () => {
      const recs992c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { unresolvedClues: ['c1', 'c2', 'c3'] } : ([8, 9].includes(i) ? { curiosityDelta: 1 } : {})));
      const res = await runBF992(recs992c);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID'), 'BELIEF_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('BELIEF_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by new curiosity within 2 scenes', async () => {
      const recs992cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, [0, 3].includes(i) ? { unresolvedClues: ['c1', 'c2', 'c3'] } : ([1, 9].includes(i) ? { curiosityDelta: 1 } : {})));
      const res = await runBF992(recs992cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID'), 'BELIEF_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 922 — beliefPass: belief stakes zone imbalance, belief revelation purpose zone imbalance, belief negative emotion zone imbalance', async () => {
    ...
    // Zone geometry n=10: Z0={0,1,2}, Z1={3,4}, Z2={5,6,7}, Z3={8,9}. Target at 0,1,2,8,9 →
    // Z0 3/5=60% (bloat), Z1 and Z2 empty → fires. Target at 0,3,5,8 → every zone touched → no-fire.
    it('BELIEF_STAKES_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of stakes-raising scenes', async () => {
      const recs922a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2, 8, 9].includes(i) ? 'raise_stakes' : 'establish_world' }));
      const res = await runBF922(recs922a);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_STAKES_ZONE_IMBALANCE'), 'BELIEF_STAKES_ZONE_IMBALANCE should fire');
    });

    it('BELIEF_STAKES_ZONE_IMBALANCE does not fire when stakes-raising scenes touch every zone', async () => {
      const recs922an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 5, 8].includes(i) ? 'raise_stakes' : 'establish_world' }));
      const res = await runBF922(recs922an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_STAKES_ZONE_IMBALANCE'), 'BELIEF_STAKES_ZONE_IMBALANCE should not fire');
    });

    it('BELIEF_REVELATION_PURPOSE_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of revelation-purposed scenes', async () => {
      const recs922b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2, 8, 9].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runBF922(recs922b);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_REVELATION_PURPOSE_ZONE_IMBALANCE'), 'BELIEF_REVELATION_PURPOSE_ZONE_IMBALANCE should fire');
    });

    it('BELIEF_REVELATION_PURPOSE_ZONE_IMBALANCE does not fire when revelation-purposed scenes touch every zone', async () => {
      const recs922bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 5, 8].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runBF922(recs922bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_REVELATION_PURPOSE_ZONE_IMBALANCE'), 'BELIEF_REVELATION_PURPOSE_ZONE_IMBALANCE should not fire');
    });

    it('BELIEF_NEGATIVE_EMOTION_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of negative-shift scenes', async () => {
      const recs922c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2, 8, 9].includes(i) ? 'negative' : 'neutral' }));
      const res = await runBF922(recs922c);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_NEGATIVE_EMOTION_ZONE_IMBALANCE'), 'BELIEF_NEGATIVE_EMOTION_ZONE_IMBALANCE should fire');
    });

    it('BELIEF_NEGATIVE_EMOTION_ZONE_IMBALANCE does not fire when negative-shift scenes touch every zone', async () => {
      const recs922cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 3, 5, 8].includes(i) ? 'negative' : 'neutral' }));
      const res = await runBF922(recs922cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_NEGATIVE_EMOTION_ZONE_IMBALANCE'), 'BELIEF_NEGATIVE_EMOTION_ZONE_IMBALANCE should not fire');
    });
  });
  ...
  describe('Wave 838 — beliefPass: belief introduce conflict drought run, belief positive emotion drought run, belief establish world zone cluster', async () => {
    ...
    // BELIEF_INTRODUCE_CONFLICT_DROUGHT_RUN fire:
    // n=10; introduce_conflict present at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('BELIEF_INTRODUCE_CONFLICT_DROUGHT_RUN fires when a long run has no new conflict', async () => {
      const recs838a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'introduce_conflict' : 'complicate' }),
      );
      const res = await runBF838(recs838a);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_INTRODUCE_CONFLICT_DROUGHT_RUN'), 'BELIEF_INTRODUCE_CONFLICT_DROUGHT_RUN should fire');
    });

    it('BELIEF_INTRODUCE_CONFLICT_DROUGHT_RUN does not fire when conflict-introducing scenes are evenly spread', async () => {
      const recs838an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'introduce_conflict' : 'complicate' }),
      );
      const res = await runBF838(recs838an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_INTRODUCE_CONFLICT_DROUGHT_RUN'), 'BELIEF_INTRODUCE_CONFLICT_DROUGHT_RUN should not fire');
    });

    // BELIEF_POSITIVE_EMOTION_DROUGHT_RUN fire:
    // n=10; positive-emotion at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('BELIEF_POSITIVE_EMOTION_DROUGHT_RUN fires when a long run has no positive-emotion charge', async () => {
      const recs838b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2].includes(i) ? 'positive' : 'neutral' }),
      );
      const res = await runBF838(recs838b);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_POSITIVE_EMOTION_DROUGHT_RUN'), 'BELIEF_POSITIVE_EMOTION_DROUGHT_RUN should fire');
    });

    it('BELIEF_POSITIVE_EMOTION_DROUGHT_RUN does not fire when positive-emotion scenes are evenly spread', async () => {
      const recs838bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 3, 6, 9].includes(i) ? 'positive' : 'neutral' }),
      );
      const res = await runBF838(recs838bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_POSITIVE_EMOTION_DROUGHT_RUN'), 'BELIEF_POSITIVE_EMOTION_DROUGHT_RUN should not fire');
    });

    // BELIEF_ESTABLISH_WORLD_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; establish_world scenes at 0,1,2 → 100% opening third
    it('BELIEF_ESTABLISH_WORLD_ZONE_CLUSTER fires when >75% of world-establishing scenes cluster in one third', async () => {
      const recs838c = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'establish_world' : 'complicate' }),
      );
      const res = await runBF838(recs838c);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_ESTABLISH_WORLD_ZONE_CLUSTER'), 'BELIEF_ESTABLISH_WORLD_ZONE_CLUSTER should fire');
    });

    it('BELIEF_ESTABLISH_WORLD_ZONE_CLUSTER does not fire when world-establishing scenes spread across thirds', async () => {
      const recs838cn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'establish_world' : 'complicate' }),
      );
      const res = await runBF838(recs838cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_ESTABLISH_WORLD_ZONE_CLUSTER'), 'BELIEF_ESTABLISH_WORLD_ZONE_CLUSTER should not fire');
    });
  });

  describe('Wave 824 — beliefPass: belief turning point drought run, belief introduce conflict zone cluster, belief positive emotion zone cluster', async () => {
    const runBF824 = async (records: ScreenplaySceneRecord[]) => {
      const { beliefPass } = await import('../../server/nvm/revision/passes/belief.ts');
      return beliefPass({ fountain: '', original: '', records, structure: {} as any, annotations: [], approvedSpans: [] });
    };

    // BELIEF_TURNING_POINT_DROUGHT_RUN fire:
    // n=10; turning_point present at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('BELIEF_TURNING_POINT_DROUGHT_RUN fires when a long run has no turning point', async () => {
      const recs824a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'turning_point' : 'complicate' }),
      );
      const res = await runBF824(recs824a);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_TURNING_POINT_DROUGHT_RUN'), 'BELIEF_TURNING_POINT_DROUGHT_RUN should fire');
    });

    it('BELIEF_TURNING_POINT_DROUGHT_RUN does not fire when turning points are evenly spread', async () => {
      const recs824an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'turning_point' : 'complicate' }),
      );
      const res = await runBF824(recs824an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_TURNING_POINT_DROUGHT_RUN'), 'BELIEF_TURNING_POINT_DROUGHT_RUN should not fire');
    });

    // BELIEF_INTRODUCE_CONFLICT_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; introduce_conflict scenes at 0,1,2 → 100% opening third
    it('BELIEF_INTRODUCE_CONFLICT_ZONE_CLUSTER fires when >75% of conflict-introducing scenes cluster in one third', async () => {
      const recs824b = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'introduce_conflict' : 'complicate' }),
      );
      const res = await runBF824(recs824b);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_INTRODUCE_CONFLICT_ZONE_CLUSTER'), 'BELIEF_INTRODUCE_CONFLICT_ZONE_CLUSTER should fire');
    });

    it('BELIEF_INTRODUCE_CONFLICT_ZONE_CLUSTER does not fire when conflict-introducing scenes spread across thirds', async () => {
      const recs824bn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'introduce_conflict' : 'complicate' }),
      );
      const res = await runBF824(recs824bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_INTRODUCE_CONFLICT_ZONE_CLUSTER'), 'BELIEF_INTRODUCE_CONFLICT_ZONE_CLUSTER should not fire');
    });

    // BELIEF_POSITIVE_EMOTION_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; positive-emotion scenes at 0,1,2 → 100% opening third
    it('BELIEF_POSITIVE_EMOTION_ZONE_CLUSTER fires when >75% of positive-emotion scenes cluster in one third', async () => {
      const recs824c = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2].includes(i) ? 'positive' : 'neutral' }),
      );
      const res = await runBF824(recs824c);
      assert.ok(res.issues.some((i: any) => i.rule === 'BELIEF_POSITIVE_EMOTION_ZONE_CLUSTER'), 'BELIEF_POSITIVE_EMOTION_ZONE_CLUSTER should fire');
    });

    it('BELIEF_POSITIVE_EMOTION_ZONE_CLUSTER does not fire when positive-emotion scenes spread across thirds', async () => {
      const recs824cn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 4, 8].includes(i) ? 'positive' : 'neutral' }),
      );
      const res = await runBF824(recs824cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'BELIEF_POSITIVE_EMOTION_ZONE_CLUSTER'), 'BELIEF_POSITIVE_EMOTION_ZONE_CLUSTER should not fire');
    });
  });
