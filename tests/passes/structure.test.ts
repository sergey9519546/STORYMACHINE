...
describe('Wave 152 — structurePass: revelation drought, false climax, act symmetry', async () => {
  ...
  describe('Wave 1171 — structurePass: structure suspense-staging aftermath void, structure suspense-dialogue-highlight aftermath void, structure seed-emotional aftermath void', async () => {
    ...
    it('STRUCTURE_SUSPENSE_STAGING_AFTERMATH_VOID fires when every suspense-rise is followed by two scenes with no heavily-staged scene', async () => {
      const recs1171a = Array.from({ length: 10 }, (_, i) => {
      ...
      const res = await runST1171(recs1171a);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_SUSPENSE_STAGING_AFTERMATH_VOID'), 'STRUCTURE_SUSPENSE_STAGING_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_SUSPENSE_STAGING_AFTERMATH_VOID does not fire when a suspense-rise is followed by a heavily-staged scene within its window', async () => {
      const recs1171an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runST1171(recs1171an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_SUSPENSE_STAGING_AFTERMATH_VOID'), 'STRUCTURE_SUSPENSE_STAGING_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_SUSPENSE_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every suspense-rise is followed by two scenes with no highlighted dialogue', async () => {
      const recs1171b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runST1171(recs1171b);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_SUSPENSE_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'STRUCTURE_SUSPENSE_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_SUSPENSE_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a suspense-rise is followed by highlighted dialogue within its window', async () => {
      const recs1171bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runST1171(recs1171bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_SUSPENSE_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'STRUCTURE_SUSPENSE_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_SEED_EMOTIONAL_AFTERMATH_VOID fires when every seed scene is followed by two scenes with no emotional shift', async () => {
      const recs1171c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['clue1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runST1171(recs1171c);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_SEED_EMOTIONAL_AFTERMATH_VOID'), 'STRUCTURE_SEED_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_SEED_EMOTIONAL_AFTERMATH_VOID does not fire when a seed scene is followed by an emotional shift within its window', async () => {
      const recs1171cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['clue1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runST1171(recs1171cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_SEED_EMOTIONAL_AFTERMATH_VOID'), 'STRUCTURE_SEED_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1157 — structurePass: structure suspense-emotional aftermath void, structure suspense-relational aftermath void, structure seed-curiosity aftermath void', async () => {
    const runST1157 = async (records: ScreenplaySceneRecord[]) => {
      const { structurePass } = await import('../../server/nvm/revision/passes/structure.ts');
      return structurePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('STRUCTURE_SUSPENSE_EMOTIONAL_AFTERMATH_VOID fires when every suspense-rise is followed by two scenes with no emotional shift', async () => {
      const recs1157a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runST1157(recs1157a);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_SUSPENSE_EMOTIONAL_AFTERMATH_VOID'), 'STRUCTURE_SUSPENSE_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_SUSPENSE_EMOTIONAL_AFTERMATH_VOID does not fire when a suspense-rise is followed by an emotional shift within its window', async () => {
      const recs1157an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runST1157(recs1157an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_SUSPENSE_EMOTIONAL_AFTERMATH_VOID'), 'STRUCTURE_SUSPENSE_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_SUSPENSE_RELATIONAL_AFTERMATH_VOID fires when every suspense-rise is followed by two scenes with no relationship shift', async () => {
      const recs1157b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runST1157(recs1157b);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_SUSPENSE_RELATIONAL_AFTERMATH_VOID'), 'STRUCTURE_SUSPENSE_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_SUSPENSE_RELATIONAL_AFTERMATH_VOID does not fire when a suspense-rise is followed by a relationship shift within its window', async () => {
      const recs1157bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runST1157(recs1157bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_SUSPENSE_RELATIONAL_AFTERMATH_VOID'), 'STRUCTURE_SUSPENSE_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_SEED_CURIOSITY_AFTERMATH_VOID fires when every seed scene is followed by two scenes with no curiosity rise', async () => {
      const recs1157c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['clue1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1157(recs1157c);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_SEED_CURIOSITY_AFTERMATH_VOID'), 'STRUCTURE_SEED_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_SEED_CURIOSITY_AFTERMATH_VOID does not fire when a seed scene is followed by a curiosity rise within its window', async () => {
      const recs1157cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['clue1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1157(recs1157cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_SEED_CURIOSITY_AFTERMATH_VOID'), 'STRUCTURE_SEED_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1101 — structurePass: structure revelation-emotional aftermath void, structure revelation-suspense aftermath void, structure revelation-relational aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('STRUCTURE_REVELATION_EMOTIONAL_AFTERMATH_VOID fires when every revelation is followed by two scenes with no emotional shift', async () => {
      const recs1101a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runST1101(recs1101a);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_REVELATION_EMOTIONAL_AFTERMATH_VOID'), 'STRUCTURE_REVELATION_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_REVELATION_EMOTIONAL_AFTERMATH_VOID does not fire when a revelation is followed by an emotional shift within its window', async () => {
      const recs1101an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runST1101(recs1101an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_REVELATION_EMOTIONAL_AFTERMATH_VOID'), 'STRUCTURE_REVELATION_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_REVELATION_SUSPENSE_AFTERMATH_VOID fires when every revelation is followed by two scenes with no suspense rise', async () => {
      const recs1101b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1101(recs1101b);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_REVELATION_SUSPENSE_AFTERMATH_VOID'), 'STRUCTURE_REVELATION_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_REVELATION_SUSPENSE_AFTERMATH_VOID does not fire when a revelation is followed by a suspense rise within its window', async () => {
      const recs1101bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1101(recs1101bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_REVELATION_SUSPENSE_AFTERMATH_VOID'), 'STRUCTURE_REVELATION_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_REVELATION_RELATIONAL_AFTERMATH_VOID fires when every revelation is followed by two scenes with no relationship shift', async () => {
      const recs1101c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runST1101(recs1101c);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_REVELATION_RELATIONAL_AFTERMATH_VOID'), 'STRUCTURE_REVELATION_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_REVELATION_RELATIONAL_AFTERMATH_VOID does not fire when a revelation is followed by a relationship shift within its window', async () => {
      const recs1101cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runST1101(recs1101cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_REVELATION_RELATIONAL_AFTERMATH_VOID'), 'STRUCTURE_REVELATION_RELATIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1087 — structurePass: structure stakes-dialogue-highlight aftermath void, structure stakes-staging aftermath void, structure revelation-curiosity aftermath void', async () => {
    const runST1087 = async (records: ScreenplaySceneRecord[]) => {
      const { structurePass } = await import('../../server/nvm/revision/passes/structure.ts');
      return structurePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('STRUCTURE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no highlighted dialogue', async () => {
      const recs1087a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runST1087(recs1087a);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'STRUCTURE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a stakes-raise is followed by highlighted dialogue within its window', async () => {
      const recs1087an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runST1087(recs1087an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'STRUCTURE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_STAKES_STAGING_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no visually dense scene', async () => {
      const recs1087b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runST1087(recs1087b);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_STAKES_STAGING_AFTERMATH_VOID'), 'STRUCTURE_STAKES_STAGING_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_STAKES_STAGING_AFTERMATH_VOID does not fire when a stakes-raise is followed by a visually dense scene within its window', async () => {
      const recs1087bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runST1087(recs1087bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_STAKES_STAGING_AFTERMATH_VOID'), 'STRUCTURE_STAKES_STAGING_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_REVELATION_CURIOSITY_AFTERMATH_VOID fires when every revelation is followed by two scenes with no curiosity rise', async () => {
      const recs1087c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1087(recs1087c);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_REVELATION_CURIOSITY_AFTERMATH_VOID'), 'STRUCTURE_REVELATION_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_REVELATION_CURIOSITY_AFTERMATH_VOID does not fire when a revelation is followed by a curiosity rise within its window', async () => {
      const recs1087cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1087(recs1087cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_REVELATION_CURIOSITY_AFTERMATH_VOID'), 'STRUCTURE_REVELATION_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1073 — structurePass: structure payoff-staging aftermath void, structure turn-suspense aftermath void, structure open-thread-suspense aftermath void', async () => {
    const runST1073 = async (records: ScreenplaySceneRecord[]) => {
      const { structurePass } = await import('../../server/nvm/revision/passes/structure.ts');
      return structurePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('STRUCTURE_PAYOFF_STAGING_AFTERMATH_VOID fires when every payoff is followed by two scenes with no visually dense scene', async () => {
      const recs1073a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runST1073(recs1073a);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_PAYOFF_STAGING_AFTERMATH_VOID'), 'STRUCTURE_PAYOFF_STAGING_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_PAYOFF_STAGING_AFTERMATH_VOID does not fire when a payoff is followed by a visually dense scene within its window', async () => {
      const recs1073an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runST1073(recs1073an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_PAYOFF_STAGING_AFTERMATH_VOID'), 'STRUCTURE_PAYOFF_STAGING_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_TURN_SUSPENSE_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no suspense rise', async () => {
      const recs1073b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1073(recs1073b);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_TURN_SUSPENSE_AFTERMATH_VOID'), 'STRUCTURE_TURN_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_TURN_SUSPENSE_AFTERMATH_VOID does not fire when a dramatic turn is followed by a suspense rise within its window', async () => {
      const recs1073bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1073(recs1073bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_TURN_SUSPENSE_AFTERMATH_VOID'), 'STRUCTURE_TURN_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no suspense rise', async () => {
      const recs1073c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1073(recs1073c);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID'), 'STRUCTURE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a suspense rise within its window', async () => {
      const recs1073cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1073(recs1073cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID'), 'STRUCTURE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1059 — structurePass: structure payoff-suspense aftermath void, structure turn-dialogue-highlight aftermath void, structure open-thread-staging aftermath void', async () => {
    const runST1059 = async (records: ScreenplaySceneRecord[]) => {
      const { structurePass } = await import('../../server/nvm/revision/passes/structure.ts');
      return structurePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('STRUCTURE_PAYOFF_SUSPENSE_AFTERMATH_VOID fires when every payoff is followed by two scenes with no suspense rise', async () => {
      const recs1059a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1059(recs1059a);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_PAYOFF_SUSPENSE_AFTERMATH_VOID'), 'STRUCTURE_PAYOFF_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_PAYOFF_SUSPENSE_AFTERMATH_VOID does not fire when a payoff is followed by a suspense rise within its window', async () => {
      const recs1059an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1059(recs1059an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_PAYOFF_SUSPENSE_AFTERMATH_VOID'), 'STRUCTURE_PAYOFF_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no highlighted dialogue', async () => {
      const recs1059b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runST1059(recs1059b);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'STRUCTURE_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a dramatic turn is followed by highlighted dialogue within its window', async () => {
      const recs1059bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runST1059(recs1059bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'STRUCTURE_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_OPEN_THREAD_STAGING_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no visually dense scene', async () => {
      const recs1059c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runST1059(recs1059c);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_OPEN_THREAD_STAGING_AFTERMATH_VOID'), 'STRUCTURE_OPEN_THREAD_STAGING_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_OPEN_THREAD_STAGING_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a visually dense scene within its window', async () => {
      const recs1059cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runST1059(recs1059cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_OPEN_THREAD_STAGING_AFTERMATH_VOID'), 'STRUCTURE_OPEN_THREAD_STAGING_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1045 — structurePass: structure turn-emotional aftermath void, structure open-thread-curiosity aftermath void, structure open-thread-relational aftermath void', async () => {
    const runST1045 = async (records: ScreenplaySceneRecord[]) => {
      const { structurePass } = await import('../../server/nvm/revision/passes/structure.ts');
      return structurePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('STRUCTURE_TURN_EMOTIONAL_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no emotional shift', async () => {
      const recs1045a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runST1045(recs1045a);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_TURN_EMOTIONAL_AFTERMATH_VOID'), 'STRUCTURE_TURN_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_TURN_EMOTIONAL_AFTERMATH_VOID does not fire when a dramatic turn is followed by an emotional shift within its window', async () => {
      const recs1045an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runST1045(recs1045an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_TURN_EMOTIONAL_AFTERMATH_VOID'), 'STRUCTURE_TURN_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no curiosity rise', async () => {
      const recs1045b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1045(recs1045b);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID'), 'STRUCTURE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a curiosity rise within its window', async () => {
      const recs1045bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1045(recs1045bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID'), 'STRUCTURE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no relationship shift', async () => {
      const recs1045c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runST1045(recs1045c);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID'), 'STRUCTURE_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a relationship shift within its window', async () => {
      const recs1045cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runST1045(recs1045cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID'), 'STRUCTURE_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1031 — structurePass: structure stakes-emotional aftermath void, structure turn-relational aftermath void, structure payoff-curiosity aftermath void', async () => {
    const runST1031 = async (records: ScreenplaySceneRecord[]) => {
      const { structurePass } = await import('../../server/nvm/revision/passes/structure.ts');
      return structurePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('STRUCTURE_STAKES_EMOTIONAL_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no emotional shift', async () => {
      const recs1031a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runST1031(recs1031a);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'STRUCTURE_STAKES_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_STAKES_EMOTIONAL_AFTERMATH_VOID does not fire when a stakes-raise is followed by an emotional shift within its window', async () => {
      const recs1031an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runST1031(recs1031an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'STRUCTURE_STAKES_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_TURN_RELATIONAL_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no relationship shift', async () => {
      const recs1031b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runST1031(recs1031b);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_TURN_RELATIONAL_AFTERMATH_VOID'), 'STRUCTURE_TURN_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_TURN_RELATIONAL_AFTERMATH_VOID does not fire when a dramatic turn is followed by a relationship shift within its window', async () => {
      const recs1031bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runST1031(recs1031bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_TURN_RELATIONAL_AFTERMATH_VOID'), 'STRUCTURE_TURN_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_PAYOFF_CURIOSITY_AFTERMATH_VOID fires when every payoff is followed by two scenes with no curiosity rise', async () => {
      const recs1031c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1031(recs1031c);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_PAYOFF_CURIOSITY_AFTERMATH_VOID'), 'STRUCTURE_PAYOFF_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_PAYOFF_CURIOSITY_AFTERMATH_VOID does not fire when a payoff is followed by a curiosity rise within its window', async () => {
      const recs1031cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1031(recs1031cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_PAYOFF_CURIOSITY_AFTERMATH_VOID'), 'STRUCTURE_PAYOFF_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1017 — structurePass: structure turn-curiosity aftermath void, structure payoff-emotional aftermath void, structure stakes-relational aftermath void', async () => {
    const runST1017 = async (records: ScreenplaySceneRecord[]) => {
      const { structurePass } = await import('../../server/nvm/revision/passes/structure.ts');
      return structurePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('STRUCTURE_TURN_CURIOSITY_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no new curiosity', async () => {
      const recs1017a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1017(recs1017a);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_TURN_CURIOSITY_AFTERMATH_VOID'), 'STRUCTURE_TURN_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_TURN_CURIOSITY_AFTERMATH_VOID does not fire when a dramatic turn is followed by new curiosity within its window', async () => {
      const recs1017an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runST1017(recs1017an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_TURN_CURIOSITY_AFTERMATH_VOID'), 'STRUCTURE_TURN_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_PAYOFF_EMOTIONAL_AFTERMATH_VOID fires when every payoff is followed by two scenes with no emotional shift', async () => {
      const recs1017b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['setup1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runST1017(recs1017b);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_PAYOFF_EMOTIONAL_AFTERMATH_VOID'), 'STRUCTURE_PAYOFF_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_PAYOFF_EMOTIONAL_AFTERMATH_VOID does not fire when a payoff is followed by an emotional shift within its window', async () => {
      const recs1017bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['setup1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runST1017(recs1017bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_PAYOFF_EMOTIONAL_AFTERMATH_VOID'), 'STRUCTURE_PAYOFF_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('STRUCTURE_STAKES_RELATIONAL_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no relationship shift', async () => {
      const recs1017c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runST1017(recs1017c);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_STAKES_RELATIONAL_AFTERMATH_VOID'), 'STRUCTURE_STAKES_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('STRUCTURE_STAKES_RELATIONAL_AFTERMATH_VOID does not fire when a stakes-raise is followed by a relationship shift within its window', async () => {
      const recs1017cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runST1017(recs1017cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_STAKES_RELATIONAL_AFTERMATH_VOID'), 'STRUCTURE_STAKES_RELATIONAL_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 933 — structurePass: structure stakes zone imbalance, structure revelation purpose zone imbalance, structure negative emotion zone imbalance', async () => {
    ...
    // Zone geometry n=10: Z0={0,1,2}, Z1={3,4}, Z2={5,6,7}, Z3={8,9}. Target at 0,1,2,8,9 →
    // Z0 3/5=60% (bloat), Z1 and Z2 empty → fires. Target at 0,3,5,8 → every zone touched → no-fire.
    it('STRUCTURE_STAKES_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of stakes-raising scenes', async () => {
      const recs933a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2, 8, 9].includes(i) ? 'raise_stakes' : 'establish_world' }));
      const res = await runST933(recs933a);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_STAKES_ZONE_IMBALANCE'), 'STRUCTURE_STAKES_ZONE_IMBALANCE should fire');
    });

    it('STRUCTURE_STAKES_ZONE_IMBALANCE does not fire when stakes-raising scenes touch every zone', async () => {
      const recs933an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 5, 8].includes(i) ? 'raise_stakes' : 'establish_world' }));
      const res = await runST933(recs933an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_STAKES_ZONE_IMBALANCE'), 'STRUCTURE_STAKES_ZONE_IMBALANCE should not fire');
    });

    it('STRUCTURE_REVELATION_PURPOSE_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of revelation-purposed scenes', async () => {
      const recs933b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2, 8, 9].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runST933(recs933b);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_REVELATION_PURPOSE_ZONE_IMBALANCE'), 'STRUCTURE_REVELATION_PURPOSE_ZONE_IMBALANCE should fire');
    });

    it('STRUCTURE_REVELATION_PURPOSE_ZONE_IMBALANCE does not fire when revelation-purposed scenes touch every zone', async () => {
      const recs933bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 5, 8].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runST933(recs933bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_REVELATION_PURPOSE_ZONE_IMBALANCE'), 'STRUCTURE_REVELATION_PURPOSE_ZONE_IMBALANCE should not fire');
    });

    it('STRUCTURE_NEGATIVE_EMOTION_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of negative-shift scenes', async () => {
      const recs933c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2, 8, 9].includes(i) ? 'negative' : 'neutral' }));
      const res = await runST933(recs933c);
      assert.ok(res.issues.some((i: any) => i.rule === 'STRUCTURE_NEGATIVE_EMOTION_ZONE_IMBALANCE'), 'STRUCTURE_NEGATIVE_EMOTION_ZONE_IMBALANCE should fire');
    });

    it('STRUCTURE_NEGATIVE_EMOTION_ZONE_IMBALANCE does not fire when negative-shift scenes touch every zone', async () => {
      const recs933cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 3, 5, 8].includes(i) ? 'negative' : 'neutral' }));
      const res = await runST933(recs933cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'STRUCTURE_NEGATIVE_EMOTION_ZONE_IMBALANCE'), 'STRUCTURE_NEGATIVE_EMOTION_ZONE_IMBALANCE should not fire');
    });
  });
