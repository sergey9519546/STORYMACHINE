...
Good riddance to you.`;
  ...
  describe('Wave 1145 — voicePass: voice revelation-relational aftermath void, voice open-thread-suspense aftermath void, voice open-thread-emotional aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('VOICE_REVELATION_RELATIONAL_AFTERMATH_VOID fires when every revelation is followed by two scenes with no relationship shift', async () => {
      const recs1145a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runV1145(recs1145a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_REVELATION_RELATIONAL_AFTERMATH_VOID'), 'VOICE_REVELATION_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('VOICE_REVELATION_RELATIONAL_AFTERMATH_VOID does not fire when a revelation is followed by a relationship shift within its window', async () => {
      const recs1145an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runV1145(recs1145an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_REVELATION_RELATIONAL_AFTERMATH_VOID'), 'VOICE_REVELATION_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('VOICE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID fires when every heavily-unresolved scene is followed by two scenes with no suspense rise', async () => {
      const recs1145b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1145(recs1145b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID'), 'VOICE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('VOICE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID does not fire when a heavily-unresolved scene is followed by a suspense rise within its window', async () => {
      const recs1145bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1145(recs1145bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID'), 'VOICE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('VOICE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID fires when every heavily-unresolved scene is followed by two scenes with no emotional shift', async () => {
      const recs1145c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runV1145(recs1145c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID'), 'VOICE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('VOICE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID does not fire when a heavily-unresolved scene is followed by an emotional shift within its window', async () => {
      const recs1145cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runV1145(recs1145cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID'), 'VOICE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1131 — voicePass: voice revelation-emotional aftermath void, voice revelation-suspense aftermath void, voice open-thread-curiosity aftermath void', async () => {
    const runV1131 = async (records: ScreenplaySceneRecord[]) => {
      const { voicePass } = await import('../../server/nvm/revision/passes/voice.ts');
      return voicePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('VOICE_REVELATION_EMOTIONAL_AFTERMATH_VOID fires when every revelation is followed by two scenes with no emotional shift', async () => {
      const recs1131a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runV1131(recs1131a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_REVELATION_EMOTIONAL_AFTERMATH_VOID'), 'VOICE_REVELATION_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('VOICE_REVELATION_EMOTIONAL_AFTERMATH_VOID does not fire when a revelation is followed by an emotional shift within its window', async () => {
      const recs1131an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runV1131(recs1131an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_REVELATION_EMOTIONAL_AFTERMATH_VOID'), 'VOICE_REVELATION_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('VOICE_REVELATION_SUSPENSE_AFTERMATH_VOID fires when every revelation is followed by two scenes with no suspense rise', async () => {
      const recs1131b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1131(recs1131b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_REVELATION_SUSPENSE_AFTERMATH_VOID'), 'VOICE_REVELATION_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('VOICE_REVELATION_SUSPENSE_AFTERMATH_VOID does not fire when a revelation is followed by a suspense rise within its window', async () => {
      const recs1131bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1131(recs1131bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_REVELATION_SUSPENSE_AFTERMATH_VOID'), 'VOICE_REVELATION_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('VOICE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID fires when every heavily-unresolved scene is followed by two scenes with no curiosity rise', async () => {
      const recs1131c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1131(recs1131c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID'), 'VOICE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('VOICE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID does not fire when a heavily-unresolved scene is followed by a curiosity rise within its window', async () => {
      const recs1131cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1131(recs1131cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID'), 'VOICE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1103 — voicePass: voice seed-staging aftermath void, voice stakes-staging aftermath void, voice turn-dialogue-highlight aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('VOICE_SEED_STAGING_AFTERMATH_VOID fires when every seed is followed by two scenes with no heavily-staged scene', async () => {
      const recs1103a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runV1103(recs1103a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_SEED_STAGING_AFTERMATH_VOID'), 'VOICE_SEED_STAGING_AFTERMATH_VOID should fire');
    });

    it('VOICE_SEED_STAGING_AFTERMATH_VOID does not fire when a seed is followed by a heavily-staged scene within its window', async () => {
      const recs1103an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runV1103(recs1103an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_SEED_STAGING_AFTERMATH_VOID'), 'VOICE_SEED_STAGING_AFTERMATH_VOID should not fire');
    });

    it('VOICE_STAKES_STAGING_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no heavily-staged scene', async () => {
      const recs1103b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runV1103(recs1103b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_STAKES_STAGING_AFTERMATH_VOID'), 'VOICE_STAKES_STAGING_AFTERMATH_VOID should fire');
    });

    it('VOICE_STAKES_STAGING_AFTERMATH_VOID does not fire when a stakes-raise is followed by a heavily-staged scene within its window', async () => {
      const recs1103bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runV1103(recs1103bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_STAKES_STAGING_AFTERMATH_VOID'), 'VOICE_STAKES_STAGING_AFTERMATH_VOID should not fire');
    });

    it('VOICE_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no highlighted dialogue', async () => {
      const recs1103c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runV1103(recs1103c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'VOICE_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('VOICE_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a dramatic turn is followed by highlighted dialogue within its window', async () => {
      const recs1103cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runV1103(recs1103cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'VOICE_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1089 — voicePass: voice seed-relational aftermath void, voice stakes-dialogue-highlight aftermath void, voice turn-staging aftermath void', async () => {
    const runV1089 = async (records: ScreenplaySceneRecord[]) => {
      const { voicePass } = await import('../../server/nvm/revision/passes/voice.ts');
      return voicePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('VOICE_SEED_RELATIONAL_AFTERMATH_VOID fires when every seed is followed by two scenes with no relationship shift', async () => {
      const recs1089a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runV1089(recs1089a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_SEED_RELATIONAL_AFTERMATH_VOID'), 'VOICE_SEED_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('VOICE_SEED_RELATIONAL_AFTERMATH_VOID does not fire when a seed is followed by a relationship shift within its window', async () => {
      const recs1089an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runV1089(recs1089an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_SEED_RELATIONAL_AFTERMATH_VOID'), 'VOICE_SEED_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('VOICE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no highlighted dialogue', async () => {
      const recs1089b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runV1089(recs1089b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'VOICE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('VOICE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a stakes-raise is followed by highlighted dialogue within its window', async () => {
      const recs1089bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runV1089(recs1089bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'VOICE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('VOICE_TURN_STAGING_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no heavily-staged scene', async () => {
      const recs1089c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runV1089(recs1089c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_TURN_STAGING_AFTERMATH_VOID'), 'VOICE_TURN_STAGING_AFTERMATH_VOID should fire');
    });

    it('VOICE_TURN_STAGING_AFTERMATH_VOID does not fire when a dramatic turn is followed by a heavily-staged scene within its window', async () => {
      const recs1089cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runV1089(recs1089cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_TURN_STAGING_AFTERMATH_VOID'), 'VOICE_TURN_STAGING_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1061 — voicePass: voice clock-emotional aftermath void, voice seed-suspense aftermath void, voice stakes-relational aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('VOICE_CLOCK_EMOTIONAL_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no emotional shift', async () => {
      const recs1061a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runV1061(recs1061a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_CLOCK_EMOTIONAL_AFTERMATH_VOID'), 'VOICE_CLOCK_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('VOICE_CLOCK_EMOTIONAL_AFTERMATH_VOID does not fire when a clock-raise is followed by an emotional shift within its window', async () => {
      const recs1061an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runV1061(recs1061an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_CLOCK_EMOTIONAL_AFTERMATH_VOID'), 'VOICE_CLOCK_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('VOICE_SEED_SUSPENSE_AFTERMATH_VOID fires when every seed is followed by two scenes with no suspense rise', async () => {
      const recs1061b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1061(recs1061b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_SEED_SUSPENSE_AFTERMATH_VOID'), 'VOICE_SEED_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('VOICE_SEED_SUSPENSE_AFTERMATH_VOID does not fire when a seed is followed by a suspense rise within its window', async () => {
      const recs1061bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1061(recs1061bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_SEED_SUSPENSE_AFTERMATH_VOID'), 'VOICE_SEED_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('VOICE_STAKES_RELATIONAL_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no relationship shift', async () => {
      const recs1061c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runV1061(recs1061c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_STAKES_RELATIONAL_AFTERMATH_VOID'), 'VOICE_STAKES_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('VOICE_STAKES_RELATIONAL_AFTERMATH_VOID does not fire when a stakes-raise is followed by a relationship shift within its window', async () => {
      const recs1061cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runV1061(recs1061cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_STAKES_RELATIONAL_AFTERMATH_VOID'), 'VOICE_STAKES_RELATIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1047 — voicePass: voice seed-curiosity aftermath void, voice payoff-emotional aftermath void, voice clock-suspense aftermath void', async () => {
    const runV1047 = async (records: ScreenplaySceneRecord[]) => {
      const { voicePass } = await import('../../server/nvm/revision/passes/voice.ts');
      return voicePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('VOICE_SEED_CURIOSITY_AFTERMATH_VOID fires when every seed is followed by two scenes with no curiosity rise', async () => {
      const recs1047a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1047(recs1047a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_SEED_CURIOSITY_AFTERMATH_VOID'), 'VOICE_SEED_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('VOICE_SEED_CURIOSITY_AFTERMATH_VOID does not fire when a seed is followed by a curiosity rise within its window', async () => {
      const recs1047an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1047(recs1047an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_SEED_CURIOSITY_AFTERMATH_VOID'), 'VOICE_SEED_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('VOICE_PAYOFF_EMOTIONAL_AFTERMATH_VOID fires when every payoff is followed by two scenes with no emotional shift', async () => {
      const recs1047b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runV1047(recs1047b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_PAYOFF_EMOTIONAL_AFTERMATH_VOID'), 'VOICE_PAYOFF_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('VOICE_PAYOFF_EMOTIONAL_AFTERMATH_VOID does not fire when a payoff is followed by an emotional shift within its window', async () => {
      const recs1047bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runV1047(recs1047bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_PAYOFF_EMOTIONAL_AFTERMATH_VOID'), 'VOICE_PAYOFF_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('VOICE_CLOCK_SUSPENSE_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no suspense rise', async () => {
      const recs1047c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1047(recs1047c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_CLOCK_SUSPENSE_AFTERMATH_VOID'), 'VOICE_CLOCK_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('VOICE_CLOCK_SUSPENSE_AFTERMATH_VOID does not fire when a clock-raise is followed by a suspense rise within its window', async () => {
      const recs1047cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1047(recs1047cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_CLOCK_SUSPENSE_AFTERMATH_VOID'), 'VOICE_CLOCK_SUSPENSE_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1033 — voicePass: voice stakes-emotional aftermath void, voice clock-relational aftermath void, voice turn-curiosity aftermath void', async () => {
    const runV1033 = async (records: ScreenplaySceneRecord[]) => {
      const { voicePass } = await import('../../server/nvm/revision/passes/voice.ts');
      return voicePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('VOICE_STAKES_EMOTIONAL_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no emotional shift', async () => {
      const recs1033a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runV1033(recs1033a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'VOICE_STAKES_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('VOICE_STAKES_EMOTIONAL_AFTERMATH_VOID does not fire when a stakes-raise is followed by an emotional shift within its window', async () => {
      const recs1033an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runV1033(recs1033an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'VOICE_STAKES_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('VOICE_CLOCK_RELATIONAL_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no relationship shift', async () => {
      const recs1033b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runV1033(recs1033b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_CLOCK_RELATIONAL_AFTERMATH_VOID'), 'VOICE_CLOCK_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('VOICE_CLOCK_RELATIONAL_AFTERMATH_VOID does not fire when a clock-raise is followed by a relationship shift within its window', async () => {
      const recs1033bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runV1033(recs1033bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_CLOCK_RELATIONAL_AFTERMATH_VOID'), 'VOICE_CLOCK_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('VOICE_TURN_CURIOSITY_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no curiosity rise', async () => {
      const recs1033c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1033(recs1033c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_TURN_CURIOSITY_AFTERMATH_VOID'), 'VOICE_TURN_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('VOICE_TURN_CURIOSITY_AFTERMATH_VOID does not fire when a dramatic turn is followed by a curiosity rise within its window', async () => {
      const recs1033cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1033(recs1033cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_TURN_CURIOSITY_AFTERMATH_VOID'), 'VOICE_TURN_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1019 — voicePass: voice stakes-suspense aftermath void, voice payoff-curiosity aftermath void, voice open-thread-staging aftermath void', async () => {
    const runV1019 = async (records: ScreenplaySceneRecord[]) => {
      const { voicePass } = await import('../../server/nvm/revision/passes/voice.ts');
      return voicePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('VOICE_STAKES_SUSPENSE_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no suspense rise', async () => {
      const recs1019a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1019(recs1019a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_STAKES_SUSPENSE_AFTERMATH_VOID'), 'VOICE_STAKES_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('VOICE_STAKES_SUSPENSE_AFTERMATH_VOID does not fire when a stakes-raise is followed by a suspense rise within its window', async () => {
      const recs1019an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1019(recs1019an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_STAKES_SUSPENSE_AFTERMATH_VOID'), 'VOICE_STAKES_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('VOICE_PAYOFF_CURIOSITY_AFTERMATH_VOID fires when every payoff setup is followed by two scenes with no curiosity rise', async () => {
      const recs1019b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1019(recs1019b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_PAYOFF_CURIOSITY_AFTERMATH_VOID'), 'VOICE_PAYOFF_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('VOICE_PAYOFF_CURIOSITY_AFTERMATH_VOID does not fire when a payoff setup is followed by a curiosity rise within its window', async () => {
      const recs1019bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1019(recs1019bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_PAYOFF_CURIOSITY_AFTERMATH_VOID'), 'VOICE_PAYOFF_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('VOICE_OPEN_THREAD_STAGING_AFTERMATH_VOID fires when every heavily-unresolved scene is followed by two scenes with no staged visual beat', async () => {
      const recs1019c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['a beat'] });
        return makeSharedRecord(i);
      });
      const res = await runV1019(recs1019c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_OPEN_THREAD_STAGING_AFTERMATH_VOID'), 'VOICE_OPEN_THREAD_STAGING_AFTERMATH_VOID should fire');
    });

    it('VOICE_OPEN_THREAD_STAGING_AFTERMATH_VOID does not fire when a heavily-unresolved scene is followed by a staged visual beat within its window', async () => {
      const recs1019cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['a beat'] });
        return makeSharedRecord(i);
      });
      const res = await runV1019(recs1019cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_OPEN_THREAD_STAGING_AFTERMATH_VOID'), 'VOICE_OPEN_THREAD_STAGING_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1005 — voicePass: voice clock-curiosity aftermath void, voice seed-emotional aftermath void, voice turn-relational aftermath void', async () => {
    const runV1005 = async (records: ScreenplaySceneRecord[]) => {
      const { voicePass } = await import('../../server/nvm/revision/passes/voice.ts');
      return voicePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('VOICE_CLOCK_CURIOSITY_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no new curiosity', async () => {
      const recs1005a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1005(recs1005a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_CLOCK_CURIOSITY_AFTERMATH_VOID'), 'VOICE_CLOCK_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('VOICE_CLOCK_CURIOSITY_AFTERMATH_VOID does not fire when a clock-raise is followed by new curiosity within its window', async () => {
      const recs1005an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV1005(recs1005an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_CLOCK_CURIOSITY_AFTERMATH_VOID'), 'VOICE_CLOCK_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('VOICE_SEED_EMOTIONAL_AFTERMATH_VOID fires when every seed is followed by two scenes with no emotional shift', async () => {
      const recs1005b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runV1005(recs1005b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_SEED_EMOTIONAL_AFTERMATH_VOID'), 'VOICE_SEED_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('VOICE_SEED_EMOTIONAL_AFTERMATH_VOID does not fire when a seed is followed by an emotional shift within its window', async () => {
      const recs1005bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runV1005(recs1005bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_SEED_EMOTIONAL_AFTERMATH_VOID'), 'VOICE_SEED_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('VOICE_TURN_RELATIONAL_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no relationship shift', async () => {
      const recs1005c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runV1005(recs1005c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_TURN_RELATIONAL_AFTERMATH_VOID'), 'VOICE_TURN_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('VOICE_TURN_RELATIONAL_AFTERMATH_VOID does not fire when a dramatic turn is followed by a relationship shift within its window', async () => {
      const recs1005cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runV1005(recs1005cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_TURN_RELATIONAL_AFTERMATH_VOID'), 'VOICE_TURN_RELATIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 991 — voicePass: voice stakes-curiosity aftermath void, voice turn-suspense aftermath void, voice payoff-relationship aftermath void', async () => {
    const runV991 = async (records: ScreenplaySceneRecord[]) => {
      const { voicePass } = await import('../../server/nvm/revision/passes/voice.ts');
      return voicePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('VOICE_STAKES_CURIOSITY_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no new curiosity', async () => {
      const recs991a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV991(recs991a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_STAKES_CURIOSITY_AFTERMATH_VOID'), 'VOICE_STAKES_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('VOICE_STAKES_CURIOSITY_AFTERMATH_VOID does not fire when a stakes-raise is followed by new curiosity within its window', async () => {
      const recs991an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV991(recs991an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_STAKES_CURIOSITY_AFTERMATH_VOID'), 'VOICE_STAKES_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('VOICE_TURN_SUSPENSE_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no rise in suspense', async () => {
      const recs991b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV991(recs991b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_TURN_SUSPENSE_AFTERMATH_VOID'), 'VOICE_TURN_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('VOICE_TURN_SUSPENSE_AFTERMATH_VOID does not fire when a dramatic turn is followed by rising suspense within its window', async () => {
      const recs991bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runV991(recs991bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_TURN_SUSPENSE_AFTERMATH_VOID'), 'VOICE_TURN_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('VOICE_PAYOFF_RELATIONSHIP_AFTERMATH_VOID fires when every payoff is followed by two scenes with no relationship shift', async () => {
      const recs991c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['setup1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runV991(recs991c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_PAYOFF_RELATIONSHIP_AFTERMATH_VOID'), 'VOICE_PAYOFF_RELATIONSHIP_AFTERMATH_VOID should fire');
    });

    it('VOICE_PAYOFF_RELATIONSHIP_AFTERMATH_VOID does not fire when a payoff is followed by a relationship shift within its window', async () => {
      const recs991cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['setup1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runV991(recs991cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_PAYOFF_RELATIONSHIP_AFTERMATH_VOID'), 'VOICE_PAYOFF_RELATIONSHIP_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 907 — voicePass: voice resolution zone imbalance, voice turning point zone imbalance, voice complicate zone imbalance', async () => {
    ...
    it('VOICE_RESOLUTION_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of resolution-purposed scenes', async () => {
      const recs907a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2, 8, 9].includes(i) ? 'resolution' : 'establish_world' }),
      );
      const res = await runV907(recs907a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_RESOLUTION_ZONE_IMBALANCE'), 'VOICE_RESOLUTION_ZONE_IMBALANCE should fire');
    });

    it('VOICE_RESOLUTION_ZONE_IMBALANCE does not fire when resolution-purposed scenes touch every zone', async () => {
      const recs907an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 5, 8].includes(i) ? 'resolution' : 'establish_world' }),
      );
      const res = await runV907(recs907an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_RESOLUTION_ZONE_IMBALANCE'), 'VOICE_RESOLUTION_ZONE_IMBALANCE should not fire');
    });

    it('VOICE_TURNING_POINT_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of turning-point scenes', async () => {
      const recs907b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2, 8, 9].includes(i) ? 'turning_point' : 'establish_world' }),
      );
      const res = await runV907(recs907b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_TURNING_POINT_ZONE_IMBALANCE'), 'VOICE_TURNING_POINT_ZONE_IMBALANCE should fire');
    });

    it('VOICE_TURNING_POINT_ZONE_IMBALANCE does not fire when turning-point scenes touch every zone', async () => {
      const recs907bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 5, 8].includes(i) ? 'turning_point' : 'establish_world' }),
      );
      const res = await runV907(recs907bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_TURNING_POINT_ZONE_IMBALANCE'), 'VOICE_TURNING_POINT_ZONE_IMBALANCE should not fire');
    });

    it('VOICE_COMPLICATE_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of complicating scenes', async () => {
      const recs907c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2, 8, 9].includes(i) ? 'complicate' : 'establish_world' }),
      );
      const res = await runV907(recs907c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_COMPLICATE_ZONE_IMBALANCE'), 'VOICE_COMPLICATE_ZONE_IMBALANCE should fire');
    });

    it('VOICE_COMPLICATE_ZONE_IMBALANCE does not fire when complicating scenes touch every zone', async () => {
      const recs907cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 5, 8].includes(i) ? 'complicate' : 'establish_world' }),
      );
      const res = await runV907(recs907cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_COMPLICATE_ZONE_IMBALANCE'), 'VOICE_COMPLICATE_ZONE_IMBALANCE should not fire');
    });
  });
  ...
  describe('Wave 879 — voicePass: voice climax drought run, voice resolution drought run, voice complicate zone cluster', async () => {
    ...
    // VOICE_CLIMAX_DROUGHT_RUN fire:
    // n=10; climax at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('VOICE_CLIMAX_DROUGHT_RUN fires when a long run has no climax-purposed scene', async () => {
      const recs879a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'climax' : 'establish_world' }),
      );
      const res = await runV879(recs879a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_CLIMAX_DROUGHT_RUN'), 'VOICE_CLIMAX_DROUGHT_RUN should fire');
    });

    it('VOICE_CLIMAX_DROUGHT_RUN does not fire when climax-purposed scenes are evenly spread', async () => {
      const recs879an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'climax' : 'establish_world' }),
      );
      const res = await runV879(recs879an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_CLIMAX_DROUGHT_RUN'), 'VOICE_CLIMAX_DROUGHT_RUN should not fire');
    });

    // VOICE_RESOLUTION_DROUGHT_RUN fire:
    // n=10; resolution at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('VOICE_RESOLUTION_DROUGHT_RUN fires when a long run has no resolution-purposed scene', async () => {
      const recs879b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'resolution' : 'establish_world' }),
      );
      const res = await runV879(recs879b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_RESOLUTION_DROUGHT_RUN'), 'VOICE_RESOLUTION_DROUGHT_RUN should fire');
    });

    it('VOICE_RESOLUTION_DROUGHT_RUN does not fire when resolution-purposed scenes are evenly spread', async () => {
      const recs879bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'resolution' : 'establish_world' }),
      );
      const res = await runV879(recs879bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_RESOLUTION_DROUGHT_RUN'), 'VOICE_RESOLUTION_DROUGHT_RUN should not fire');
    });

    // VOICE_COMPLICATE_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; complicate scenes at 0,1,2 → 100% opening third
    it('VOICE_COMPLICATE_ZONE_CLUSTER fires when >75% of complicating scenes cluster in one third', async () => {
      const recs879c = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'complicate' : 'establish_world' }),
      );
      const res = await runV879(recs879c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_COMPLICATE_ZONE_CLUSTER'), 'VOICE_COMPLICATE_ZONE_CLUSTER should fire');
    });

    it('VOICE_COMPLICATE_ZONE_CLUSTER does not fire when complicating scenes spread across thirds', async () => {
      const recs879cn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'complicate' : 'establish_world' }),
      );
      const res = await runV879(recs879cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_COMPLICATE_ZONE_CLUSTER'), 'VOICE_COMPLICATE_ZONE_CLUSTER should not fire');
    });
  });

  describe('Wave 865 — voicePass: voice establish world drought run, voice climax zone cluster, voice resolution zone cluster', async () => {
    const runV865 = async (records: ScreenplaySceneRecord[]) => {
      const { voicePass } = await import('../../server/nvm/revision/passes/voice.ts');
      return voicePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // VOICE_ESTABLISH_WORLD_DROUGHT_RUN fire:
    // n=10; establish_world at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('VOICE_ESTABLISH_WORLD_DROUGHT_RUN fires when a long run has no world-establishing scene', async () => {
      const recs865a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'establish_world' : 'complicate' }),
      );
      const res = await runV865(recs865a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_ESTABLISH_WORLD_DROUGHT_RUN'), 'VOICE_ESTABLISH_WORLD_DROUGHT_RUN should fire');
    });

    it('VOICE_ESTABLISH_WORLD_DROUGHT_RUN does not fire when world-establishing scenes are evenly spread', async () => {
      const recs865an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'establish_world' : 'complicate' }),
      );
      const res = await runV865(recs865an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_ESTABLISH_WORLD_DROUGHT_RUN'), 'VOICE_ESTABLISH_WORLD_DROUGHT_RUN should not fire');
    });

    // VOICE_CLIMAX_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; climax scenes at 0,1,2 → 100% opening third
    it('VOICE_CLIMAX_ZONE_CLUSTER fires when >75% of climax-purposed scenes cluster in one third', async () => {
      const recs865b = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'climax' : 'complicate' }),
      );
      const res = await runV865(recs865b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_CLIMAX_ZONE_CLUSTER'), 'VOICE_CLIMAX_ZONE_CLUSTER should fire');
    });

    it('VOICE_CLIMAX_ZONE_CLUSTER does not fire when climax-purposed scenes spread across thirds', async () => {
      const recs865bn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'climax' : 'complicate' }),
      );
      const res = await runV865(recs865bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_CLIMAX_ZONE_CLUSTER'), 'VOICE_CLIMAX_ZONE_CLUSTER should not fire');
    });

    // VOICE_RESOLUTION_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; resolution scenes at 0,1,2 → 100% opening third
    it('VOICE_RESOLUTION_ZONE_CLUSTER fires when >75% of resolution-purposed scenes cluster in one third', async () => {
      const recs865c = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'resolution' : 'complicate' }),
      );
      const res = await runV865(recs865c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_RESOLUTION_ZONE_CLUSTER'), 'VOICE_RESOLUTION_ZONE_CLUSTER should fire');
    });

    it('VOICE_RESOLUTION_ZONE_CLUSTER does not fire when resolution-purposed scenes spread across thirds', async () => {
      const recs865cn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'resolution' : 'complicate' }),
      );
      const res = await runV865(recs865cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_RESOLUTION_ZONE_CLUSTER'), 'VOICE_RESOLUTION_ZONE_CLUSTER should not fire');
    });
  });
  ...
  describe('Wave 809 — voicePass: voice revelation drought run, voice revelation peak uncaused, voice negative emotion zone cluster', async () => {
    ...
    // VOICE_REVELATION_DROUGHT_RUN fire:
    // n=10; revelation present at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('VOICE_REVELATION_DROUGHT_RUN fires when a long run has no revelation', async () => {
      const recs809a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { revelation: [0, 1, 2].includes(i) ? 'truth revealed' : null }),
      );
      const res = await runV809(recs809a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_REVELATION_DROUGHT_RUN'), 'VOICE_REVELATION_DROUGHT_RUN should fire');
    });

    it('VOICE_REVELATION_DROUGHT_RUN does not fire when revelations are evenly spread', async () => {
      const recs809an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { revelation: [0, 3, 6, 9].includes(i) ? 'truth revealed' : null }),
      );
      const res = await runV809(recs809an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_REVELATION_DROUGHT_RUN'), 'VOICE_REVELATION_DROUGHT_RUN should not fire');
    });

    // VOICE_REVELATION_PEAK_UNCAUSED fire:
    ...
    it('VOICE_REVELATION_PEAK_UNCAUSED fires when the revelation scene has no dramatic turn nearby', async () => {
      const recs809b = Array.from({ length: 8 }, (_, i) => makeSharedRecord(i));
      recs809b[2] = makeSharedRecord(2, { revelation: 'truth revealed' });
      recs809b[5] = makeSharedRecord(5, { revelation: 'truth revealed' });
      const res = await runV809(recs809b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_REVELATION_PEAK_UNCAUSED'), 'VOICE_REVELATION_PEAK_UNCAUSED should fire');
    });

    it('VOICE_REVELATION_PEAK_UNCAUSED does not fire when a dramatic turn precedes the revelation scene', async () => {
      const recs809bn = Array.from({ length: 8 }, (_, i) => makeSharedRecord(i));
      recs809bn[2] = makeSharedRecord(2, { revelation: 'truth revealed' });
      recs809bn[5] = makeSharedRecord(5, { revelation: 'truth revealed' });
      recs809bn[1] = makeSharedRecord(1, { dramaticTurn: 'reversal' });
      const res = await runV809(recs809bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_REVELATION_PEAK_UNCAUSED'), 'VOICE_REVELATION_PEAK_UNCAUSED should not fire');
    });

    // VOICE_NEGATIVE_EMOTION_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; negative-emotion scenes at 0,1,2 → 100% opening third
    it('VOICE_NEGATIVE_EMOTION_ZONE_CLUSTER fires when >75% of negative-emotion scenes cluster in one third', async () => {
      const recs809c = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2].includes(i) ? 'negative' : 'neutral' }),
      );
      const res = await runV809(recs809c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_NEGATIVE_EMOTION_ZONE_CLUSTER'), 'VOICE_NEGATIVE_EMOTION_ZONE_CLUSTER should fire');
    });

    it('VOICE_NEGATIVE_EMOTION_ZONE_CLUSTER does not fire when negative-emotion scenes spread across thirds', async () => {
      const recs809cn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 4, 8].includes(i) ? 'negative' : 'neutral' }),
      );
      const res = await runV809(recs809cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_NEGATIVE_EMOTION_ZONE_CLUSTER'), 'VOICE_NEGATIVE_EMOTION_ZONE_CLUSTER should not fire');
    });
  });

  describe('Wave 795 — voicePass: voice turn zone cluster, voice emotion drought run, voice revelation zone cluster', async () => {
    const runV795 = async (records: ScreenplaySceneRecord[]) => {
      const { voicePass } = await import('../../server/nvm/revision/passes/voice.ts');
      return voicePass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: {} as any, annotations: Array.from({ length: records.length }, () => ({} as any)),
    ...
    // VOICE_TURN_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; turn scenes at 0,1,2 → 100% opening third
    it('VOICE_TURN_ZONE_CLUSTER fires when >75% of turn scenes cluster in one third', async () => {
      const recs795a = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { dramaticTurn: [0, 1, 2].includes(i) ? 'reversal' : 'nothing' }),
      );
      const res = await runV795(recs795a);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_TURN_ZONE_CLUSTER'), 'VOICE_TURN_ZONE_CLUSTER should fire');
    });

    it('VOICE_TURN_ZONE_CLUSTER does not fire when turn scenes spread across thirds', async () => {
      const recs795an = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { dramaticTurn: [0, 4, 8].includes(i) ? 'reversal' : 'nothing' }),
      );
      const res = await runV795(recs795an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_TURN_ZONE_CLUSTER'), 'VOICE_TURN_ZONE_CLUSTER should not fire');
    });

    // VOICE_EMOTION_DROUGHT_RUN fire:
    // n=10; emotionally charged (non-neutral) at 0,1,2 only, then a run of 7 consecutive scenes
    // (3-9) that are all neutral.
    it('VOICE_EMOTION_DROUGHT_RUN fires when a long run has no emotional charge', async () => {
      const recs795b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2].includes(i) ? 'positive' : 'neutral' }),
      );
      const res = await runV795(recs795b);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_EMOTION_DROUGHT_RUN'), 'VOICE_EMOTION_DROUGHT_RUN should fire');
    });

    it('VOICE_EMOTION_DROUGHT_RUN does not fire when emotional charge is evenly spread', async () => {
      const recs795bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 3, 6, 9].includes(i) ? 'positive' : 'neutral' }),
      );
      const res = await runV795(recs795bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_EMOTION_DROUGHT_RUN'), 'VOICE_EMOTION_DROUGHT_RUN should not fire');
    });

    // VOICE_REVELATION_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; revelation scenes at 0,1,2 → 100% opening third
    it('VOICE_REVELATION_ZONE_CLUSTER fires when >75% of revelation scenes cluster in one third', async () => {
      const recs795c = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { revelation: [0, 1, 2].includes(i) ? 'truth revealed' : null }),
      );
      const res = await runV795(recs795c);
      assert.ok(res.issues.some((i: any) => i.rule === 'VOICE_REVELATION_ZONE_CLUSTER'), 'VOICE_REVELATION_ZONE_CLUSTER should fire');
    });

    it('VOICE_REVELATION_ZONE_CLUSTER does not fire when revelation scenes spread across thirds', async () => {
      const recs795cn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { revelation: [0, 4, 8].includes(i) ? 'truth revealed' : null }),
      );
      const res = await runV795(recs795cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'VOICE_REVELATION_ZONE_CLUSTER'), 'VOICE_REVELATION_ZONE_CLUSTER should not fire');
    });
  });
