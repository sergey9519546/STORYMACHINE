...
import { relationshipArcPass } from '../../server/nvm/revision/passes/relationship-arc.ts';
  ...
  describe('Wave 1184 — pacingPass (Program v2, Type 3 — genre-conditioned): ENERGY_MONOTONE and PACING_PLATEAU genre thresholds', async () => {
    ...
    const noAnnotations1184 = (n: number) => Array.from({ length: n }, () => ({ revelation: false } as any));
    function makeFountainWithLengths1184(sceneLinesArray: number[]): string {
      return sceneLinesArray.map((len, i) => {
        const body = Array.from({ length: len }, (_, j) => `Scene ${i} action line ${j + 1}.`).join('\n');
        return `INT. SC${i} - DAY\n\n${body}\n`;
      }).join('\n');
    ...
    // ── ENERGY_MONOTONE genre threshold ──────────────────────────────────────
    // Craft argument: a thriller's contract is forward momentum in every scene
    // (GENRE_MODIFIERS.thriller — "no scene ends in the same place it began"), so even a
    // moderately-varied rhythm (CoV ≈ 0.39, comfortably above the generic 0.35 floor)
    // already reads as a stall under thriller's stricter 0.45 cutoff.
    it('pacingPass: ENERGY_MONOTONE does not fire generically at CoV≈0.39 but fires under thriller\'s stricter cutoff', async () => {
      const { pacingPass } = await import('../../server/nvm/revision/passes/pacing.ts');
      const sceneLens = [10, 10, 10, 10, 10, 10, 10, 24]; // coefficient of variation ≈ 0.394
      const records = Array.from({ length: 8 }, (_, i) => makeRec1184(i));
      const baseInput = {
        fountain: makeFountainWithLengths1184(sceneLens),
        original: '', records: records as any, structure: {} as any,
        annotations: noAnnotations1184(8), approvedSpans: [],
      };
      const genericResult = await pacingPass(baseInput);
      assert.ok(
        !genericResult.issues.some(i => i.rule === 'ENERGY_MONOTONE'),
        'Should NOT fire generically at CoV≈0.39 (above the generic 0.35 cutoff)',
      );
      ...
      assert.ok(monotone.length >= 1, `Expected ENERGY_MONOTONE under thriller; got: ${thrillerResult.issues.map(i => i.rule).join(', ')}`);
      assert.ok(monotone[0].description.includes('(threshold adjusted for thriller)'), 'thriller-shifted issue must explain why');
    });

    it('pacingPass: ENERGY_MONOTONE genre-absent path is byte-identical to the pre-Wave-1184 rule', async () => {
      const { pacingPass } = await import('../../server/nvm/revision/passes/pacing.ts');
      const sceneLens = [10, 10, 10, 10, 10, 10, 10, 10]; // CoV = 0, fires under any threshold
      const records = Array.from({ length: 8 }, (_, i) => makeRec1184(i));
      const result = await pacingPass({
        fountain: makeFountainWithLengths1184(sceneLens),
        original: '', records: records as any, structure: {} as any,
      ...
      assert.ok(monotone.length >= 1, 'Should fire generically with zero-variance scene lengths');
      assert.equal(
        monotone[0].description,
        "Scene lengths are monotone: all scenes are 10 ±0 lines. There's no rhythm — no short punchy scenes, no long contemplative ones.",
        'Genre-absent description must match the pre-wave literal exactly, with no genre note appended',
      );
    });

    // ── PACING_PLATEAU genre threshold ───────────────────────────────────────
    // Craft argument: drama's register is grounded restraint (GENRE_MODIFIERS.drama —
    // "silence and restraint do heavy lifting"), so a sustained near-uniform cadence
    // (a 15%-spread window, comfortably inside the generic ±20% plateau band) is a
    // legitimate stylistic choice under drama's tighter ±10% band, not a defect.
    it("pacingPass: PACING_PLATEAU fires generically at a 15%-spread window but not under drama's tighter ratio", async () => {
      const { pacingPass } = await import('../../server/nvm/revision/passes/pacing.ts');
      const sceneLens = [100, 105, 110, 115, 5, 50, 3, 60];
      const records = Array.from({ length: 8 }, (_, i) => makeRec1184(i));
      const baseInput = {
        fountain: makeFountainWithLengths1184(sceneLens),
        original: '', records: records as any, structure: {} as any,
      ...
      const genericResult = await pacingPass(baseInput);
      const plateau = genericResult.issues.filter(i => i.rule === 'PACING_PLATEAU');
      assert.ok(plateau.length >= 1, `Expected PACING_PLATEAU generically; got: ${genericResult.issues.map(i => i.rule).join(', ')}`);
      const dramaResult = await pacingPass({ ...baseInput, storyContext: { genre: 'drama' } });
      assert.ok(
        !dramaResult.issues.some(i => i.rule === 'PACING_PLATEAU'),
        "Should NOT fire under drama's tighter ±10% plateau ratio at a 15% spread",
      );
    });

    it('pacingPass: PACING_PLATEAU genre-absent path is byte-identical to the pre-Wave-1184 rule', async () => {
      const { pacingPass } = await import('../../server/nvm/revision/passes/pacing.ts');
      const sceneLens = [10, 10, 10, 10, 2, 25, 3, 22, 4];
      const records = Array.from({ length: 9 }, (_, i) => makeRec1184(i));
      const result = await pacingPass({
        fountain: makeFountainWithLengths1184(sceneLens),
        original: '', records: records as any, structure: {} as any,
        annotations: noAnnotations1184(9), approvedSpans: [],
      });
      const plateau = result.issues.filter(i => i.rule === 'PACING_PLATEAU');
  ...
  describe('Wave 1153 — pacingPass: pacing suspense-staging aftermath void, pacing emotion-staging aftermath void, pacing suspense-recurrence aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PACING_SUSPENSE_STAGING_AFTERMATH_VOID fires when every suspense-rise is followed by two scenes with no heavily-staged scene', async () => {
      const recs1153a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runP1153(recs1153a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_SUSPENSE_STAGING_AFTERMATH_VOID'), 'PACING_SUSPENSE_STAGING_AFTERMATH_VOID should fire');
    });

    it('PACING_SUSPENSE_STAGING_AFTERMATH_VOID does not fire when a suspense-rise is followed by a heavily-staged scene within its window', async () => {
      const recs1153an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runP1153(recs1153an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_SUSPENSE_STAGING_AFTERMATH_VOID'), 'PACING_SUSPENSE_STAGING_AFTERMATH_VOID should not fire');
    });

    it('PACING_EMOTION_STAGING_AFTERMATH_VOID fires when every emotionally-charged scene is followed by two scenes with no heavily-staged scene', async () => {
      const recs1153b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { emotionalShift: 'positive' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runP1153(recs1153b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_EMOTION_STAGING_AFTERMATH_VOID'), 'PACING_EMOTION_STAGING_AFTERMATH_VOID should fire');
    });

    it('PACING_EMOTION_STAGING_AFTERMATH_VOID does not fire when an emotionally-charged scene is followed by a heavily-staged scene within its window', async () => {
      const recs1153bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { emotionalShift: 'positive' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runP1153(recs1153bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_EMOTION_STAGING_AFTERMATH_VOID'), 'PACING_EMOTION_STAGING_AFTERMATH_VOID should not fire');
    });

    it('PACING_SUSPENSE_RECURRENCE_AFTERMATH_VOID fires when every suspense-rise is followed by two scenes with no further suspense rise', async () => {
      const recs1153c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runP1153(recs1153c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_SUSPENSE_RECURRENCE_AFTERMATH_VOID'), 'PACING_SUSPENSE_RECURRENCE_AFTERMATH_VOID should fire');
    });

    it('PACING_SUSPENSE_RECURRENCE_AFTERMATH_VOID does not fire when a suspense-rise is followed by a further suspense rise within its window', async () => {
      const recs1153cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runP1153(recs1153cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_SUSPENSE_RECURRENCE_AFTERMATH_VOID'), 'PACING_SUSPENSE_RECURRENCE_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1125 — pacingPass: pacing suspense-emotional aftermath void, pacing suspense-relational aftermath void, pacing emotion-curiosity aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PACING_SUSPENSE_EMOTIONAL_AFTERMATH_VOID fires when every suspense-rise is followed by two scenes with no emotional shift', async () => {
      const recs1125a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runP1125(recs1125a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_SUSPENSE_EMOTIONAL_AFTERMATH_VOID'), 'PACING_SUSPENSE_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('PACING_SUSPENSE_EMOTIONAL_AFTERMATH_VOID does not fire when a suspense-rise is followed by an emotional shift within its window', async () => {
      const recs1125an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runP1125(recs1125an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_SUSPENSE_EMOTIONAL_AFTERMATH_VOID'), 'PACING_SUSPENSE_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('PACING_SUSPENSE_RELATIONAL_AFTERMATH_VOID fires when every suspense-rise is followed by two scenes with no relationship shift', async () => {
      const recs1125b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1125(recs1125b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_SUSPENSE_RELATIONAL_AFTERMATH_VOID'), 'PACING_SUSPENSE_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PACING_SUSPENSE_RELATIONAL_AFTERMATH_VOID does not fire when a suspense-rise is followed by a relationship shift within its window', async () => {
      const recs1125bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1125(recs1125bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_SUSPENSE_RELATIONAL_AFTERMATH_VOID'), 'PACING_SUSPENSE_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('PACING_EMOTION_CURIOSITY_AFTERMATH_VOID fires when every emotionally-charged scene is followed by two scenes with no curiosity rise', async () => {
      const recs1125c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { emotionalShift: 'positive' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runP1125(recs1125c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_EMOTION_CURIOSITY_AFTERMATH_VOID'), 'PACING_EMOTION_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('PACING_EMOTION_CURIOSITY_AFTERMATH_VOID does not fire when an emotionally-charged scene is followed by a curiosity rise within its window', async () => {
      const recs1125cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { emotionalShift: 'positive' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runP1125(recs1125cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_EMOTION_CURIOSITY_AFTERMATH_VOID'), 'PACING_EMOTION_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1111 — pacingPass: pacing clock-delta-dialogue-highlight aftermath void, pacing suspense-curiosity aftermath void, pacing emotion-relational aftermath void', async () => {
    const runP1111 = async (records: ScreenplaySceneRecord[]) => {
      const { pacingPass } = await import('../../server/nvm/revision/passes/pacing.ts');
      return pacingPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PACING_CLOCK_DELTA_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every clock-delta scene is followed by two scenes with no highlighted dialogue', async () => {
      const recs1111a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockDelta: 2 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runP1111(recs1111a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_CLOCK_DELTA_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PACING_CLOCK_DELTA_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('PACING_CLOCK_DELTA_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a clock-delta scene is followed by highlighted dialogue within its window', async () => {
      const recs1111an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockDelta: 2 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runP1111(recs1111an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_CLOCK_DELTA_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PACING_CLOCK_DELTA_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('PACING_SUSPENSE_CURIOSITY_AFTERMATH_VOID fires when every suspense-rise is followed by two scenes with no curiosity rise', async () => {
      const recs1111b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runP1111(recs1111b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_SUSPENSE_CURIOSITY_AFTERMATH_VOID'), 'PACING_SUSPENSE_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('PACING_SUSPENSE_CURIOSITY_AFTERMATH_VOID does not fire when a suspense-rise is followed by a curiosity rise within its window', async () => {
      const recs1111bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runP1111(recs1111bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_SUSPENSE_CURIOSITY_AFTERMATH_VOID'), 'PACING_SUSPENSE_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('PACING_EMOTION_RELATIONAL_AFTERMATH_VOID fires when every emotionally-charged scene is followed by two scenes with no relationship shift', async () => {
      const recs1111c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { emotionalShift: 'positive' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1111(recs1111c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_EMOTION_RELATIONAL_AFTERMATH_VOID'), 'PACING_EMOTION_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PACING_EMOTION_RELATIONAL_AFTERMATH_VOID does not fire when an emotionally-charged scene is followed by a relationship shift within its window', async () => {
      const recs1111cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { emotionalShift: 'positive' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1111(recs1111cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_EMOTION_RELATIONAL_AFTERMATH_VOID'), 'PACING_EMOTION_RELATIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1097 — pacingPass: pacing clock-delta-emotional aftermath void, pacing clock-delta-relational aftermath void, pacing clock-delta-staging aftermath void', async () => {
    const runP1097 = async (records: ScreenplaySceneRecord[]) => {
      const { pacingPass } = await import('../../server/nvm/revision/passes/pacing.ts');
      return pacingPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PACING_CLOCK_DELTA_EMOTIONAL_AFTERMATH_VOID fires when every clock-delta scene is followed by two scenes with no emotional shift', async () => {
      const recs1097a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockDelta: 2 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runP1097(recs1097a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_CLOCK_DELTA_EMOTIONAL_AFTERMATH_VOID'), 'PACING_CLOCK_DELTA_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('PACING_CLOCK_DELTA_EMOTIONAL_AFTERMATH_VOID does not fire when a clock-delta scene is followed by an emotional shift within its window', async () => {
      const recs1097an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockDelta: 2 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runP1097(recs1097an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_CLOCK_DELTA_EMOTIONAL_AFTERMATH_VOID'), 'PACING_CLOCK_DELTA_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('PACING_CLOCK_DELTA_RELATIONAL_AFTERMATH_VOID fires when every clock-delta scene is followed by two scenes with no relationship shift', async () => {
      const recs1097b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockDelta: 2 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1097(recs1097b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_CLOCK_DELTA_RELATIONAL_AFTERMATH_VOID'), 'PACING_CLOCK_DELTA_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PACING_CLOCK_DELTA_RELATIONAL_AFTERMATH_VOID does not fire when a clock-delta scene is followed by a relationship shift within its window', async () => {
      const recs1097bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockDelta: 2 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1097(recs1097bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_CLOCK_DELTA_RELATIONAL_AFTERMATH_VOID'), 'PACING_CLOCK_DELTA_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('PACING_CLOCK_DELTA_STAGING_AFTERMATH_VOID fires when every clock-delta scene is followed by two scenes with no visually dense scene', async () => {
      const recs1097c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockDelta: 2 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runP1097(recs1097c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_CLOCK_DELTA_STAGING_AFTERMATH_VOID'), 'PACING_CLOCK_DELTA_STAGING_AFTERMATH_VOID should fire');
    });

    it('PACING_CLOCK_DELTA_STAGING_AFTERMATH_VOID does not fire when a clock-delta scene is followed by a visually dense scene within its window', async () => {
      const recs1097cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockDelta: 2 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runP1097(recs1097cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_CLOCK_DELTA_STAGING_AFTERMATH_VOID'), 'PACING_CLOCK_DELTA_STAGING_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1069 — pacingPass: pacing payoff-dialogue-highlight aftermath void, pacing stakes-dialogue-highlight aftermath void, pacing open-thread-staging aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PACING_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every payoff is followed by two scenes with no highlighted dialogue', async () => {
      const recs1069a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runP1069(recs1069a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PACING_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('PACING_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a payoff is followed by highlighted dialogue within its window', async () => {
      const recs1069an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runP1069(recs1069an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PACING_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('PACING_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no highlighted dialogue', async () => {
      const recs1069b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runP1069(recs1069b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PACING_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('PACING_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a stakes-raise is followed by highlighted dialogue within its window', async () => {
      const recs1069bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runP1069(recs1069bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PACING_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('PACING_OPEN_THREAD_STAGING_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no visually dense scene', async () => {
      const recs1069c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runP1069(recs1069c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_OPEN_THREAD_STAGING_AFTERMATH_VOID'), 'PACING_OPEN_THREAD_STAGING_AFTERMATH_VOID should fire');
    });

    it('PACING_OPEN_THREAD_STAGING_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a visually dense scene within its window', async () => {
      const recs1069cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runP1069(recs1069cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_OPEN_THREAD_STAGING_AFTERMATH_VOID'), 'PACING_OPEN_THREAD_STAGING_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1055 — pacingPass: pacing stakes-relational aftermath void, pacing open-thread-relational aftermath void, pacing stakes-staging aftermath void', async () => {
    const runP1055 = async (records: ScreenplaySceneRecord[]) => {
      const { pacingPass } = await import('../../server/nvm/revision/passes/pacing.ts');
      return pacingPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PACING_STAKES_RELATIONAL_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no relationship shift', async () => {
      const recs1055a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1055(recs1055a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_STAKES_RELATIONAL_AFTERMATH_VOID'), 'PACING_STAKES_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PACING_STAKES_RELATIONAL_AFTERMATH_VOID does not fire when a stakes-raise is followed by a relationship shift within its window', async () => {
      const recs1055an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1055(recs1055an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_STAKES_RELATIONAL_AFTERMATH_VOID'), 'PACING_STAKES_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('PACING_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no relationship shift', async () => {
      const recs1055b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1055(recs1055b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID'), 'PACING_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PACING_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a relationship shift within its window', async () => {
      const recs1055bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1055(recs1055bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID'), 'PACING_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('PACING_STAKES_STAGING_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no substantial staging', async () => {
      const recs1055c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat1', 'beat2'] });
        return makeSharedRecord(i);
      });
      const res = await runP1055(recs1055c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_STAKES_STAGING_AFTERMATH_VOID'), 'PACING_STAKES_STAGING_AFTERMATH_VOID should fire');
    });

    it('PACING_STAKES_STAGING_AFTERMATH_VOID does not fire when a stakes-raise is followed by substantial staging within its window', async () => {
      const recs1055cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat1', 'beat2'] });
        return makeSharedRecord(i);
      });
      const res = await runP1055(recs1055cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_STAKES_STAGING_AFTERMATH_VOID'), 'PACING_STAKES_STAGING_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1041 — pacingPass: pacing revelation-dialogue-highlight aftermath void, pacing clock-dialogue-highlight aftermath void, pacing turn-dialogue-highlight aftermath void', async () => {
    const runP1041 = async (records: ScreenplaySceneRecord[]) => {
      const { pacingPass } = await import('../../server/nvm/revision/passes/pacing.ts');
      return pacingPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PACING_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every revelation is followed by two scenes with no highlighted dialogue', async () => {
      const recs1041a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runP1041(recs1041a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PACING_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('PACING_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a revelation is followed by highlighted dialogue within its window', async () => {
      const recs1041an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runP1041(recs1041an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PACING_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('PACING_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no highlighted dialogue', async () => {
      const recs1041b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runP1041(recs1041b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PACING_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('PACING_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a clock-raise is followed by highlighted dialogue within its window', async () => {
      const recs1041bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runP1041(recs1041bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PACING_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('PACING_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no highlighted dialogue', async () => {
      const recs1041c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runP1041(recs1041c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PACING_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('PACING_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a dramatic turn is followed by highlighted dialogue within its window', async () => {
      const recs1041cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runP1041(recs1041cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PACING_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1013 — pacingPass: pacing turn-relational aftermath void, pacing clock-relational aftermath void, pacing clock-staging aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PACING_TURN_RELATIONAL_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no relationship shift', async () => {
      const recs1013a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1013(recs1013a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_TURN_RELATIONAL_AFTERMATH_VOID'), 'PACING_TURN_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PACING_TURN_RELATIONAL_AFTERMATH_VOID does not fire when a dramatic turn is followed by a relationship shift within its window', async () => {
      const recs1013an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1013(recs1013an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_TURN_RELATIONAL_AFTERMATH_VOID'), 'PACING_TURN_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('PACING_CLOCK_RELATIONAL_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no relationship shift', async () => {
      const recs1013b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1013(recs1013b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_CLOCK_RELATIONAL_AFTERMATH_VOID'), 'PACING_CLOCK_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PACING_CLOCK_RELATIONAL_AFTERMATH_VOID does not fire when a clock-raise is followed by a relationship shift within its window', async () => {
      const recs1013bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runP1013(recs1013bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_CLOCK_RELATIONAL_AFTERMATH_VOID'), 'PACING_CLOCK_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('PACING_CLOCK_STAGING_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no substantial staging', async () => {
      const recs1013c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat1', 'beat2'] });
        return makeSharedRecord(i);
      });
      const res = await runP1013(recs1013c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_CLOCK_STAGING_AFTERMATH_VOID'), 'PACING_CLOCK_STAGING_AFTERMATH_VOID should fire');
    });

    it('PACING_CLOCK_STAGING_AFTERMATH_VOID does not fire when a clock-raise is followed by substantial staging within its window', async () => {
      const recs1013cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat1', 'beat2'] });
        return makeSharedRecord(i);
      });
      const res = await runP1013(recs1013cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_CLOCK_STAGING_AFTERMATH_VOID'), 'PACING_CLOCK_STAGING_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 943 — pacingPass: pacing revelation purpose zone imbalance, pacing curiosity zone imbalance, pacing seed zone imbalance', async () => {
    ...
    // Zone geometry n=10: Z0={0,1,2}, Z1={3,4}, Z2={5,6,7}, Z3={8,9}. Target at 0,1,2,8,9 →
    // Z0 3/5=60% (bloat), Z1 and Z2 empty → fires. Target at 0,3,5,8 → every zone touched → no-fire.
    it('PACING_REVELATION_PURPOSE_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of revelation-purposed scenes', async () => {
      const recs943a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2, 8, 9].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runP943(recs943a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_REVELATION_PURPOSE_ZONE_IMBALANCE'), 'PACING_REVELATION_PURPOSE_ZONE_IMBALANCE should fire');
    });

    it('PACING_REVELATION_PURPOSE_ZONE_IMBALANCE does not fire when revelation-purposed scenes touch every zone', async () => {
      const recs943an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 5, 8].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runP943(recs943an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_REVELATION_PURPOSE_ZONE_IMBALANCE'), 'PACING_REVELATION_PURPOSE_ZONE_IMBALANCE should not fire');
    });

    it('PACING_CURIOSITY_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of curiosity-raising scenes', async () => {
      const recs943b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { curiosityDelta: [0, 1, 2, 8, 9].includes(i) ? 1 : 0 }));
      const res = await runP943(recs943b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_CURIOSITY_ZONE_IMBALANCE'), 'PACING_CURIOSITY_ZONE_IMBALANCE should fire');
    });

    it('PACING_CURIOSITY_ZONE_IMBALANCE does not fire when curiosity-raising scenes touch every zone', async () => {
      const recs943bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { curiosityDelta: [0, 3, 5, 8].includes(i) ? 1 : 0 }));
      const res = await runP943(recs943bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_CURIOSITY_ZONE_IMBALANCE'), 'PACING_CURIOSITY_ZONE_IMBALANCE should not fire');
    });

    it('PACING_SEED_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of seeding scenes', async () => {
      const recs943c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { seededClueIds: [0, 1, 2, 8, 9].includes(i) ? ['c1'] : [] }));
      const res = await runP943(recs943c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_SEED_ZONE_IMBALANCE'), 'PACING_SEED_ZONE_IMBALANCE should fire');
    });

    it('PACING_SEED_ZONE_IMBALANCE does not fire when seeding scenes touch every zone', async () => {
      const recs943cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { seededClueIds: [0, 3, 5, 8].includes(i) ? ['c1'] : [] }));
      const res = await runP943(recs943cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_SEED_ZONE_IMBALANCE'), 'PACING_SEED_ZONE_IMBALANCE should not fire');
    });
  });

  describe('Wave 929 — pacingPass: pacing revelation purpose zone cluster, pacing revelation purpose drought run, pacing negative emotion zone imbalance', async () => {
    const runP929 = async (records: ScreenplaySceneRecord[]) => {
      const { pacingPass } = await import('../../server/nvm/revision/passes/pacing.ts');
      return pacingPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // PACING_REVELATION_PURPOSE_ZONE_CLUSTER fire: n=9, 3 thirds; revelation-purposed scenes at
    // 0,1,2 (opening third) → 3/3 = 100% > 75%. Filler 'establish_world'.
    it('PACING_REVELATION_PURPOSE_ZONE_CLUSTER fires when >75% of revelation-purposed scenes cluster in one third', async () => {
      const recs929a = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runP929(recs929a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_REVELATION_PURPOSE_ZONE_CLUSTER'), 'PACING_REVELATION_PURPOSE_ZONE_CLUSTER should fire');
    });

    it('PACING_REVELATION_PURPOSE_ZONE_CLUSTER does not fire when revelation-purposed scenes spread across thirds', async () => {
      const recs929an = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runP929(recs929an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_REVELATION_PURPOSE_ZONE_CLUSTER'), 'PACING_REVELATION_PURPOSE_ZONE_CLUSTER should not fire');
    });

    // PACING_REVELATION_PURPOSE_DROUGHT_RUN fire: n=10, revelation-purposed scenes at 0, 8, 9
    // (minPresentCount 3), leaving a 7-scene gap (indices 1-7) — run of 7 >= threshold 6.
    it('PACING_REVELATION_PURPOSE_DROUGHT_RUN fires when a long run has no revelation-purposed scene', async () => {
      const recs929b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 8, 9].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runP929(recs929b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_REVELATION_PURPOSE_DROUGHT_RUN'), 'PACING_REVELATION_PURPOSE_DROUGHT_RUN should fire');
    });

    it('PACING_REVELATION_PURPOSE_DROUGHT_RUN does not fire when revelation-purposed scenes are evenly spread', async () => {
      const recs929bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'revelation' : 'establish_world' }));
      const res = await runP929(recs929bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_REVELATION_PURPOSE_DROUGHT_RUN'), 'PACING_REVELATION_PURPOSE_DROUGHT_RUN should not fire');
    });

    // PACING_NEGATIVE_EMOTION_ZONE_IMBALANCE fire: n=10, Z0={0,1,2}, Z1={3,4}, Z2={5,6,7}, Z3={8,9};
    // negative at 0,1,2,8,9 → Z0 3/5=60% (bloat), Z1 and Z2 empty. Filler 'neutral'.
    it('PACING_NEGATIVE_EMOTION_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of negative-shift scenes', async () => {
      const recs929c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2, 8, 9].includes(i) ? 'negative' : 'neutral' }));
      const res = await runP929(recs929c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_NEGATIVE_EMOTION_ZONE_IMBALANCE'), 'PACING_NEGATIVE_EMOTION_ZONE_IMBALANCE should fire');
    });

    it('PACING_NEGATIVE_EMOTION_ZONE_IMBALANCE does not fire when negative-shift scenes touch every zone', async () => {
      const recs929cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 3, 5, 8].includes(i) ? 'negative' : 'neutral' }));
      const res = await runP929(recs929cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_NEGATIVE_EMOTION_ZONE_IMBALANCE'), 'PACING_NEGATIVE_EMOTION_ZONE_IMBALANCE should not fire');
    });
  });
  ...
  describe('Wave 873 — pacingPass: pacing climax drought run, pacing resolution drought run, pacing complicate zone cluster', async () => {
    ...
    // PACING_CLIMAX_DROUGHT_RUN fire:
    // n=10; climax at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('PACING_CLIMAX_DROUGHT_RUN fires when a long run has no climax-purposed scene', async () => {
      const recs873a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'climax' : 'complicate' }),
      );
      const res = await runP873(recs873a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_CLIMAX_DROUGHT_RUN'), 'PACING_CLIMAX_DROUGHT_RUN should fire');
    });

    it('PACING_CLIMAX_DROUGHT_RUN does not fire when climax-purposed scenes are evenly spread', async () => {
      const recs873an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'climax' : 'complicate' }),
      );
      const res = await runP873(recs873an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_CLIMAX_DROUGHT_RUN'), 'PACING_CLIMAX_DROUGHT_RUN should not fire');
    });

    // PACING_RESOLUTION_DROUGHT_RUN fire:
    // n=10; resolution at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('PACING_RESOLUTION_DROUGHT_RUN fires when a long run has no resolution-purposed scene', async () => {
      const recs873b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'resolution' : 'complicate' }),
      );
      const res = await runP873(recs873b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_RESOLUTION_DROUGHT_RUN'), 'PACING_RESOLUTION_DROUGHT_RUN should fire');
    });

    it('PACING_RESOLUTION_DROUGHT_RUN does not fire when resolution-purposed scenes are evenly spread', async () => {
      const recs873bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'resolution' : 'complicate' }),
      );
      const res = await runP873(recs873bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_RESOLUTION_DROUGHT_RUN'), 'PACING_RESOLUTION_DROUGHT_RUN should not fire');
    });

    // PACING_COMPLICATE_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; complicate scenes at 0,1,2 → 100% opening third
    it('PACING_COMPLICATE_ZONE_CLUSTER fires when >75% of complicating scenes cluster in one third', async () => {
      const recs873c = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'complicate' : 'establish_world' }),
      );
      const res = await runP873(recs873c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_COMPLICATE_ZONE_CLUSTER'), 'PACING_COMPLICATE_ZONE_CLUSTER should fire');
    });

    it('PACING_COMPLICATE_ZONE_CLUSTER does not fire when complicating scenes spread across thirds', async () => {
      const recs873cn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'complicate' : 'establish_world' }),
      );
      const res = await runP873(recs873cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_COMPLICATE_ZONE_CLUSTER'), 'PACING_COMPLICATE_ZONE_CLUSTER should not fire');
    });
  });
  ...
  describe('Wave 845 — pacingPass: pacing introduce conflict drought run, pacing positive emotion drought run, pacing establish world zone cluster', async () => {
    ...
    // PACING_INTRODUCE_CONFLICT_DROUGHT_RUN fire:
    // n=10; introduce_conflict at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('PACING_INTRODUCE_CONFLICT_DROUGHT_RUN fires when a long run has no new conflict', async () => {
      const recs845a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'introduce_conflict' : 'complicate' }),
      );
      const res = await runP845(recs845a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_INTRODUCE_CONFLICT_DROUGHT_RUN'), 'PACING_INTRODUCE_CONFLICT_DROUGHT_RUN should fire');
    });

    it('PACING_INTRODUCE_CONFLICT_DROUGHT_RUN does not fire when conflict-introducing scenes are evenly spread', async () => {
      const recs845an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'introduce_conflict' : 'complicate' }),
      );
      const res = await runP845(recs845an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_INTRODUCE_CONFLICT_DROUGHT_RUN'), 'PACING_INTRODUCE_CONFLICT_DROUGHT_RUN should not fire');
    });

    // PACING_POSITIVE_EMOTION_DROUGHT_RUN fire:
    // n=10; positive-emotion at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('PACING_POSITIVE_EMOTION_DROUGHT_RUN fires when a long run has no positive-emotion charge', async () => {
      const recs845b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2].includes(i) ? 'positive' : 'neutral' }),
      );
      const res = await runP845(recs845b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_POSITIVE_EMOTION_DROUGHT_RUN'), 'PACING_POSITIVE_EMOTION_DROUGHT_RUN should fire');
    });

    it('PACING_POSITIVE_EMOTION_DROUGHT_RUN does not fire when positive-emotion scenes are evenly spread', async () => {
      const recs845bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 3, 6, 9].includes(i) ? 'positive' : 'neutral' }),
      );
      const res = await runP845(recs845bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_POSITIVE_EMOTION_DROUGHT_RUN'), 'PACING_POSITIVE_EMOTION_DROUGHT_RUN should not fire');
    });

    // PACING_ESTABLISH_WORLD_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; establish_world scenes at 0,1,2 → 100% opening third
    it('PACING_ESTABLISH_WORLD_ZONE_CLUSTER fires when >75% of world-establishing scenes cluster in one third', async () => {
      const recs845c = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'establish_world' : 'complicate' }),
      );
      const res = await runP845(recs845c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_ESTABLISH_WORLD_ZONE_CLUSTER'), 'PACING_ESTABLISH_WORLD_ZONE_CLUSTER should fire');
    });

    it('PACING_ESTABLISH_WORLD_ZONE_CLUSTER does not fire when world-establishing scenes spread across thirds', async () => {
      const recs845cn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'establish_world' : 'complicate' }),
      );
      const res = await runP845(recs845cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_ESTABLISH_WORLD_ZONE_CLUSTER'), 'PACING_ESTABLISH_WORLD_ZONE_CLUSTER should not fire');
    });
  });
  ...
  describe('Wave 803 — pacingPass: pacing revelation peak uncaused, pacing negative emotion zone cluster, pacing negative emotion drought run', async () => {
    ...
    // PACING_REVELATION_PEAK_UNCAUSED fire:
    ...
    it('PACING_REVELATION_PEAK_UNCAUSED fires when the revelation scene has no dramatic turn nearby', async () => {
      const recs803a = Array.from({ length: 8 }, (_, i) => makeSharedRecord(i));
      recs803a[2] = makeSharedRecord(2, { revelation: 'truth revealed' });
      recs803a[5] = makeSharedRecord(5, { revelation: 'truth revealed' });
      const res = await runP803(recs803a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_REVELATION_PEAK_UNCAUSED'), 'PACING_REVELATION_PEAK_UNCAUSED should fire');
    });

    it('PACING_REVELATION_PEAK_UNCAUSED does not fire when a dramatic turn precedes the revelation scene', async () => {
      const recs803an = Array.from({ length: 8 }, (_, i) => makeSharedRecord(i));
      recs803an[2] = makeSharedRecord(2, { revelation: 'truth revealed' });
      recs803an[5] = makeSharedRecord(5, { revelation: 'truth revealed' });
      recs803an[1] = makeSharedRecord(1, { dramaticTurn: 'reversal' });
      const res = await runP803(recs803an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_REVELATION_PEAK_UNCAUSED'), 'PACING_REVELATION_PEAK_UNCAUSED should not fire');
    });

    // PACING_NEGATIVE_EMOTION_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; negative-emotion scenes at 0,1,2 → 100% opening third
    it('PACING_NEGATIVE_EMOTION_ZONE_CLUSTER fires when >75% of negative-emotion scenes cluster in one third', async () => {
      const recs803b = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2].includes(i) ? 'negative' : 'neutral' }),
      );
      const res = await runP803(recs803b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_NEGATIVE_EMOTION_ZONE_CLUSTER'), 'PACING_NEGATIVE_EMOTION_ZONE_CLUSTER should fire');
    });

    it('PACING_NEGATIVE_EMOTION_ZONE_CLUSTER does not fire when negative-emotion scenes spread across thirds', async () => {
      const recs803bn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 4, 8].includes(i) ? 'negative' : 'neutral' }),
      );
      const res = await runP803(recs803bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_NEGATIVE_EMOTION_ZONE_CLUSTER'), 'PACING_NEGATIVE_EMOTION_ZONE_CLUSTER should not fire');
    });

    // PACING_NEGATIVE_EMOTION_DROUGHT_RUN fire:
    // n=10; negative-emotion at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('PACING_NEGATIVE_EMOTION_DROUGHT_RUN fires when a long run has no negative-emotion charge', async () => {
      const recs803c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2].includes(i) ? 'negative' : 'neutral' }),
      );
      const res = await runP803(recs803c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_NEGATIVE_EMOTION_DROUGHT_RUN'), 'PACING_NEGATIVE_EMOTION_DROUGHT_RUN should fire');
    });

    it('PACING_NEGATIVE_EMOTION_DROUGHT_RUN does not fire when negative-emotion scenes are evenly spread', async () => {
      const recs803cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 3, 6, 9].includes(i) ? 'negative' : 'neutral' }),
      );
      const res = await runP803(recs803cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_NEGATIVE_EMOTION_DROUGHT_RUN'), 'PACING_NEGATIVE_EMOTION_DROUGHT_RUN should not fire');
    });
  });

  describe('Wave 789 — pacingPass: pacing emotion zone cluster, pacing revelation zone cluster, pacing revelation drought run', async () => {
    const runP789 = async (records: ScreenplaySceneRecord[]) => {
      const { pacingPass } = await import('../../server/nvm/revision/passes/pacing.ts');
      return pacingPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // PACING_EMOTION_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; emotionally charged scenes at 0,1,2 → 100% opening third
    it('PACING_EMOTION_ZONE_CLUSTER fires when >75% of emotionally charged scenes cluster in one third', async () => {
      const recs789a = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2].includes(i) ? 'negative' : 'neutral' }),
      );
      const res = await runP789(recs789a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_EMOTION_ZONE_CLUSTER'), 'PACING_EMOTION_ZONE_CLUSTER should fire');
    });

    it('PACING_EMOTION_ZONE_CLUSTER does not fire when emotionally charged scenes spread across thirds', async () => {
      const recs789an = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 4, 8].includes(i) ? 'negative' : 'neutral' }),
      );
      const res = await runP789(recs789an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_EMOTION_ZONE_CLUSTER'), 'PACING_EMOTION_ZONE_CLUSTER should not fire');
    });

    // PACING_REVELATION_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; revelation scenes at 0,1,2 → 100% opening third
    it('PACING_REVELATION_ZONE_CLUSTER fires when >75% of revelation scenes cluster in one third', async () => {
      const recs789b = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { revelation: [0, 1, 2].includes(i) ? 'truth revealed' : null }),
      );
      const res = await runP789(recs789b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_REVELATION_ZONE_CLUSTER'), 'PACING_REVELATION_ZONE_CLUSTER should fire');
    });

    it('PACING_REVELATION_ZONE_CLUSTER does not fire when revelation scenes spread across thirds', async () => {
      const recs789bn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { revelation: [0, 4, 8].includes(i) ? 'truth revealed' : null }),
      );
      const res = await runP789(recs789bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_REVELATION_ZONE_CLUSTER'), 'PACING_REVELATION_ZONE_CLUSTER should not fire');
    });

    // PACING_REVELATION_DROUGHT_RUN fire:
    // n=10; revelation present at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('PACING_REVELATION_DROUGHT_RUN fires when a long run has no revelation', async () => {
      const recs789c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { revelation: [0, 1, 2].includes(i) ? 'truth revealed' : null }),
      );
      const res = await runP789(recs789c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_REVELATION_DROUGHT_RUN'), 'PACING_REVELATION_DROUGHT_RUN should fire');
    });

    it('PACING_REVELATION_DROUGHT_RUN does not fire when revelations are evenly spread', async () => {
      const recs789cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { revelation: [0, 3, 6, 9].includes(i) ? 'truth revealed' : null }),
      );
      const res = await runP789(recs789cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_REVELATION_DROUGHT_RUN'), 'PACING_REVELATION_DROUGHT_RUN should not fire');
    });
  });
  ...
  describe('Wave 747 — pacingPass: pacing relationship drought run, pacing clock delta zone cluster, pacing stakes drought run', async () => {
    ...
    // PACING_RELATIONSHIP_DROUGHT_RUN fire:
    // n=10; scenes 0,1,2 carry a relationship shift (>=3 present overall); scenes 3-9 (7 scenes) have none
    it('PACING_RELATIONSHIP_DROUGHT_RUN fires when the longest no-relationship-shift run reaches 6', async () => {
      const recs747a = Array.from({ length: 10 }, (_, i) => makeSharedRecord(i));
      recs747a[0] = makeSharedRecord(0, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
      recs747a[1] = makeSharedRecord(1, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
      recs747a[2] = makeSharedRecord(2, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
      const res = await runP747(recs747a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_RELATIONSHIP_DROUGHT_RUN'), 'PACING_RELATIONSHIP_DROUGHT_RUN should fire');
    });

    // PACING_RELATIONSHIP_DROUGHT_RUN no-fire:
    // relationship-shift scenes spread out so no gap reaches 6 consecutive scenes
    it('PACING_RELATIONSHIP_DROUGHT_RUN does not fire when relationship shifts are spread through the story', async () => {
      const recs747an = Array.from({ length: 10 }, (_, i) => makeSharedRecord(i));
      recs747an[0] = makeSharedRecord(0, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
      recs747an[3] = makeSharedRecord(3, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
      recs747an[6] = makeSharedRecord(6, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
      recs747an[9] = makeSharedRecord(9, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
      const res = await runP747(recs747an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_RELATIONSHIP_DROUGHT_RUN'), 'PACING_RELATIONSHIP_DROUGHT_RUN should not fire');
    });

    // PACING_CLOCK_DELTA_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; clock-shifting scenes at 0,1,2 → 100% opening third
    it('PACING_CLOCK_DELTA_ZONE_CLUSTER fires when >75% of clock-shifting scenes cluster in one third', async () => {
      const recs747b = Array.from({ length: 9 }, (_, i) => makeSharedRecord(i));
      recs747b[0] = makeSharedRecord(0, { clockDelta: 1 });
      recs747b[1] = makeSharedRecord(1, { clockDelta: -1 });
      recs747b[2] = makeSharedRecord(2, { clockDelta: 1 });
      const res = await runP747(recs747b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_CLOCK_DELTA_ZONE_CLUSTER'), 'PACING_CLOCK_DELTA_ZONE_CLUSTER should fire');
    });

    // PACING_CLOCK_DELTA_ZONE_CLUSTER no-fire:
    // clock-shifting scenes at 0, 4, 7 (one per third) → maxZone/total = 1/3
    it('PACING_CLOCK_DELTA_ZONE_CLUSTER does not fire when clock movement is distributed across thirds', async () => {
      const recs747bn = Array.from({ length: 9 }, (_, i) => makeSharedRecord(i));
      recs747bn[0] = makeSharedRecord(0, { clockDelta: 1 });
      recs747bn[4] = makeSharedRecord(4, { clockDelta: -1 });
      recs747bn[7] = makeSharedRecord(7, { clockDelta: 1 });
      const res = await runP747(recs747bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_CLOCK_DELTA_ZONE_CLUSTER'), 'PACING_CLOCK_DELTA_ZONE_CLUSTER should not fire');
    });

    // PACING_STAKES_DROUGHT_RUN fire:
    // n=10; scenes 0,1,2 purposed to raise stakes (>=3 present overall); scenes 3-9 (7 scenes) purposed otherwise
    it('PACING_STAKES_DROUGHT_RUN fires when the longest no-stakes-raise run reaches 6', async () => {
      const recs747c = Array.from({ length: 10 }, (_, i) => makeSharedRecord(i));
      recs747c[0] = makeSharedRecord(0, { purpose: 'raise_stakes' });
      recs747c[1] = makeSharedRecord(1, { purpose: 'raise_stakes' });
      recs747c[2] = makeSharedRecord(2, { purpose: 'raise_stakes' });
      const res = await runP747(recs747c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_STAKES_DROUGHT_RUN'), 'PACING_STAKES_DROUGHT_RUN should fire');
    });

    // PACING_STAKES_DROUGHT_RUN no-fire:
    // stakes-raising scenes spread out so no gap reaches 6 consecutive scenes
    it('PACING_STAKES_DROUGHT_RUN does not fire when stakes-raising scenes are spread through the story', async () => {
      const recs747cn = Array.from({ length: 10 }, (_, i) => makeSharedRecord(i));
      recs747cn[0] = makeSharedRecord(0, { purpose: 'raise_stakes' });
      recs747cn[3] = makeSharedRecord(3, { purpose: 'raise_stakes' });
      recs747cn[6] = makeSharedRecord(6, { purpose: 'raise_stakes' });
      recs747cn[9] = makeSharedRecord(9, { purpose: 'raise_stakes' });
      const res = await runP747(recs747cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_STAKES_DROUGHT_RUN'), 'PACING_STAKES_DROUGHT_RUN should not fire');
    });
  });
  ...
  describe('Wave 705 — pacingPass: pacing seed zone cluster, pacing open thread peak uncaused, pacing payoff peak uncaused', async () => {
    ...
    // PACING_SEED_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; seed scenes at 0,1,2 → 100% opening third
    it('PACING_SEED_ZONE_CLUSTER fires when >75% of seed scenes cluster in one third', async () => {
      const recs705a = Array.from({ length: 9 }, (_, i) => makeSharedRecord(i));
      recs705a[0] = makeSharedRecord(0, { seededClueIds: ['clue-a'] });
      recs705a[1] = makeSharedRecord(1, { seededClueIds: ['clue-b'] });
      recs705a[2] = makeSharedRecord(2, { seededClueIds: ['clue-c'] });
      const res = await runP705(recs705a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_SEED_ZONE_CLUSTER'), 'PACING_SEED_ZONE_CLUSTER should fire');
    });

    // PACING_SEED_ZONE_CLUSTER no-fire:
    // seed scenes at 0, 4, 7 (one per third) → maxZone/total = 1/3
    it('PACING_SEED_ZONE_CLUSTER does not fire when seed scenes are distributed across thirds', async () => {
      const recs705an = Array.from({ length: 9 }, (_, i) => makeSharedRecord(i));
      recs705an[0] = makeSharedRecord(0, { seededClueIds: ['clue-a'] });
      recs705an[4] = makeSharedRecord(4, { seededClueIds: ['clue-b'] });
      recs705an[7] = makeSharedRecord(7, { seededClueIds: ['clue-c'] });
      const res = await runP705(recs705an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_SEED_ZONE_CLUSTER'), 'PACING_SEED_ZONE_CLUSTER should not fire');
    });

    // PACING_OPEN_THREAD_PEAK_UNCAUSED fire:
    // 8 scenes; open threads at 2 (1) and 6 (5, the peak); no dramaticTurn or revelation at 6, 5, or 4
    it('PACING_OPEN_THREAD_PEAK_UNCAUSED fires when the peak open-thread scene has no dramatic turn or revelation nearby', async () => {
      const recs705b = Array.from({ length: 8 }, (_, i) => makeSharedRecord(i));
      recs705b[2] = makeSharedRecord(2, { unresolvedClues: ['a'] });
      recs705b[6] = makeSharedRecord(6, { unresolvedClues: ['a', 'b', 'c', 'd', 'e'] });
      const res = await runP705(recs705b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_OPEN_THREAD_PEAK_UNCAUSED'), 'PACING_OPEN_THREAD_PEAK_UNCAUSED should fire');
    });

    // PACING_OPEN_THREAD_PEAK_UNCAUSED no-fire:
    // dramatic turn at scene 5, within the peak's 2-scene lookback (6-1=5)
    it('PACING_OPEN_THREAD_PEAK_UNCAUSED does not fire when a dramatic turn precedes the peak within the lookback', async () => {
      const recs705bn = Array.from({ length: 8 }, (_, i) => makeSharedRecord(i));
      recs705bn[2] = makeSharedRecord(2, { unresolvedClues: ['a'] });
      recs705bn[5] = makeSharedRecord(5, { dramaticTurn: 'reversal' });
      recs705bn[6] = makeSharedRecord(6, { unresolvedClues: ['a', 'b', 'c', 'd', 'e'] });
      const res = await runP705(recs705bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_OPEN_THREAD_PEAK_UNCAUSED'), 'PACING_OPEN_THREAD_PEAK_UNCAUSED should not fire');
    });

    // PACING_PAYOFF_PEAK_UNCAUSED fire:
    // 8 scenes; payoffs at 2 (1) and 6 (5, the peak); no dramaticTurn or revelation at 6, 5, or 4
    it('PACING_PAYOFF_PEAK_UNCAUSED fires when the peak payoff scene has no dramatic turn or revelation nearby', async () => {
      const recs705c = Array.from({ length: 8 }, (_, i) => makeSharedRecord(i));
      recs705c[2] = makeSharedRecord(2, { payoffSetupIds: ['thread-a'] });
      recs705c[6] = makeSharedRecord(6, { payoffSetupIds: ['a', 'b', 'c', 'd', 'e'] });
      const res = await runP705(recs705c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PACING_PAYOFF_PEAK_UNCAUSED'), 'PACING_PAYOFF_PEAK_UNCAUSED should fire');
    });

    // PACING_PAYOFF_PEAK_UNCAUSED no-fire:
    // dramatic turn at scene 5, within the peak's 2-scene lookback (6-1=5)
    it('PACING_PAYOFF_PEAK_UNCAUSED does not fire when a dramatic turn precedes the peak within the lookback', async () => {
      const recs705cn = Array.from({ length: 8 }, (_, i) => makeSharedRecord(i));
      recs705cn[2] = makeSharedRecord(2, { payoffSetupIds: ['thread-a'] });
      recs705cn[5] = makeSharedRecord(5, { dramaticTurn: 'reversal' });
      recs705cn[6] = makeSharedRecord(6, { payoffSetupIds: ['a', 'b', 'c', 'd', 'e'] });
      const res = await runP705(recs705cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PACING_PAYOFF_PEAK_UNCAUSED'), 'PACING_PAYOFF_PEAK_UNCAUSED should not fire');
    });
  });
