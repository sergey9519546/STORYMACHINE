...
describe('Wave 1186 — conflictPass (Program v2, Type 1 — signal channel): power-balance flatline, climax-uncontested, interrogation monopoly', async () => {
  ...
  describe('Wave 1164 — conflictPass: conflict revelation-dialogue-highlight aftermath void, conflict suspense-emotional aftermath void, conflict suspense-curiosity aftermath void', async () => {
    ...
    const runCF1164 = async (records: any[]) => {
      const { conflictPass } = await import('../../server/nvm/revision/passes/conflict.ts');
      return conflictPass({
        fountain: '', original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    it('CONFLICT_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every revelation is followed by two scenes with no highlighted dialogue', async () => {
      const recs1164a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1164(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeRec1164(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1164(i);
      });
      const res = await runCF1164(recs1164a);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'CONFLICT_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a revelation is followed by highlighted dialogue within its window', async () => {
      const recs1164an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1164(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeRec1164(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1164(i);
      });
      const res = await runCF1164(recs1164an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'CONFLICT_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_SUSPENSE_EMOTIONAL_AFTERMATH_VOID fires when every suspense-spike is followed by two scenes with no emotional shift', async () => {
      const recs1164b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1164(i, { suspenseDelta: 1 });
        if (i === 8 || i === 9) return makeRec1164(i, { emotionalShift: 'positive' });
        return makeRec1164(i);
      });
      const res = await runCF1164(recs1164b);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_SUSPENSE_EMOTIONAL_AFTERMATH_VOID'), 'CONFLICT_SUSPENSE_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_SUSPENSE_EMOTIONAL_AFTERMATH_VOID does not fire when a suspense-spike is followed by an emotional shift within its window', async () => {
      const recs1164bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1164(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeRec1164(i, { emotionalShift: 'positive' });
        return makeRec1164(i);
      });
      const res = await runCF1164(recs1164bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_SUSPENSE_EMOTIONAL_AFTERMATH_VOID'), 'CONFLICT_SUSPENSE_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_SUSPENSE_CURIOSITY_AFTERMATH_VOID fires when every suspense-spike is followed by two scenes with no curiosity rise', async () => {
      const recs1164c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1164(i, { suspenseDelta: 1 });
        if (i === 8 || i === 9) return makeRec1164(i, { curiosityDelta: 1 });
        return makeRec1164(i);
      });
      const res = await runCF1164(recs1164c);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_SUSPENSE_CURIOSITY_AFTERMATH_VOID'), 'CONFLICT_SUSPENSE_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_SUSPENSE_CURIOSITY_AFTERMATH_VOID does not fire when a suspense-spike is followed by a curiosity rise within its window', async () => {
      const recs1164cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1164(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeRec1164(i, { curiosityDelta: 1 });
        return makeRec1164(i);
      });
      const res = await runCF1164(recs1164cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_SUSPENSE_CURIOSITY_AFTERMATH_VOID'), 'CONFLICT_SUSPENSE_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1122 — conflictPass: conflict revelation-emotional aftermath void, conflict revelation-suspense aftermath void, conflict clock-curiosity aftermath void', async () => {
    ...
    const runCF1122 = async (records: any[]) => {
      const { conflictPass } = await import('../../server/nvm/revision/passes/conflict.ts');
      return conflictPass({
        fountain: '', original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('CONFLICT_REVELATION_EMOTIONAL_AFTERMATH_VOID fires when every revelation is followed by two scenes with no emotional shift', async () => {
      const recs1122a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1122(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeRec1122(i, { emotionalShift: 'positive' });
        return makeRec1122(i);
      });
      const res = await runCF1122(recs1122a);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_REVELATION_EMOTIONAL_AFTERMATH_VOID'), 'CONFLICT_REVELATION_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_REVELATION_EMOTIONAL_AFTERMATH_VOID does not fire when a revelation is followed by an emotional shift within its window', async () => {
      const recs1122an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1122(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeRec1122(i, { emotionalShift: 'positive' });
        return makeRec1122(i);
      });
      const res = await runCF1122(recs1122an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_REVELATION_EMOTIONAL_AFTERMATH_VOID'), 'CONFLICT_REVELATION_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_REVELATION_SUSPENSE_AFTERMATH_VOID fires when every revelation is followed by two scenes with no suspense rise', async () => {
      const recs1122b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1122(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeRec1122(i, { suspenseDelta: 1 });
        return makeRec1122(i);
      });
      const res = await runCF1122(recs1122b);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_REVELATION_SUSPENSE_AFTERMATH_VOID'), 'CONFLICT_REVELATION_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_REVELATION_SUSPENSE_AFTERMATH_VOID does not fire when a revelation is followed by a suspense rise within its window', async () => {
      const recs1122bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1122(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeRec1122(i, { suspenseDelta: 1 });
        return makeRec1122(i);
      });
      const res = await runCF1122(recs1122bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_REVELATION_SUSPENSE_AFTERMATH_VOID'), 'CONFLICT_REVELATION_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_CLOCK_CURIOSITY_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no curiosity rise', async () => {
      const recs1122c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1122(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeRec1122(i, { curiosityDelta: 1 });
        return makeRec1122(i);
      });
      const res = await runCF1122(recs1122c);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_CLOCK_CURIOSITY_AFTERMATH_VOID'), 'CONFLICT_CLOCK_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_CLOCK_CURIOSITY_AFTERMATH_VOID does not fire when a clock-raise is followed by a curiosity rise within its window', async () => {
      const recs1122cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1122(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeRec1122(i, { curiosityDelta: 1 });
        return makeRec1122(i);
      });
      const res = await runCF1122(recs1122cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_CLOCK_CURIOSITY_AFTERMATH_VOID'), 'CONFLICT_CLOCK_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1108 — conflictPass: conflict seed-dialogue-highlight aftermath void, conflict revelation-curiosity aftermath void, conflict clock-suspense aftermath void', async () => {
    const makeRec1108 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: `INT. SC${idx} - DAY`,
      emotionalShift: 'neutral', suspenseDelta: 0, curiosityDelta: 0,
      clockRaised: false, clockDelta: 0,
      dialogueHighlights: [], revelation: null,
    ...
    const runCF1108 = async (records: any[]) => {
      const { conflictPass } = await import('../../server/nvm/revision/passes/conflict.ts');
      return conflictPass({
        fountain: '', original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('CONFLICT_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every seed is followed by two scenes with no highlighted dialogue', async () => {
      const recs1108a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1108(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeRec1108(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1108(i);
      });
      const res = await runCF1108(recs1108a);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'CONFLICT_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a seed is followed by highlighted dialogue within its window', async () => {
      const recs1108an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1108(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeRec1108(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1108(i);
      });
      const res = await runCF1108(recs1108an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'CONFLICT_SEED_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_REVELATION_CURIOSITY_AFTERMATH_VOID fires when every revelation is followed by two scenes with no curiosity rise', async () => {
      const recs1108b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1108(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeRec1108(i, { curiosityDelta: 1 });
        return makeRec1108(i);
      });
      const res = await runCF1108(recs1108b);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_REVELATION_CURIOSITY_AFTERMATH_VOID'), 'CONFLICT_REVELATION_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_REVELATION_CURIOSITY_AFTERMATH_VOID does not fire when a revelation is followed by a curiosity rise within its window', async () => {
      const recs1108bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1108(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeRec1108(i, { curiosityDelta: 1 });
        return makeRec1108(i);
      });
      const res = await runCF1108(recs1108bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_REVELATION_CURIOSITY_AFTERMATH_VOID'), 'CONFLICT_REVELATION_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_CLOCK_SUSPENSE_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no suspense rise', async () => {
      const recs1108c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1108(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeRec1108(i, { suspenseDelta: 1 });
        return makeRec1108(i);
      });
      const res = await runCF1108(recs1108c);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_CLOCK_SUSPENSE_AFTERMATH_VOID'), 'CONFLICT_CLOCK_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_CLOCK_SUSPENSE_AFTERMATH_VOID does not fire when a clock-raise is followed by a suspense rise within its window', async () => {
      const recs1108cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1108(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeRec1108(i, { suspenseDelta: 1 });
        return makeRec1108(i);
      });
      const res = await runCF1108(recs1108cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_CLOCK_SUSPENSE_AFTERMATH_VOID'), 'CONFLICT_CLOCK_SUSPENSE_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1066 — conflictPass: conflict stakes-staging aftermath void, conflict payoff-dialogue-highlight aftermath void, conflict open-thread-suspense aftermath void', async () => {
    ...
    const runCF1066 = async (records: any[]) => {
      const { conflictPass } = await import('../../server/nvm/revision/passes/conflict.ts');
      return conflictPass({
        fountain: '', original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('CONFLICT_STAKES_STAGING_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no visually dense scene', async () => {
      const recs1066a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1066(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeRec1066(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1066(i);
      });
      const res = await runCF1066(recs1066a);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_STAKES_STAGING_AFTERMATH_VOID'), 'CONFLICT_STAKES_STAGING_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_STAKES_STAGING_AFTERMATH_VOID does not fire when a stakes-raise is followed by a visually dense scene within its window', async () => {
      const recs1066an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1066(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeRec1066(i, { visualBeats: ['beat one', 'beat two'] });
        return makeRec1066(i);
      });
      const res = await runCF1066(recs1066an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_STAKES_STAGING_AFTERMATH_VOID'), 'CONFLICT_STAKES_STAGING_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every payoff is followed by two scenes with no highlighted dialogue', async () => {
      const recs1066b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1066(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeRec1066(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1066(i);
      });
      const res = await runCF1066(recs1066b);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'CONFLICT_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a payoff is followed by highlighted dialogue within its window', async () => {
      const recs1066bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1066(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeRec1066(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1066(i);
      });
      const res = await runCF1066(recs1066bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'CONFLICT_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no suspense rise', async () => {
      const recs1066c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1066(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeRec1066(i, { suspenseDelta: 1 });
        return makeRec1066(i);
      });
      const res = await runCF1066(recs1066c);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID'), 'CONFLICT_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a suspense rise within its window', async () => {
      const recs1066cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1066(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeRec1066(i, { suspenseDelta: 1 });
        return makeRec1066(i);
      });
      const res = await runCF1066(recs1066cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID'), 'CONFLICT_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1052 — conflictPass: conflict stakes-dialogue-highlight aftermath void, conflict payoff-staging aftermath void, conflict open-thread-dialogue-highlight aftermath void', async () => {
    const makeRec1052 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: `INT. SC${idx} - DAY`,
      emotionalShift: 'neutral', suspenseDelta: 0, curiosityDelta: 0,
      clockRaised: false, clockDelta: 0,
      dialogueHighlights: [], revelation: null,
    ...
    const runCF1052 = async (records: any[]) => {
      const { conflictPass } = await import('../../server/nvm/revision/passes/conflict.ts');
      return conflictPass({
        fountain: '', original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('CONFLICT_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no highlighted dialogue', async () => {
      const recs1052a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1052(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeRec1052(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1052(i);
      });
      const res = await runCF1052(recs1052a);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'CONFLICT_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a stakes-raise is followed by highlighted dialogue within its window', async () => {
      const recs1052an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1052(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeRec1052(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1052(i);
      });
      const res = await runCF1052(recs1052an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'CONFLICT_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_PAYOFF_STAGING_AFTERMATH_VOID fires when every payoff is followed by two scenes with no substantial staging', async () => {
      const recs1052b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1052(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeRec1052(i, { visualBeats: ['beat1', 'beat2'] });
        return makeRec1052(i);
      });
      const res = await runCF1052(recs1052b);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_PAYOFF_STAGING_AFTERMATH_VOID'), 'CONFLICT_PAYOFF_STAGING_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_PAYOFF_STAGING_AFTERMATH_VOID does not fire when a payoff is followed by substantial staging within its window', async () => {
      const recs1052bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1052(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeRec1052(i, { visualBeats: ['beat1', 'beat2'] });
        return makeRec1052(i);
      });
      const res = await runCF1052(recs1052bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_PAYOFF_STAGING_AFTERMATH_VOID'), 'CONFLICT_PAYOFF_STAGING_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no highlighted dialogue', async () => {
      const recs1052c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1052(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeRec1052(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1052(i);
      });
      const res = await runCF1052(recs1052c);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'CONFLICT_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by highlighted dialogue within its window', async () => {
      const recs1052cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1052(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeRec1052(i, { dialogueHighlights: ['a memorable line'] });
        return makeRec1052(i);
      });
      const res = await runCF1052(recs1052cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'CONFLICT_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1024 — conflictPass: conflict stakes-relational aftermath void, conflict payoff-curiosity aftermath void, conflict turn-emotional aftermath void', async () => {
    ...
    const runCF1024 = async (records: any[]) => {
      const { conflictPass } = await import('../../server/nvm/revision/passes/conflict.ts');
      return conflictPass({
        fountain: '', original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('CONFLICT_STAKES_RELATIONAL_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no relationship shift', async () => {
      const recs1024a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1024(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeRec1024(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeRec1024(i);
      });
      const res = await runCF1024(recs1024a);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_STAKES_RELATIONAL_AFTERMATH_VOID'), 'CONFLICT_STAKES_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_STAKES_RELATIONAL_AFTERMATH_VOID does not fire when a stakes-raise is followed by a relationship shift within its window', async () => {
      const recs1024an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1024(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeRec1024(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeRec1024(i);
      });
      const res = await runCF1024(recs1024an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_STAKES_RELATIONAL_AFTERMATH_VOID'), 'CONFLICT_STAKES_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_PAYOFF_CURIOSITY_AFTERMATH_VOID fires when every payoff is followed by two scenes with no curiosity rise', async () => {
      const recs1024b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1024(i, { payoffSetupIds: ['p1'] });
        if (i === 8 || i === 9) return makeRec1024(i, { curiosityDelta: 1 });
        return makeRec1024(i);
      });
      const res = await runCF1024(recs1024b);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_PAYOFF_CURIOSITY_AFTERMATH_VOID'), 'CONFLICT_PAYOFF_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_PAYOFF_CURIOSITY_AFTERMATH_VOID does not fire when a payoff is followed by a curiosity rise within its window', async () => {
      const recs1024bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1024(i, { payoffSetupIds: ['p1'] });
        if (i === 1 || i === 9) return makeRec1024(i, { curiosityDelta: 1 });
        return makeRec1024(i);
      });
      const res = await runCF1024(recs1024bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_PAYOFF_CURIOSITY_AFTERMATH_VOID'), 'CONFLICT_PAYOFF_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_TURN_EMOTIONAL_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no emotional shift', async () => {
      const recs1024c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1024(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeRec1024(i, { emotionalShift: 'positive' });
        return makeRec1024(i);
      });
      const res = await runCF1024(recs1024c);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_TURN_EMOTIONAL_AFTERMATH_VOID'), 'CONFLICT_TURN_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_TURN_EMOTIONAL_AFTERMATH_VOID does not fire when a dramatic turn is followed by an emotional shift within its window', async () => {
      const recs1024cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1024(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeRec1024(i, { emotionalShift: 'positive' });
        return makeRec1024(i);
      });
      const res = await runCF1024(recs1024cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_TURN_EMOTIONAL_AFTERMATH_VOID'), 'CONFLICT_TURN_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1010 — conflictPass: conflict turn-curiosity aftermath void, conflict payoff-suspense aftermath void, conflict stakes-emotional aftermath void', async () => {
    const makeRec1010 = (idx: number, overrides: any = {}): any => ({
      sceneIdx: idx, slug: `INT. SC${idx} - DAY`,
      emotionalShift: 'neutral', suspenseDelta: 0, curiosityDelta: 0,
      clockRaised: false, clockDelta: 0,
      dialogueHighlights: [], revelation: null,
    ...
    const runCF1010 = async (records: any[]) => {
      const { conflictPass } = await import('../../server/nvm/revision/passes/conflict.ts');
      return conflictPass({
        fountain: '', original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('CONFLICT_TURN_CURIOSITY_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no new curiosity', async () => {
      const recs1010a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1010(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeRec1010(i, { curiosityDelta: 1 });
        return makeRec1010(i);
      });
      const res = await runCF1010(recs1010a);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_TURN_CURIOSITY_AFTERMATH_VOID'), 'CONFLICT_TURN_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_TURN_CURIOSITY_AFTERMATH_VOID does not fire when a dramatic turn is followed by new curiosity within its window', async () => {
      const recs1010an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1010(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeRec1010(i, { curiosityDelta: 1 });
        return makeRec1010(i);
      });
      const res = await runCF1010(recs1010an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_TURN_CURIOSITY_AFTERMATH_VOID'), 'CONFLICT_TURN_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_PAYOFF_SUSPENSE_AFTERMATH_VOID fires when every payoff is followed by two scenes with no rise in suspense', async () => {
      const recs1010b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1010(i, { payoffSetupIds: ['setup1'] });
        if (i === 8 || i === 9) return makeRec1010(i, { suspenseDelta: 1 });
        return makeRec1010(i);
      });
      const res = await runCF1010(recs1010b);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_PAYOFF_SUSPENSE_AFTERMATH_VOID'), 'CONFLICT_PAYOFF_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_PAYOFF_SUSPENSE_AFTERMATH_VOID does not fire when a payoff is followed by rising suspense within its window', async () => {
      const recs1010bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1010(i, { payoffSetupIds: ['setup1'] });
        if (i === 1 || i === 9) return makeRec1010(i, { suspenseDelta: 1 });
        return makeRec1010(i);
      });
      const res = await runCF1010(recs1010bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_PAYOFF_SUSPENSE_AFTERMATH_VOID'), 'CONFLICT_PAYOFF_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('CONFLICT_STAKES_EMOTIONAL_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no emotional shift', async () => {
      const recs1010c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1010(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeRec1010(i, { emotionalShift: 'positive' });
        return makeRec1010(i);
      });
      const res = await runCF1010(recs1010c);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'CONFLICT_STAKES_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_STAKES_EMOTIONAL_AFTERMATH_VOID does not fire when a stakes-raise is followed by an emotional shift within its window', async () => {
      const recs1010cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeRec1010(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeRec1010(i, { emotionalShift: 'positive' });
        return makeRec1010(i);
      });
      const res = await runCF1010(recs1010cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'CONFLICT_STAKES_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 982 — conflictPass: conflict emotion zone imbalance, conflict highlight zone imbalance, conflict stakes curiosity aftermath void', async () => {
    ...
    const runCF982 = async (records: any[]) => {
      const { conflictPass } = await import('../../server/nvm/revision/passes/conflict.ts');
      return conflictPass({
        fountain: '', original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    it('CONFLICT_EMOTION_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of emotionally-charged scenes', async () => {
      const recs982a = Array.from({ length: 10 }, (_, i) =>
        makeRec982(i, [0, 1, 2, 8, 9].includes(i) ? { emotionalShift: 'positive' } : {}));
      const res = await runCF982(recs982a);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_EMOTION_ZONE_IMBALANCE'), 'CONFLICT_EMOTION_ZONE_IMBALANCE should fire');
    });

    it('CONFLICT_EMOTION_ZONE_IMBALANCE does not fire when emotionally-charged scenes touch every zone', async () => {
      const recs982an = Array.from({ length: 10 }, (_, i) =>
        makeRec982(i, [0, 3, 5, 8].includes(i) ? { emotionalShift: 'positive' } : {}));
      const res = await runCF982(recs982an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_EMOTION_ZONE_IMBALANCE'), 'CONFLICT_EMOTION_ZONE_IMBALANCE should not fire');
    });

    it('CONFLICT_HIGHLIGHT_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of dialogue-highlight scenes', async () => {
      const recs982b = Array.from({ length: 10 }, (_, i) =>
        makeRec982(i, [0, 1, 2, 8, 9].includes(i) ? { dialogueHighlights: ['a memorable line'] } : {}));
      const res = await runCF982(recs982b);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_HIGHLIGHT_ZONE_IMBALANCE'), 'CONFLICT_HIGHLIGHT_ZONE_IMBALANCE should fire');
    });

    it('CONFLICT_HIGHLIGHT_ZONE_IMBALANCE does not fire when dialogue-highlight scenes touch every zone', async () => {
      const recs982bn = Array.from({ length: 10 }, (_, i) =>
        makeRec982(i, [0, 3, 5, 8].includes(i) ? { dialogueHighlights: ['a memorable line'] } : {}));
      const res = await runCF982(recs982bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_HIGHLIGHT_ZONE_IMBALANCE'), 'CONFLICT_HIGHLIGHT_ZONE_IMBALANCE should not fire');
    });
    ...
    it('CONFLICT_STAKES_CURIOSITY_AFTERMATH_VOID fires when every stakes-raise has no curiosity raised within 2 scenes', async () => {
      const recs982c = Array.from({ length: 10 }, (_, i) =>
        makeRec982(i, [0, 3].includes(i) ? { purpose: 'raise_stakes' } : ([8, 9].includes(i) ? { curiosityDelta: 1 } : {})));
      const res = await runCF982(recs982c);
      assert.ok(res.issues.some((i: any) => i.rule === 'CONFLICT_STAKES_CURIOSITY_AFTERMATH_VOID'), 'CONFLICT_STAKES_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('CONFLICT_STAKES_CURIOSITY_AFTERMATH_VOID does not fire when a stakes-raise is followed by curiosity within 2 scenes', async () => {
      const recs982cn = Array.from({ length: 10 }, (_, i) =>
        makeRec982(i, [0, 3].includes(i) ? { purpose: 'raise_stakes' } : ([1, 9].includes(i) ? { curiosityDelta: 1 } : {})));
      const res = await runCF982(recs982cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'CONFLICT_STAKES_CURIOSITY_AFTERMATH_VOID'), 'CONFLICT_STAKES_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });
