...
import { relationshipArcPass } from '../../server/nvm/revision/passes/relationship-arc.ts';
  ...
  describe('Wave 1168 — payoffPass: payoff suspense-relational aftermath void, payoff emotion-suspense aftermath void, payoff clock-delta-relational aftermath void', async () => {
    ...
    it('PAYOFF_SUSPENSE_RELATIONAL_AFTERMATH_VOID fires when every suspense-rise is followed by two scenes with no relationship shift', async () => {
      const recs1168a = Array.from({ length: 10 }, (_, i) => {
      ...
      const res = await runPY1168(recs1168a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_SUSPENSE_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_SUSPENSE_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_SUSPENSE_RELATIONAL_AFTERMATH_VOID does not fire when a suspense-rise is followed by a relationship shift within its window', async () => {
      const recs1168an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1168(recs1168an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_SUSPENSE_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_SUSPENSE_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_EMOTION_SUSPENSE_AFTERMATH_VOID fires when every emotionally-charged scene is followed by two scenes with no suspense rise', async () => {
      const recs1168b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { emotionalShift: 'positive' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1168(recs1168b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_EMOTION_SUSPENSE_AFTERMATH_VOID'), 'PAYOFF_EMOTION_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_EMOTION_SUSPENSE_AFTERMATH_VOID does not fire when an emotionally-charged scene is followed by a suspense rise within its window', async () => {
      const recs1168bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { emotionalShift: 'positive' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1168(recs1168bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_EMOTION_SUSPENSE_AFTERMATH_VOID'), 'PAYOFF_EMOTION_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_CLOCK_DELTA_RELATIONAL_AFTERMATH_VOID fires when every clock-delta shift is followed by two scenes with no relationship shift', async () => {
      const recs1168c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockDelta: 1 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1168(recs1168c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_CLOCK_DELTA_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_CLOCK_DELTA_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_CLOCK_DELTA_RELATIONAL_AFTERMATH_VOID does not fire when a clock-delta shift is followed by a relationship shift within its window', async () => {
      const recs1168cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1168(recs1168cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_CLOCK_DELTA_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_CLOCK_DELTA_RELATIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1154 — payoffPass: payoff suspense-emotional aftermath void, payoff emotion-curiosity aftermath void, payoff clock-delta-emotional aftermath void', async () => {
    const runPY1154 = async (records: ScreenplaySceneRecord[]) => {
      const { payoffPass } = await import('../../server/nvm/revision/passes/payoff.ts');
      return payoffPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PAYOFF_SUSPENSE_EMOTIONAL_AFTERMATH_VOID fires when every suspense-rise is followed by two scenes with no emotional shift', async () => {
      const recs1154a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1154(recs1154a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_SUSPENSE_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_SUSPENSE_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_SUSPENSE_EMOTIONAL_AFTERMATH_VOID does not fire when a suspense-rise is followed by an emotional shift within its window', async () => {
      const recs1154an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { suspenseDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1154(recs1154an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_SUSPENSE_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_SUSPENSE_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_EMOTION_CURIOSITY_AFTERMATH_VOID fires when every emotionally-charged scene is followed by two scenes with no curiosity rise', async () => {
      const recs1154b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { emotionalShift: 'positive' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1154(recs1154b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_EMOTION_CURIOSITY_AFTERMATH_VOID'), 'PAYOFF_EMOTION_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_EMOTION_CURIOSITY_AFTERMATH_VOID does not fire when an emotionally-charged scene is followed by a curiosity rise within its window', async () => {
      const recs1154bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { emotionalShift: 'positive' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1154(recs1154bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_EMOTION_CURIOSITY_AFTERMATH_VOID'), 'PAYOFF_EMOTION_CURIOSITY_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_CLOCK_DELTA_EMOTIONAL_AFTERMATH_VOID fires when every clock-delta shift is followed by two scenes with no emotional shift', async () => {
      const recs1154c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockDelta: 1 });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1154(recs1154c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_CLOCK_DELTA_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_CLOCK_DELTA_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_CLOCK_DELTA_EMOTIONAL_AFTERMATH_VOID does not fire when a clock-delta shift is followed by an emotional shift within its window', async () => {
      const recs1154cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockDelta: 1 });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1154(recs1154cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_CLOCK_DELTA_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_CLOCK_DELTA_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1126 — payoffPass: payoff open-thread-staging aftermath void, payoff revelation-staging aftermath void, payoff revelation-dialogue-highlight aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PAYOFF_OPEN_THREAD_STAGING_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no heavily-staged scene', async () => {
      const recs1126a = Array.from({ length: 10 }, (_, i) => {
      ...
      const res = await runPY1126(recs1126a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_OPEN_THREAD_STAGING_AFTERMATH_VOID'), 'PAYOFF_OPEN_THREAD_STAGING_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_OPEN_THREAD_STAGING_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a heavily-staged scene within its window', async () => {
      const recs1126an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runPY1126(recs1126an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_OPEN_THREAD_STAGING_AFTERMATH_VOID'), 'PAYOFF_OPEN_THREAD_STAGING_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_REVELATION_STAGING_AFTERMATH_VOID fires when every revelation is followed by two scenes with no heavily-staged scene', async () => {
      const recs1126b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'the truth about the letter' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runPY1126(recs1126b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_REVELATION_STAGING_AFTERMATH_VOID'), 'PAYOFF_REVELATION_STAGING_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_REVELATION_STAGING_AFTERMATH_VOID does not fire when a revelation is followed by a heavily-staged scene within its window', async () => {
      const recs1126bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'the truth about the letter' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runPY1126(recs1126bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_REVELATION_STAGING_AFTERMATH_VOID'), 'PAYOFF_REVELATION_STAGING_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every revelation is followed by two scenes with no highlighted dialogue', async () => {
      const recs1126c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'the truth about the letter' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runPY1126(recs1126c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PAYOFF_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a revelation is followed by highlighted dialogue within its window', async () => {
      const recs1126cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'the truth about the letter' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runPY1126(recs1126cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PAYOFF_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1112 — payoffPass: payoff stakes-dialogue-highlight aftermath void, payoff stakes-staging aftermath void, payoff open-thread-dialogue-highlight aftermath void', async () => {
    const runPY1112 = async (records: ScreenplaySceneRecord[]) => {
      const { payoffPass } = await import('../../server/nvm/revision/passes/payoff.ts');
      return payoffPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PAYOFF_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no highlighted dialogue', async () => {
      const recs1112a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runPY1112(recs1112a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PAYOFF_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a stakes-raise is followed by highlighted dialogue within its window', async () => {
      const recs1112an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runPY1112(recs1112an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PAYOFF_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_STAKES_STAGING_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no heavily-staged scene', async () => {
      const recs1112b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runPY1112(recs1112b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_STAKES_STAGING_AFTERMATH_VOID'), 'PAYOFF_STAKES_STAGING_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_STAKES_STAGING_AFTERMATH_VOID does not fire when a stakes-raise is followed by a heavily-staged scene within its window', async () => {
      const recs1112bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { visualBeats: ['beat one', 'beat two'] });
        return makeSharedRecord(i);
      });
      const res = await runPY1112(recs1112bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_STAKES_STAGING_AFTERMATH_VOID'), 'PAYOFF_STAKES_STAGING_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no highlighted dialogue', async () => {
      const recs1112c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runPY1112(recs1112c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PAYOFF_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by highlighted dialogue within its window', async () => {
      const recs1112cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { dialogueHighlights: ['a memorable line'] });
        return makeSharedRecord(i);
      });
      const res = await runPY1112(recs1112cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID'), 'PAYOFF_OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1084 — payoffPass: payoff seed-relational aftermath void, payoff clock-suspense aftermath void, payoff turn-emotional aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PAYOFF_SEED_RELATIONAL_AFTERMATH_VOID fires when every seed is followed by two scenes with no relationship shift', async () => {
      const recs1084a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1084(recs1084a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_SEED_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_SEED_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_SEED_RELATIONAL_AFTERMATH_VOID does not fire when a seed is followed by a relationship shift within its window', async () => {
      const recs1084an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1084(recs1084an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_SEED_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_SEED_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_CLOCK_SUSPENSE_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no suspense rise', async () => {
      const recs1084b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1084(recs1084b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_CLOCK_SUSPENSE_AFTERMATH_VOID'), 'PAYOFF_CLOCK_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_CLOCK_SUSPENSE_AFTERMATH_VOID does not fire when a clock-raise is followed by a suspense rise within its window', async () => {
      const recs1084bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1084(recs1084bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_CLOCK_SUSPENSE_AFTERMATH_VOID'), 'PAYOFF_CLOCK_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_TURN_EMOTIONAL_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no emotional shift', async () => {
      const recs1084c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1084(recs1084c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_TURN_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_TURN_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_TURN_EMOTIONAL_AFTERMATH_VOID does not fire when a dramatic turn is followed by an emotional shift within its window', async () => {
      const recs1084cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1084(recs1084cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_TURN_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_TURN_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1070 — payoffPass: payoff stakes-relational aftermath void, payoff open-thread-relational aftermath void, payoff revelation-suspense aftermath void', async () => {
    const runPY1070 = async (records: ScreenplaySceneRecord[]) => {
      const { payoffPass } = await import('../../server/nvm/revision/passes/payoff.ts');
      return payoffPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PAYOFF_STAKES_RELATIONAL_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no relationship shift', async () => {
      const recs1070a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1070(recs1070a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_STAKES_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_STAKES_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_STAKES_RELATIONAL_AFTERMATH_VOID does not fire when a stakes-raise is followed by a relationship shift within its window', async () => {
      const recs1070an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1070(recs1070an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_STAKES_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_STAKES_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no relationship shift', async () => {
      const recs1070b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1070(recs1070b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by a relationship shift within its window', async () => {
      const recs1070bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1070(recs1070bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_OPEN_THREAD_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_REVELATION_SUSPENSE_AFTERMATH_VOID fires when every revelation is followed by two scenes with no suspense rise', async () => {
      const recs1070c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1070(recs1070c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_REVELATION_SUSPENSE_AFTERMATH_VOID'), 'PAYOFF_REVELATION_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_REVELATION_SUSPENSE_AFTERMATH_VOID does not fire when a revelation is followed by a suspense rise within its window', async () => {
      const recs1070cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1070(recs1070cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_REVELATION_SUSPENSE_AFTERMATH_VOID'), 'PAYOFF_REVELATION_SUSPENSE_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1056 — payoffPass: payoff seed-suspense aftermath void, payoff clock-emotional aftermath void, payoff turn-relational aftermath void', async () => {
    const runPY1056 = async (records: ScreenplaySceneRecord[]) => {
      const { payoffPass } = await import('../../server/nvm/revision/passes/payoff.ts');
      return payoffPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PAYOFF_SEED_SUSPENSE_AFTERMATH_VOID fires when every seed is followed by two scenes with no suspense rise', async () => {
      const recs1056a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1056(recs1056a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_SEED_SUSPENSE_AFTERMATH_VOID'), 'PAYOFF_SEED_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_SEED_SUSPENSE_AFTERMATH_VOID does not fire when a seed is followed by a suspense rise within its window', async () => {
      const recs1056an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { seededClueIds: ['c1'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1056(recs1056an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_SEED_SUSPENSE_AFTERMATH_VOID'), 'PAYOFF_SEED_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_CLOCK_EMOTIONAL_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no emotional shift', async () => {
      const recs1056b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1056(recs1056b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_CLOCK_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_CLOCK_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_CLOCK_EMOTIONAL_AFTERMATH_VOID does not fire when a clock-raise is followed by an emotional shift within its window', async () => {
      const recs1056bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1056(recs1056bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_CLOCK_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_CLOCK_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_TURN_RELATIONAL_AFTERMATH_VOID fires when every dramatic turn is followed by two scenes with no relationship shift', async () => {
      const recs1056c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1056(recs1056c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_TURN_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_TURN_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_TURN_RELATIONAL_AFTERMATH_VOID does not fire when a dramatic turn is followed by a relationship shift within its window', async () => {
      const recs1056cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { dramaticTurn: 'reversal' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1056(recs1056cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_TURN_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_TURN_RELATIONAL_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 1028 — payoffPass: payoff stakes-emotional aftermath void, payoff clock-relational aftermath void, payoff revelation-emotional aftermath void', async () => {
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PAYOFF_STAKES_EMOTIONAL_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no emotional shift', async () => {
      const recs1028a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1028(recs1028a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_STAKES_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_STAKES_EMOTIONAL_AFTERMATH_VOID does not fire when a stakes-raise is followed by an emotional shift within its window', async () => {
      const recs1028an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1028(recs1028an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_STAKES_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_STAKES_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_CLOCK_RELATIONAL_AFTERMATH_VOID fires when every clock-raise is followed by two scenes with no relationship shift', async () => {
      const recs1028b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 8 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1028(recs1028b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_CLOCK_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_CLOCK_RELATIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_CLOCK_RELATIONAL_AFTERMATH_VOID does not fire when a clock-raise is followed by a relationship shift within its window', async () => {
      const recs1028bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { clockRaised: true });
        if (i === 1 || i === 9) return makeSharedRecord(i, { relationshipShifts: [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] });
        return makeSharedRecord(i);
      });
      const res = await runPY1028(recs1028bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_CLOCK_RELATIONAL_AFTERMATH_VOID'), 'PAYOFF_CLOCK_RELATIONAL_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_REVELATION_EMOTIONAL_AFTERMATH_VOID fires when every revelation is followed by two scenes with no emotional shift', async () => {
      const recs1028c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1028(recs1028c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_REVELATION_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_REVELATION_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_REVELATION_EMOTIONAL_AFTERMATH_VOID does not fire when a revelation is followed by an emotional shift within its window', async () => {
      const recs1028cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1028(recs1028cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_REVELATION_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_REVELATION_EMOTIONAL_AFTERMATH_VOID should not fire');
    });
  });

  describe('Wave 1014 — payoffPass: payoff stakes-suspense aftermath void, payoff open-thread-emotional aftermath void, payoff revelation-curiosity aftermath void', async () => {
    const runPY1014 = async (records: ScreenplaySceneRecord[]) => {
      const { payoffPass } = await import('../../server/nvm/revision/passes/payoff.ts');
      return payoffPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // FIRE: aftermath signal placed only at {8,9} — outside both trigger windows {1,2} and {4,5}.
    // NO-FIRE: aftermath at {1,9} — index 1 falls inside trigger 0's window, breaking voidness.
    it('PAYOFF_STAKES_SUSPENSE_AFTERMATH_VOID fires when every stakes-raise is followed by two scenes with no rise in suspense', async () => {
      const recs1014a = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1014(recs1014a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_STAKES_SUSPENSE_AFTERMATH_VOID'), 'PAYOFF_STAKES_SUSPENSE_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_STAKES_SUSPENSE_AFTERMATH_VOID does not fire when a stakes-raise is followed by rising suspense within its window', async () => {
      const recs1014an = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { purpose: 'raise_stakes' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { suspenseDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1014(recs1014an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_STAKES_SUSPENSE_AFTERMATH_VOID'), 'PAYOFF_STAKES_SUSPENSE_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID fires when every heavy clue-debt scene is followed by two scenes with no emotional shift', async () => {
      const recs1014b = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 8 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1014(recs1014b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID does not fire when a heavy clue-debt scene is followed by an emotional shift within its window', async () => {
      const recs1014bn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { unresolvedClues: ['c1', 'c2', 'c3'] });
        if (i === 1 || i === 9) return makeSharedRecord(i, { emotionalShift: 'positive' });
        return makeSharedRecord(i);
      });
      const res = await runPY1014(recs1014bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID'), 'PAYOFF_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID should not fire');
    });

    it('PAYOFF_REVELATION_CURIOSITY_AFTERMATH_VOID fires when every revelation is followed by two scenes with no new curiosity', async () => {
      const recs1014c = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 8 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1014(recs1014c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_REVELATION_CURIOSITY_AFTERMATH_VOID'), 'PAYOFF_REVELATION_CURIOSITY_AFTERMATH_VOID should fire');
    });

    it('PAYOFF_REVELATION_CURIOSITY_AFTERMATH_VOID does not fire when a revelation is followed by new curiosity within its window', async () => {
      const recs1014cn = Array.from({ length: 10 }, (_, i) => {
        if (i === 0 || i === 3) return makeSharedRecord(i, { revelation: 'a hidden truth surfaces' });
        if (i === 1 || i === 9) return makeSharedRecord(i, { curiosityDelta: 1 });
        return makeSharedRecord(i);
      });
      const res = await runPY1014(recs1014cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_REVELATION_CURIOSITY_AFTERMATH_VOID'), 'PAYOFF_REVELATION_CURIOSITY_AFTERMATH_VOID should not fire');
    });
  });
  ...
  describe('Wave 944 — payoffPass: payoff positive emotion zone imbalance, payoff suspense zone imbalance, payoff relationship zone imbalance', async () => {
    ...
    // Zone geometry n=10: Z0={0,1,2}, Z1={3,4}, Z2={5,6,7}, Z3={8,9}. Target at 0,1,2,8,9 →
    // Z0 3/5=60% (bloat), Z1 and Z2 empty → fires. Target at 0,3,5,8 → every zone touched → no-fire.
    it('PAYOFF_POSITIVE_EMOTION_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of positive-shift scenes', async () => {
      const recs944a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2, 8, 9].includes(i) ? 'positive' : 'neutral' }));
      const res = await runPY944(recs944a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_POSITIVE_EMOTION_ZONE_IMBALANCE'), 'PAYOFF_POSITIVE_EMOTION_ZONE_IMBALANCE should fire');
    });

    it('PAYOFF_POSITIVE_EMOTION_ZONE_IMBALANCE does not fire when positive-shift scenes touch every zone', async () => {
      const recs944an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 3, 5, 8].includes(i) ? 'positive' : 'neutral' }));
      const res = await runPY944(recs944an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_POSITIVE_EMOTION_ZONE_IMBALANCE'), 'PAYOFF_POSITIVE_EMOTION_ZONE_IMBALANCE should not fire');
    });

    it('PAYOFF_SUSPENSE_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of suspense-raising scenes', async () => {
      const recs944b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { suspenseDelta: [0, 1, 2, 8, 9].includes(i) ? 1 : 0 }));
      const res = await runPY944(recs944b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_SUSPENSE_ZONE_IMBALANCE'), 'PAYOFF_SUSPENSE_ZONE_IMBALANCE should fire');
    });

    it('PAYOFF_SUSPENSE_ZONE_IMBALANCE does not fire when suspense-raising scenes touch every zone', async () => {
      const recs944bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { suspenseDelta: [0, 3, 5, 8].includes(i) ? 1 : 0 }));
      const res = await runPY944(recs944bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_SUSPENSE_ZONE_IMBALANCE'), 'PAYOFF_SUSPENSE_ZONE_IMBALANCE should not fire');
    });

    it('PAYOFF_RELATIONSHIP_ZONE_IMBALANCE fires when one zone is empty while another holds >=50% of relationship-shift scenes', async () => {
      const recs944c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { relationshipShifts: [0, 1, 2, 8, 9].includes(i) ? [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] : [] }));
      const res = await runPY944(recs944c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_RELATIONSHIP_ZONE_IMBALANCE'), 'PAYOFF_RELATIONSHIP_ZONE_IMBALANCE should fire');
    });

    it('PAYOFF_RELATIONSHIP_ZONE_IMBALANCE does not fire when relationship-shift scenes touch every zone', async () => {
      const recs944cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { relationshipShifts: [0, 3, 5, 8].includes(i) ? [{ pairKey: 'a|b', dimension: 'trust', amount: 1 }] : [] }));
      const res = await runPY944(recs944cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_RELATIONSHIP_ZONE_IMBALANCE'), 'PAYOFF_RELATIONSHIP_ZONE_IMBALANCE should not fire');
    });
  });
  ...
  describe('Wave 860 — payoffPass: payoff climax drought run, payoff establish world drought run, payoff resolution zone cluster', async () => {
    ...
    // PAYOFF_CLIMAX_DROUGHT_RUN fire:
    // n=10; climax at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('PAYOFF_CLIMAX_DROUGHT_RUN fires when a long run has no climax-purposed scene', async () => {
      const recs860a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'climax' : 'complicate' }),
      );
      const res = await runPY860(recs860a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_CLIMAX_DROUGHT_RUN'), 'PAYOFF_CLIMAX_DROUGHT_RUN should fire');
    });

    it('PAYOFF_CLIMAX_DROUGHT_RUN does not fire when climax-purposed scenes are evenly spread', async () => {
      const recs860an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'climax' : 'complicate' }),
      );
      const res = await runPY860(recs860an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_CLIMAX_DROUGHT_RUN'), 'PAYOFF_CLIMAX_DROUGHT_RUN should not fire');
    });

    // PAYOFF_ESTABLISH_WORLD_DROUGHT_RUN fire:
    // n=10; establish_world at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('PAYOFF_ESTABLISH_WORLD_DROUGHT_RUN fires when a long run has no world-establishing scene', async () => {
      const recs860b = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'establish_world' : 'complicate' }),
      );
      const res = await runPY860(recs860b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_ESTABLISH_WORLD_DROUGHT_RUN'), 'PAYOFF_ESTABLISH_WORLD_DROUGHT_RUN should fire');
    });

    it('PAYOFF_ESTABLISH_WORLD_DROUGHT_RUN does not fire when world-establishing scenes are evenly spread', async () => {
      const recs860bn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'establish_world' : 'complicate' }),
      );
      const res = await runPY860(recs860bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_ESTABLISH_WORLD_DROUGHT_RUN'), 'PAYOFF_ESTABLISH_WORLD_DROUGHT_RUN should not fire');
    });

    // PAYOFF_RESOLUTION_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; resolution scenes at 0,1,2 → 100% opening third
    it('PAYOFF_RESOLUTION_ZONE_CLUSTER fires when >75% of resolution-purposed scenes cluster in one third', async () => {
      const recs860c = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'resolution' : 'complicate' }),
      );
      const res = await runPY860(recs860c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_RESOLUTION_ZONE_CLUSTER'), 'PAYOFF_RESOLUTION_ZONE_CLUSTER should fire');
    });

    it('PAYOFF_RESOLUTION_ZONE_CLUSTER does not fire when resolution-purposed scenes spread across thirds', async () => {
      const recs860cn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'resolution' : 'complicate' }),
      );
      const res = await runPY860(recs860cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_RESOLUTION_ZONE_CLUSTER'), 'PAYOFF_RESOLUTION_ZONE_CLUSTER should not fire');
    });
  });

  describe('Wave 846 — payoffPass: payoff positive emotion drought run, payoff establish world zone cluster, payoff climax zone cluster', async () => {
    const runPY846 = async (records: ScreenplaySceneRecord[]) => {
      const { payoffPass } = await import('../../server/nvm/revision/passes/payoff.ts');
      return payoffPass({
        fountain: buildPlainFountain(records.length), original: '', records,
        structure: { escalating: true, avgSuspensePerScene: 0, completionPercent: 50,
    ...
    // PAYOFF_POSITIVE_EMOTION_DROUGHT_RUN fire:
    // n=10; positive-emotion at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('PAYOFF_POSITIVE_EMOTION_DROUGHT_RUN fires when a long run has no positive-emotion charge', async () => {
      const recs846a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 1, 2].includes(i) ? 'positive' : 'neutral' }),
      );
      const res = await runPY846(recs846a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_POSITIVE_EMOTION_DROUGHT_RUN'), 'PAYOFF_POSITIVE_EMOTION_DROUGHT_RUN should fire');
    });

    it('PAYOFF_POSITIVE_EMOTION_DROUGHT_RUN does not fire when positive-emotion scenes are evenly spread', async () => {
      const recs846an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { emotionalShift: [0, 3, 6, 9].includes(i) ? 'positive' : 'neutral' }),
      );
      const res = await runPY846(recs846an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_POSITIVE_EMOTION_DROUGHT_RUN'), 'PAYOFF_POSITIVE_EMOTION_DROUGHT_RUN should not fire');
    });

    // PAYOFF_ESTABLISH_WORLD_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; establish_world scenes at 0,1,2 → 100% opening third
    it('PAYOFF_ESTABLISH_WORLD_ZONE_CLUSTER fires when >75% of world-establishing scenes cluster in one third', async () => {
      const recs846b = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'establish_world' : 'complicate' }),
      );
      const res = await runPY846(recs846b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_ESTABLISH_WORLD_ZONE_CLUSTER'), 'PAYOFF_ESTABLISH_WORLD_ZONE_CLUSTER should fire');
    });

    it('PAYOFF_ESTABLISH_WORLD_ZONE_CLUSTER does not fire when world-establishing scenes spread across thirds', async () => {
      const recs846bn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'establish_world' : 'complicate' }),
      );
      const res = await runPY846(recs846bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_ESTABLISH_WORLD_ZONE_CLUSTER'), 'PAYOFF_ESTABLISH_WORLD_ZONE_CLUSTER should not fire');
    });

    // PAYOFF_CLIMAX_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; climax scenes at 0,1,2 → 100% opening third
    it('PAYOFF_CLIMAX_ZONE_CLUSTER fires when >75% of climax-purposed scenes cluster in one third', async () => {
      const recs846c = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'climax' : 'complicate' }),
      );
      const res = await runPY846(recs846c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_CLIMAX_ZONE_CLUSTER'), 'PAYOFF_CLIMAX_ZONE_CLUSTER should fire');
    });

    it('PAYOFF_CLIMAX_ZONE_CLUSTER does not fire when climax-purposed scenes spread across thirds', async () => {
      const recs846cn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'climax' : 'complicate' }),
      );
      const res = await runPY846(recs846cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_CLIMAX_ZONE_CLUSTER'), 'PAYOFF_CLIMAX_ZONE_CLUSTER should not fire');
    });
  });
  ...
  describe('Wave 818 — payoffPass: payoff character moment drought run, payoff turning point zone cluster, payoff turning point drought run', async () => {
    ...
    // PAYOFF_CHARACTER_MOMENT_DROUGHT_RUN fire:
    // n=10; character_moment at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('PAYOFF_CHARACTER_MOMENT_DROUGHT_RUN fires when a long run has no character moment', async () => {
      const recs818a = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'character_moment' : 'complicate' }),
      );
      const res = await runPY818(recs818a);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_CHARACTER_MOMENT_DROUGHT_RUN'), 'PAYOFF_CHARACTER_MOMENT_DROUGHT_RUN should fire');
    });

    it('PAYOFF_CHARACTER_MOMENT_DROUGHT_RUN does not fire when character moments are evenly spread', async () => {
      const recs818an = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'character_moment' : 'complicate' }),
      );
      const res = await runPY818(recs818an);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_CHARACTER_MOMENT_DROUGHT_RUN'), 'PAYOFF_CHARACTER_MOMENT_DROUGHT_RUN should not fire');
    });

    // PAYOFF_TURNING_POINT_ZONE_CLUSTER fire:
    // n=9; thirds=[0-2],[3-5],[6-8]; turning_point scenes at 0,1,2 → 100% opening third
    it('PAYOFF_TURNING_POINT_ZONE_CLUSTER fires when >75% of turning-point scenes cluster in one third', async () => {
      const recs818b = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'turning_point' : 'complicate' }),
      );
      const res = await runPY818(recs818b);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_TURNING_POINT_ZONE_CLUSTER'), 'PAYOFF_TURNING_POINT_ZONE_CLUSTER should fire');
    });

    it('PAYOFF_TURNING_POINT_ZONE_CLUSTER does not fire when turning-point scenes spread across thirds', async () => {
      const recs818bn = Array.from({ length: 9 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 4, 8].includes(i) ? 'turning_point' : 'complicate' }),
      );
      const res = await runPY818(recs818bn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_TURNING_POINT_ZONE_CLUSTER'), 'PAYOFF_TURNING_POINT_ZONE_CLUSTER should not fire');
    });

    // PAYOFF_TURNING_POINT_DROUGHT_RUN fire:
    // n=10; turning_point at 0,1,2 only, then a run of 7 consecutive scenes (3-9) with none.
    it('PAYOFF_TURNING_POINT_DROUGHT_RUN fires when a long run has no turning point', async () => {
      const recs818c = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 1, 2].includes(i) ? 'turning_point' : 'complicate' }),
      );
      const res = await runPY818(recs818c);
      assert.ok(res.issues.some((i: any) => i.rule === 'PAYOFF_TURNING_POINT_DROUGHT_RUN'), 'PAYOFF_TURNING_POINT_DROUGHT_RUN should fire');
    });

    it('PAYOFF_TURNING_POINT_DROUGHT_RUN does not fire when turning points are evenly spread', async () => {
      const recs818cn = Array.from({ length: 10 }, (_, i) =>
        makeSharedRecord(i, { purpose: [0, 3, 6, 9].includes(i) ? 'turning_point' : 'complicate' }),
      );
      const res = await runPY818(recs818cn);
      assert.ok(!res.issues.some((i: any) => i.rule === 'PAYOFF_TURNING_POINT_DROUGHT_RUN'), 'PAYOFF_TURNING_POINT_DROUGHT_RUN should not fire');
    });
  });
