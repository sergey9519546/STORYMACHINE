...
// doctor-worker.ts's `import('./doctor.ts')` does on every pool thread, and
// what a `tsx server.ts` main thread does). Any named function expression on
// that call path therefore throws `TypeError: __name is not a function`,
// reference.ts's `catch` swallows it into the empty distribution, and every
// report the process ever produces ships with no `percentile`, no
// `percentileDescriptor` and no `healthPercentile` — under tsx only, so the
// test suite and the dev server never see it.
//
// WHERE THIS CAME FROM. The defect was found on 2026-09-21 on the
// feature-length candidate branch (`lane/land-feature-length-defects`), which
// had added `const sig = (x: number) => …` inside `subDensityCurve` — on the
// `computeRawCraftScore` path. `verify:production`'s dev-vs-prod check caught
// it at 70/71 of a browser battery, as `first differing top-level key:
// "dimensions"`; nothing in `npm test` did. See
// `docs/audits/2026-09-21-prod-loader-guard/README.md` for the mechanism, the
// loader evidence, and this test's fail-first probe on THIS tree. This
// branch has no named function expression on that path today; this test is
// what keeps it that way, and it was proven able to fail by reintroducing
// one (see the audit record).
//
...
// take the doctor down. That fallback now logs through server/lib/logger.ts
// (see reference.ts's `settleDistribution`), and this test also asserts the
// production child printed NO such log line — so a swallowed corpus build is
// caught twice: by the absent percentiles and by the presence of the log.
//
// The test spawns the REAL tsx loader as a child — the same `tsx/dist/cli.mjs`
// the Dockerfile CMD resolves — rather than importing the doctor into the
// test process, where the defect is invisible by construction.
...
import { CALIBRATION_UNAVAILABLE_LOG_MSG } from '../../server/nvm/analyze/calibration/reference.ts';
...
describe('calibration under the production loader (tsx)', () => {
  it('builds the reference distribution and reports percentiles on the main thread AND on a pool worker, and the two reports are byte-identical', async () => {
    ...
    // Last, the child's stderr. reference.ts's fallback is loud now
    // (logger.error, to stderr): a production child that printed it
    // swallowed a corpus-build failure in SOME thread — including one the
    // assertions above could not see, such as a worker the pool retired
    // before its report was read. And the raw esbuild symptom, in case a
    // future change moves the throw outside reference.ts's catch.
    assert.ok(
      !stderr.includes(CALIBRATION_UNAVAILABLE_LOG_MSG),
      `reference.ts logged that the calibration corpus build threw in the production child:\n${stderr}`,
    );
