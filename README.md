...
| `SHUTDOWN_DRAIN_MS` | Optional | Delays `server.close()` by this many ms *after* `GET /ready` has already flipped to `503 {reason:"draining"}` on `SIGTERM`/`SIGINT`/an uncaught exception (default `0`: today's timing — `setDraining()` and `server.close()` run in the same tick). **Measured directly, for both probe shapes**: a caller reusing an already-open, keep-alive connection sees the `503` regardless of this setting, the same tick draining begins. A prober that opens a **fresh connection per check** — most load balancer/orchestrator healthchecks, and this repo's own `wget`-based Dockerfile/docker-compose healthcheck — does **not**: with the default `0`, a fresh connection attempted ~6ms after the signal was measured getting `ECONNREFUSED`, never seeing the `503` at all, because `server.close()` had already stopped accepting by then. Set this above your healthcheck's poll interval to fix that for a fresh-connection prober specifically (docker-compose.yml sets `35000`, comfortably past its own 30s `interval`); a keep-alive prober gets no benefit from raising it. The 10s hard-kill timer for in-flight requests now counts from when `close()` actually starts (i.e. after this delay), not from the original signal. **Constraint that must hold regardless of the above**: this value plus that 10s hard-kill must sit BELOW your orchestrator's own termination grace period (`docker stop`/`docker compose stop`/`down` default to 10s via `stop_grace_period`; Kubernetes' `terminationGracePeriodSeconds` defaults to 30s) — the runtime SIGKILLs the process at ITS deadline regardless of what this app is still waiting on, so a `SHUTDOWN_DRAIN_MS` set too high never lets `server.close()`'s callback run at all: the SQLite WAL flush is skipped, in-flight requests are cut mid-response, and the exit code is always 137, not this app's own 0/1. See docker-compose.yml's own `stop_grace_period` (raised alongside `SHUTDOWN_DRAIN_MS` there, with the same reasoning in its comment) for a worked example. |
