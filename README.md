...
| `DOCTOR_QUEUE_ADMISSION` | Optional | Whether the queue budget above is also enforced at **submission** (default on; `off` or `0` reverts to wait-then-shed). With it on, a submission the pool already cannot serve inside `DOCTOR_QUEUE_BUDGET_MS` is refused immediately — same `503`, same `Retry-After`, same sentence — instead of sitting in the queue for the full budget first. Measured on a saturated 2-worker pool: the first `503` of an incoming wave arrived at **60,361 ms** with this off and **1,055 ms** with it on, with no measurable change in how many submissions were served. The check is deliberately biased toward admitting (it excludes the job's own execution from the estimated wait, and requires the estimate to exceed the budget by 1.5x — the measured over-statement of the estimator at the shed boundary), so a submission near the line still goes through and still gets the timer's answer later. Turn it off if you would rather every submission wait its turn. |
