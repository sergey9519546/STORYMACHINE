...
| `DOCTOR_ANALYSIS_BUDGET_MS` | Optional | Per-analysis wall-clock budget for the Script Doctor (default `30000`; `0` or `off` disables it). One analysis that runs longer than this is stopped the same way the panel's Cancel stops one — the worker thread running it is terminated — and the writer is told so: JSON routes answer `400 {"error": "This draft took longer to analyze than this server's per-analysis budget (30s), so the run was stopped and nothing was scored. Try again, or split the draft into shorter files and analyze them separately."}`, and `POST /api/scriptide/doctor/stream` sends the same sentence in a `doctor_error` frame. The default is derived from the measured accepted worst case (~14 s for a 400-scene draft with a large cast) with 2× headroom, and sits below the panel's own 120 s watchdog so the writer sees this sentence rather than a generic timeout — see `docs/DECISION_LOG.md` Decision #7 for the full derivation and `server/lib/doctor-budget.ts` for the mechanism. **Applies to the worker path only**: with `DOCTOR_WORKER_POOL=off`, on a host that cannot spawn workers, or on the deep-read route, the analysis runs in-process where there is nothing to terminate — the same carve-out Cancel already has. Raise it only with a measurement of your own; lowering it below ~15 s starts to reach legitimate feature-length drafts. |
