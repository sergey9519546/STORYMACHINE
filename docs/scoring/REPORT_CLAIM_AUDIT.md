...
| 4.6 | "The clock isn't set once and forgotten…" (stakes continuity) | `buildStakesContinuityStrength`, `doctor.ts:894-910`; guard at `1379-1380` | **substantiated** (with corpus evidence) | Checks `clockRaised` appears in both halves (≥6 scenes). Corpus-verified: 2/5 strong, 0/15 others (`doctor.ts:888-893`). | None — the guard is well-evidenced and the wording names the measured fact (specific scene numbers). |
| 4.7 | Relationship dynamism / emotional range / scene-purpose variety / suspense shaping / cold-open / climax placement / acceleration / dramatic-turn density strengths | `doctor.ts:943-1412` (eight more guards), aggregated `1382-1412` | **substantiated** (individually) | Each is a separate thresholded guard with its own corpus-evidence comment. None is padded. | Per-guard audit would lengthen this ledger without changing the pattern: each is one measured signal phrased as a craft strength. Same imprecise-as-authority caveat as 4.1 applies uniformly. |
