...
9. **Line anchors in prose had to be updated three times**, because doctor.ts
...
* **`compiled.fountain = normalizeScreenplay(fountain)` on every path.** The
  stronger version of the pipeline fix. On a double-spaced import the normalizer
  also uppercases cues and reflows action paragraphs, and the documents taking
  that branch are exactly the scraped PDFs of the private corpus — a scoring
  change whose entire blast radius is invisible from this tree. Recorded as
