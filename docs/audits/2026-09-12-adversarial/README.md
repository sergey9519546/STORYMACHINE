...
| `smoke-gate-boot` — the smoke gate drives the built bundle (dist/ rebuilt when any audited build input is newer; the served mode read back off the wire and a mismatch fatal; every boot in every suite names what it serves); five docs that said the opposite corrected; the verdict-poll scanner follows a hoisted read and a local call, and caught a real site on main | p0-flow lane §5; writer review round-3 scanner escape | REVISE (`public/` missing from the build-input set) → MERGE | 9d0b832d (main) |
...
| `scoring/forced-cue` — Fountain's `@` cue honoured at the parser seam and in every renderer (32/32 → 0/32); a ninth shape-guard bypass class the parser change itself opened, closed | scoring review round 2, non-blocking 1 | READY-FOR-OWNER → READY-FOR-OWNER (round 2, no further items) | 089bec91 (stacked on 4cf5b2f3; owner-gated) |
| `scoring/renderer-residuals` — `>` forced transitions typed at the parser seam and right-aligned by every renderer; `>text<` centering a structural line at the analysis seam; one case rule across layout/PDF/DOCX with FDX preserving the writer's bytes; the PDF importer's false round-trip claim fixed; a first suite for `canonical-fountain.ts`; the `@`-out-of-position hypothesis rejected from the spec and pinned; the probe's `>tr` column | forced-cue §6.7 and its review | REVISE 6 (centering absorbed at the seam; centered text lost through FDX; mixed-case transitions printed two ways; pdf-import's identical false claim; an untested widening; a receipt file count) → MERGE | 56b96765 (stacked on 089bec91; owner-gated, never merged here) |
| `vite-cache-isolation` — one Vite dependency-optimizer cache per worktree and per boot (slot lock, released after the child's death and on SIGINT/SIGTERM/SIGHUP, pid-liveness reclaim); a two-server harness showing main's config rewriting the other server's live cache (+46/−44) and this tree +0/−0; the Docker build context and a context test that derives its required paths from the config's imports; eight browser suites / seven dev-middleware boot sites, count asserted | p0-flow lane (504 Outdated Optimize Dep), smoke-gate lane §5 | REVISE 5 (the Docker builder stage could not build the tree; the slot released on signal not death; the exit hook silent on SIGTERM; a self-comparing assertion; the lock untested) → MERGE | ce093a0c (main) |
| `vitecache-notes` — the three non-blocking notes of the vite-cache review: the production-build suite's inline teardown routed through `shutdown()` so its slot is released for concurrent gates; `distStaleness()` counts `vite.config.ts`'s build-time imports through one shared walker; the signal handler re-installs per signal, with the listener count pinned against a mutant; the caller and call-site counts derived, not typed | vitecache review round 2 (a)–(c) | REVISE 4 (a rationale invented beside a correct fix; SIGKILL escalation dropped silently; "eight call sites" was eleven; an unpinned listener claim) → MERGE | ef529c05 (main) |
| `owner-measure` (2026-09-13, `docs/audits/2026-09-13-owner-measure/`) — `npm run owner:measure`: the owner's corpus measurement as one command — plan read from a committed record a test checks against the owner note, pre-flight refusals, probe on branch and base, `measure-real` per branch in a detached worktree, the three-scan receipt conversion verified with the gate's own exports, accept/reject between the first branch and the stack, in-place manifest re-lock, `lock-auc24` last; a no-corpus end-to-end mode on the 32 public scripts drives every path in CI; the two AUC-24 recipes named beside their numbers and declared not comparable | the owner's blocking P1 step (seven documents of procedure) | REVISE 10 (the measure-real number mislabelled as the v2 recipe; an unconditional probe claim in the receipt; a report gate treated as accepted; a converted entry contradicting itself; public mode able to overwrite the real manifest; a NUL byte; five smaller) → MERGE | 7b05a943 (main) |
