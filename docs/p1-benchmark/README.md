...
**None of the numbers in that table runs in CI**, and neither does the AUC-24
ratchet: the corpus is local-only for copyright reasons. Since 2026-09-06 one
discrimination measurement DOES run on every CI run, on the 32 screenplays
this repository can legally redistribute — `PUBLIC_BENCHMARK_2026-09-06.md`,
`npm run benchmark:public`, asserted by `tests/core/public-benchmark.test.ts`.
It is a **different, smaller** statistic (32 scripts at 9–14 scenes; the
feature-scale deductions never fire) and both of its AUCs land near chance
(0.5586 and 0.4673, both 95% intervals containing 0.5). Read it as a ratchet
that makes a regression visible, never as a substitute for the table above.
...
| `READINESS_ASSESSMENT_2026-07-28.md` | Pre-dates the work above. Originally concluded P1 was blocked on P0; its status header was updated 2026-08-11 to reflect the P0 hard-gate retirement (Decision #2). Read it for the still-accurate inventory of what's pre-buildable vs. genuinely corpus-gated. |
| `BLIND_PAIRS_2026-09-04.md` | Six matched excellent/bad pairs written blind to the engine, scored against the calibration corpus: the score orders 1 of 6 blind pairs vs 5 of 5 calibration pairs, the rule channel is at chance on both, and nine of twelve blind scripts tie at one health value because the density penalty saturates. |
| `BLIND_PAIRS_ON_BRANCHES_2026-09-04.md` | The same twelve fixtures scored on the two pending scoring branches (R5 verbosity-bias, advice-rule-fixes). R5 un-pins the tie but only exposes raw weighted-issue order (3 of 6); advice-rule-fixes changes nothing; the branches conflict on five files, so the stack could not be scored. Corrects the owner merge-order guidance. |
| `PUBLIC_BENCHMARK_2026-09-06.md` | The first discrimination measurement that runs in CI: 32 distributable screenplays (20 CC0 + 12 blind-pair fixtures), two degradations (shuffle-drop, and a scene-count-preserving climax-relocate), AUCs 0.5586 and 0.4673 with seeded bootstrap CIs, a pre-registered sha256 split, a 32-row manifest lock, floors in `scripts/lib/auc.ts`, and the same harness run unmerged on the three pushed scoring branches. Also the measurement that refuted the scene-count-artifact prediction: scarcity rises +5.693 while density falls −7.632, so degradation RAISES health here. |
