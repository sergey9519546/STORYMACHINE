...
| `PUBLIC_BENCHMARK_2026-09-06.md` | The first discrimination measurement that runs in CI: 32 distributable screenplays (20 CC0 + 12 blind-pair fixtures), two degradations (shuffle-drop, and a scene-count-preserving climax-relocate), AUCs 0.5586 and 0.4673 with seeded bootstrap CIs, a pre-registered sha256 split, a 32-row manifest lock, floors in `scripts/lib/auc.ts`, and the same harness run unmerged on the three pushed scoring branches. Also the measurement that refuted the scene-count-artifact prediction: scarcity rises +5.693 while density falls −7.625, so degradation RAISES health here. |
