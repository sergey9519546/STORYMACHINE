...
| 12 | *(round 3)* the shape guard learns the forced cue — ROUND 9 of the bypass family | Teaching the parser `@` opened the ninth instance of `server/lib/validation.ts`'s documented bypass pattern, and the first to arrive with a parser change rather than with a review: all three of `isCueLikeLine`'s disjuncts start at a cased-script capital, which is exactly what the marker escapes. On the round-5/6 payload (distinct=600, occurrences=12,000, cue adjacent to its speech): `guardCueOccurrences` **12,000 → 0** while the pipeline still made **12,000** character blocks, so the round-8 oracle went **TRUE → FALSE** and the payload reached `MAX_FOUNTAIN_VOICE_ELIGIBLE_WEIGHT`, which runs a real parse, instead of the cheap pre-parse bound. Fixed by a fourth, wider disjunct: guard **12,000**, oracle TRUE, rejected by `MAX_FOUNTAIN_FREQUENT_CUE_LINES` again. Caught by this repository's own suite on the lane's first full `npm test`. No scoring-path file is involved and no number in row 11 moved: benchmark unchanged to the digit, 45 of 45 output-identity fixtures byte-identical. | `docs/audits/2026-09-12-adversarial/forcedcue-lane-report.md` §5b |
