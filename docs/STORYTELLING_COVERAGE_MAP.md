...
≈ 0.938 (`doctor.ts:2544-2545`, `docs/p1-benchmark/DISCRIMINATION_BASELINE_2026-07-29.md`).
...
| Chekhov's gun / setup-payoff | **COVERED** (lexicon layer, both directions) + **ABSENT** (typed layer, built but unwired) | `CHEKHOV_GUN_UNFIRED` in `server/nvm/revision/passes/causality.ts` (~line 936, content-word clue seeded in first half with no later payoff) — the forward direction (planted, never fired). `INVERSE_CHEKHOV_GUN` (same file, added 2026-08-07 in response to the pilot session's Chet's-blade miss — see `pilot-session-2026-08-07/PILOT_SESSION_REPORT.md` §5) covers the reverse direction: a concrete weapon/tool/device that first appears in the story's peak-intensity scene and is used to resolve it, with no prior mention anywhere earlier — a payoff with no setup. Deliberately conservative (narrow instrument lexicon, same-sentence instrumental-use-verb requirement, cross-document absence check, climax-zone gating) — measured at 0 fires across the 71/73 real-corpus scripts it could evaluate before shipping (2 scripts excluded for pipeline runtime unrelated to this rule; see `docs/p1-benchmark/MEASUREMENT_RECEIPTS.md`'s 2026-08-07 entry). Separately, `server/nvm/analyze/typed-promises.ts` implements a much more rigorous **typed** ledger (`chekhov_object`/`stated_goal`/`prophecy`/`threat`/`mystery_question`, strict-later-scene-index discipline) | The lexicon rules fire and route through the ordinary density-normalized issue channel (the one measured at AUC ≈0.076). `typed-promises.ts` is stricter and genuinely order-sensitive but is consumed only by `integrity-rate.ts`, which is itself consumed only by its own test — fully orphaned. It needs a text→typed-event extractor that doesn't exist for arbitrary scripts. |
