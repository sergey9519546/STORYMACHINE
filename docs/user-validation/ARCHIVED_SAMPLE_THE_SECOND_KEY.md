# Archived P0 sample — "The Second Key" (retired 2026-08-04)

**Status: retired reference, not live.** This was the built-in "Try a sample
script" stimulus in `src/lib/sample-script.ts` from P0's inception through
2026-08-04. It was replaced by `data/screenplays/dead-frequency.fountain`
("Dead Frequency") — see the dated addendum in `FIELDING_DECISION_BRIEF.md`
and the decision-log entry in `PHASE_TRACKER.md` for the full rationale
(corpus-density thinness: ~47.5 words/scene against a real-corpus median of
~161–181; documented as defect D5 in `docs/p1-benchmark/DETECTOR_DEFECTS_2026-08-03.md`).

This file exists per the maintainer's standing rule — never blindly remove,
keep-as-reference is the default — and is kept verbatim, including its
original design-intent commentary, exactly as it read in `sample-script.ts`
immediately before the swap. It is not imported anywhere and has no effect
on the running app; it is historical record only.

## Original module header (verbatim)
...
## Original stimulus facts (last measured before retirement, HEAD at time of
retirement: commit `0cf12c9`)

- Title: "The Second Key"
- 665 words / 14 scenes ≈ 47.5 words/scene
- health 68.9, verdict CONSIDER, sceneCount 14
- contentHash `33dcf214…` (see git history of `src/lib/sample-script.ts`
  prior to the 2026-08-04 swap for the full value)

## Original fountain text (verbatim)

```fountain
INT. PAWNSHOP BACK ROOM - NIGHT
