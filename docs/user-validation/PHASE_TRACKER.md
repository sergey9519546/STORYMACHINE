...
*(Correction, 2026-09-19: the "contains no session files" claim above was
itself false from 2026-08-12 to 2026-09-19 — five harness-generated
records, `P0-S01.md` through `P0-S05.md`, were tracked in
`docs/user-validation/sessions/` during that window. See the 2026-09-19
decision-log row below for the full incident; the counters in this table
were never inflated by those files and remain accurate throughout.)*
...
| 2026-09-19 | P0 | **Fabricated session records removed (records-truth repair; no real session data ever existed to change).** Five tracked files — `docs/user-validation/sessions/P0-S01.md` through `P0-S05.md` — were discovered to be harness-generated output (moderator `research-harness-v1`, observer `automated-async-logger`, session mode "remote / async research portal") carrying invented writer quotes and STRONG_PULL-style verdicts, with an "Application commit SHA" field that was actually the sample report's `contentHash`, not a commit. They entered `main` via merge commit `5a125054` (2026-08-12), present in neither of that merge's two parents, four days after `docs/superpowers/plans/2026-08-08-main-consolidation.md:74` recorded the explicit decision "Do not add ignored P0-S01 through P0-S05 harness records to Git; leave them uncounted and document why," and despite `docs/audits/2026-08-08-main-consolidation.md` stating on the same date that they "were not committed" (see the correction note added to that audit on this date). They were produced by the now-superseded `docs/user-validation/P0_WORKAROUND_HARNESS.md` async harness, were never referenced by any test or code path, and were removed with `git rm` on 2026-09-19 (recoverable, for the record, at `5a125054:docs/user-validation/sessions/P0-S01.md` etc.). **No counter in this file or in `P0_EVIDENCE_SUMMARY.md` was ever inflated by these files** — every counter and session table read 0 / "No sessions documented" both before and after this removal; this is a repository-hygiene correction, not a change to the P0 evidence state. A tripwire test (`tests/core/p0-session-records.test.ts`) now blocks recurrence of this class of file. | This entry; `docs/audits/2026-08-08-main-consolidation.md` (correction note); `docs/superpowers/plans/2026-08-08-main-consolidation.md:74`; `tests/core/p0-session-records.test.ts` | Agent session (records-truth repair, lane `p0-records-truth`) | Session counters remain **0 of >=5** after this removal, unchanged from before it. |
...
| Session artifact directory | `docs/user-validation/sessions/` — empty (`.gitkeep` only) *(Correction, 2026-09-19: true again as of this date; five harness-generated records were tracked here from 2026-08-12 to 2026-09-19 — see the 2026-09-19 decision-log row above.)* |
