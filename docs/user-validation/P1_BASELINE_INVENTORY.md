...
3. **Produced health range:** test comment says measured `88.9–98.3` (`tests/core/real-script-corpus.test.ts:40-44`), while committed manifest includes values below 88.9 and up to 98.9 (for examples, the entry at `87.3`, lines 99–104, and the entry at `98.9`, lines 315–320 — titles redacted per corpus de-identification, see `docs/p1-benchmark/CORPUS_IDENTIFICATION.md`; locate them by manifest line number or the private crosswalk). Treat the comment as stale.
