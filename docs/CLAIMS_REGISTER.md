...
| 41 | Shape & rhythm (descriptive, not part of the score): Talk/action swing … → … / Action-prose variation … → … | src/components/scriptide/ScriptDoctorPanel.tsx (FixReceiptCard structural-signals delta, 2026-09-04) | measured-in-repo | tests/core/shape-rhythm-panel-copy.test.ts; tests/routes/scriptide-fix.test.ts. Reachability note (audit, 2026-09-04 evening): the field is gated on `usedLLM`, so on the keyless deploy the browser battery certifies this copy cannot render; a lane is adding the deterministic writer-supplied-candidate path. | supported (qualified) |
