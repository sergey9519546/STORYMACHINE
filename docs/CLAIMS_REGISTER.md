...
| 54 | top … of a 20-sample, hand-authored synthetic reference set | src/components/scriptide/SnapshotManager.tsx (`SnapshotPercentileAndRankLine`, compact form of row 49's sentence via `compactPercentileNote()`, src/lib/percentile-copy.ts — 2026-09-04 review fix: the first version of this line had silently dropped "hand-authored synthetic") | measured-in-repo | tests/core/percentile-copy-consistency.test.ts (the "compactPercentileNote() ALSO carries the hand-authored synthetic qualifier" and "SnapshotManager.tsx renders the compact note via the shared compactPercentileNote()" tests) | supported |
