...
| 46 | The Script Doctor re-reads both drafts and shows exactly what moved — no AI, no key needed. | src/components/scriptide/ScriptDoctorPanel.tsx (Verify my rewrite, 2026-09-04) | measured-in-repo | tests/routes/scriptide-fix.test.ts ("writer-supplied candidate: 200 with the full receipt, usedLLM:false, source:\"writer\"" — runs against a keyless test server with a provider that throws if called); scripts/verify-p2-p3-surfaces.mjs (P2-generative, keyless server, Labs OFF) | supported |
| 47 | Measured by the Script Doctor. No AI was used. | src/components/scriptide/ScriptDoctorPanel.tsx (FixReceiptCard, writer-supplied candidate, 2026-09-04) | measured-in-repo | tests/routes/scriptide-fix.test.ts (the writer path is asserted to answer 200 while the configured provider throws on any call); server/routes/scriptide.ts (the candidateFountain branch returns before fixAndVerify is imported) | supported |
