 ...
 *
 * CALLED WITH NO RESULT (2026-09-20 merge) it renders the SAME caveats with
 * every live figure replaced by the sentence that says no measurement was run.
 * That is what `PUBLIC_BENCHMARK_LIMITS` below is, and what
 * `npm run benchmark:public -- --limits` prints in ~0.3 s instead of 6 s —
 * `main`'s tests/core/public-benchmark-limits.test.ts greps those real bytes
 * for finding 6's corrected sentence. One renderer, so the cheap flag cannot
 * drift away from what a full run prints, and no frozen number anywhere.
 */
export function publicBenchmarkLimits(result?: BenchmarkResult): string {
  ...
  const pinned = result === undefined ? 0 : pinnedScriptCount(result);
  const blind = result === undefined
    ? { ordered: 0, of: 0, meanGap: NaN }
    : blindPairOrdering(result);
  ...
  return [
    ...
    ...(result === undefined
      ? [
        '    control: the score catches it on a majority of the corpus through a channel neither',
        '    measurement degradation touches. So a near-chance reading on the other two channels is',
      ]
      : [
        `    control: the score catches it on ${control?.ordered ?? 0} of ${control?.n ?? 0} scripts with ${control?.tied ?? 0} ties (matched-pair AUC`,
        `    ${f4(control?.aucPaired)}, ${(control?.meanGap ?? 0).toFixed(2)} points). So a near-chance reading on the other two channels is`,
      ]),
    '    the SCORE being blind, not the instrument being broken — the one hypothesis a benchmark',
    '    with only null readings can never rule out.',
    '',
    ...(result === undefined
      ? [
        'WHAT IT SAYS TODAY',
        '  * This invocation measured nothing, so it quotes no figure. Run',
        '    `npm run benchmark:public` for this tree\'s shuffle-drop, climax-relocate and control',
        '    AUCs, their bootstrap intervals, and whether each interval still contains 0.5.',
      ]
      : [
    ...
    ...(result === undefined
      ? [
        '  * A moving reading on every CLIMAX_RELOCATE pair. Scripts pinned at exactly health',
        `    ${PINNED_HEALTH.toFixed(1)} (density penalty at its 10-point cap plus a 14.0 scarcity term) make EXACT ties,`,
        '    which contribute 0.5 apiece by construction, so a narrow interval on that channel can',
        '    reflect pinning rather than precision. Do not read it as the more precise of the two.',
      ]
      : [
    ...
    ...(result === undefined
      ? ['    (tests/core/blind-pairs-discrimination.test.ts) is the']
    ...
    '    some length. It fires at no length.) Since 2026-09-11 the scarcity term also saturates at',
    '    12 scenes, which on THIS corpus still moves (every document is 6-14 scenes) and on a',
    '    feature-length corpus contributes exactly zero to shuffle-drop.',
...
/**
 * The runless rendering of the caveats above — the same renderer, called with
 * no measurement. `scripts/benchmark-public.ts --limits` prints exactly these
 * bytes and tests/core/public-benchmark-limits.test.ts greps them, so the
 * claim that is printed is the claim that is checked.
 */
export const PUBLIC_BENCHMARK_LIMITS = publicBenchmarkLimits();
