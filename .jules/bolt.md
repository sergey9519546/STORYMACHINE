...
## 2024-06-05 - Optimize ScriptIDE Keystroke Rendering
**Learning:** High-frequency rendering paths in React, especially text editors where users are typing constantly, suffer severely from garbage collection spikes caused by string array allocations (`.split()`) and repeated `.substring()` calls that create unnecessary intermediate string instances.
**Action:** Replaced `.split(/\s+/)` with a zero-allocation `charCodeAt(i) > 32` loop for word counting. Replaced `.substring()` with `.slice()` and used `lastIndexOf` or `indexOf` to avoid `.split('\n')` inside keystroke handling. This minimizes string allocations per keystroke, preventing jank in the main thread.
