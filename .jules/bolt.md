...
## 2024-06-24 - [Zero-Allocation String Operations in React]
**Learning:** Relying on `split` to traverse or inspect text content in high-frequency events (like keystrokes in ScriptIDE) triggers excessive garbage collection, even for simple tasks like finding the current line or counting words.
**Action:** Always replace `.split()` with zero-allocation alternatives such as `RegExp.exec` loops or direct index checks using `indexOf` and `lastIndexOf` in hot paths.
