...
## 2024-06-23 - Avoid String.prototype.split in High-Frequency React Paths
**Learning:** Using `.split()` inside React render methods or keystroke handlers creates significant garbage collection spikes via intermediate array allocation, causing frame stuttering. This is an anti-pattern for text editors.
**Action:** Always prefer zero-allocation string methods like `.lastIndexOf`, `.slice()`, and regex execution loops (`/.exec()/`) when extracting substrings or counting elements.
