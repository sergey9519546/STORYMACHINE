...
## 2026-06-27 - [Zero-Allocation String Parsing]
**Learning:** Found O(N^2) memory scaling caused by .split('
') array allocations in high-frequency React render paths like keystroke handlers in ScriptIDE.tsx.
**Action:** Replaced String.prototype.split() with zero-allocation character code loops and string index methods (indexOf, lastIndexOf) and regex execution loops (/\S+/g.exec) to prevent frame stuttering on the main thread.
