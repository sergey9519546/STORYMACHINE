...
## 2024-05-20 - [Memory-Optimized Word Count]
**Learning:** Using .split() for word counting creates large intermediate arrays and causes GC spikes.
**Action:** Use (string.match(/\S+/g) || []).length instead for high-frequency React components to prevent double-allocations and avoid `.trim()` edge cases.
