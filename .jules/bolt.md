...
## 2024-05-24 - Zero-Allocation string navigation and analysis
**Learning:** High-frequency rendering paths that trigger on every keystroke (like `ScriptIDE.tsx`) shouldn't use `String.prototype.split('\n')` or `String.prototype.split(/\s+/)` to calculate character offsets, extract substrings, or count words. They create huge arrays that stress the garbage collector and cause frame stutter on the main thread.
**Action:** Use zero-allocation methods: `RegExp.exec()` in a while loop for word counting (handles unicode whitespace better than `charCodeAt`), and string index loops (`indexOf('\n')`, `lastIndexOf('\n')`) combined with `.slice()` for fast line extraction.
