...
## 2024-05-24 - [String Optimization in ScriptIDE]
**Learning:** Replaced `String.prototype.split()` calls with zero-allocation index loops (`indexOf`, `lastIndexOf`) and RegEx execution (`/\S+/g.exec()`), and replaced `String.prototype.substring()` calls with `.slice()` in high-frequency React render paths.
**Action:** Always prefer zero-allocation character code loops and string index methods for string analysis in high-frequency paths to avoid garbage collection spikes. Use `.slice()` over `.substring()` to prevent unexpected argument swapping.
