...
## 2024-07-22 - [Zero-Allocation Highlighting]
**Learning:** Avoid redundant `.split()`. When parsing flat line-based formats like Fountain, blocks natively correspond to single lines. Calling `.split("\\n")` on single lines during rapid keystroke renders creates O(N) garbage collection thrash.
**Action:** Always verify if a string property intrinsically represents a single line before blindly splitting it in high-frequency rendering paths.
