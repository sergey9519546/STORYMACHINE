...
## 2024-08-01 - [Fast word count without arrays]
**Learning:** High-frequency `.split(/\s+/).filter(Boolean).length` usage throughout analysis passes generates substantial GC pressure for simply counting words.
**Action:** Replace this pattern with `fastWordCount(str)` which loops without intermediate array allocation.
