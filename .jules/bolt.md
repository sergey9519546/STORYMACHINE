...
## 2026-06-18 - [Zero-Allocation String Parsing]
**Learning:** Found an unexpected memory scaling issue caused by `String.prototype.split()` being used in high-frequency React render paths (such as keystroke updates in `ScriptIDE`) for string analysis like word counting or newline tracking, causing garbage collection spikes.
**Action:** Replaced `.split()` with zero-allocation character code loops and string index methods like `indexOf` and `lastIndexOf`.
