...
## 2024-05-27 - [ScriptIDE String Split Optimization]
**Learning:** `String.prototype.split('\n')` and `/\s+/` allocations within high-frequency UI render loops (like React keystroke updates) cause O(N^2) memory scaling, garbage collection spikes, and subsequent input lag, especially on large script texts.
**Action:** Replace array-generating `.split()` calls with zero-allocation `charCodeAt` character code loops or direct iterations over pre-parsed structures (like `FountainBlock.text`) wherever text processing is invoked per keystroke.
