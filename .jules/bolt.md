...
## 2024-06-11 - [ScriptIDE Text Split Overhead]
**Learning:** High frequency React components (like the code editor in ScriptIDE handling keystrokes) suffer severe GC spikes if they repeatedly allocate array chunks using `.split('\n')` or `/\s+/` regex logic on large text blocks.
**Action:** Always replace these allocations with zero-allocation character index scanning algorithms using `.charCodeAt(i) > 32` or string manipulation primitives `.indexOf()` and `.lastIndexOf()` with `.slice()` for React re-render hotpaths.
