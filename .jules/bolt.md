...
## 2024-03-12 - ScriptIDE Performance Optimization
**Learning:** Avoid using `String.prototype.split('\n')` in high-frequency React render paths like keystroke updates, as it creates large intermediate arrays and causes garbage collection spikes. Use zero-allocation methods like `indexOf` and `slice` instead.
**Action:** Implemented zero-allocation `while`-loops with `.indexOf('\n')` and `.slice()` to extract substrings in `ScriptIDE.tsx` text processing, navigation, and rendering to improve UI performance and reduce frame stuttering on the main thread.
