...
## 2024-06-12 - [ScriptIDE] Avoid `String.prototype.split()` for zero-allocation performance
**Learning:** In high-frequency React render paths like the editor update cycle in `ScriptIDE.tsx`, using `.split("\n")` to count words or navigate lines allocates many intermediate arrays, triggering severe garbage collection spikes.
**Action:** Replaced `.split()` with zero-allocation index methods (`.indexOf`, `.lastIndexOf`, `.slice()`) and RegEx looping (`/[^\s]+/g.exec()`) for text measurements to optimize text manipulation.
