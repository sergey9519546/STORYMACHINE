...
// Wave 1037 additions: ARC_STAKES_EMOTIONAL_AFTERMATH_VOID gives raise_stakes a fourth channel
// (previously paired with relationshipShifts/curiosityDelta/suspenseDelta, now paired with
// emotionalShift), ARC_REVELATION_SUSPENSE_AFTERMATH_VOID gives revelation a fourth channel
// (previously paired with relationshipShifts/emotionalShift/curiosityDelta, now paired with
...
// Wave 1135 additions: re-verifying against actual rule: names (not just a channel count),
// suspenseDelta-as-trigger was actually at five of the five non-self standard channels
// (curiosityDelta/relationshipShifts/emotionalShift/dialogueHighlights/visualBeats) after Wave
// 1121, not six as that wave's header stated — ARC_SUSPENSE_RECURRENCE_AFTERMATH_VOID adds the
// self-referential sixth channel (another suspenseDelta rise elsewhere), following the same
// self-pairing convention used for relationshipShifts-as-trigger triggers in other pass files.
// dramaticTurn-as-trigger was at four (emotionalShift/relationshipShifts/curiosityDelta/
// suspenseDelta) — ARC_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID and ARC_TURN_STAGING_AFTERMATH_
// VOID give it its fifth and sixth channels (dialogueHighlights, visualBeats), completing full
// six-channel saturation for this trigger.
// Wave 1149 additions: with all eight main triggers (dramaticTurn, clockRaised, raise_stakes,
// payoffSetupIds, seededClueIds, revelation, unresolvedClues-debt, suspenseDelta) now fully
// saturated at six channels each, this wave advances the two secondary emotion-valence
// triggers (emotionalShift === 'positive'/'negative'), each of which stood at only one of six
// channels (ARC_POSITIVE_RELATIONAL_AFTERMATH_VOID, ARC_NEGATIVE_RELATIONAL_AFTERMATH_VOID,
// both Wave 477/547 — relational channel only). ARC_POSITIVE_CURIOSITY_AFTERMATH_VOID and
...
export async function characterArcPass(input: PassInput): Promise<PassResult> {
  ...
  // ARC_STAKES_EMOTIONAL_AFTERMATH_VOID — Sequence/aftermath × raise_stakes trigger →
  // emotionalShift absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  ...
  // while such shifts occur elsewhere. Distinct from ARC_STAKES_RELATIONAL_AFTERMATH_VOID, ARC_
  // STAKES_CURIOSITY_AFTERMATH_VOID, and ARC_STAKES_SUSPENSE_AFTERMATH_VOID (same trigger paired
  // with relationshipShifts/curiosityDelta/suspenseDelta respectively) — this is the fourth
  // consequence channel for this trigger in this pass.
  {
    const r1037a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.purpose === 'raise_stakes',
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1037a.fires) {
      issues.push({
        location: `${r1037a.triggerCount} raise-stakes aftermath(s) — no emotional shift within 2 scenes`,
        rule: 'ARC_STAKES_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every stakes-raising scene in the story (${r1037a.triggerCount} of them) is followed by two emotionally neutral scenes, even though ${r1037a.aftermathCount} emotionally-charged scenes exist elsewhere. A stakes-raise that isn't matched by any feeling in the scenes right after it leaves the arc's escalation registering as a declared beat rather than something the protagonist visibly feels the weight of.`,
        suggestedFix: `In the two scenes following at least one stakes-raise, let the protagonist's feelings visibly register the new danger so the escalation lands emotionally, not only structurally.`,
      });
    }
  }

  // ARC_REVELATION_SUSPENSE_AFTERMATH_VOID — Sequence/aftermath × revelation trigger →
  ...
  // Fires when every revelation's two-scene aftermath carries no suspense rise, while such rises
  // occur elsewhere. Distinct from ARC_REVELATION_RELATIONAL_AFTERMATH_VOID, ARC_REVELATION_
  // EMOTIONAL_AFTERMATH_VOID, and ARC_REVELATION_CURIOSITY_AFTERMATH_VOID (same trigger paired
  // with relationshipShifts/emotionalShift/curiosityDelta respectively) — this is the fourth
  // consequence channel for this trigger.
  {
    const r1037b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.revelation != null,
      isAftermath: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r1037b.fires) {
      issues.push({
        location: `${r1037b.triggerCount} revelation aftermath(s) — no suspense rise within 2 scenes`,
        rule: 'ARC_REVELATION_SUSPENSE_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every revelation in the story (${r1037b.triggerCount} discoveries) is followed by two scenes with no rise in suspense, even though ${r1037b.aftermathCount} such rises occur elsewhere. A discovery that never tightens the felt sense of danger right after it leaves the arc's revelations landing as flat information rather than a beat that raises the pressure on the protagonist.`,
        suggestedFix: `In the two scenes following at least one revelation, let the tension climb so the arc's discoveries keep building pressure rather than settling once the truth is out.`,
      });
    }
  }

  // ARC_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID — Sequence/aftermath × heavy unresolvedClues debt
  ...
  // no curiosity rise, while such rises occur elsewhere. Distinct from this pass's original
  // unresolvedClues → emotionalShift rule and ARC_OPEN_THREAD_STAGING_AFTERMATH_VOID (same
  // trigger paired with emotionalShift and visualBeats respectively) — this is the third
  // consequence channel for this trigger.
  {
    const r1037c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.unresolvedClues ?? []).length >= 3,
      isAftermath: r => (r.curiosityDelta ?? 0) > 0,
    });
    if (r1037c.fires) {
      issues.push({
        location: `${r1037c.triggerCount} heavy clue-debt scene(s) — no curiosity rise within 2 scenes of any`,
        rule: 'ARC_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID',
        severity: 'minor',
  ...
  // ARC_SUSPENSE_RECURRENCE_AFTERMATH_VOID — Sequence/aftermath × suspenseDelta (>0) trigger →
  // a further suspenseDelta rise elsewhere. Built on checkAftermathVoid from the shared checks
  // library. n≥8, ≥2 qualifying suspense-spike scenes (pos<n-2), ≥2 further suspense-rise scenes
  // anywhere, 2-scene lookahead. Fires when every suspense-spike's two-scene aftermath carries no
  // further rise in suspense, while such rises occur elsewhere in the script. Distinct from
  // ARC_SUSPENSE_CURIOSITY_AFTERMATH_VOID, ARC_SUSPENSE_RELATIONAL_AFTERMATH_VOID, ARC_SUSPENSE_
  // EMOTIONAL_AFTERMATH_VOID, ARC_SUSPENSE_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID, and ARC_SUSPENSE_
  // STAGING_AFTERMATH_VOID (Wave 1121, same trigger paired with curiosityDelta/relationshipShifts/
  // emotionalShift/dialogueHighlights/visualBeats) — this self-referential pairing (suspenseDelta
  // as both trigger and channel) is the sixth and final consequence channel for this trigger,
  // genuinely completing saturation after Wave 1121's header undercounted it at five.
  {
    const r1135a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.suspenseDelta ?? 0) > 0,
      isAftermath: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r1135a.fires) {
      issues.push({
        location: `${r1135a.triggerCount} suspense-spike scene(s) — no further suspense rise within 2 scenes of any`,
        rule: 'ARC_SUSPENSE_RECURRENCE_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every one of the story's ${r1135a.triggerCount} suspense-spike scenes is followed by two scenes with no further rise in suspense, even though ${r1135a.aftermathCount} such rises occur elsewhere in the script. A tension spike that never compounds into a second wave of danger right after it leaves the arc's suspense reading as isolated jolts rather than a mounting, escalating pressure.`,
        suggestedFix: `In the two scenes following at least one suspense spike, let the tension build further rather than settling, so the arc's danger compounds instead of resetting to baseline after every scare.`,
      });
    }
  }

  // ARC_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID — Sequence/aftermath × dramaticTurn trigger →
  // dialogueHighlights absence. Built on checkAftermathVoid from the shared checks library.
  // n≥8, ≥2 qualifying dramatic-turn scenes (pos<n-2), ≥2 scenes anywhere with a highlighted
  // line of dialogue, 2-scene lookahead. Fires when every turn's two-scene aftermath contains no
  // highlighted dialogue, while such dialogue occurs elsewhere. Distinct from ARC_TURN_
  // CURIOSITY_AFTERMATH_VOID and ARC_TURN_SUSPENSE_AFTERMATH_VOID (Wave 1121), and ARC_TURN_
  // EMOTIONAL_AFTERMATH_VOID / ARC_DRAMATIC_TURN_EMOTIONAL_AFTERMATH_VOID and ARC_DRAMATIC_
  // TURN_RELATIONAL_AFTERMATH_VOID (same trigger paired with curiosityDelta/suspenseDelta/
  // emotionalShift/relationshipShifts) — this is the fifth consequence channel for this trigger.
  {
    const r1135b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.dramaticTurn ?? 'nothing') !== 'nothing',
      isAftermath: r => (r.dialogueHighlights ?? []).length > 0,
    });
    if (r1135b.fires) {
      issues.push({
        location: `${r1135b.triggerCount} dramatic-turn aftermath(s) — no highlighted dialogue within 2 scenes`,
        rule: 'ARC_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every dramatic-turn scene in the story (${r1135b.triggerCount} pivots) is followed by two scenes with no highlighted dialogue, even though ${r1135b.aftermathCount} such scenes exist elsewhere in the script. A pivot that never earns a memorable line right after it happens leaves the arc's turns unvoiced — no character's speech processes what the reversal just changed.`,
  ...
  // ARC_TURN_STAGING_AFTERMATH_VOID — Sequence/aftermath × dramaticTurn trigger → visualBeats
  // absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2 qualifying
  ...
  // elsewhere. Distinct from ARC_TURN_CURIOSITY_AFTERMATH_VOID, ARC_TURN_SUSPENSE_AFTERMATH_VOID
  // (Wave 1121), ARC_TURN_EMOTIONAL_AFTERMATH_VOID / ARC_DRAMATIC_TURN_EMOTIONAL_AFTERMATH_VOID,
  // ARC_DRAMATIC_TURN_RELATIONAL_AFTERMATH_VOID, and ARC_TURN_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID
  // (this wave, same trigger paired with curiosityDelta/suspenseDelta/emotionalShift/
  ...
  {
    const r1135c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.dramaticTurn ?? 'nothing') !== 'nothing',
      isAftermath: r => (r.visualBeats ?? []).length >= 2,
    });
    if (r1135c.fires) {
      issues.push({
        location: `${r1135c.triggerCount} dramatic-turn aftermath(s) — no heavily-staged scene within 2 scenes`,
        rule: 'ARC_TURN_STAGING_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every dramatic-turn scene in the story (${r1135c.triggerCount} pivots) is followed by two scenes with no heavily-staged visual beat, even though ${r1135c.aftermathCount} such scenes exist elsewhere in the script. A pivot that never earns a visually charged follow-through leaves the arc's turns registering as narrated information rather than something the story visibly dwells on.`,
  ...
  // ARC_POSITIVE_CURIOSITY_AFTERMATH_VOID — Sequence/aftermath × positive-emotion trigger →
  // curiosityDelta absence. Built on checkAftermathVoid from the shared checks library. n≥8,
  // ≥2 qualifying positive-emotion scenes (pos<n-2), ≥2 curiosity-spike scenes anywhere, 2-scene
  // lookahead. Fires when every positive beat's two-scene aftermath carries no rise in curiosity,
  // while such rises occur elsewhere. Distinct from ARC_POSITIVE_RELATIONAL_AFTERMATH_VOID (Wave
  // 477: same trigger, relationshipShifts channel — this is the second channel for the positive-
  // emotion trigger), ARC_REVELATION_CURIOSITY_AFTERMATH_VOID and ARC_PAYOFF_CURIOSITY_AFTERMATH_
  // VOID (different triggers), and ARC_SUSPENSE_CURIOSITY_AFTERMATH_VOID (suspenseDelta, not
  // emotionalShift, as the trigger). A triumph that generates no forward-looking question in its
  // wake treats joy as a closed, self-contained beat rather than one that opens the story further.
  {
    const r1149a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
    ...
    if (r1149a.fires) {
      issues.push({
        location: `${r1149a.triggerCount} positive-emotion scene aftermath(s) — no curiosity rise within 2 scenes`,
        rule: 'ARC_POSITIVE_CURIOSITY_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every positive-emotion scene (${r1149a.triggerCount} qualifying) is followed by two scenes with no rise in curiosity, even though ${r1149a.aftermathCount} such rises occur elsewhere in the script. A triumph or joy is most narratively useful when it also opens a new question — what this success now makes possible, or costs, or risks. When every positive beat closes without provoking any fresh curiosity in its wake, the story's wins read as endpoints rather than pivots that keep pulling the audience forward.`,
        suggestedFix: `After at least one positive-emotion scene, let the following scene or two raise a new question — what the win now exposes, threatens, or sets in motion — so triumph generates momentum instead of only closure.`,
      });
    }
  }

  // ARC_POSITIVE_SUSPENSE_AFTERMATH_VOID — Sequence/aftermath × positive-emotion trigger →
  // suspenseDelta absence. Built on checkAftermathVoid from the shared checks library. n≥8,
  // ≥2 qualifying positive-emotion scenes (pos<n-2), ≥2 suspense-rise scenes anywhere, 2-scene
  // lookahead. Fires when every positive beat's two-scene aftermath carries no rise in suspense,
  // while such rises occur elsewhere. Distinct from ARC_POSITIVE_RELATIONAL_AFTERMATH_VOID (Wave
  // 477) and ARC_POSITIVE_CURIOSITY_AFTERMATH_VOID (this wave, same trigger paired with
  // relationshipShifts/curiosityDelta) — this is the third channel for the positive-emotion
  // trigger — and from ARC_SUSPENSE_RECURRENCE_AFTERMATH_VOID (Wave 1135: suspenseDelta as
  // trigger, not aftermath). A victory that never tightens the story's tension in its wake
  // reads as a safe plateau rather than the fragile, temporary relief that keeps a story taut.
  {
    const r1149b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.emotionalShift === 'positive',
      isAftermath: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r1149b.fires) {
      issues.push({
        location: `${r1149b.triggerCount} positive-emotion scene aftermath(s) — no suspense rise within 2 scenes`,
        rule: 'ARC_POSITIVE_SUSPENSE_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every positive-emotion scene (${r1149b.triggerCount} qualifying) is followed by two scenes with no rise in suspense, even though ${r1149b.aftermathCount} such rises occur elsewhere in the script. A win that is never shadowed by rising tension in its immediate aftermath reads as a safe, permanent plateau — the story never lets the audience feel that this relief is fragile or that it might cost something later.`,
        suggestedFix: `After at least one positive-emotion scene, let the following scene or two tighten suspense — a new threat forming, a cost becoming visible, a complication surfacing — so triumph stays precarious rather than settled.`,
      });
    }
  }

  // ARC_NEGATIVE_CURIOSITY_AFTERMATH_VOID — Sequence/aftermath × negative-emotion trigger →
  // curiosityDelta absence. Built on checkAftermathVoid from the shared checks library. n≥8,
  // ≥2 qualifying negative-emotion scenes (pos<n-2), ≥2 curiosity-spike scenes anywhere, 2-scene
  // lookahead. Fires when every negative beat's two-scene aftermath carries no rise in curiosity,
  // while such rises occur elsewhere. Distinct from ARC_NEGATIVE_RELATIONAL_AFTERMATH_VOID (Wave
  // 547: same trigger, relationshipShifts channel — this is the second channel for the negative-
  // emotion trigger) and ARC_POSITIVE_CURIOSITY_AFTERMATH_VOID (this wave, opposite emotional
  // polarity). A defeat that raises no question about what happens next treats loss as a dead
  // end rather than the pivot that reroutes the story toward a new uncertainty.
  {
    const r1149c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
    ...
    if (r1149c.fires) {
      issues.push({
        location: `${r1149c.triggerCount} negative-emotion scene aftermath(s) — no curiosity rise within 2 scenes`,
        rule: 'ARC_NEGATIVE_CURIOSITY_AFTERMATH_VOID',
        severity: 'minor',
