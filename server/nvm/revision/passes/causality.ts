...
// Wave 811 additions (closes the sixteenth rotation cycle, 798-811): CAUSALITY_TURNING_
// POINT_ZONE_CLUSTER (distribution/timing × purpose === 'turning_point' × structural thirds —
// this purpose value has only ever appeared inside the tensionReleasingPurposes composite set
// [union with 'resolution', 'climax', 'revelation']; it has never been audited as its own
// standalone signal by any of the three shared-library trio modes), CAUSALITY_TURNING_
// POINT_DROUGHT_RUN (run-based × purpose === 'turning_point' absence — completing 2 of 3 slots
// for this purpose value alongside the zone-cluster mode added in this same wave; peak mode
// conventionally skipped for this categorical field), CAUSALITY_INTRODUCE_CONFLICT_
// ZONE_CLUSTER (distribution/timing × purpose === 'introduce_conflict' × structural thirds —
...
//
// Wave 853 additions (closes the nineteenth rotation cycle, 840-853): CAUSALITY_CLIMAX_DROUGHT_RUN
// (run-based × purpose === 'climax' absence — completes 2 of 3 slots for this purpose value
// alongside the zone-cluster mode added in Wave 839; peak mode conventionally skipped for this
// categorical field), CAUSALITY_RESOLUTION_DROUGHT_RUN (run-based × purpose === 'resolution'
// absence — completes 2 of 3 slots for this purpose value alongside the zone-cluster mode added
// in Wave 839; peak mode conventionally skipped for this categorical field),
...
//
// Wave 895 additions (closes the twenty-second rotation cycle, 882-895): continuing the
// checkZoneImbalance rollout from Wave 881, this wave applies the 4-zone bloat+empty-zone mode
...
//
// Wave 923 additions (closes the twenty-fourth rotation cycle, 910-923): purpose === 'revelation'
// has never been referenced anywhere in this pass (the pre-existing CAUSALITY_REVELATION_ZONE_
// CLUSTER/DROUGHT_RUN audit the separate revelation string|null field, not this purpose enum value)
// -- a genuinely virgin field. This wave adds CAUSALITY_REVELATION_PURPOSE_ZONE_CLUSTER and
// CAUSALITY_REVELATION_PURPOSE_DROUGHT_RUN (peak mode conventionally skipped for this categorical
...
// Wave 965 additions (closes the twenty-seventh rotation cycle, 952-965): auditing the three remaining
// trio-complete signals in this pass, spanning three distinct classes: CAUSALITY_REVELATION_ZONE_
...
// Wave 1007 additions (opens the thirty-first rotation cycle for this pass): this wave gives
// three more existing aftermath-void triggers a fresh consequence channel: CAUSALITY_STAKES_
// SUSPENSE_AFTERMATH_VOID (raise_stakes, previously paired with emotionalShift and curiosityDelta,
// now paired with suspenseDelta), CAUSALITY_SEED_EMOTIONAL_AFTERMATH_VOID (seededClueIds,
// previously only paired with curiosityDelta, now paired with emotionalShift), and CAUSALITY_
// PAYOFF_RELATIONAL_AFTERMATH_VOID (payoffSetupIds, previously only paired with emotionalShift,
// now paired with relationshipShifts).
// Wave 1021 additions (closes the thirty-first rotation cycle, 1008-1021): three more fresh
// channels for existing triggers: CAUSALITY_STAKES_RELATIONAL_AFTERMATH_VOID (raise_stakes,
// previously paired with emotionalShift/curiosityDelta/suspenseDelta, now a fourth channel with
// relationshipShifts), CAUSALITY_PAYOFF_SUSPENSE_AFTERMATH_VOID (payoffSetupIds, previously paired
...
// Wave 1105 additions (closes the thirty-seventh rotation cycle, 1092-1105): this wave gives
// clockRaised and dramaticTurn further channels — CAUSALITY_CLOCK_EMOTIONAL_AFTERMATH_VOID and
// CAUSALITY_CLOCK_RELATIONAL_AFTERMATH_VOID pair clockRaised with emotionalShift and
// relationshipShifts respectively (second and third channels for this trigger), and CAUSALITY_
// TURN_CURIOSITY_AFTERMATH_VOID pairs dramaticTurn with curiosityDelta (second channel for this
// trigger).
// Wave 1119 additions (opens the thirty-ninth rotation cycle continuation of cycle 38, 1106-1119):
// clockRaised is now three channels deep (curiosityDelta/emotionalShift/relationshipShifts) and
// dramaticTurn two (suspenseDelta/curiosityDelta) — this wave advances both toward saturation.
// CAUSALITY_CLOCK_SUSPENSE_AFTERMATH_VOID pairs clockRaised with suspenseDelta (fourth channel)
...
// Wave 1161 additions: revelation and suspenseDelta each had only their one Wave-1147 channel.
// CAUSALITY_REVELATION_EMOTIONAL_AFTERMATH_VOID and CAUSALITY_REVELATION_RELATIONAL_AFTERMATH_
...
export async function causalityPass(input: PassInput): Promise<PassResult> {
  ...
  // ── Wave 811: CAUSALITY_TURNING_POINT_ZONE_CLUSTER, CAUSALITY_TURNING_POINT_DROUGHT_RUN,
  //              CAUSALITY_INTRODUCE_CONFLICT_ZONE_CLUSTER ──────────────────────────────────────

  // CAUSALITY_TURNING_POINT_ZONE_CLUSTER — Distribution/timing × purpose === 'turning_point' ×
  // structural thirds. Built on checkZoneCluster from the shared checks library. n≥9, ≥3
  ...
  {
    const r811a = checkZoneCluster({
      records, minRecords: 9, minCount: 3, ratioThreshold: 0.75,
      isPresent: r => r.purpose === 'turning_point',
    });
    if (r811a.fires) {
      issues.push({
        location: `${r811a.zoneNames[r811a.maxZoneIdx]} third — ${r811a.maxZoneCount} of ${r811a.count} turning-point scenes`,
        rule: 'CAUSALITY_TURNING_POINT_ZONE_CLUSTER',
        severity: 'minor',
        description: `${Math.round((r811a.maxZoneCount / r811a.count) * 100)}% of the story's turning-point scenes cluster in the ${r811a.zoneNames[r811a.maxZoneIdx]} third. When every scene purposed as a turning point lands in the same structural window, the causal chain has no redirection anywhere else across the story.`,
        suggestedFix: `Purpose at least one scene outside the ${r811a.zoneNames[r811a.maxZoneIdx]} third as a turning point so the causal chain keeps redirecting events more evenly across the story.`,
      });
    }
  }

  // CAUSALITY_TURNING_POINT_DROUGHT_RUN — Run-based × purpose === 'turning_point' absence. Built
  ...
  {
    const r811b = checkDroughtRun({
      records, minRecords: 10, minPresentCount: 3, runThreshold: 6,
      isPresent: r => r.purpose === 'turning_point',
    });
    if (r811b.fires) {
      issues.push({
        location: `longest stretch with no turning point: ${r811b.longestRun} consecutive scenes`,
        rule: 'CAUSALITY_TURNING_POINT_DROUGHT_RUN',
        severity: 'minor',
        description: `The story contains a run of ${r811b.longestRun} consecutive scenes with no turning-point purpose at all, even though ${r811b.presentCount} scenes elsewhere redirect events. A long unbroken stretch with no redirection leaves the causal chain coasting without a pivot for an extended run.`,
        suggestedFix: `Purpose at least one scene within the ${r811b.longestRun}-scene stretch as a turning point so the causal chain keeps redirecting events throughout that stretch.`,
      });
    }
  }

  // CAUSALITY_INTRODUCE_CONFLICT_ZONE_CLUSTER — Distribution/timing × purpose ===
  ...
  // despite being thematically central to a causality analysis; none of the three shared-library
  // trio modes has ever been applied to it.
  {
    const r811c = checkZoneCluster({
      records, minRecords: 9, minCount: 3, ratioThreshold: 0.75,
      isPresent: r => r.purpose === 'introduce_conflict',
    });
    if (r811c.fires) {
      issues.push({
        location: `${r811c.zoneNames[r811c.maxZoneIdx]} third — ${r811c.maxZoneCount} of ${r811c.count} conflict-introducing scenes`,
        rule: 'CAUSALITY_INTRODUCE_CONFLICT_ZONE_CLUSTER',
        severity: 'minor',
  ...
  // ── Wave 853: CAUSALITY_CLIMAX_DROUGHT_RUN, CAUSALITY_RESOLUTION_DROUGHT_RUN,
  //              CAUSALITY_ESTABLISH_WORLD_DROUGHT_RUN ──────────────────────────────────────

  // CAUSALITY_CLIMAX_DROUGHT_RUN — Run-based × purpose === 'climax' absence. Built on
  ...
  // 839 (peak mode conventionally skipped for this categorical field).
  {
    const r853a = checkDroughtRun({
      records, minRecords: 10, minPresentCount: 3, runThreshold: 6,
      isPresent: r => r.purpose === 'climax',
    });
    if (r853a.fires) {
      issues.push({
        location: `longest stretch with no climax-purposed scene: ${r853a.longestRun} consecutive scenes`,
        rule: 'CAUSALITY_CLIMAX_DROUGHT_RUN',
        severity: 'minor',
        description: `The story contains a run of ${r853a.longestRun} consecutive scenes purposed otherwise than the climax, even though ${r853a.presentCount} scenes elsewhere are dedicated to the story's peak. A long unbroken stretch with no climactic scene leaves the causal chain without a payoff to build toward for an extended run.`,
        suggestedFix: `Purpose at least one scene within the ${r853a.longestRun}-scene stretch as the climax so the causal chain keeps a payoff to build toward throughout that stretch.`,
      });
    }
  }

  // CAUSALITY_RESOLUTION_DROUGHT_RUN — Run-based × purpose === 'resolution' absence. Built on
  ...
  // 839 (peak mode conventionally skipped for this categorical field).
  {
    const r853b = checkDroughtRun({
      records, minRecords: 10, minPresentCount: 3, runThreshold: 6,
      isPresent: r => r.purpose === 'resolution',
    });
    if (r853b.fires) {
      issues.push({
        location: `longest stretch with no resolution-purposed scene: ${r853b.longestRun} consecutive scenes`,
        rule: 'CAUSALITY_RESOLUTION_DROUGHT_RUN',
        severity: 'minor',
        description: `The story contains a run of ${r853b.longestRun} consecutive scenes purposed otherwise than resolving the story, even though ${r853b.presentCount} scenes elsewhere settle a thread. A long unbroken stretch with nothing resolved leaves the causal chain's threads dangling for an extended run.`,
        suggestedFix: `Purpose at least one scene within the ${r853b.longestRun}-scene stretch to resolve the story so the causal chain keeps settling its threads throughout that stretch.`,
      });
    }
  }
  ...
  // added in Wave 839 (peak mode conventionally skipped for this categorical field).
  {
    const r853c = checkDroughtRun({
      records, minRecords: 10, minPresentCount: 3, runThreshold: 6,
      isPresent: r => r.purpose === 'establish_world',
    });
    if (r853c.fires) {
      issues.push({
        location: `longest stretch with no world-building: ${r853c.longestRun} consecutive scenes`,
        rule: 'CAUSALITY_ESTABLISH_WORLD_DROUGHT_RUN',
        severity: 'minor',
  ...
  // ── Wave 895: CAUSALITY_RESOLUTION_ZONE_IMBALANCE, CAUSALITY_COMPLICATE_ZONE_IMBALANCE,
  //              CAUSALITY_TURNING_POINT_ZONE_IMBALANCE ──────────────────────────────────────

  // CAUSALITY_RESOLUTION_ZONE_IMBALANCE — Underweight/bloat × purpose === 'resolution' × four
  // structural zones. Built on checkZoneImbalance from the shared checks library. n≥10, ≥4
  ...
  // existing 3-zone CAUSALITY_RESOLUTION_ZONE_CLUSTER and run-based CAUSALITY_RESOLUTION_
  // DROUGHT_RUN — the first application of the 4-zone bloat+empty-zone mode to this purpose
  // value.
  {
    const r895a = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => r.purpose === 'resolution',
    });
    if (r895a.fires) {
      const emptyNames895a = r895a.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName895a = FOUR_ZONE_NAMES[r895a.bloatZoneIdx];
      issues.push({
        location: `${emptyNames895a} empty; ${bloatName895a} has ${r895a.counts[r895a.bloatZoneIdx]}/${r895a.totalCount} resolution-purposed scenes`,
        rule: 'CAUSALITY_RESOLUTION_ZONE_IMBALANCE',
        severity: 'minor',
        description: `The story's ${r895a.totalCount} resolution-purposed scenes are unevenly distributed across its four structural zones: ${bloatName895a} contains ${r895a.counts[r895a.bloatZoneIdx]} of them (${Math.round((r895a.counts[r895a.bloatZoneIdx] / r895a.totalCount) * 100)}%) while ${emptyNames895a} contains none. Settling beats bloat in one structural quarter and vanish from another, giving the causal chain's closure an uneven structural rhythm.`,
        suggestedFix: `Redistribute settling beats: move at least one resolution-purposed scene into the empty zone(s) — ${emptyNames895a} — so the causal chain's threads settle more evenly across every structural quarter, not only the quarter currently carrying most of them.`,
      });
    }
  }

  // CAUSALITY_COMPLICATE_ZONE_IMBALANCE — Underweight/bloat × purpose === 'complicate' × four
  // structural zones. Built on checkZoneImbalance from the shared checks library. n≥10, ≥4
  ...
  // 3-zone CAUSALITY_COMPLICATE_ZONE_CLUSTER and run-based CAUSALITY_COMPLICATE_DROUGHT_RUN —
  // the first application of the 4-zone bloat+empty-zone mode to this purpose value.
  {
    const r895b = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => r.purpose === 'complicate',
    });
    if (r895b.fires) {
      const emptyNames895b = r895b.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName895b = FOUR_ZONE_NAMES[r895b.bloatZoneIdx];
      issues.push({
        location: `${emptyNames895b} empty; ${bloatName895b} has ${r895b.counts[r895b.bloatZoneIdx]}/${r895b.totalCount} complicating scenes`,
        rule: 'CAUSALITY_COMPLICATE_ZONE_IMBALANCE',
        severity: 'minor',
        description: `The story's ${r895b.totalCount} complicating scenes are unevenly distributed across its four structural zones: ${bloatName895b} contains ${r895b.counts[r895b.bloatZoneIdx]} of them (${Math.round((r895b.counts[r895b.bloatZoneIdx] / r895b.totalCount) * 100)}%) while ${emptyNames895b} contains none. Complications bloat in one structural quarter and vanish from another, giving the causal chain's escalating trouble an uneven structural rhythm.`,
        suggestedFix: `Redistribute complications: move at least one complicate-purposed scene into the empty zone(s) — ${emptyNames895b} — so the causal chain keeps deepening its cause-and-effect more evenly across every structural quarter, not only the quarter currently carrying most of them.`,
      });
    }
  }

  // CAUSALITY_TURNING_POINT_ZONE_IMBALANCE — Underweight/bloat × purpose === 'turning_point' ×
  ...
  // zone has zero such scenes while another holds ≥50% of the total. Distinct from the existing
  // 3-zone CAUSALITY_TURNING_POINT_ZONE_CLUSTER and run-based CAUSALITY_TURNING_POINT_
  // DROUGHT_RUN — the first application of the 4-zone bloat+empty-zone mode to this purpose
  // value.
  {
    const r895c = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => r.purpose === 'turning_point',
    });
    if (r895c.fires) {
      const emptyNames895c = r895c.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName895c = FOUR_ZONE_NAMES[r895c.bloatZoneIdx];
      issues.push({
        location: `${emptyNames895c} empty; ${bloatName895c} has ${r895c.counts[r895c.bloatZoneIdx]}/${r895c.totalCount} turning-point scenes`,
        rule: 'CAUSALITY_TURNING_POINT_ZONE_IMBALANCE',
        severity: 'minor',
  ...
  // CAUSALITY_REVELATION_PURPOSE_ZONE_CLUSTER — Distribution/timing × purpose === 'revelation' ×
  ...
  // Named distinctly from CAUSALITY_REVELATION_ZONE_CLUSTER, which audits the separate revelation
  ...
  {
    const r923a = checkZoneCluster({
      records, minRecords: 9, minCount: 3, ratioThreshold: 0.75,
      isPresent: r => r.purpose === 'revelation',
    });
    if (r923a.fires) {
      issues.push({
        location: `${r923a.zoneNames[r923a.maxZoneIdx]} third — ${r923a.maxZoneCount} of ${r923a.count} revelation-purposed scenes`,
        rule: 'CAUSALITY_REVELATION_PURPOSE_ZONE_CLUSTER',
        severity: 'minor',
        description: `${Math.round((r923a.maxZoneCount / r923a.count) * 100)}% of the scenes purposed as a revelation cluster in the ${r923a.zoneNames[r923a.maxZoneIdx]} third. When every purpose-built disclosure lands in the same structural window, the causal chain turns on new information in only one part of the story instead of across its whole shape.`,
        suggestedFix: `Purpose at least one scene outside the ${r923a.zoneNames[r923a.maxZoneIdx]} third as a revelation so the causal chain keeps turning on new information more evenly across the story.`,
      });
    }
  }

  // CAUSALITY_REVELATION_PURPOSE_DROUGHT_RUN — Run-based × purpose === 'revelation' absence. Built
  ...
  {
    const r923b = checkDroughtRun({
      records, minRecords: 10, minPresentCount: 3, runThreshold: 6,
      isPresent: r => r.purpose === 'revelation',
    });
    if (r923b.fires) {
      issues.push({
        location: `longest stretch with no revelation-purposed scene: ${r923b.longestRun} consecutive scenes`,
        rule: 'CAUSALITY_REVELATION_PURPOSE_DROUGHT_RUN',
        severity: 'minor',
        description: `The story contains a run of ${r923b.longestRun} consecutive scenes with no scene purposed as a revelation, even though ${r923b.presentCount} scenes elsewhere disclose information by purpose. A long unbroken stretch with no turns of new information leaves the causal chain running on old premises for an extended run.`,
        suggestedFix: `Purpose a scene within the ${r923b.longestRun}-scene stretch as a revelation so the causal chain keeps turning on new information throughout that stretch.`,
      });
    }
  }

  // CAUSALITY_NEGATIVE_EMOTION_ZONE_IMBALANCE — Underweight/bloat × emotionalShift === 'negative' ×
  ...
  // another holds ≥50% of the total. Distinct from the existing 3-zone CAUSALITY_NEGATIVE_EMOTION_
  // ZONE_CLUSTER and run-based CAUSALITY_NEGATIVE_EMOTION_DROUGHT_RUN — the first application of the
  // 4-zone bloat+empty-zone mode to this valence signal.
  {
    const r923c = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => r.emotionalShift === 'negative',
    });
    if (r923c.fires) {
      const emptyNames923c = r923c.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName923c = FOUR_ZONE_NAMES[r923c.bloatZoneIdx];
      issues.push({
        location: `${emptyNames923c} empty; ${bloatName923c} has ${r923c.counts[r923c.bloatZoneIdx]}/${r923c.totalCount} negative-shift scenes`,
        rule: 'CAUSALITY_NEGATIVE_EMOTION_ZONE_IMBALANCE',
        severity: 'minor',
  ...
  // CAUSALITY_REVELATION_ZONE_IMBALANCE — Underweight/bloat × (revelation != null) × four structural
  // zones. Built on checkZoneImbalance from the shared checks library. n≥10, ≥4 revelation scenes
  // total, divided across four equal structural zones. Fires only when one zone has zero such scenes
  // while another holds ≥50% of the total. Distinct from the existing 3-zone CAUSALITY_REVELATION_
  // ZONE_CLUSTER and run-based CAUSALITY_REVELATION_DROUGHT_RUN — the first application of the 4-zone
  // bloat+empty-zone mode to the revelation STRING field (revelation != null), and distinct from
  // CAUSALITY_REVELATION_PURPOSE_ZONE_IMBALANCE, which audits the separate purpose === 'revelation' enum.
  {
    const r965a = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => r.revelation != null,
    });
    if (r965a.fires) {
      const emptyNames965a = r965a.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName965a = FOUR_ZONE_NAMES[r965a.bloatZoneIdx];
      issues.push({
        location: `${emptyNames965a} empty; ${bloatName965a} has ${r965a.counts[r965a.bloatZoneIdx]}/${r965a.totalCount} revelation scenes`,
        rule: 'CAUSALITY_REVELATION_ZONE_IMBALANCE',
        severity: 'minor',
        description: `The story's ${r965a.totalCount} revelation scenes are unevenly distributed across its four structural zones: ${bloatName965a} contains ${r965a.counts[r965a.bloatZoneIdx]} of them (${Math.round((r965a.counts[r965a.bloatZoneIdx] / r965a.totalCount) * 100)}%) while ${emptyNames965a} contains none. Disclosures bloat in one structural quarter and never land in another, so the new information that redirects the causal chain surfaces in only part of the story.`,
        suggestedFix: `Redistribute disclosures: land a revelation in at least one scene inside the empty zone(s) — ${emptyNames965a} — so new information keeps redirecting the causal chain across every structural quarter, not only the quarter currently carrying most of them.`,
      });
    }
  }

  // CAUSALITY_RELATIONSHIP_ZONE_IMBALANCE — Underweight/bloat × (relationshipShifts.length > 0) × four
  ...
  // CAUSALITY_RELATIONSHIP_ZONE_CLUSTER and run-based CAUSALITY_RELATIONSHIP_DROUGHT_RUN — the first
  // application of the 4-zone bloat+empty-zone mode to the relationshipShifts array field, distinct
  // from the payoffSetupIds and seededClueIds arrays audited in Wave 951.
  {
    const r965b = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => (r.relationshipShifts ?? []).length > 0,
    });
    if (r965b.fires) {
      const emptyNames965b = r965b.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName965b = FOUR_ZONE_NAMES[r965b.bloatZoneIdx];
      issues.push({
        location: `${emptyNames965b} empty; ${bloatName965b} has ${r965b.counts[r965b.bloatZoneIdx]}/${r965b.totalCount} relationship-shift scenes`,
        rule: 'CAUSALITY_RELATIONSHIP_ZONE_IMBALANCE',
        severity: 'minor',
        description: `The story's ${r965b.totalCount} scenes with a relationship shift are unevenly distributed across its four structural zones: ${bloatName965b} contains ${r965b.counts[r965b.bloatZoneIdx]} of them (${Math.round((r965b.counts[r965b.bloatZoneIdx] / r965b.totalCount) * 100)}%) while ${emptyNames965b} contains none. Bonds change in a bloated cluster in one structural quarter and stay static in another, so the relational consequences that drive the causal chain concentrate in only part of the story.`,
        suggestedFix: `Redistribute relational change: give at least one scene inside the empty zone(s) — ${emptyNames965b} — a relationship shift so relational cause-and-effect runs across every structural quarter, not only the quarter currently carrying most of them.`,
      });
    }
  }

  // CAUSALITY_TURN_ZONE_IMBALANCE — Underweight/bloat × (dramaticTurn !== 'nothing') × four structural
  ...
  // predicate as the existing 3-zone CAUSALITY_TURN_ZONE_CLUSTER and run-based CAUSALITY_TURN_DROUGHT_
  // RUN — the first application of the 4-zone bloat+empty-zone mode to the dramatic-turn categorical
  // signal, closing the twenty-seventh rotation cycle (952-965).
  {
    const r965c = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => (r.dramaticTurn ?? 'nothing') !== 'nothing',
    });
    if (r965c.fires) {
      const emptyNames965c = r965c.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName965c = FOUR_ZONE_NAMES[r965c.bloatZoneIdx];
      issues.push({
        location: `${emptyNames965c} empty; ${bloatName965c} has ${r965c.counts[r965c.bloatZoneIdx]}/${r965c.totalCount} dramatic-turn scenes`,
        rule: 'CAUSALITY_TURN_ZONE_IMBALANCE',
        severity: 'minor',
  ...
  // CAUSALITY_STAKES_SUSPENSE_AFTERMATH_VOID — Sequence/aftermath × raise_stakes trigger →
  ...
  // tension does rise elsewhere. Distinct from CAUSALITY_STAKES_EMOTIONAL_AFTERMATH_VOID (Wave
  // 979, emotionalShift) and CAUSALITY_STAKES_CURIOSITY_AFTERMATH_VOID (Wave 993, curiosityDelta)
  // — this is the third consequence channel for this trigger in this pass.
  {
    const r1007a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.purpose === 'raise_stakes',
      isAftermath: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r1007a.fires) {
      issues.push({
        location: `${r1007a.triggerCount} stakes-raise aftermath(s) — no suspense raised within 2 scenes`,
        rule: 'CAUSALITY_STAKES_SUSPENSE_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every stakes-raising scene (${r1007a.triggerCount} escalations) is followed by two scenes with no rise in tension, even though ${r1007a.aftermathCount} such rises occur elsewhere. Escalating danger should usually tighten the felt sense of jeopardy in the causal chain's next link; when every stakes-raise's aftermath registers no suspense, the escalation reads as a stated fact rather than a threat that propagates forward.`,
        suggestedFix: `In the two scenes following at least one stakes-raise, tighten the tension — a ticking complication or a near-miss — so escalating danger propagates through the causal chain as a felt threat, not just a stated one.`,
      });
    }
  }

  // CAUSALITY_SEED_EMOTIONAL_AFTERMATH_VOID — Sequence/aftermath × seededClueIds trigger →
  // emotionalShift absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  ...
  // elsewhere. Distinct from CAUSALITY_SEED_CURIOSITY_AFTERMATH_VOID (Wave 979, same trigger
  // paired with curiosityDelta) — this pairs seededClueIds with emotionalShift for the first time
  // in this pass.
  {
    const r1007b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.seededClueIds ?? []).length > 0,
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1007b.fires) {
      issues.push({
        location: `${r1007b.triggerCount} seed aftermath(s) — no emotional shift within 2 scenes`,
        rule: 'CAUSALITY_SEED_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every clue-seeding scene (${r1007b.triggerCount} plants) is followed by two emotionally neutral scenes, even though ${r1007b.aftermathCount} emotionally-charged scenes exist elsewhere. Planting a clue usually carries some charge — unease, curiosity's cousin dread, quiet hope; when every seed's aftermath is affectively flat, the causal chain's groundwork lands as pure information with no felt weight.`,
  ...
  // CAUSALITY_PAYOFF_RELATIONAL_AFTERMATH_VOID — Sequence/aftermath × payoffSetupIds trigger →
  ...
  // such shifts occur elsewhere. Distinct from CAUSALITY_PAYOFF_EMOTIONAL_AFTERMATH_VOID (Wave
  // 993, same trigger paired with emotionalShift) — this pairs payoffSetupIds with
  // relationshipShifts for the first time in this pass.
  {
    const r1007c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.payoffSetupIds ?? []).length > 0,
    ...
    if (r1007c.fires) {
      issues.push({
        location: `${r1007c.triggerCount} payoff aftermath(s) — no relationship shift within 2 scenes`,
        rule: 'CAUSALITY_PAYOFF_RELATIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every payoff scene (${r1007c.triggerCount} cashed-in setups) is followed by two scenes with no shift in any relationship, even though ${r1007c.aftermathCount} such shifts occur elsewhere. A callback that never bears on how characters treat each other in the scenes right after it lands as narrative bookkeeping rather than a beat that ripples through the causal chain's relationships.`,
        suggestedFix: `In the two scenes following at least one payoff, let the resolved setup strain or shift a relationship so the callback pays off interpersonally, not only causally.`,
      });
    }
  }

  // CAUSALITY_STAKES_RELATIONAL_AFTERMATH_VOID — Sequence/aftermath × raise_stakes trigger →
  // relationshipShifts absence. Built on checkAftermathVoid from the shared checks library. n≥8,
  ...
  // such changes occur elsewhere. Distinct from CAUSALITY_STAKES_CURIOSITY_AFTERMATH_VOID (Wave
  // 993), the original raise_stakes → emotionalShift rule, and CAUSALITY_STAKES_SUSPENSE_
  // AFTERMATH_VOID (Wave 1007) — this is the fourth consequence channel for this trigger in this
  // pass.
  {
    const r1021a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.purpose === 'raise_stakes',
      isAftermath: r => (r.relationshipShifts ?? []).length > 0,
    });
    if (r1021a.fires) {
      issues.push({
        location: `${r1021a.triggerCount} raise-stakes aftermath(s) — no relationship shift within 2 scenes`,
        rule: 'CAUSALITY_STAKES_RELATIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every stakes-raising scene in the story (${r1021a.triggerCount} of them) is followed by two scenes with no shift in any relationship, even though ${r1021a.aftermathCount} such shifts occur elsewhere. Raising the stakes without it ever bearing on how characters treat each other in the scenes right after leaves the causal chain's escalation registering as plot mechanics rather than interpersonal consequence.`,
        suggestedFix: `In the two scenes following at least one stakes-raise, let the new danger strain or shift a relationship so the escalation ripples through the causal chain's relationships, not just its plot.`,
      });
    }
  }

  // CAUSALITY_PAYOFF_SUSPENSE_AFTERMATH_VOID — Sequence/aftermath × payoffSetupIds trigger →
  // suspenseDelta absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  // qualifying payoff scenes (pos<n-2), ≥2 suspense-rising scenes anywhere, 2-scene lookahead.
  // Fires when every payoff's two-scene aftermath carries no suspense rise, while such rises occur
  // elsewhere. Distinct from CAUSALITY_PAYOFF_EMOTIONAL_AFTERMATH_VOID (Wave 993) and CAUSALITY_
  // PAYOFF_RELATIONAL_AFTERMATH_VOID (Wave 1007) — this is the third consequence channel for this
  // trigger in this pass.
  {
    const r1021b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.payoffSetupIds ?? []).length > 0,
      isAftermath: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r1021b.fires) {
      issues.push({
        location: `${r1021b.triggerCount} payoff aftermath(s) — no suspense rise within 2 scenes`,
        rule: 'CAUSALITY_PAYOFF_SUSPENSE_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every payoff scene in the story (${r1021b.triggerCount} cashed-in setups) is followed by two scenes with no rise in suspense, even though ${r1021b.aftermathCount} such rises occur elsewhere. A callback that closes cleanly with no fresh tension in its immediate wake leaves the causal chain feeling settled rather than still building toward what comes next.`,
        suggestedFix: `In the two scenes following at least one payoff, let a new tension rise so the causal chain keeps building rather than going slack right after a resolution.`,
      });
    }
  }

  // CAUSALITY_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID — Sequence/aftermath × unresolvedClues trigger
  // → emotionalShift absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  // qualifying scenes with an open thread (pos<n-2), ≥2 emotionally-charged scenes anywhere,
  // 2-scene lookahead. Fires when every open-thread scene's two-scene aftermath carries no
  // emotional shift, while such shifts occur elsewhere. Distinct from CAUSALITY_OPEN_THREAD_
  // SUSPENSE_AFTERMATH_VOID (Wave 993, same trigger paired with suspenseDelta) — this pairs
  // unresolvedClues with emotionalShift for the first time in this pass.
  {
    const r1021c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.unresolvedClues ?? []).length > 0,
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1021c.fires) {
      issues.push({
        location: `${r1021c.triggerCount} open-thread aftermath(s) — no emotional shift within 2 scenes`,
        rule: 'CAUSALITY_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
  ...
  // CAUSALITY_CLOCK_EMOTIONAL_AFTERMATH_VOID — Sequence/aftermath × clockRaised trigger →
  // emotionalShift absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  ...
  // while such shifts occur elsewhere. Distinct from CAUSALITY_CLOCK_CURIOSITY_AFTERMATH_VOID
  // (Wave 1091, same trigger paired with curiosityDelta) — this is the second consequence
  // channel for this trigger.
  {
    const r1105a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.clockRaised === true,
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1105a.fires) {
      issues.push({
        location: `${r1105a.triggerCount} clock-raise scene(s) — no emotional shift within 2 scenes of any`,
        rule: 'CAUSALITY_CLOCK_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every one of the story's ${r1105a.triggerCount} scenes that raise the ticking clock is followed by two scenes with no emotional shift, even though ${r1105a.aftermathCount} such shifts occur elsewhere. Time pressure that never registers emotionally right after it tightens leaves the causal chain's clock feeling procedural — a deadline moving without anyone visibly reacting to it.`,
        suggestedFix: `In the two scenes following at least one clock-raise, let a character's emotional register shift in response to the new urgency, so the ticking clock carries felt weight, not just mechanical pressure.`,
      });
    }
  }

  // CAUSALITY_CLOCK_RELATIONAL_AFTERMATH_VOID — Sequence/aftermath × clockRaised trigger →
  // relationshipShifts absence. Built on checkAftermathVoid from the shared checks library. n≥8,
  // ≥2 qualifying clock-raise scenes (pos<n-2), ≥2 scenes anywhere with a recorded relationship
  // shift, 2-scene lookahead. Fires when every clock-raise's two-scene aftermath carries no
  // relationship movement, while such movement occurs elsewhere. Distinct from CAUSALITY_CLOCK_
  // CURIOSITY_AFTERMATH_VOID and CAUSALITY_CLOCK_EMOTIONAL_AFTERMATH_VOID (same trigger paired
  // with curiosityDelta/emotionalShift) — this is the third consequence channel for this
  // trigger.
  {
    const r1105b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.clockRaised === true,
      isAftermath: r => (r.relationshipShifts ?? []).length > 0,
    });
    if (r1105b.fires) {
      issues.push({
        location: `${r1105b.triggerCount} clock-raise scene(s) — no relationship shift within 2 scenes of any`,
        rule: 'CAUSALITY_CLOCK_RELATIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every one of the story's ${r1105b.triggerCount} scenes that raise the ticking clock is followed by two scenes with no recorded relationship shift, even though ${r1105b.aftermathCount} such shifts occur elsewhere. Time pressure that never moves how characters stand with each other leaves the causal chain's clock isolated from the interpersonal stakes it should eventually complicate.`,
  ...
  // CAUSALITY_TURN_CURIOSITY_AFTERMATH_VOID — Sequence/aftermath × dramaticTurn trigger →
  // curiosityDelta absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  ...
  // such rises occur elsewhere. Distinct from CAUSALITY_TURN_SUSPENSE_AFTERMATH_VOID (Wave 1091,
  // same trigger paired with suspenseDelta) — this is the second consequence channel for this
  // trigger.
  {
    const r1105c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.dramaticTurn ?? 'nothing') !== 'nothing',
      isAftermath: r => (r.curiosityDelta ?? 0) > 0,
    });
    if (r1105c.fires) {
      issues.push({
        location: `${r1105c.triggerCount} dramatic-turn aftermath(s) — no curiosity rise within 2 scenes`,
        rule: 'CAUSALITY_TURN_CURIOSITY_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every dramatic-turn scene in the story (${r1105c.triggerCount} pivots) is followed by two scenes with no rise in curiosity, even though ${r1105c.aftermathCount} such rises occur elsewhere. A pivot that never opens a fresh question right after it happens leaves the causal chain's turns registering as closed events rather than links that generate the next thing to wonder about.`,
        suggestedFix: `In the two scenes following at least one dramatic turn, let a new question surface from the pivot so the causal chain keeps generating curiosity, not just settling into the new state of affairs.`,
      });
    }
  }

  // CAUSALITY_CLOCK_SUSPENSE_AFTERMATH_VOID — Sequence/aftermath × clockRaised trigger →
  // suspenseDelta absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  ...
  // while such rises occur elsewhere. Distinct from CAUSALITY_CLOCK_CURIOSITY_AFTERMATH_VOID
  // (Wave 1091), CAUSALITY_CLOCK_EMOTIONAL_AFTERMATH_VOID, and CAUSALITY_CLOCK_RELATIONAL_
  // AFTERMATH_VOID (Wave 1105, same trigger paired with curiosityDelta/emotionalShift/
  // relationshipShifts) — this is the fourth consequence channel for this trigger.
  {
    const r1119a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.clockRaised === true,
      isAftermath: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r1119a.fires) {
      issues.push({
        location: `${r1119a.triggerCount} clock-raise scene(s) — no suspense rise within 2 scenes of any`,
        rule: 'CAUSALITY_CLOCK_SUSPENSE_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every one of the story's ${r1119a.triggerCount} scenes that raise the ticking clock is followed by two scenes with no rise in suspense, even though ${r1119a.aftermathCount} such rises occur elsewhere. A deadline that tightens without making the outcome feel more threatening leaves the causal chain's clock registering as a schedule rather than a source of dread.`,
  ...
  // CAUSALITY_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID — Sequence/aftermath × clockRaised
  // trigger → dialogueHighlights absence. Built on checkAftermathVoid from the shared checks
  // library. n≥8, ≥2 qualifying clock-raise scenes (pos<n-2), ≥2 highlighted-dialogue scenes
  // anywhere, 2-scene lookahead. Fires when every clock-raise's two-scene aftermath has no
  // memorable line, while such lines occur elsewhere. Distinct from CAUSALITY_CLOCK_CURIOSITY_
  // AFTERMATH_VOID, CAUSALITY_CLOCK_EMOTIONAL_AFTERMATH_VOID, CAUSALITY_CLOCK_RELATIONAL_
  // AFTERMATH_VOID, and CAUSALITY_CLOCK_SUSPENSE_AFTERMATH_VOID (same trigger paired with
  // curiosityDelta/emotionalShift/relationshipShifts/suspenseDelta) — this is the fifth
  // consequence channel for this trigger.
  {
    const r1119b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.clockRaised === true,
      isAftermath: r => (r.dialogueHighlights ?? []).length > 0,
    });
    if (r1119b.fires) {
      issues.push({
        location: `${r1119b.triggerCount} clock-raise scene(s) — no highlighted dialogue within 2 scenes of any`,
        rule: 'CAUSALITY_CLOCK_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every one of the story's ${r1119b.triggerCount} scenes that raise the ticking clock is followed by two scenes with no memorable line, even though ${r1119b.aftermathCount} such lines exist elsewhere in the script. A deadline that tightens without earning a line that reckons with it leaves the causal chain's clock voiced only in narration, never in what a character says.`,
  ...
  // CAUSALITY_TURN_EMOTIONAL_AFTERMATH_VOID — Sequence/aftermath × dramaticTurn trigger →
  // emotionalShift absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  ...
  // such shifts occur elsewhere. Distinct from CAUSALITY_TURN_SUSPENSE_AFTERMATH_VOID (Wave
  // 1091) and CAUSALITY_TURN_CURIOSITY_AFTERMATH_VOID (Wave 1105, same trigger paired with
  // suspenseDelta/curiosityDelta) — this is the third consequence channel for this trigger.
  {
    const r1119c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.dramaticTurn ?? 'nothing') !== 'nothing',
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1119c.fires) {
      issues.push({
        location: `${r1119c.triggerCount} dramatic-turn aftermath(s) — no emotional shift within 2 scenes`,
        rule: 'CAUSALITY_TURN_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
  ...
  // CAUSALITY_REVELATION_EMOTIONAL_AFTERMATH_VOID — Sequence/aftermath × revelation trigger →
  // emotionalShift absence. Built on checkAftermathVoid from the shared checks library. n≥8,
  // ≥2 qualifying revelation scenes (pos<n-2), ≥2 emotionally-shifted scenes anywhere, 2-scene
  // lookahead. Fires when every revelation's two-scene aftermath registers no emotional shift,
  // while such shifts occur elsewhere. Distinct from CAUSALITY_REVELATION_CURIOSITY_AFTERMATH_
  // VOID (Wave 1147, same trigger paired with curiosityDelta) — this is the second consequence
  // channel for this trigger.
  {
    const r1161a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.revelation != null,
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1161a.fires) {
      issues.push({
        location: `${r1161a.triggerCount} revelation scene(s) — no emotional shift within 2 scenes of any`,
        rule: 'CAUSALITY_REVELATION_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every one of the story's ${r1161a.triggerCount} revelation scenes is followed by two scenes with no emotional shift, even though ${r1161a.aftermathCount} such shifts occur elsewhere. A truth that surfaces without ever registering on a character's felt state right after it lands leaves the causal chain's disclosures reading as information delivered to the audience rather than something that visibly changes how anyone feels.`,
  ...
  // CAUSALITY_REVELATION_RELATIONAL_AFTERMATH_VOID — Sequence/aftermath × revelation trigger →
  // relationshipShifts absence. Built on checkAftermathVoid from the shared checks library.
  ...
  // CAUSALITY_REVELATION_CURIOSITY_AFTERMATH_VOID (Wave 1147) and CAUSALITY_REVELATION_
  ...
  {
    const r1161b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.revelation != null,
      isAftermath: r => (r.relationshipShifts ?? []).length > 0,
    });
    if (r1161b.fires) {
      issues.push({
        location: `${r1161b.triggerCount} revelation scene(s) — no relationship shift within 2 scenes of any`,
        rule: 'CAUSALITY_REVELATION_RELATIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every one of the story's ${r1161b.triggerCount} revelation scenes is followed by two scenes with no recorded relationship shift, even though ${r1161b.aftermathCount} such shifts occur elsewhere. A truth that surfaces without ever moving how a pair of characters stand with each other leaves the causal chain's disclosures isolated from the interpersonal stakes they should eventually complicate.`,
  ...
  // CAUSALITY_SUSPENSE_CURIOSITY_AFTERMATH_VOID — Sequence/aftermath × suspenseDelta (>0)
  ...
  // anywhere, 2-scene lookahead. Fires when every suspense-spike's two-scene aftermath carries
  // no rise in curiosity, while such rises occur elsewhere. Distinct from CAUSALITY_SUSPENSE_
  // EMOTIONAL_AFTERMATH_VOID (Wave 1147, same trigger paired with emotionalShift) — this is the
  // second consequence channel for this trigger.
  {
    const r1161c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.suspenseDelta ?? 0) > 0,
      isAftermath: r => (r.curiosityDelta ?? 0) > 0,
    });
    if (r1161c.fires) {
      issues.push({
        location: `${r1161c.triggerCount} suspense-spike scene(s) — no curiosity rise within 2 scenes of any`,
        rule: 'CAUSALITY_SUSPENSE_CURIOSITY_AFTERMATH_VOID',
        severity: 'minor',
