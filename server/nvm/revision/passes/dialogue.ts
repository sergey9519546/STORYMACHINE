...
// Waves 644/700 applied the backward-cause peak and zone-cluster modes to dialogueHighlights; the
// drought-run mode has never been applied to it, completing the trio; named distinctly from
// structure.ts's Wave 597 DIALOGUE_HIGHLIGHT_DROUGHT_RUN to avoid a cross-file rule-name
// collision), DIALOGUE_SEED_ZONE_CLUSTER (distribution/timing × seededClueIds × structural
// thirds — Waves 658/700 applied the drought-run and backward-cause peak modes to seededClueIds;
// the zone-cluster mode has never been applied to it, completing the trio), DIALOGUE_PAYOFF_PEAK_
...
// Wave 952 additions (opens the twenty-seventh rotation cycle): extending the checkZoneImbalance
// rollout to three more trio-complete signals spanning three distinct signal classes: DIALOGUE_
// REVELATION_PURPOSE_ZONE_IMBALANCE (purpose === 'revelation', trio completed Wave 938), DIALOGUE_
...
// this pass).
// Wave 1008 additions: this wave gives three existing aftermath-void triggers a fresh consequence
// channel: DIALOGUE_STAKES_EMOTIONAL_AFTERMATH_VOID (raise_stakes, previously paired with
// dialogueHighlights and curiosityDelta, now paired with emotionalShift), DIALOGUE_OPEN_THREAD_
// CURIOSITY_AFTERMATH_VOID (heavy unresolvedClues debt, previously only paired with
// dialogueHighlights, now paired with curiosityDelta), and DIALOGUE_SEED_RELATIONAL_AFTERMATH_VOID
// (seededClueIds, previously only paired with suspenseDelta, now paired with relationshipShifts).
// Wave 1022 additions (opens the thirty-second rotation cycle for this pass): three more fresh
// channels for existing triggers: DIALOGUE_STAKES_SUSPENSE_AFTERMATH_VOID (raise_stakes,
// previously paired with dialogueHighlights/curiosityDelta/emotionalShift, now a fourth channel
// with suspenseDelta), DIALOGUE_SEED_CURIOSITY_AFTERMATH_VOID (seededClueIds, previously paired
// with suspenseDelta/relationshipShifts, now a third channel with curiosityDelta), and DIALOGUE_
// OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID (heavy unresolvedClues debt, previously paired with
// dialogueHighlights/curiosityDelta, now a third channel with emotionalShift).
// Wave 1036 additions (opens the thirty-third rotation cycle for this pass): with raise_stakes now
// at four channels, this wave targets the less-saturated triggers instead: DIALOGUE_OPEN_THREAD_
// SUSPENSE_AFTERMATH_VOID (heavy unresolvedClues debt, previously paired with dialogueHighlights/
// curiosityDelta/emotionalShift, now a fourth channel with suspenseDelta), DIALOGUE_SEED_EMOTIONAL_
// AFTERMATH_VOID (seededClueIds, previously paired with suspenseDelta/relationshipShifts/
...
// Wave 1106 additions (opens the thirty-eighth rotation cycle for this pass): this wave gives
// clockRaised and dramaticTurn further checkAftermathVoid channels — DIALOGUE_CLOCK_EMOTIONAL_
// AFTERMATH_VOID and DIALOGUE_CLOCK_RELATIONAL_AFTERMATH_VOID pair clockRaised with
// emotionalShift and relationshipShifts respectively (second and third channels for this
// trigger), and DIALOGUE_TURN_CURIOSITY_AFTERMATH_VOID pairs dramaticTurn with curiosityDelta
// (second channel for this trigger).
// Wave 1120 additions (opens the thirty-ninth rotation cycle for this pass): clockRaised is now
// three channels deep (curiosityDelta/emotionalShift/relationshipShifts) and dramaticTurn two
// (suspenseDelta/curiosityDelta) -- this wave advances both toward saturation.
...
// ⚡ Bolt: Use zero-allocation fastWordCount to avoid O(N) array allocations during dialogue analysis passes.
...
export async function dialoguePass(input: PassInput): Promise<PassResult> {
  ...
  // ── Wave 714: HIGHLIGHTED_DIALOGUE_DROUGHT_RUN, DIALOGUE_SEED_ZONE_CLUSTER,
  //              DIALOGUE_PAYOFF_PEAK_UNCAUSED ─────────────────────────────────────────────────

  // HIGHLIGHTED_DIALOGUE_DROUGHT_RUN — Run-based × dialogueHighlights absence. Built on
  ...
  // Waves 644/700 applied the backward-cause peak and zone-cluster modes to dialogueHighlights;
  // the drought-run mode has never been applied to it, completing the trio. Named distinctly from
  // structure.ts's Wave 597 DIALOGUE_HIGHLIGHT_DROUGHT_RUN to avoid a cross-file rule-name
  // collision.
  {
    const r714a = checkDroughtRun({
      records, minRecords: 10, minPresentCount: 3, runThreshold: 6,
      isPresent: r => (r.dialogueHighlights ?? []).length > 0,
    });
    if (r714a.fires) {
      issues.push({
        location: `longest stretch with no highlighted dialogue: ${r714a.longestRun} consecutive scenes`,
        rule: 'HIGHLIGHTED_DIALOGUE_DROUGHT_RUN',
        severity: 'minor',
        description: `The story contains a run of ${r714a.longestRun} consecutive scenes with no highlighted dialogue at all, even though ${r714a.presentCount} scenes elsewhere carry a standout line. A long unbroken stretch with nothing verbally memorable leaves the dialogue running on unremarkable lines for an extended run.`,
        suggestedFix: `Give at least one scene within the ${r714a.longestRun}-scene stretch a standout line of dialogue — keeping the verbal register alive throughout that stretch.`,
      });
    }
  }

  // DIALOGUE_SEED_ZONE_CLUSTER — Distribution/timing × seededClueIds × structural thirds. Built
  // on checkZoneCluster from the shared checks library. n≥9, ≥3 seed scenes, fires when >75% of
  // them fall in a single structural third. Waves 658/700 applied the drought-run and backward-
  // cause peak modes to seededClueIds; the zone-cluster mode has never been applied to it,
  // completing the trio.
  {
    const r714b = checkZoneCluster({
      records, minRecords: 9, minCount: 3, ratioThreshold: 0.75,
      isPresent: r => (r.seededClueIds ?? []).length > 0,
    });
    if (r714b.fires) {
      const zoneName714b = r714b.zoneNames[r714b.maxZoneIdx];
      issues.push({
        location: `${zoneName714b} third — ${r714b.maxZoneCount}/${r714b.count} seed scenes`,
        rule: 'DIALOGUE_SEED_ZONE_CLUSTER',
        severity: 'minor',
        description: `${r714b.maxZoneCount} of the story's ${r714b.count} clue-planting scenes (${Math.round((r714b.maxZoneCount / r714b.count) * 100)}%) cluster in the ${zoneName714b} third. Foreshadowing concentrates almost exclusively in that stretch of the story rather than surfacing throughout, leaving other structural thirds with no new seed for dialogue to press against.`,
        suggestedFix: `Plant at least one clue outside the ${zoneName714b} third — spreading foreshadowing across the story gives dialogue in every structural third something fresh to draw on.`,
      });
    }
  }

  // DIALOGUE_PAYOFF_PEAK_UNCAUSED — Single-peak isolation/backward-cause × payoffSetupIds
  ...
  // revelation. Waves 658/700 applied the zone-cluster and drought-run modes to payoffSetupIds;
  // the backward-cause peak mode has never been applied to it, completing the trio.
  {
    const r714c = checkPeakUncaused({
      records, minRecords: 8, minQualifying: 2, lookback: 2,
    ...
    if (r714c.fires) {
      issues.push({
        location: `scene ${r714c.peakIdx + 1} — peak payoff density (${r714c.peakMagnitude}) with no dramatic turn or revelation nearby`,
        rule: 'DIALOGUE_PAYOFF_PEAK_UNCAUSED',
        severity: 'minor',
  ...
  // DIALOGUE_REVELATION_PURPOSE_ZONE_IMBALANCE -- Underweight/bloat x purpose === 'revelation' x four
  // structural zones. Built on checkZoneImbalance from the shared checks library, closing the 4-zone
  // gap for this purpose value (its 3-zone/run trio was completed in Wave 938). n>=10, >=4 revelation-
  // purposed scenes total, divided across four equal structural zones. Fires only when one zone has
  // zero such scenes while another holds >=50% of the total. Distinct from DIALOGUE_REVELATION_PURPOSE_
  // ZONE_CLUSTER/DROUGHT_RUN (Wave 938) -- the first application of the 4-zone bloat+empty-zone mode
  // to this purpose value, and distinct from the separate revelation-FIELD rules (DIALOGUE_REVELATION_
  // ZONE_CLUSTER/DROUGHT_RUN audit the revelation string, not the purpose enum).
  {
    const r952a = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => r.purpose === 'revelation',
    });
    if (r952a.fires) {
      const emptyNames952a = r952a.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName952a = FOUR_ZONE_NAMES[r952a.bloatZoneIdx];
      issues.push({
        location: `${emptyNames952a} empty; ${bloatName952a} has ${r952a.counts[r952a.bloatZoneIdx]}/${r952a.totalCount} revelation-purposed scenes`,
        rule: 'DIALOGUE_REVELATION_PURPOSE_ZONE_IMBALANCE',
        severity: 'minor',
        description: `The story's ${r952a.totalCount} revelation-purposed scenes are unevenly distributed across its four structural zones: ${bloatName952a} contains ${r952a.counts[r952a.bloatZoneIdx]} of them (${Math.round((r952a.counts[r952a.bloatZoneIdx] / r952a.totalCount) * 100)}%) while ${emptyNames952a} contains none. Scenes built to disclose bloat in one quarter and vanish from another, so dialogue's register of revelation carries only part of the story.`,
        suggestedFix: `Redistribute disclosures: purpose at least one scene inside the empty zone(s) -- ${emptyNames952a} -- as a revelation so dialogue's disclosing register recurs across every structural quarter, not only the quarter currently carrying most of them.`,
      });
    }
  }

  // DIALOGUE_SUSPENSE_ZONE_IMBALANCE -- Underweight/bloat x (suspenseDelta > 0) x four structural
  ...
  // SUSPENSE_ZONE_CLUSTER and run-based DIALOGUE_SUSPENSE_DROUGHT_RUN -- the first application of the
  ...
  {
    const r952b = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r952b.fires) {
      const emptyNames952b = r952b.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName952b = FOUR_ZONE_NAMES[r952b.bloatZoneIdx];
      issues.push({
        location: `${emptyNames952b} empty; ${bloatName952b} has ${r952b.counts[r952b.bloatZoneIdx]}/${r952b.totalCount} suspense-raising scenes`,
        rule: 'DIALOGUE_SUSPENSE_ZONE_IMBALANCE',
        severity: 'minor',
        description: `The story's ${r952b.totalCount} suspense-raising scenes are unevenly distributed across its four structural zones: ${bloatName952b} contains ${r952b.counts[r952b.bloatZoneIdx]} of them (${Math.round((r952b.counts[r952b.bloatZoneIdx] / r952b.totalCount) * 100)}%) while ${emptyNames952b} contains none. Tension bloats in one quarter and flatlines in another, giving dialogue's taut register an uneven structural rhythm.`,
        suggestedFix: `Redistribute suspense: move or add a scene that raises suspense (suspenseDelta > 0) into the empty zone(s) -- ${emptyNames952b} -- so dialogue's taut register recurs across every structural quarter, not only the quarter currently carrying most of it.`,
      });
    }
  }

  // DIALOGUE_OPEN_THREAD_ZONE_IMBALANCE -- Underweight/bloat x (unresolvedClues.length > 0) x four
  ...
  // 3-zone DIALOGUE_OPEN_THREAD_ZONE_CLUSTER and run-based DIALOGUE_OPEN_THREAD_DROUGHT_RUN, and
  // distinct from the already-audited DIALOGUE_PAYOFF_ZONE_IMBALANCE (payoffSetupIds) -- this keys on
  // the unresolvedClues array, a genuinely different field.
  {
    const r952c = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => (r.unresolvedClues ?? []).length > 0,
    });
    if (r952c.fires) {
      const emptyNames952c = r952c.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName952c = FOUR_ZONE_NAMES[r952c.bloatZoneIdx];
      issues.push({
        location: `${emptyNames952c} empty; ${bloatName952c} has ${r952c.counts[r952c.bloatZoneIdx]}/${r952c.totalCount} open-thread scenes`,
        rule: 'DIALOGUE_OPEN_THREAD_ZONE_IMBALANCE',
        severity: 'minor',
  ...
  // DIALOGUE_STAKES_EMOTIONAL_AFTERMATH_VOID -- Sequence/aftermath x raise_stakes trigger ->
  ...
  // charged scenes occur elsewhere. Distinct from DIALOGUE_STAKES_CURIOSITY_AFTERMATH_VOID (Wave
  // 994, curiosityDelta) and RAISE_STAKES_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID (dialogueHighlights) --
  // this is the third consequence channel for this trigger in this pass.
  {
    const r1008a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.purpose === 'raise_stakes',
    ...
    if (r1008a.fires) {
      issues.push({
        location: `${r1008a.triggerCount} stakes-raise aftermath(s) -- no emotional shift within 2 scenes`,
        rule: 'DIALOGUE_STAKES_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every stakes-raising scene (${r1008a.triggerCount} escalations) is followed by two emotionally neutral scenes, even though ${r1008a.aftermathCount} emotionally-charged scenes exist elsewhere. Escalating danger should usually carry felt weight for the characters living through it; when every stakes-raise's aftermath is affectively flat, dialogue never voices what the escalation costs anyone.`,
        suggestedFix: `Let at least one stakes-raise carry feeling in its aftermath: in the scene or two after the danger sharpens, give a character a line that voices fear, resolve, or dread.`,
      });
    }
  }

  // DIALOGUE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID -- Sequence/aftermath x heavy unresolved-clue-
  ...
  // curiosity-raising scenes anywhere, 2-scene lookahead. Fires when every heavy-debt scene's
  // two-scene aftermath opens no new curiosity, while curiosity does occur elsewhere. Distinct
  // from OPEN_THREAD_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID (same trigger paired with
  // dialogueHighlights) -- this pairs heavy clue-debt with curiosityDelta for the first time in
  // this pass.
  {
    const r1008b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.unresolvedClues ?? []).length >= 3,
      isAftermath: r => (r.curiosityDelta ?? 0) > 0,
    });
    if (r1008b.fires) {
      issues.push({
        location: `${r1008b.triggerCount} heavy clue-debt scene(s) -- no curiosity raised within 2 scenes of any`,
        rule: 'DIALOGUE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every scene carrying heavy unresolved clue-debt (${r1008b.triggerCount} instances) is followed by two full scenes that raise no new curiosity, even though ${r1008b.aftermathCount} such rises occur elsewhere. Accumulated mystery should usually compound into fresh questions rather than sit as inert backlog; when every heavy-debt scene's aftermath opens nothing new, dialogue never voices the next question the debt provokes.`,
        suggestedFix: `In the two scenes following at least one heavy clue-debt moment, give a character a line that raises a new question so accumulated mystery keeps compounding rather than sitting in a learnable lull.`,
      });
    }
  }

  // DIALOGUE_SEED_RELATIONAL_AFTERMATH_VOID -- Sequence/aftermath x seededClueIds trigger ->
  // relationshipShifts absence. Built on checkAftermathVoid from the shared checks library. n>=8,
  // >=2 qualifying seed scenes (pos<n-2), >=2 relationship-shift scenes anywhere, 2-scene
  // lookahead. Fires when every seed's two-scene aftermath carries no relationship shift, while
  // such shifts occur elsewhere. Distinct from DIALOGUE_SEED_SUSPENSE_AFTERMATH_VOID (same
  // trigger paired with suspenseDelta) -- this pairs seededClueIds with relationshipShifts for the
  // first time in this pass.
  {
    const r1008c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.seededClueIds ?? []).length > 0,
    ...
    if (r1008c.fires) {
      issues.push({
        location: `${r1008c.triggerCount} seed aftermath(s) -- no relationship shift within 2 scenes`,
        rule: 'DIALOGUE_SEED_RELATIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every clue-seeding scene (${r1008c.triggerCount} plants) is followed by two scenes with no shift in any relationship, even though ${r1008c.aftermathCount} such shifts occur elsewhere. A planted clue that never bears on how characters treat each other nearby leaves the seed purely mechanical, disconnected from what's actually said between people.`,
        suggestedFix: `In the two scenes following at least one clue-seeding moment, let the planted material strain or shift a relationship so dialogue carries interpersonal weight alongside its narrative function.`,
      });
    }
  }

  // DIALOGUE_STAKES_SUSPENSE_AFTERMATH_VOID -- Sequence/aftermath x raise_stakes trigger ->
  // suspenseDelta absence. Built on checkAftermathVoid from the shared checks library. n>=8, >=2
  // qualifying raise_stakes scenes (pos<n-2), >=2 suspense-rising scenes anywhere, 2-scene
  // lookahead. Fires when every stakes-raise's two-scene aftermath carries no suspense rise, while
  // such rises occur elsewhere. Distinct from DIALOGUE_STAKES_CURIOSITY_AFTERMATH_VOID and
  // DIALOGUE_STAKES_EMOTIONAL_AFTERMATH_VOID (same trigger paired with curiosityDelta and
  // emotionalShift respectively) -- this is the fourth consequence channel for this trigger.
  {
    const r1022a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.purpose === 'raise_stakes',
      isAftermath: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r1022a.fires) {
      issues.push({
        location: `${r1022a.triggerCount} raise-stakes aftermath(s) -- no suspense rise within 2 scenes`,
        rule: 'DIALOGUE_STAKES_SUSPENSE_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every stakes-raising scene in the story (${r1022a.triggerCount} of them) is followed by two scenes with no rise in suspense, even though ${r1022a.aftermathCount} such rises occur elsewhere. A stakes-raise that isn't matched by climbing tension in the dialogue right after it leaves the escalation as a stated fact rather than something characters are visibly grappling with.`,
        suggestedFix: `In the two scenes following at least one stakes-raise, let the tension in the dialogue itself climb so characters' words carry the weight of what's now at risk.`,
      });
    }
  }

  // DIALOGUE_SEED_CURIOSITY_AFTERMATH_VOID -- Sequence/aftermath x seededClueIds trigger ->
  ...
  // elsewhere. Distinct from DIALOGUE_SEED_SUSPENSE_AFTERMATH_VOID and DIALOGUE_SEED_RELATIONAL_
  // AFTERMATH_VOID (same trigger paired with suspenseDelta and relationshipShifts respectively) --
  // this is the third consequence channel for this trigger.
  {
    const r1022b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.seededClueIds ?? []).length > 0,
      isAftermath: r => (r.curiosityDelta ?? 0) > 0,
    });
    if (r1022b.fires) {
      issues.push({
        location: `${r1022b.triggerCount} seed aftermath(s) -- no curiosity rise within 2 scenes`,
        rule: 'DIALOGUE_SEED_CURIOSITY_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every clue-seeding scene (${r1022b.triggerCount} plants) is followed by two scenes with no rise in curiosity, even though ${r1022b.aftermathCount} such rises occur elsewhere. Planting a clue without characters' dialogue visibly wondering about it right after leaves the seed buried rather than voiced as a live question.`,
        suggestedFix: `In the two scenes following at least one clue-seeding moment, let a character's line raise a new question about it so the seed's curiosity is voiced, not just planted.`,
      });
    }
  }

  // DIALOGUE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID -- Sequence/aftermath x heavy unresolvedClues
  // debt trigger -> emotionalShift absence. Built on checkAftermathVoid from the shared checks
  // library. n>=8, >=2 qualifying heavy-debt scenes (pos<n-2, threshold >=3, same as DIALOGUE_
  // OPEN_THREAD_CURIOSITY_AFTERMATH_VOID), >=2 emotionally-charged scenes anywhere, 2-scene
  // lookahead. Fires when every heavy-debt scene's two-scene aftermath carries no emotional shift,
  // while such shifts occur elsewhere. Distinct from DIALOGUE_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID
  // (same trigger paired with curiosityDelta) -- this is the third consequence channel for this
  // trigger.
  {
    const r1022c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.unresolvedClues ?? []).length >= 3,
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1022c.fires) {
      issues.push({
        location: `${r1022c.triggerCount} heavy clue-debt scene(s) -- no emotional shift within 2 scenes of any`,
        rule: 'DIALOGUE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every scene carrying heavy unresolved clue-debt (${r1022c.triggerCount} instances) is followed by two emotionally neutral scenes, even though ${r1022c.aftermathCount} emotionally-charged scenes exist elsewhere. A pile-up of open questions that never registers as felt in the dialogue right after it leaves the mounting mystery purely intellectual rather than something anyone visibly carries.`,
        suggestedFix: `In the two scenes following a heavy clue-debt moment, let a character's dialogue register the weight of the unresolved questions emotionally, not just informationally.`,
      });
    }
  }

  // DIALOGUE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID -- Sequence/aftermath x heavy unresolvedClues
  ...
  // carries no suspense rise, while such rises occur elsewhere. Distinct from DIALOGUE_OPEN_
  // THREAD_CURIOSITY_AFTERMATH_VOID and DIALOGUE_OPEN_THREAD_EMOTIONAL_AFTERMATH_VOID (same
  // trigger paired with curiosityDelta and emotionalShift respectively) -- this is the fourth
  // consequence channel for this trigger.
  {
    const r1036a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.unresolvedClues ?? []).length >= 3,
      isAftermath: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r1036a.fires) {
      issues.push({
        location: `${r1036a.triggerCount} heavy clue-debt scene(s) -- no suspense rise within 2 scenes of any`,
        rule: 'DIALOGUE_OPEN_THREAD_SUSPENSE_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every scene carrying heavy unresolved clue-debt (${r1036a.triggerCount} instances) is followed by two scenes with no rise in suspense, even though ${r1036a.aftermathCount} such rises occur elsewhere. Accumulated mystery should usually tighten the felt sense of danger rather than sit as inert backlog; when every heavy-debt scene's aftermath carries no rising tension, dialogue never voices the pressure the debt should be generating.`,
        suggestedFix: `In the two scenes following at least one heavy clue-debt moment, let the tension rise in what characters say so accumulated mystery keeps building pressure rather than sitting in a learnable lull.`,
      });
    }
  }

  // DIALOGUE_SEED_EMOTIONAL_AFTERMATH_VOID -- Sequence/aftermath x seededClueIds trigger ->
  // emotionalShift absence. Built on checkAftermathVoid from the shared checks library. n>=8, >=2
  ...
  // occur elsewhere. Distinct from DIALOGUE_SEED_SUSPENSE_AFTERMATH_VOID, DIALOGUE_SEED_
  // RELATIONAL_AFTERMATH_VOID, and DIALOGUE_SEED_CURIOSITY_AFTERMATH_VOID (same trigger paired
  // with suspenseDelta/relationshipShifts/curiosityDelta respectively) -- this is the fourth
  // consequence channel for this trigger.
  {
    const r1036b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.seededClueIds ?? []).length > 0,
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1036b.fires) {
      issues.push({
        location: `${r1036b.triggerCount} seed aftermath(s) -- no emotional shift within 2 scenes`,
        rule: 'DIALOGUE_SEED_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every clue-seeding scene (${r1036b.triggerCount} plants) is followed by two emotionally neutral scenes, even though ${r1036b.aftermathCount} emotionally-charged scenes exist elsewhere. A planted clue that never registers as felt in the dialogue right after it leaves the seed purely mechanical, disconnected from what characters actually feel about the new information.`,
        suggestedFix: `In the two scenes following at least one clue-seeding moment, let a character's line carry some feeling about the new information so the seed lands emotionally, not just as plot data.`,
      });
    }
  }

  // DIALOGUE_SHIFT_CURIOSITY_AFTERMATH_VOID -- Sequence/aftermath x relationshipShifts
  // (amount>=0.3) trigger -> curiosityDelta absence. Built on checkAftermathVoid from the shared
  // checks library. n>=8, >=2 qualifying shift scenes (pos<n-2), >=2 curiosity-rising scenes
  // anywhere, 2-scene lookahead. Fires when every shift's two-scene aftermath carries no curiosity
  // rise, while such rises occur elsewhere. Distinct from DIALOGUE_SHIFT_STAGING_AFTERMATH_VOID
  // (same trigger paired with visualBeats) -- this pairs the shift trigger with curiosityDelta for
  // the first time in this pass.
  {
    const r1036c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.relationshipShifts ?? []).some(s => Math.abs(s.amount) >= 0.3),
      isAftermath: r => (r.curiosityDelta ?? 0) > 0,
    });
    if (r1036c.fires) {
      issues.push({
        location: `${r1036c.triggerCount} relationship-shift scene(s) -- no curiosity rise within 2 scenes of any`,
        rule: 'DIALOGUE_SHIFT_CURIOSITY_AFTERMATH_VOID',
        severity: 'minor',
  ...
  // DIALOGUE_CLOCK_EMOTIONAL_AFTERMATH_VOID -- Sequence/aftermath x clockRaised trigger ->
  // emotionalShift absence. Built on checkAftermathVoid from the shared checks library. n>=8,
  // >=2 qualifying clock-raise scenes (pos<n-2), >=2 emotionally-charged scenes anywhere,
  // 2-scene lookahead. Fires when every clock-raise's two-scene aftermath carries no emotional
  // shift, while such shifts occur elsewhere. Distinct from DIALOGUE_CLOCK_CURIOSITY_AFTERMATH_
  // VOID (Wave 1092, same trigger paired with curiosityDelta) and every dialogue-presence/peak/
  // zone/drought clockRaised check in this file -- this is the second checkAftermathVoid-based
  // channel for this trigger.
  {
    const r1106a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.clockRaised === true,
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1106a.fires) {
      issues.push({
        location: `${r1106a.triggerCount} clock-raise scene(s) -- no emotional shift within 2 scenes of any`,
        rule: 'DIALOGUE_CLOCK_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every one of the story's ${r1106a.triggerCount} scenes that raise the ticking clock is followed by two scenes with no emotional shift, even though ${r1106a.aftermathCount} such shifts occur elsewhere. Time pressure that never registers emotionally right after it tightens leaves the dialogue layer with urgency to voice but nothing felt to react to -- characters can argue about the deadline, but nobody visibly feels its weight.`,
        suggestedFix: `In the two scenes following at least one clock-raise, let a character's emotional register shift in response -- in dialogue or event -- so the ticking clock carries felt weight, not just mechanical pressure.`,
      });
    }
  }

  // DIALOGUE_CLOCK_RELATIONAL_AFTERMATH_VOID -- Sequence/aftermath x clockRaised trigger ->
  ...
  // DIALOGUE_CLOCK_CURIOSITY_AFTERMATH_VOID and DIALOGUE_CLOCK_EMOTIONAL_AFTERMATH_VOID (same
  // trigger paired with curiosityDelta/emotionalShift) -- this is the third checkAftermathVoid-
  // based channel for this trigger.
  {
    const r1106b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.clockRaised === true,
      isAftermath: r => (r.relationshipShifts ?? []).length > 0,
    });
    if (r1106b.fires) {
      issues.push({
        location: `${r1106b.triggerCount} clock-raise scene(s) -- no relationship shift within 2 scenes of any`,
        rule: 'DIALOGUE_CLOCK_RELATIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every one of the story's ${r1106b.triggerCount} scenes that raise the ticking clock is followed by two scenes with no recorded relationship shift, even though ${r1106b.aftermathCount} such shifts occur elsewhere. Time pressure that never moves how characters stand with each other leaves the dialogue layer's clock isolated from the interpersonal stakes it should eventually complicate.`,
  ...
  // DIALOGUE_TURN_CURIOSITY_AFTERMATH_VOID -- Sequence/aftermath x dramaticTurn trigger ->
  // curiosityDelta absence. Built on checkAftermathVoid from the shared checks library. n>=8,
  ...
  // such rises occur elsewhere. Distinct from DIALOGUE_TURN_SUSPENSE_AFTERMATH_VOID (Wave 1092,
  // same trigger paired with suspenseDelta) and DIALOGUE_DRAMATIC_TURN_AFTERMATH_SILENT (Wave
  // 532, hand-rolled, dialogue-presence aftermath) -- this is the second checkAftermathVoid-
  // based channel for this trigger.
  {
    const r1106c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.dramaticTurn ?? 'nothing') !== 'nothing',
      isAftermath: r => (r.curiosityDelta ?? 0) > 0,
    });
    if (r1106c.fires) {
      issues.push({
        location: `${r1106c.triggerCount} dramatic-turn aftermath(s) -- no curiosity rise within 2 scenes`,
        rule: 'DIALOGUE_TURN_CURIOSITY_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every dramatic-turn scene in the story (${r1106c.triggerCount} pivots) is followed by two scenes with no rise in curiosity, even though ${r1106c.aftermathCount} such rises occur elsewhere. A pivot that never opens a fresh question right after it happens leaves the dialogue layer's turns registering as closed events rather than links that generate the next thing to wonder about.`,
        suggestedFix: `In the two scenes following at least one dramatic turn, let a new question surface -- in dialogue or event -- so the pivot keeps generating curiosity instead of settling into the new state of affairs.`,
      });
    }
  }

  // DIALOGUE_CLOCK_SUSPENSE_AFTERMATH_VOID -- Sequence/aftermath x clockRaised trigger ->
  // suspenseDelta absence. Built on checkAftermathVoid from the shared checks library. n>=8, >=2
  ...
  // while such rises occur elsewhere. Distinct from DIALOGUE_CLOCK_CURIOSITY_AFTERMATH_VOID
  // (Wave 1092), DIALOGUE_CLOCK_EMOTIONAL_AFTERMATH_VOID, and DIALOGUE_CLOCK_RELATIONAL_
  // AFTERMATH_VOID (Wave 1106, same trigger paired with curiosityDelta/emotionalShift/
  // relationshipShifts) -- this is the fourth consequence channel for this trigger.
  {
    const r1120a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.clockRaised === true,
      isAftermath: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r1120a.fires) {
      issues.push({
        location: `${r1120a.triggerCount} clock-raise scene(s) -- no suspense rise within 2 scenes of any`,
        rule: 'DIALOGUE_CLOCK_SUSPENSE_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every one of the story's ${r1120a.triggerCount} scenes that raise the ticking clock is followed by two scenes with no rise in suspense, even though ${r1120a.aftermathCount} such rises occur elsewhere. A deadline that tightens without making the outcome feel more threatening leaves the dialogue layer's clock registering as a schedule rather than a source of dread the characters voice.`,
  ...
  // DIALOGUE_CLOCK_HIGHLIGHT_AFTERMATH_VOID -- Sequence/aftermath x clockRaised trigger ->
  // dialogueHighlights absence. Built on checkAftermathVoid from the shared checks library.
  // n>=8, >=2 qualifying clock-raise scenes (pos<n-2), >=2 highlighted-dialogue scenes anywhere,
  // 2-scene lookahead. Fires when every clock-raise's two-scene aftermath has no memorable line,
  // while such lines occur elsewhere. Distinct from DIALOGUE_CLOCK_CURIOSITY_AFTERMATH_VOID,
  // DIALOGUE_CLOCK_EMOTIONAL_AFTERMATH_VOID, DIALOGUE_CLOCK_RELATIONAL_AFTERMATH_VOID, and
  // DIALOGUE_CLOCK_SUSPENSE_AFTERMATH_VOID (same trigger paired with curiosityDelta/
  // emotionalShift/relationshipShifts/suspenseDelta) -- this is the fifth consequence channel for
  // this trigger, and the first that pairs clockRaised with this pass's own central signal.
  {
    const r1120b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.clockRaised === true,
      isAftermath: r => (r.dialogueHighlights ?? []).length > 0,
    });
    if (r1120b.fires) {
      issues.push({
        location: `${r1120b.triggerCount} clock-raise scene(s) -- no highlighted dialogue within 2 scenes of any`,
        rule: 'DIALOGUE_CLOCK_HIGHLIGHT_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every one of the story's ${r1120b.triggerCount} scenes that raise the ticking clock is followed by two scenes with no memorable line, even though ${r1120b.aftermathCount} such lines exist elsewhere in the script. A deadline that tightens without earning a line that reckons with it leaves the clock voiced only in narration, never in what a character says.`,
  ...
  // DIALOGUE_TURN_EMOTIONAL_AFTERMATH_VOID -- Sequence/aftermath x dramaticTurn trigger ->
  // emotionalShift absence. Built on checkAftermathVoid from the shared checks library. n>=8, >=2
  ...
  // shifts occur elsewhere. Distinct from DIALOGUE_TURN_SUSPENSE_AFTERMATH_VOID (Wave 1092) and
  // DIALOGUE_TURN_CURIOSITY_AFTERMATH_VOID (Wave 1106, same trigger paired with suspenseDelta/
  // curiosityDelta) -- this is the third consequence channel for this trigger.
  {
    const r1120c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.dramaticTurn ?? 'nothing') !== 'nothing',
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1120c.fires) {
      issues.push({
        location: `${r1120c.triggerCount} dramatic-turn aftermath(s) -- no emotional shift within 2 scenes`,
        rule: 'DIALOGUE_TURN_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
