...
// Wave 820 additions: RHYTHM_CHARACTER_MOMENT_DROUGHT_RUN (run-based × purpose ===
// 'character_moment' absence — completing 2 of 3 slots for this purpose value alongside the
// zone-cluster mode added in Wave 806; peak mode conventionally skipped for this categorical
// field), RHYTHM_TURNING_POINT_ZONE_CLUSTER (distribution/timing × purpose === 'turning_point'
...
//
// Wave 932 additions: purpose === 'revelation' has never been referenced anywhere in this pass
// (the pre-existing RHYTHM_REVELATION_ZONE_CLUSTER/DROUGHT_RUN audit the separate revelation
// string|null field, not this purpose enum value) -- a genuinely virgin field. This wave adds
// RHYTHM_REVELATION_PURPOSE_ZONE_CLUSTER and RHYTHM_REVELATION_PURPOSE_DROUGHT_RUN (peak mode
...
// Wave 1030 additions: RHYTHM_STAKES_EMOTIONAL_AFTERMATH_VOID gives raise_stakes a fourth channel
// (previously paired with curiosityDelta/suspenseDelta/relationshipShifts, now paired with
// emotionalShift), RHYTHM_PAYOFF_SUSPENSE_AFTERMATH_VOID gives payoffSetupIds a third channel
...
// Wave 1058 additions: RHYTHM_REVELATION_STAGING_AFTERMATH_VOID gives revelation its sixth and
// final standard channel (previously paired with emotionalShift/relationshipShifts/
// curiosityDelta/suspenseDelta/dialogueHighlights, now also paired with visualBeats, completing
// full saturation). RHYTHM_TURN_SUSPENSE_AFTERMATH_VOID gives dramaticTurn a fourth channel
// (previously paired with payoffSetupIds/relationshipShifts/emotionalShift/curiosityDelta, now
...
export async function rhythmPass(input: PassInput): Promise<PassResult> {
      ...
      const hasRecovery442b = qualifying442b.some(ql => {
        ...
        return (
          (qIdx + 1 < actionLines.length &&
            fastWordCount(actionLines[qIdx + 1].text) <= 7) ||
          (qIdx + 2 < actionLines.length &&
            fastWordCount(actionLines[qIdx + 2].text) <= 7)
        );
      });
      if (!hasRecovery442b) {
        issues.push({
  ...
  // ── Wave 820: RHYTHM_CHARACTER_MOMENT_DROUGHT_RUN, RHYTHM_TURNING_POINT_ZONE_CLUSTER,
  //              RHYTHM_TURNING_POINT_DROUGHT_RUN ──────────────────────────────────────

  // RHYTHM_CHARACTER_MOMENT_DROUGHT_RUN — Run-based × purpose === 'character_moment' absence.
  ...
  // added in Wave 806 (peak mode conventionally skipped for this categorical field).
  {
    const r820a = checkDroughtRun({
      records, minRecords: 10, minPresentCount: 3, runThreshold: 6,
      isPresent: r => r.purpose === 'character_moment',
    });
    if (r820a.fires) {
      issues.push({
        location: `longest stretch with no character moment: ${r820a.longestRun} consecutive scenes`,
        rule: 'RHYTHM_CHARACTER_MOMENT_DROUGHT_RUN',
        severity: 'minor',
        description: `The story contains a run of ${r820a.longestRun} consecutive scenes with no character-moment purpose at all, even though ${r820a.presentCount} scenes elsewhere pause for interior reflection. A long unbroken stretch with nothing but momentum leaves the story's rhythm without room to breathe for an extended run.`,
        suggestedFix: `Purpose at least one scene within the ${r820a.longestRun}-scene stretch as a character moment so the rhythm keeps room to breathe throughout that stretch.`,
      });
    }
  }

  // RHYTHM_TURNING_POINT_ZONE_CLUSTER — Distribution/timing × purpose === 'turning_point' ×
  // structural thirds. Built on checkZoneCluster from the shared checks library. n≥9, ≥3
  // turning-point scenes, fires when more than 75% of them fall in a single structural third.
  // This purpose value has never been referenced anywhere in this pass; none of the three
  // shared-library trio modes has ever been applied to it.
  {
    const r820b = checkZoneCluster({
      records, minRecords: 9, minCount: 3, ratioThreshold: 0.75,
      isPresent: r => r.purpose === 'turning_point',
    });
    if (r820b.fires) {
      issues.push({
        location: `${r820b.zoneNames[r820b.maxZoneIdx]} third — ${r820b.maxZoneCount} of ${r820b.count} turning-point scenes`,
        rule: 'RHYTHM_TURNING_POINT_ZONE_CLUSTER',
        severity: 'minor',
        description: `${Math.round((r820b.maxZoneCount / r820b.count) * 100)}% of the story's turning-point scenes cluster in the ${r820b.zoneNames[r820b.maxZoneIdx]} third. When every scene purposed as a turning point lands in the same structural window, the story's rhythm has no pivot to react to anywhere else across the story.`,
        suggestedFix: `Purpose at least one scene outside the ${r820b.zoneNames[r820b.maxZoneIdx]} third as a turning point so the rhythm keeps a pivot to react to more evenly across the story.`,
      });
    }
  }

  // RHYTHM_TURNING_POINT_DROUGHT_RUN — Run-based × purpose === 'turning_point' absence. Built on
  ...
  // same wave (peak mode conventionally skipped for this categorical field).
  {
    const r820c = checkDroughtRun({
      records, minRecords: 10, minPresentCount: 3, runThreshold: 6,
      isPresent: r => r.purpose === 'turning_point',
    });
    if (r820c.fires) {
      issues.push({
        location: `longest stretch with no turning point: ${r820c.longestRun} consecutive scenes`,
        rule: 'RHYTHM_TURNING_POINT_DROUGHT_RUN',
        severity: 'minor',
  ...
  // RHYTHM_REVELATION_PURPOSE_ZONE_CLUSTER — Distribution/timing × purpose === 'revelation' ×
  ...
  // Named distinctly from RHYTHM_REVELATION_ZONE_CLUSTER, which audits the separate revelation
  ...
  {
    const r932a = checkZoneCluster({
      records, minRecords: 9, minCount: 3, ratioThreshold: 0.75,
      isPresent: r => r.purpose === 'revelation',
    });
    if (r932a.fires) {
      issues.push({
        location: `${r932a.zoneNames[r932a.maxZoneIdx]} third — ${r932a.maxZoneCount} of ${r932a.count} revelation-purposed scenes`,
        rule: 'RHYTHM_REVELATION_PURPOSE_ZONE_CLUSTER',
        severity: 'minor',
        description: `${Math.round((r932a.maxZoneCount / r932a.count) * 100)}% of the scenes purposed as a revelation cluster in the ${r932a.zoneNames[r932a.maxZoneIdx]} third. When every purpose-built disclosure lands in the same structural window, the rhythm surges with new information in one part of the story and stays flat elsewhere.`,
        suggestedFix: `Purpose at least one scene outside the ${r932a.zoneNames[r932a.maxZoneIdx]} third as a revelation so the rhythm keeps pulsing with new information more evenly across the story.`,
      });
    }
  }

  // RHYTHM_REVELATION_PURPOSE_DROUGHT_RUN — Run-based × purpose === 'revelation' absence. Built on
  ...
  // mode conventionally skipped for this categorical field).
  {
    const r932b = checkDroughtRun({
      records, minRecords: 10, minPresentCount: 3, runThreshold: 6,
      isPresent: r => r.purpose === 'revelation',
    });
    if (r932b.fires) {
      issues.push({
        location: `longest stretch with no revelation-purposed scene: ${r932b.longestRun} consecutive scenes`,
        rule: 'RHYTHM_REVELATION_PURPOSE_DROUGHT_RUN',
        severity: 'minor',
        description: `The story contains a run of ${r932b.longestRun} consecutive scenes with no scene purposed as a revelation, even though ${r932b.presentCount} scenes elsewhere disclose information by purpose. A long unbroken stretch with no turns of new information leaves the rhythm flat for an extended run.`,
        suggestedFix: `Purpose a scene within the ${r932b.longestRun}-scene stretch as a revelation so the rhythm keeps pulsing with new information throughout that stretch.`,
      });
    }
  }

  // RHYTHM_COMPLICATE_ZONE_IMBALANCE — Underweight/bloat × purpose === 'complicate' × four
  // structural zones. Built on checkZoneImbalance from the shared checks library, continuing the
  // rollout begun in Wave 890. n≥10, ≥4 complicating scenes total, divided across four equal
  // structural zones. Fires only when one zone has zero such scenes while another holds ≥50% of
  // the total. Distinct from the 3-zone RHYTHM_COMPLICATE_ZONE_CLUSTER and run-based RHYTHM_
  // COMPLICATE_DROUGHT_RUN (both Wave 918) — the first application of the 4-zone bloat+empty-zone
  // mode to this purpose value.
  {
    const r932c = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => r.purpose === 'complicate',
    });
    if (r932c.fires) {
      const emptyNames932c = r932c.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName932c = FOUR_ZONE_NAMES[r932c.bloatZoneIdx];
      issues.push({
        location: `${emptyNames932c} empty; ${bloatName932c} has ${r932c.counts[r932c.bloatZoneIdx]}/${r932c.totalCount} complicating scenes`,
        rule: 'RHYTHM_COMPLICATE_ZONE_IMBALANCE',
        severity: 'minor',
  ...
  // RHYTHM_STAKES_EMOTIONAL_AFTERMATH_VOID — Sequence/aftermath × raise_stakes trigger →
  // emotionalShift absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  ...
  // while such shifts occur elsewhere. Distinct from RHYTHM_STAKES_CURIOSITY_AFTERMATH_VOID,
  // RHYTHM_STAKES_SUSPENSE_AFTERMATH_VOID, and RHYTHM_STAKES_RELATIONAL_AFTERMATH_VOID (same
  // trigger paired with curiosityDelta/suspenseDelta/relationshipShifts respectively) — this is
  // the fourth consequence channel for this trigger in this pass.
  {
    const r1030a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.purpose === 'raise_stakes',
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1030a.fires) {
      issues.push({
        location: `${r1030a.triggerCount} raise-stakes aftermath(s) — no emotional shift within 2 scenes`,
        rule: 'RHYTHM_STAKES_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every stakes-raising scene in the story (${r1030a.triggerCount} of them) is followed by two emotionally neutral scenes, even though ${r1030a.aftermathCount} emotionally-charged scenes exist elsewhere. A stakes-raise that isn't matched by any feeling in the scenes right after it leaves the rhythm's escalation registering as a declared beat rather than something anyone visibly feels the weight of.`,
        suggestedFix: `In the two scenes following at least one stakes-raise, let someone's feelings visibly register the new danger so escalating pressure lands emotionally, giving the rhythm a felt beat alongside its structural one.`,
      });
    }
  }

  // RHYTHM_PAYOFF_SUSPENSE_AFTERMATH_VOID — Sequence/aftermath × payoffSetupIds trigger →
  // suspenseDelta absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  ...
  // elsewhere. Distinct from RHYTHM_PAYOFF_CURIOSITY_AFTERMATH_VOID and the original payoffSetupIds
  // → emotionalShift rule (same trigger paired with curiosityDelta and emotionalShift
  // respectively) — this is the third consequence channel for this trigger.
  {
    const r1030b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.payoffSetupIds ?? []).length > 0,
      isAftermath: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r1030b.fires) {
      issues.push({
        location: `${r1030b.triggerCount} payoff aftermath(s) — no suspense rise within 2 scenes`,
        rule: 'RHYTHM_PAYOFF_SUSPENSE_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every payoff scene in the story (${r1030b.triggerCount} cashed-in setups) is followed by two scenes with no rise in suspense, even though ${r1030b.aftermathCount} such rises occur elsewhere. A resolution that closes cleanly with no fresh tension in its wake leaves the rhythm going slack right after a beat rather than building into the next one.`,
        suggestedFix: `In the two scenes following at least one payoff, let a new tension rise so the rhythm keeps building momentum rather than settling after a resolution.`,
      });
    }
  }

  // RHYTHM_REVELATION_SUSPENSE_AFTERMATH_VOID — Sequence/aftermath × revelation trigger →
  // suspenseDelta absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  ...
  // occur elsewhere. Distinct from RHYTHM_REVELATION_RELATIONAL_AFTERMATH_VOID, RHYTHM_REVELATION_
  // CURIOSITY_AFTERMATH_VOID, and the original revelation → emotionalShift rule (same trigger
  // paired with relationshipShifts/curiosityDelta/emotionalShift respectively) — this is the
  // fourth consequence channel for this trigger.
  {
    const r1030c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.revelation != null,
      isAftermath: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r1030c.fires) {
      issues.push({
        location: `${r1030c.triggerCount} revelation aftermath(s) — no suspense rise within 2 scenes`,
        rule: 'RHYTHM_REVELATION_SUSPENSE_AFTERMATH_VOID',
        severity: 'minor',
  ...
  // RHYTHM_REVELATION_STAGING_AFTERMATH_VOID — Sequence/aftermath × revelation trigger →
  // visualBeats absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  // qualifying revelation scenes (pos<n-2), ≥2 visually-dense scenes anywhere (visualBeats
  // length≥2), 2-scene lookahead. Fires when every revelation's two-scene aftermath contains no
  // visually dense scene, while such scenes occur elsewhere. Distinct from RHYTHM_REVELATION_
  // RELATIONAL_AFTERMATH_VOID, RHYTHM_REVELATION_CURIOSITY_AFTERMATH_VOID, RHYTHM_REVELATION_
  // SUSPENSE_AFTERMATH_VOID, and RHYTHM_REVELATION_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID (same trigger
  // paired with relationshipShifts/curiosityDelta/suspenseDelta/dialogueHighlights respectively,
  // plus the original emotionalShift pairing) — this is the sixth and final standard-channel
  // pairing for this trigger, completing full saturation.
  {
    const r1058a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.revelation != null,
    ...
    if (r1058a.fires) {
      issues.push({
        location: `${r1058a.triggerCount} revelation aftermath(s) — no visually dense scene within 2 scenes`,
        rule: 'RHYTHM_REVELATION_STAGING_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every revelation in the story (${r1058a.triggerCount} discoveries) is followed by two scenes with no substantial physical staging, even though ${r1058a.aftermathCount} such scenes exist elsewhere in the script. A truth that lands with no physical presence around it in the immediate aftermath leaves the rhythm's revelations registering only in dialogue and interiority, never given a body in the world.`,
        suggestedFix: `After at least one revelation, let one of the following two scenes carry substantial physical staging — an action or gesture that gives the discovery a physical anchor before the rhythm moves on.`,
      });
    }
  }

  // RHYTHM_TURN_SUSPENSE_AFTERMATH_VOID — Sequence/aftermath × dramaticTurn trigger →
  // suspenseDelta absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  ...
  // rises occur elsewhere. Distinct from RHYTHM_TURN_RELATIONAL_AFTERMATH_VOID, RHYTHM_TURN_
  // EMOTIONAL_AFTERMATH_VOID, and RHYTHM_TURN_CURIOSITY_AFTERMATH_VOID (same trigger paired with
  // relationshipShifts/emotionalShift/curiosityDelta respectively, plus the original payoffSetupIds
  // pairing) — this is the fourth standard-channel pairing for this trigger.
  {
    const r1058b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.dramaticTurn ?? 'nothing') !== 'nothing',
      isAftermath: r => (r.suspenseDelta ?? 0) > 0,
    });
    if (r1058b.fires) {
      issues.push({
        location: `${r1058b.triggerCount} dramatic-turn aftermath(s) — no suspense rise within 2 scenes`,
        rule: 'RHYTHM_TURN_SUSPENSE_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every dramatic-turn scene in the story (${r1058b.triggerCount} pivots) is followed by two scenes with no rise in suspense, even though ${r1058b.aftermathCount} such rises occur elsewhere. A pivot that never tightens tension right after it lands leaves the rhythm's turns registering as isolated events rather than pressure the story keeps building on.`,
  ...
  // RHYTHM_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID — Sequence/aftermath × payoffSetupIds trigger
  // → dialogueHighlights absence. Built on checkAftermathVoid from the shared checks library. n≥8,
  // ≥2 qualifying payoff scenes (pos<n-2), ≥2 scenes anywhere with a highlighted line of dialogue,
  // 2-scene lookahead. Fires when every payoff's two-scene aftermath contains no highlighted
  // dialogue, while such dialogue occurs elsewhere. Distinct from RHYTHM_PAYOFF_EMOTIONAL_
  // AFTERMATH_VOID, RHYTHM_PAYOFF_CURIOSITY_AFTERMATH_VOID, RHYTHM_PAYOFF_SUSPENSE_AFTERMATH_VOID,
  // and RHYTHM_PAYOFF_RELATIONAL_AFTERMATH_VOID (same trigger paired with emotionalShift/
  // curiosityDelta/suspenseDelta/relationshipShifts respectively) — this is the fifth
  // standard-channel pairing for this trigger.
  {
    const r1058c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.payoffSetupIds ?? []).length > 0,
      isAftermath: r => (r.dialogueHighlights ?? []).length > 0,
    });
    if (r1058c.fires) {
      issues.push({
        location: `${r1058c.triggerCount} payoff scene(s) — no highlighted dialogue within 2 scenes of any`,
        rule: 'RHYTHM_PAYOFF_DIALOGUE_HIGHLIGHT_AFTERMATH_VOID',
        severity: 'minor',
