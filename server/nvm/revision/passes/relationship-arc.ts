...
// modes in this pass).
//
// Wave 847 additions: RELATIONAL_POSITIVE_EMOTION_DROUGHT_RUN (run-based × emotionalShift ===
// 'positive' absence — completes 2 of 3 slots for this valence alongside the zone-cluster mode
// added in Wave 833; peak mode conventionally skipped for this categorical field),
// RELATIONAL_ESTABLISH_WORLD_ZONE_CLUSTER (distribution/timing × purpose === 'establish_world' ×
...
//
// Wave 931 additions: continuing the checkZoneImbalance rollout, this wave applies the 4-zone
// bloat+empty-zone mode to three more signals that each already have a complete 3-zone/run-based
// trio but had never been audited by it: RELATIONAL_STAKES_ZONE_IMBALANCE (purpose ===
...
// Wave 973 additions: auditing the three remaining trio-complete signals in this pass, spanning three
// distinct classes: RELATIONAL_CLOCK_ZONE_IMBALANCE (clockRaised boolean — whether a ticking clock is
...
// with emotionalShift).
// Wave 1015 additions: this wave gives three more triggers a third consequence channel:
// RELATIONAL_SEED_EMOTIONAL_AFTERMATH_VOID (seededClueIds, previously paired with visualBeats and
// curiosityDelta, now paired with emotionalShift), RELATIONAL_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID
// (heavy unresolvedClues debt, previously paired with visualBeats and emotionalShift, now paired
// with curiosityDelta), and RELATIONAL_STAKES_SUSPENSE_AFTERMATH_VOID (raise_stakes, previously
// paired with relationshipShifts and curiosityDelta, now paired with suspenseDelta).
// Wave 1029 additions: RELATIONAL_STAKES_EMOTIONAL_AFTERMATH_VOID gives raise_stakes a fourth
// checkAftermathVoid channel (previously paired with relationshipShifts/curiosityDelta/
// suspenseDelta, now paired with emotionalShift). The other two checks flip direction on an
// existing pairing rather than duplicating it: RELATIONAL_SEED_AFTERMATH_VOID pairs seededClueIds
// as TRIGGER with relationshipShifts as channel — distinct from the existing RELATIONSHIP_SHIFT_
// SEED_AFTERMATH_VOID, which runs the same two fields in the opposite direction (relationshipShifts
// as trigger, seededClueIds as channel) — and RELATIONAL_OPEN_THREAD_AFTERMATH_VOID pairs heavy
...
export async function relationshipArcPass(input: PassInput): Promise<PassResult> {
  ...
  // ── Wave 847: RELATIONAL_POSITIVE_EMOTION_DROUGHT_RUN, RELATIONAL_ESTABLISH_WORLD_ZONE_CLUSTER,
  //              RELATIONAL_CLIMAX_ZONE_CLUSTER ──────────────────────────────────────

  // RELATIONAL_POSITIVE_EMOTION_DROUGHT_RUN — Run-based × emotionalShift === 'positive' absence.
  ...
  // Wave 833 (peak mode conventionally skipped for this categorical field).
  {
    const r847a = checkDroughtRun({
      records, minRecords: 10, minPresentCount: 3, runThreshold: 6,
      isPresent: r => r.emotionalShift === 'positive',
    });
    if (r847a.fires) {
      issues.push({
        location: `longest stretch with no positive-emotion charge: ${r847a.longestRun} consecutive scenes`,
        rule: 'RELATIONAL_POSITIVE_EMOTION_DROUGHT_RUN',
        severity: 'minor',
        description: `The story contains a run of ${r847a.longestRun} consecutive scenes with no positive-emotion charge at all, even though ${r847a.presentCount} scenes elsewhere carry one. A long unbroken stretch with no warmth leaves the relationship without an emotional reward for an extended run.`,
        suggestedFix: `Give the story a moment of warmth within the ${r847a.longestRun}-scene stretch so the relationship keeps registering its emotional reward throughout that stretch.`,
      });
    }
  }
  ...
  {
    const r847b = checkZoneCluster({
      records, minRecords: 9, minCount: 3, ratioThreshold: 0.75,
      isPresent: r => r.purpose === 'establish_world',
    });
    if (r847b.fires) {
      issues.push({
        location: `${r847b.zoneNames[r847b.maxZoneIdx]} third — ${r847b.maxZoneCount} of ${r847b.count} world-establishing scenes`,
        rule: 'RELATIONAL_ESTABLISH_WORLD_ZONE_CLUSTER',
        severity: 'minor',
        description: `${Math.round((r847b.maxZoneCount / r847b.count) * 100)}% of the scenes purposed to establish the world cluster in the ${r847b.zoneNames[r847b.maxZoneIdx]} third. When every act of world-building concentrates in one structural window, the relationship loses fresh ground to develop against anywhere else across the story.`,
        suggestedFix: `Purpose at least one scene outside the ${r847b.zoneNames[r847b.maxZoneIdx]} third to establish the world so the relationship keeps fresh ground to develop against more evenly across the story.`,
      });
    }
  }

  // RELATIONAL_CLIMAX_ZONE_CLUSTER — Distribution/timing × purpose === 'climax' × structural
  ...
  {
    const r847c = checkZoneCluster({
      records, minRecords: 9, minCount: 3, ratioThreshold: 0.75,
      isPresent: r => r.purpose === 'climax',
    });
    if (r847c.fires) {
      issues.push({
        location: `${r847c.zoneNames[r847c.maxZoneIdx]} third — ${r847c.maxZoneCount} of ${r847c.count} climax-purposed scenes`,
        rule: 'RELATIONAL_CLIMAX_ZONE_CLUSTER',
        severity: 'minor',
  ...
  // RELATIONAL_STAKES_ZONE_IMBALANCE — Underweight/bloat × purpose === 'raise_stakes' × four
  // structural zones. Built on checkZoneImbalance from the shared checks library, continuing the
  // rollout begun in Wave 889. n≥10, ≥4 stakes-raising scenes total, divided across four equal
  // structural zones. Fires only when one zone has zero such scenes while another holds ≥50% of
  // the total. Distinct from the existing 3-zone RELATIONAL_STAKES_ZONE_CLUSTER and run-based
  // RELATIONAL_STAKES_DROUGHT_RUN — the first application of the 4-zone bloat+empty-zone mode to
  // this purpose value.
  {
    const r931a = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => r.purpose === 'raise_stakes',
    });
    if (r931a.fires) {
      const emptyNames931a = r931a.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName931a = FOUR_ZONE_NAMES[r931a.bloatZoneIdx];
      issues.push({
        location: `${emptyNames931a} empty; ${bloatName931a} has ${r931a.counts[r931a.bloatZoneIdx]}/${r931a.totalCount} stakes-raising scenes`,
        rule: 'RELATIONAL_STAKES_ZONE_IMBALANCE',
        severity: 'minor',
        description: `The story's ${r931a.totalCount} stakes-raising scenes are unevenly distributed across its four structural zones: ${bloatName931a} contains ${r931a.counts[r931a.bloatZoneIdx]} of them (${Math.round((r931a.counts[r931a.bloatZoneIdx] / r931a.totalCount) * 100)}%) while ${emptyNames931a} contains none. Stakes bloat upward in one structural quarter and never rise at all in another, giving the relationship's tests of loyalty an uneven structural rhythm.`,
        suggestedFix: `Redistribute stakes-raising beats: move at least one raise_stakes-purposed scene into the empty zone(s) — ${emptyNames931a} — so the relationship's loyalty is tested across every structural quarter, not only the quarter currently carrying most of them.`,
      });
    }
  }

  // RELATIONAL_REVELATION_PURPOSE_ZONE_IMBALANCE — Underweight/bloat × purpose === 'revelation' ×
  // four structural zones. Built on checkZoneImbalance from the shared checks library, closing the
  // 4-zone gap for this purpose value (its 3-zone/run trio was completed in Wave 917). n≥10, ≥4
  // revelation-purposed scenes total, divided across four equal structural zones. Fires only when
  // one zone has zero such scenes while another holds ≥50% of the total. Distinct from RELATIONAL_
  // REVELATION_PURPOSE_ZONE_CLUSTER/DROUGHT_RUN (Wave 917) and from the revelation-string-field
  // rules — the first application of the 4-zone bloat+empty-zone mode to this purpose value.
  {
    const r931b = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => r.purpose === 'revelation',
    });
    if (r931b.fires) {
      const emptyNames931b = r931b.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName931b = FOUR_ZONE_NAMES[r931b.bloatZoneIdx];
      issues.push({
        location: `${emptyNames931b} empty; ${bloatName931b} has ${r931b.counts[r931b.bloatZoneIdx]}/${r931b.totalCount} revelation-purposed scenes`,
        rule: 'RELATIONAL_REVELATION_PURPOSE_ZONE_IMBALANCE',
        severity: 'minor',
        description: `The story's ${r931b.totalCount} revelation-purposed scenes are unevenly distributed across its four structural zones: ${bloatName931b} contains ${r931b.counts[r931b.bloatZoneIdx]} of them (${Math.round((r931b.counts[r931b.bloatZoneIdx] / r931b.totalCount) * 100)}%) while ${emptyNames931b} contains none. Purpose-built disclosures bloat in one structural quarter and vanish from another, so the relationship is reshaped by new truth in only part of the story.`,
        suggestedFix: `Redistribute disclosures: move at least one revelation-purposed scene into the empty zone(s) — ${emptyNames931b} — so the relationship keeps being reshaped by new truth across every structural quarter, not only the quarter currently carrying most of them.`,
      });
    }
  }

  // RELATIONAL_NEGATIVE_EMOTION_ZONE_IMBALANCE — Underweight/bloat × emotionalShift === 'negative'
  ...
  // another holds ≥50% of the total. Distinct from the existing 3-zone RELATIONAL_NEGATIVE_EMOTION_
  // ZONE_CLUSTER and run-based RELATIONAL_NEGATIVE_EMOTION_DROUGHT_RUN — the first application of
  // the 4-zone bloat+empty-zone mode to this valence signal.
  {
    const r931c = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => r.emotionalShift === 'negative',
    });
    if (r931c.fires) {
      const emptyNames931c = r931c.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName931c = FOUR_ZONE_NAMES[r931c.bloatZoneIdx];
      issues.push({
        location: `${emptyNames931c} empty; ${bloatName931c} has ${r931c.counts[r931c.bloatZoneIdx]}/${r931c.totalCount} negative-shift scenes`,
        rule: 'RELATIONAL_NEGATIVE_EMOTION_ZONE_IMBALANCE',
        severity: 'minor',
  ...
  // RELATIONAL_CLOCK_ZONE_IMBALANCE — Underweight/bloat × (clockRaised === true) × four structural
  ...
  // existing 3-zone RELATIONAL_CLOCK_ZONE_CLUSTER and run-based RELATIONAL_CLOCK_DROUGHT_RUN — the
  ...
  {
    const r973a = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => r.clockRaised === true,
    });
    if (r973a.fires) {
      const emptyNames973a = r973a.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName973a = FOUR_ZONE_NAMES[r973a.bloatZoneIdx];
      issues.push({
        location: `${emptyNames973a} empty; ${bloatName973a} has ${r973a.counts[r973a.bloatZoneIdx]}/${r973a.totalCount} clock-raising scenes`,
        rule: 'RELATIONAL_CLOCK_ZONE_IMBALANCE',
        severity: 'minor',
        description: `The story's ${r973a.totalCount} clock-raising scenes are unevenly distributed across its four structural zones: ${bloatName973a} contains ${r973a.counts[r973a.bloatZoneIdx]} of them (${Math.round((r973a.counts[r973a.bloatZoneIdx] / r973a.totalCount) * 100)}%) while ${emptyNames973a} contains none. Ticking clocks bloat in one structural quarter and are never introduced in another, so the relationship operates under deadline pressure in only part of the story.`,
        suggestedFix: `Redistribute ticking clocks: introduce a time pressure (clockRaised) in at least one scene inside the empty zone(s) — ${emptyNames973a} — so the relationship keeps operating under deadline pressure across every structural quarter, not only the quarter currently carrying most of them.`,
      });
    }
  }

  // RELATIONAL_CLOCK_DELTA_ZONE_IMBALANCE — Underweight/bloat × (clockDelta !== 0) × four structural
  ...
  // 3-zone RELATIONAL_CLOCK_DELTA_ZONE_CLUSTER and run-based RELATIONAL_CLOCK_DELTA_DROUGHT_RUN — the
  ...
  {
    const r973b = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => (r.clockDelta ?? 0) !== 0,
    });
    if (r973b.fires) {
      const emptyNames973b = r973b.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName973b = FOUR_ZONE_NAMES[r973b.bloatZoneIdx];
      issues.push({
        location: `${emptyNames973b} empty; ${bloatName973b} has ${r973b.counts[r973b.bloatZoneIdx]}/${r973b.totalCount} clock-moving scenes`,
        rule: 'RELATIONAL_CLOCK_DELTA_ZONE_IMBALANCE',
        severity: 'minor',
        description: `The story's ${r973b.totalCount} clock-moving scenes are unevenly distributed across its four structural zones: ${bloatName973b} contains ${r973b.counts[r973b.bloatZoneIdx]} of them (${Math.round((r973b.counts[r973b.bloatZoneIdx] / r973b.totalCount) * 100)}%) while ${emptyNames973b} contains none. Deadline pressure bloats in one structural quarter and never moves in another, so the relationship's urgency comes and goes in only part of the story.`,
        suggestedFix: `Redistribute clock movement: move or add a scene that changes the clock (clockDelta ≠ 0) into the empty zone(s) — ${emptyNames973b} — so the relationship keeps racing a deadline across every structural quarter, not only the quarter currently carrying most of it.`,
      });
    }
  }

  // RELATIONAL_TURN_ZONE_IMBALANCE — Underweight/bloat × (dramaticTurn !== 'nothing') × four
  // structural zones. Built on checkZoneImbalance from the shared checks library. n≥10, ≥4 scenes
  // with a dramatic turn total, divided across four equal structural zones. Fires only when one zone
  // has zero such scenes while another holds ≥50% of the total. Uses the same dramaticTurn !==
  // 'nothing' predicate as the existing 3-zone RELATIONAL_TURN_ZONE_CLUSTER and run-based RELATIONAL_
  // TURN_DROUGHT_RUN — the first application of the 4-zone bloat+empty-zone mode to the dramatic-turn
  // categorical signal.
  {
    const r973c = checkZoneImbalance({
      records, minRecords: 10, minCount: 4, bloatRatio: 0.5,
      isPresent: r => (r.dramaticTurn ?? 'nothing') !== 'nothing',
    });
    if (r973c.fires) {
      const emptyNames973c = r973c.emptyZoneIdxs.map(i => FOUR_ZONE_NAMES[i]).join(', ');
      const bloatName973c = FOUR_ZONE_NAMES[r973c.bloatZoneIdx];
      issues.push({
        location: `${emptyNames973c} empty; ${bloatName973c} has ${r973c.counts[r973c.bloatZoneIdx]}/${r973c.totalCount} dramatic-turn scenes`,
        rule: 'RELATIONAL_TURN_ZONE_IMBALANCE',
        severity: 'minor',
  ...
  // RELATIONAL_SEED_EMOTIONAL_AFTERMATH_VOID — Sequence/aftermath × seededClueIds trigger →
  ...
  // elsewhere. Distinct from RELATIONAL_SEED_STAGING_AFTERMATH_VOID (visualBeats) and
  // RELATIONAL_SEED_CURIOSITY_AFTERMATH_VOID (Wave 1001, curiosityDelta) — this is the third
  // consequence channel for this trigger in this pass.
  {
    const r1015a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.seededClueIds ?? []).length > 0,
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1015a.fires) {
      issues.push({
        location: `${r1015a.triggerCount} seed aftermath(s) — no emotional shift within 2 scenes`,
        rule: 'RELATIONAL_SEED_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every clue-seeding scene (${r1015a.triggerCount} plants) is followed by two emotionally neutral scenes, even though ${r1015a.aftermathCount} emotionally-charged scenes exist elsewhere. Planting a clue usually carries some charge for whoever notices it; when every seed's aftermath is affectively flat, the arc's groundwork lands as pure information disconnected from the relationship's felt stakes.`,
        suggestedFix: `Let at least one seed carry feeling in its aftermath: in the scene or two after a clue is planted, show someone reacting to it emotionally in a way that bears on the relationship.`,
      });
    }
  }

  // RELATIONAL_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID — Sequence/aftermath × heavy unresolved-
  ...
  // RELATIONAL_OPEN_THREAD_STAGING_AFTERMATH_VOID (visualBeats) and RELATIONAL_OPEN_THREAD_
  // EMOTIONAL_AFTERMATH_VOID (Wave 1001, emotionalShift) — this is the third consequence channel
  // for this trigger in this pass.
  {
    const r1015b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.unresolvedClues ?? []).length >= 3,
      isAftermath: r => (r.curiosityDelta ?? 0) > 0,
    });
    if (r1015b.fires) {
      issues.push({
        location: `${r1015b.triggerCount} heavy clue-debt scene(s) — no curiosity raised within 2 scenes of any`,
        rule: 'RELATIONAL_OPEN_THREAD_CURIOSITY_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every scene carrying heavy unresolved clue-debt (${r1015b.triggerCount} instances) is followed by two full scenes that raise no new curiosity, even though ${r1015b.aftermathCount} such rises occur elsewhere. Accumulated mystery about the relationship should usually compound into fresh questions rather than sit as inert backlog; when every heavy-debt scene's aftermath opens nothing new, the arc's uncertainty stalls instead of deepening.`,
        suggestedFix: `In the two scenes following at least one heavy clue-debt moment, plant a new open question tied to the relationship so accumulated mystery keeps compounding rather than sitting in a learnable lull.`,
      });
    }
  }

  // RELATIONAL_STAKES_SUSPENSE_AFTERMATH_VOID — Sequence/aftermath × raise_stakes trigger →
  ...
  // tension does rise elsewhere. Distinct from RELATIONAL_STAKES_AFTERMATH_VOID (Wave 987,
  // relationshipShifts) and RELATIONAL_STAKES_CURIOSITY_AFTERMATH_VOID (Wave 1001, curiosityDelta)
  // — this is the third consequence channel for this trigger in this pass.
  {
    const r1015c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.purpose === 'raise_stakes',
    ...
    if (r1015c.fires) {
      issues.push({
        location: `${r1015c.triggerCount} stakes-raise aftermath(s) — no suspense raised within 2 scenes`,
        rule: 'RELATIONAL_STAKES_SUSPENSE_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every stakes-raising scene (${r1015c.triggerCount} escalations) is followed by two scenes with no rise in tension, even though ${r1015c.aftermathCount} such rises occur elsewhere. Escalating danger that never tightens the felt sense of jeopardy in the beats right after it leaves the relational arc's stakes reading as stated rather than felt.`,
        suggestedFix: `In the two scenes following at least one stakes-raise, tighten the tension — a ticking complication or a near-miss that bears on the relationship — so escalating danger registers as felt, not just stated.`,
      });
    }
  }

  // RELATIONAL_STAKES_EMOTIONAL_AFTERMATH_VOID — Sequence/aftermath × raise_stakes trigger →
  // emotionalShift absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2
  ...
  // while such shifts occur elsewhere. Distinct from RELATIONAL_STAKES_AFTERMATH_VOID
  // (relationshipShifts), RELATIONAL_STAKES_CURIOSITY_AFTERMATH_VOID, and RELATIONAL_STAKES_
  // SUSPENSE_AFTERMATH_VOID (same trigger paired with curiosityDelta and suspenseDelta
  // respectively) — this is the fourth consequence channel for this trigger in this pass.
  {
    const r1029a = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => r.purpose === 'raise_stakes',
      isAftermath: r => (r.emotionalShift ?? 'neutral') !== 'neutral',
    });
    if (r1029a.fires) {
      issues.push({
        location: `${r1029a.triggerCount} stakes-raise aftermath(s) — no emotional shift within 2 scenes`,
        rule: 'RELATIONAL_STAKES_EMOTIONAL_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every stakes-raising scene (${r1029a.triggerCount} escalations) is followed by two emotionally neutral scenes, even though ${r1029a.aftermathCount} emotionally-charged scenes exist elsewhere. Escalating danger that never registers as felt in the beats right after it leaves the relational arc's stakes reading as declared rather than something anyone in the bond visibly carries.`,
        suggestedFix: `In the two scenes following at least one stakes-raise, let someone's feelings visibly register the new danger so escalating pressure lands emotionally within the relationship, not only as stated fact.`,
      });
    }
  }

  // RELATIONAL_SEED_AFTERMATH_VOID — Sequence/aftermath × seededClueIds trigger → relationshipShifts
  // absence. Built on checkAftermathVoid from the shared checks library. n≥8, ≥2 qualifying seed
  // scenes (pos<n-2), ≥2 relationship-shift scenes anywhere, 2-scene lookahead. Fires when every
  // seed's two-scene aftermath carries no bond change, while such changes occur elsewhere. Distinct
  // from RELATIONSHIP_SHIFT_SEED_AFTERMATH_VOID, which runs the same two fields in the opposite
  // direction (relationshipShifts as trigger, seededClueIds as channel) — this rule makes
  // seededClueIds the trigger and relationshipShifts the channel, a fourth channel for this trigger
  // alongside visualBeats/curiosityDelta/emotionalShift.
  {
    const r1029b = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.seededClueIds ?? []).length > 0,
      isAftermath: r => (r.relationshipShifts ?? []).length > 0,
    });
    if (r1029b.fires) {
      issues.push({
        location: `${r1029b.triggerCount} seed aftermath(s) — no relationship shift within 2 scenes`,
        rule: 'RELATIONAL_SEED_AFTERMATH_VOID',
        severity: 'minor',
        description: `Every clue-seeding scene (${r1029b.triggerCount} plants) is followed by two scenes with no shift in any relationship, even though ${r1029b.aftermathCount} such shifts occur elsewhere. A planted clue that never bears on how characters treat each other nearby leaves the seed purely mechanical, disconnected from the bonds the arc is meant to be tracking.`,
        suggestedFix: `In the two scenes following at least one clue-seeding moment, let the planted material strain or shift a relationship so the seed carries relational weight alongside its narrative function.`,
      });
    }
  }

  // RELATIONAL_OPEN_THREAD_AFTERMATH_VOID — Sequence/aftermath × heavy unresolvedClues debt
  ...
  // carries no bond change, while such changes occur elsewhere. Distinct from this pass's
  // OPEN_THREAD_RELATIONSHIP_SHIFT_DECOUPLED (co-occurrence mode, not sequence/aftermath) — this is
  // the fourth consequence channel for this trigger alongside visualBeats/emotionalShift/
  // curiosityDelta.
  {
    const r1029c = checkAftermathVoid({
      records, minRecords: 8, minTriggerCount: 2, minAftermathCount: 2, window: 2,
      isTrigger: r => (r.unresolvedClues ?? []).length >= 3,
      isAftermath: r => (r.relationshipShifts ?? []).length > 0,
    });
    if (r1029c.fires) {
      issues.push({
        location: `${r1029c.triggerCount} heavy clue-debt scene(s) — no relationship shift within 2 scenes of any`,
        rule: 'RELATIONAL_OPEN_THREAD_AFTERMATH_VOID',
        severity: 'minor',
