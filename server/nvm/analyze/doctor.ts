...
//
// This file imports calibration/reference.ts; since 2026-09-21 reference.ts
// does NOT import this file back. It scores its corpus through
// computeRawCraftScore from ./craft-formula.ts — a leaf with no imports, where
// the whole craft formula (densityPenalty, scarcityPenalty, craftPenalty,
// computeRawCraftScore) now lives and is re-exported from here. The old
// doctor.ts <-> reference.ts cycle, and the two silent failures it carried on
// the corpus-scoring path (a module-level formula const in its temporal dead
// zone; esbuild's `__name` helper under tsx), are gone by construction on
// that path; the function-local-constant style is kept as convention, and
// tests/core/doctor-calibration-under-tsx.test.ts still runs.
...
// ── The craft formula lives in ./craft-formula.ts (a leaf; 2026-09-21) ──────
// densityPenalty, scarcityPenalty, craftPenalty and computeRawCraftScore —
// and the whole opportunity-based design comment that used to sit above them
// here — moved verbatim to craft-formula.ts, a module that imports NOTHING,
// so that calibration/reference.ts can score its corpus through
// computeRawCraftScore WITHOUT importing this file. That removes the
// doctor.ts <-> reference.ts cycle from the corpus-scoring path, and with it
// the two silent failures the cycle carried (a module-level const in its
// temporal dead zone; esbuild's `__name` helper under tsx) — see the leaf's
// header and docs/audits/2026-09-21-craft-formula-leaf/README.md.
// Re-exported here so every existing import site (tests, scripts, branches)
// still resolves `computeRawCraftScore` from doctor.ts.
export { computeRawCraftScore };
...
// score (see craftPenalty's design comment in craft-formula.ts: "a tiny script can no
 ...
 *  function-local for the same TDZ reason densityPenalty/scarcityPenalty
 *  (now in craft-formula.ts, a leaf) originally gave — this function stays
 *  in doctor.ts, which still imports calibration/reference.ts, so the
 *  convention is kept here as a matter of style (reachable from
 *  computeDimensionRawScore, called by buildDimensions, called from
 *  aggregateReport — none of which the corpus build runs). */
...
export function aggregateReport(result: RevisionResult, analysis: FountainAnalysis, fountain: string): ScriptDoctorReport {
  ...
  return {
    ...
    subtextRatio: analysis.subtextRatio,
    questionLatencyOverall: analysis.questionLatencyOverall,
    analysisComplete,
