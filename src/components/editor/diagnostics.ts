...
function buildDiagnosticDecorations(state: { doc: { lines: number; line(n: number): { from: number; to: number } } }, issues: LocatedIssue[]): DecorationSet {
  ...
  for (const lineNo of [...lineWorst.keys()].sort((a, b) => a - b)) {
    ...
    ranges.push(Decoration.mark({ class: `cm-diagnostic-${severity}` }).range(line.from, line.to));
  }
...
const diagnosticsDecorationPlugin = ViewPlugin.fromClass(
  class {
    ...
    constructor(view: EditorView) {
      this.decorations = buildDiagnosticDecorations(view.state, view.state.field(diagnosticsField));
    }
    update(update: ViewUpdate) {
      if (
        ...
        this.decorations = buildDiagnosticDecorations(update.state, update.state.field(diagnosticsField));
      }
    }
  },
