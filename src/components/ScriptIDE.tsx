...
const renderHighlightedText = (_text: string, blocks: FountainBlock[]) => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    while (startIdx <= blockTextLength) {
      let endIdx = block.text.indexOf('\n', startIdx);
      if (endIdx === -1) endIdx = blockTextLength;

      const lineText = block.text.slice(startIdx, endIdx);
      const isLastBlock = i === blocks.length - 1;
      const isLastLineInBlock = endIdx === blockTextLength;
...
export default function ScriptIDE({
  ...
  const stats = useMemo(() => {
    ...
    if (trimmedScriptText !== "") {
      const wordRegex = /[^\s]+/g;
      while (wordRegex.exec(trimmedScriptText)) {
        wordCount++;
      }
    }
