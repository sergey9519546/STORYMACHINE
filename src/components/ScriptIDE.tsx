...
const renderHighlightedText = (_text: string, blocks: FountainBlock[]) => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    while (startIndex <= blockText.length) {
      let nextNewline = blockText.indexOf('\n', startIndex);
      if (nextNewline === -1) {
        nextNewline = blockText.length;
      }

      const lineText = blockText.slice(startIndex, nextNewline);
      const isLastBlock = i === blocks.length - 1;
      const isLastLineInBlock = nextNewline === blockText.length;
      ...
      if (nextNewline === blockText.length) {
        break;
      }
...
export default function ScriptIDE({
  ...
  const stats = useMemo(() => {
    ...
    if (trimmedText !== "") {
      const regex = /\S+/g;
      while (regex.exec(trimmedText) !== null) {
        wordCount++;
      }
    }
