...
export default function ScriptIDE({
  ...
  const stats = useMemo(() => {
    ...
    // Performance optimization: Replace allocation-heavy string parsing (e.g. .trim().split(/\s+/))
    // with zero-allocation charCodeAt loop for high-frequency string counting
    let wordCount = 0;
    let inWord = false;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // Performance optimization: Avoid .substring() and .split("\n") for currentLine extraction
    // using zero-allocation reverse search
  ...
  const handleNavigate = (lineIndex: number) => {
    ...
    // Performance optimization: Replace scriptText.split("\n") with indexOf loop
    // to avoid massive array allocation during navigation
    let charCount = 0;
