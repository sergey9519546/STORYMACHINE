...
export default function ScriptIDE({
  ...
  const stats = useMemo(() => {
    ...
    let dialogueLines = 0;
    let actionLines = 0;
    // ⚡ Bolt Performance Optimization: Use RegEx exec loop to avoid O(N) array allocation
    let wordCount = 0;
    const trimmedScript = scriptText.trim();
    if (trimmedScript !== '') {
      const regex = /[^\s]+/g;
      while (regex.exec(trimmedScript) !== null) {
        wordCount++;
      }
    }
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // ⚡ Bolt Performance Optimization: Replace .split('\n') with .lastIndexOf for zero-allocation
    const textBeforeCursor = scriptText.slice(0, cursor);
    const lastNewlineIdx = textBeforeCursor.lastIndexOf('\n');
    const currentLine = textBeforeCursor.slice(lastNewlineIdx === -1 ? 0 : lastNewlineIdx + 1);
