...
const renderHighlightedText = (_text: string, blocks: FountainBlock[]) => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    // ⚡ Bolt: Replaced split() with zero-allocation indexOf loop
    let startIndex = 0;
    const isLastBlock = i === blocks.length - 1;
    while (startIndex <= block.text.length) {
      let newlineIndex = block.text.indexOf("\n", startIndex);
      const isLastLineInBlock = newlineIndex === -1;
      if (isLastLineInBlock) newlineIndex = block.text.length;

      const lineText = block.text.slice(startIndex, newlineIndex);
...
export default function ScriptIDE({
  ...
  const stats = useMemo(() => {
    ...
    let dialogueLines = 0;
    let actionLines = 0;
    // ⚡ Bolt: Replaced split() with zero-allocation RegExp match loop
    let wordCount = 0;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // ⚡ Bolt: Replaced substring/split with lastIndexOf/slice for zero-allocation
