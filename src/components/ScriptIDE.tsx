...
const renderHighlightedText = (_text: string, blocks: FountainBlock[]) => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    let currentIndex = 0;
    while (currentIndex <= block.text.length) {
      const nextNewline = block.text.indexOf('\n', currentIndex);
      const hasNextNewline = nextNewline !== -1;

      const end = hasNextNewline ? nextNewline : block.text.length;
      const lineText = block.text.slice(currentIndex, end);
      const isLastBlock = i === blocks.length - 1;
      const isLastLineInBlock = !hasNextNewline;
