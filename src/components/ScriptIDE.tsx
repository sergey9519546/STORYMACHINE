...
const renderHighlightedText = (_text: string, blocks: FountainBlock[]) => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    while (startIndex <= textLength) {
      let nlIndex = block.text.indexOf("\n", startIndex);
      if (nlIndex === -1) {
        nlIndex = textLength;
        isLastLineInBlock = true;
      }

      const lineText = block.text.slice(startIndex, nlIndex);
...
export default function ScriptIDE({
  ...
  const handleNavigate = (lineIndex: number) => {
    ...
    while (currentLineIndex < lineIndex && charCount < scriptText.length) {
      let nextNl = scriptText.indexOf("\n", charCount);
      if (nextNl === -1) break;
      charCount = nextNl + 1;
      currentLineIndex++;
    }
