...
const renderHighlightedText = (_text: string, blocks: FountainBlock[]) => {
  ...
  // Removed redundant `split('\n')` inside the loop to avoid O(N^2) memory allocations.
  // `parseFountain` already parses the text into single-line blocks, so mapping directly over
  // `blocks` reduces memory allocations and lowers UI render latency during typing.
