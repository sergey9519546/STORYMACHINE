...
export default function ScriptIDE({
  ...
  const stats = useMemo(() => {
    ...
    let dialogueLines = 0;
    let actionLines = 0;
    // ⚡ Bolt: Zero-allocation word counting to prevent array allocation on keystrokes
    let wordCount = 0;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // ⚡ Bolt: Zero-allocation line extraction using string index search instead of split
  ...
  const handleNavigate = (lineIndex: number) => {
    if (!editorRef.current) return;
    // ⚡ Bolt: Zero-allocation string scanning to prevent array allocation when navigating
    let charCount = 0;
