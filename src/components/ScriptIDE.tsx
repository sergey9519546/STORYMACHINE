...
const renderHighlightedText = (_text: string, blocks: FountainBlock[]) => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    // ⚡ Bolt Performance Optimization: Zero-allocation line extraction for rendering
    let startIndex = 0;
    let isLastLineInBlock = false;
    while (!isLastLineInBlock) {
      let newlineIdx = block.text.indexOf("\n", startIndex);
      if (newlineIdx === -1) {
        newlineIdx = block.text.length;
        isLastLineInBlock = true;
      }
      const lineText = block.text.slice(startIndex, newlineIdx);
...
export default function ScriptIDE({
  ...
  const stats = useMemo(() => {
    ...
    let dialogueLines = 0;
    let actionLines = 0;
    // ⚡ Bolt Performance Optimization: Zero-allocation word counting
    let wordCount = 0;
    let inWord = false;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // ⚡ Bolt Performance Optimization: Zero-allocation line extraction
  ...
  const handleNavigate = (lineIndex: number) => {
    if (!editorRef.current) return;
    // ⚡ Bolt Performance Optimization: Zero-allocation line navigation
    let charCount = 0;
