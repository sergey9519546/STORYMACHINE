...
export default function ScriptIDE({
  ...
  const stats = useMemo(() => {
    ...
    // ⚡ Bolt Performance Optimization:
    // Replaced `.split(/\s+/)` with zero-allocation `charCodeAt(i) > 32` loop
    // avoiding O(N) intermediate array creation on large script analysis
    let wordCount = 0;
    let inWord = false;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // avoiding O(N) intermediate array creation on every keystroke.
  ...
  const handleNavigate = (lineIndex: number) => {
    ...
    // avoiding O(N) intermediate array creation during navigation jumps.
    let charCount = 0;
