...
const renderHighlightedText = (_text: string, blocks: FountainBlock[]) => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    while (startIndex <= blockText.length) {
      ...
      const lineText = isLastLineInBlock ? blockText.slice(startIndex) : blockText.slice(startIndex, nextNewline);
...
export default function ScriptIDE({
  ...
  const handleNavigate = (lineIndex: number) => {
    ...
    for (let i = 0; i < lineIndex; i++) {
      const nextNewlineIndex = scriptText.indexOf('\n', charCount);
      if (nextNewlineIndex === -1) break;
      charCount = nextNewlineIndex + 1;
    }
    editorRef.current.focus();
