...
export default function ScriptIDE({
  ...
  const handleNavigate = (lineIndex: number) => {
    ...
    while (currentLineIdx < lineIndex && searchIdx <= scriptText.length) {
      ...
      if (nextNewline === -1) {
        // If no more newlines, the rest of the string is the last line
        charCount += (scriptText.length - searchIdx) + 1;
        break;
      }
      // Add the length of this line plus the newline character
      charCount += (nextNewline - searchIdx) + 1;
