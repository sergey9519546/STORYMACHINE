...
const renderHighlightedText = (_text: string, blocks: FountainBlock[]) => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    let j = 0;
    let nextIndex;
    while ((nextIndex = block.text.indexOf('\n', lastIndex)) !== -1) {
      const lineText = block.text.slice(lastIndex, nextIndex);
      const isLastBlock = i === blocks.length - 1;
      const isLastLineInBlock = false;
    ...
    const lineText = block.text.slice(lastIndex);
    const isLastBlock = i === blocks.length - 1;
    const isLastLineInBlock = true;
    result.push(
      <span key={lineIdx} className={className || ""}>
        {lineText || " "}
...
export default function ScriptIDE({
  ...
  const handleNavigate = (lineIndex: number) => {
    ...
    while (currentLineIdx < lineIndex && charCount < scriptText.length) {
      if (scriptText.charCodeAt(charCount) === 10) { // 10 is newline
        currentLineIdx++;
      }
      charCount++;
    }
