...
export default function ScriptIDE({
  ...
  const stats = useMemo(() => {
    ...
    let dialogueLines = 0;
    let actionLines = 0;
    // ⚡ Bolt Performance Optimization: Replace O(N) split allocation with zero-allocation regex loop for word counting
    let wordCount = 0;
    if (scriptText.trim() !== "") {
      const regex = /[^\s]+/g;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // ⚡ Bolt Performance Optimization: Avoid intermediate array allocation from split() during high-frequency keydown events
  ...
  const handleNavigate = (lineIndex: number) => {
    if (!editorRef.current) return;
    // ⚡ Bolt Performance Optimization: Replace text.split() array instantiation with direct string navigation
    let charCount = 0;
    let currentLine = 0;
    let index = 0;
    while (currentLine < lineIndex && index < scriptText.length) {
