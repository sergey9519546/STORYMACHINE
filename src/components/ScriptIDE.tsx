...
const renderHighlightedText = (_text: string, blocks: FountainBlock[]) => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    while (startIndex <= block.text.length) {
      const nlIndex = block.text.indexOf("\n", startIndex);
      const isLastLineInBlock = nlIndex === -1;
      const lineText = isLastLineInBlock ? block.text.slice(startIndex) : block.text.slice(startIndex, nlIndex);
...
export default function ScriptIDE({
  ...
  const stats = useMemo(() => {
    ...
    if (trimmedText !== "") {
      const wordRegex = /[^\s]+/g;
      while (wordRegex.exec(trimmedText) !== null) {
        wordCount++;
      }
    }
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    const lastNlIndex = textBeforeCursor.lastIndexOf("\n");
    const currentLine = lastNlIndex === -1 ? textBeforeCursor : textBeforeCursor.slice(lastNlIndex + 1);
  ...
  const handleNavigate = (lineIndex: number) => {
    ...
    while (currentLine < lineIndex && startIndex <= scriptText.length) {
      const nlIndex = scriptText.indexOf("\n", startIndex);
      if (nlIndex === -1) {
        startIndex = scriptText.length + 1;
        break;
      }
      startIndex = nlIndex + 1;
      currentLine++;
    ...
    // the target index handles out of bounds since setSelectionRange caps it
    const charCount = Math.min(startIndex, scriptText.length);
    editorRef.current.focus();
