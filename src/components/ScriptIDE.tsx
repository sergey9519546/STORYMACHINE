...
export default function ScriptIDE({
  ...
  const stats = useMemo(() => {
    ...
    // ⚡ Bolt Performance Optimization:
    // Replaced `scriptText.trim().split(/\s+/).length` with zero-allocation Regex execution loop.
    // Avoids creating huge intermediate arrays for word counting in large scripts.
    let wordCount = 0;
    const wordRegex = /[^\s]+/g;
    while (wordRegex.exec(scriptText)) {
      wordCount++;
    }
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // ⚡ Bolt Performance Optimization:
    // Replaced `.split("\n")` with zero-allocation `.lastIndexOf` and `.slice`
    // to extract the current line without massive intermediate array allocations on keystrokes.
    const lastNewline = scriptText.lastIndexOf("\n", cursor - 1);
    const currentLine = scriptText.slice(lastNewline + 1, cursor);
  ...
  const handleNavigate = (lineIndex: number) => {
    ...
    // ⚡ Bolt Performance Optimization:
    // Replaced `.split("\n")` with zero-allocation loop using `.indexOf`.
    let charCount = 0;
    for (let i = 0; i < lineIndex; i++) {
      const nextNewline = scriptText.indexOf("\n", charCount);
      if (nextNewline === -1) break; // Reached end of string
