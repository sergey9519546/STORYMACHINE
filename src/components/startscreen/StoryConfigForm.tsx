...
export function StoryConfigForm({
  ...
  const inferCategory = (filename: string): FileCategory => {
    ...
    const ext = lastDot !== -1 && lastDot < lower.length - 1 ? lower.slice(lastDot + 1) : lower;
            ...
            <h4 className="font-bold uppercase tracking-widest text-xs border-b-2 border-black pb-2 mb-3 flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                {uploadedFiles.length > 0 && (
                  <button onClick={() => onUploadedFilesChange([])} className="text-[10px] hover:text-[#FF4444] transition-colors uppercase font-bold">Clear All</button>
                )}
                  ...
                  <div key={idx} className="flex flex-col gap-2 bg-white brutal-border p-2 group hover:border-[#FF4444] brutal-shadow-hover">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <button onClick={() => onPreviewFile(file)} className="text-gray-400 hover:text-black transition-colors" title="Preview">
