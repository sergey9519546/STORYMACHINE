...
import type { CharacterSheet, Location, ActionLogEntry } from '../../server/engine/types';
  ...
  return (
    <div className="min-h-screen bg-[#f4f4f0] text-black p-8 font-sans">
      <header className="mb-8 border-b-4 border-black pb-4 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold uppercase tracking-widest text-black">Story Machine</h1>
          <p className="text-gray-600 text-sm mt-1 font-mono uppercase">OASIS Architecture Prototype</p>
        </div>
        <div className="flex gap-4">
          <button 
            onClick={() => window.dispatchEvent(new CustomEvent('close-story-machine'))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-8">
          <section>
            <h2 className="text-xl font-bold mb-4 uppercase text-black border-b-2 border-black pb-2">The Stage</h2>
            <div className="space-y-4">
              ...
              {nodes.length === 0 && <p className="text-gray-500 italic font-mono text-sm">No nodes initialized.</p>}
            </div>
          </section>

          <section>
            <h2 className="text-xl font-bold mb-4 uppercase text-black border-b-2 border-black pb-2">Agents</h2>
            <div className="space-y-4">
              {agents.map(agent => (
                <div key={agent.char_id} className="bg-white p-4 brutal-border-thick brutal-shadow">
                  <div className="flex justify-between items-start border-b-2 border-black pb-2 mb-2">
                    <h3 className="font-bold text-lg text-black uppercase tracking-wider">{agent.name}</h3>
                    <span className="text-[10px] bg-black text-white px-2 py-1 uppercase font-bold tracking-widest">
                      Loc: {nodes.find(n => n.location_id === agent.current_location_id)?.name || agent.current_location_id}
                    </span>
                  </div>
                  <div className="mt-3 space-y-2 text-xs font-mono">
                    <p><span className="font-bold uppercase text-black">Mask:</span> {agent.public_mask}</p>
                    <p><span className="font-bold uppercase text-black">Shadow:</span> {agent.hidden_motive}</p>
                    <p><span className="font-bold uppercase text-black">Suspicion:</span> <span className={agent.suspicion_score > 50 ? 'text-[#FF4444] font-bold' : 'text-black'}>{agent.suspicion_score}/100</span></p>
                    <div className="border-t-2 border-dashed border-gray-300 pt-2 mt-2">
                      <span className="font-bold uppercase text-black">Knowledge:</span>
              ...
              {agents.length === 0 && <p className="text-gray-500 italic font-mono text-sm">No agents initialized.</p>}
            </div>
          </section>
        </div>
        ...
        <div className="lg:col-span-2">
          <section className="h-full flex flex-col">
            <h2 className="text-xl font-bold mb-4 uppercase text-black border-b-2 border-black pb-2">Script Ledger</h2>
            <div className="flex-1 bg-white brutal-border-thick brutal-shadow p-6 overflow-y-auto font-mono text-sm space-y-6 min-h-[600px]">
              {ledger.map(entry => {
                ...
                return (
                  <div key={entry.action_id} className="border-l-4 border-black pl-4 py-2 bg-gray-50">
                    ...
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      {entry.target_char_id && entry.action_type === 'RELOCATE' && (
                        <span className="text-blue-600 font-bold uppercase text-xs mt-0.5">→ {nodes.find(n => n.location_id === entry.target_char_id)?.name || entry.target_char_id}</span>
                      )}
                      {entry.target_char_id && entry.action_type === 'SPEAK' && (
                        <span className="text-[#FF4444] font-bold uppercase text-xs mt-0.5">to {agents.find(a => a.char_id === entry.target_char_id)?.name || entry.target_char_id}</span>
                      )}
                    </div>
              ...
              {ledger.length === 0 && <p className="text-gray-500 italic">The stage is silent...</p>}
            </div>
          </section>
        </div>
      </div>
